...
export type SchemaImportMapping = {
    key: string;
    type: 'Room' | 'Feature' | 'Knowledge' | 'Map' | 'Moment' | 'Message' | 'Lens'
}
...
export const isSchemaImportMapping = (schema: any): schema is SchemaImportMapping => (
    checkTypes({
        ...
        values: { type: (type: string) => (['Room', 'Feature', 'Knowledge', 'Map', 'Moment', 'Message', 'Lens'].includes(type)) }
    })(schema)
)

export const isSchemaImportMappingType = (value: string): value is SchemaImportMapping["type"] => (['Room', 'Feature', 'Knowledge', 'Map', 'Moment', 'Message', 'Lens'].includes(value))
