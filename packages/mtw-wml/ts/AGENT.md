...
**⚠️ CRITICAL (Feature and Knowledge)** — *Migration in progress (Phase 1+).* Task plan: [`taskPlanning/packages/mtw-wml/standardize/AGENT.featureKnowledgeExamples.planning.md`](../../../taskPlanning/packages/mtw-wml/standardize/AGENT.featureKnowledgeExamples.planning.md).

**Target steady state (v1):**

- **Storage:** **`situations`** homogeneous facet list on Feature/Knowledge (same pattern as Room **`SituationRoomFacetList`**). Each facet references a **`Situation`** and carries a **payload** (DisplayName / Summary / Description for that parent in that world-state).
...
- **Marks:** On **`Situation`** components, not on Example children.
- **Ephemera wire:** Resolved prose on **`render`** (`SituationRoomFacetPayload` shape), mirroring Room (**D1**). Playing UI reads **`render`**, then DEFAULT asset facet when wire is absent.
- **Situation entity:** **`Situation`** is an **independent** WML component; Room/Feature/Knowledge reference it via facets but do **not** own it (**D8**). Sharing one Situation across parents is allowed; no cross-parent validator in v1.
- **No production data migration:** Extant database has no Feature/Knowledge/Example content; Phase 4 is code/test cleanup only.

**Current code (until Phase 1 ships):** Display content still lives in child **`Example`** components referenced via **`examples`** on Feature/Knowledge. See [Standard Components documentation](standardize/components/AGENT.md) for implementation status.
...
- **`standardizeMode`**: Asset vs ephemera wire payload vocabulary, optional **`StandardFormConstructionOptions`** on **`StandardForm`** and generated **`Standard*`** constructors (WML/schema paths), and threading through **`fromSchema`** --- see [`standardize/AGENT.md`](./standardize/AGENT.md) (section **Payload vocabulary vs semantic mode (`standardizeMode`)**). Component-level concepts also link from [`standardize/components/AGENT.md`](./standardize/components/AGENT.md). **Ephemera wire** adds **`<Object uuid=(id)><ShortName>...</ShortName></Object>`** under **`Room`** only; canonical handles are **`OBJECT#...`** ([`packages/mtw-utilities/ts/types.ts`](../../mtw-utilities/ts/types.ts) **`enforceTypedKey`/`stripTypedKey`**). **`Object`** is not a **`StandardComponent`**. **Ephemera wire** also adds **`<Render>`** under **`Room`** with **DisplayName**, **Summary**, and **Description** children; **`StandardRoom.render`** stores the same JSON shape as a situation-room facet payload (**`SituationRoomFacetPayloadType`**). **Asset** mode rejects **`Render`** like **`Object`**. Ephemera integration context: [`lambda/ephemera/dataSource/AGENT.multiChannel.contract.md`](../../../lambda/ephemera/dataSource/AGENT.multiChannel.contract.md), [`taskPlanning/lambda/ephemera/dataSource/objects/AGENT.objectHandling.plan.md`](../../../taskPlanning/lambda/ephemera/dataSource/objects/AGENT.objectHandling.plan.md).
