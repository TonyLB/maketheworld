...
**Still Phase 2+ (lambdas / client):** Ephemera wire **`render`** on F/K (**D1**), playing UI and editors. Assets **`componentExamples`**: F/K parent facet branch, Situation fan-out, and Example path gated on **`parentIds`** shipped (2026-05-19). **`StandardExample`** remains for marks-only Example components until Phase 4.
