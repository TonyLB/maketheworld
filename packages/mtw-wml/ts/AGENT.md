...
**Still Phase 3+ (client):** Playing UI (**`ComponentDescription`**) and editors (**D5**). Ephemera wire **`render`** on F/K (**D1**) and assets/ephemera cache pipeline shipped (2026-05-19). **`StandardExample`** remains for marks-only Example components until Phase 4.
