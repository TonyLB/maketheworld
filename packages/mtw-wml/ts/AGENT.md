...
**Still Phase 2+ (lambdas / client):** Ephemera wire **`render`** on F/K (**D1**), **`componentExamples`** parent branch, playing UI and editors. **`StandardExample`** remains for marks-only Example components until Phase 4.
