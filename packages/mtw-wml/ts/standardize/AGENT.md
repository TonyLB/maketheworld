...
**Authored `Room.objects` is a `not yet` gate, not a permanent prohibition (LG-7).** The **`objects`** row above (non-empty **`objects`** forbidden on asset **`StandardRoom`**) exists because no cache-time path currently derives room contents from authored **`<Object>`** nesting the way **`containmentPopulationSteps.ts`** derives Feature/Room presence --- object placement is runtime-improvisational today. Authored objects are wanted eventually, and this clause is waiting on: (1) Room's wire carrying a scoped ludic-graph subgraph in place of **`_objects`** (so an authored object has somewhere non-ambiguous to live); (2) **`roomHeaderPhaseC.ts`**'s room-contents line deriving from graph nodes instead of **`objects`**, so retiring **`_objects`** doesn't silently break the `Contents: a, b, and c` line; and (3) settling the same identity-across-merge and identity-across-recache questions presence nodes raise (LG-6) --- two assets authoring the same object into the same room, and what happens to an authored object's identity across a recache. Until those land, this **`objects`** check stays exactly as written above; when they do, it is replaced (not just deleted) by whatever authoring surface those items produce.
