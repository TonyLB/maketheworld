...
|-----------|--------------------------|
| **`StandardRoom`** | non-empty **`exits`**, an **`Object`**-tagged node in **`ludicGraph`**, defined **`render`** |
| **`StandardFeature`** | defined **`render`** |
| **`StandardKnowledge`** | defined **`render`** |

**Presence nodes are type-permitted, unauthored, and un-linted (LG-6).** `StandardLudicGraphData`'s node list can represent a `Presence` node (added in Slice 1, mirroring `EphemeraLudicGraphNode`), but no WML tag or JSON-authoring surface exists for one yet --- only raw `StandardLudicGraphData` passed directly into a component's data can populate it, per the lift rule (type now, behavior/authoring on demand). **`validateAssetWirePolicy` does not check for presence nodes** --- a deliberate choice (2026-09-21), not an oversight: WML authoring is **definitely** going to gain a way to author presence, and a temporary rejection would only have to be removed again when it does, for no safety gained in the meantime (nothing today produces a presence node without hand-authored JSON in the first place). The reason a lint would otherwise be tempting: ephemera's cache-time containment derivation (**`containmentPopulationSteps.ts`**) is currently the sole minter of presence bindings, keyed by **`fromHostId === parentId`**, and if WML could also author one, two independent mechanisms could produce conflicting bindings for the same host --- an arity-2 hazard where exactly one binding is correct. That hazard is real, but it is a problem for whichever future authoring surface mints presence nodes to solve (via a stable identity scheme, e.g. derived from `(hostedThing, fromHostId)` rather than a minted uuid --- see `AGENT.edges.md`'s worked precedent for `Exit` `edgeId`), not something to guard against with a block that has no author path to protect against today.

**Authored objects-in-rooms are a `not yet` gate, not a permanent prohibition (LG-7).** `_objects`/`StandardRoomObjectData` (a flat `{uuid, shortName}` list independent of `ludicGraph`) were retired in Slice 5: Room's wire now carries a `ludicGraph` (item 1 below), and `roomHeaderPhaseC.ts`'s room-contents line derives from graph nodes (item 2), both landed. Unlike presence, this one **does** have a WML surface tag: a nested `<Object>` under `<Room>` parses as a `ludicGraph` membership reference (mirroring `<Feature>`), and its full inline definition (`uuid` + `<ShortName>`) is independently promoted to a real top-level `Object` component by the same `returnRemainderAddition` mechanism Feature/Character already use. **The gate is enforced entirely by `assetWirePolicy`, the same seam as `exits`/`render`** --- the tag parses in every mode, and the check runs only when `standardizeMode === 'asset'`. It blocks that path on asset forms because settling it requires the same identity-across-merge and identity-across-recache questions presence nodes raise (LG-6) --- two assets authoring the same object into the same room, and what happens to an authored object's identity across a recache --- neither of which is settled. Until those land, the check stays exactly as written above; when they do, it is replaced (not just deleted) by whatever mechanism settles those questions.
...
**`Object` (dual wire contexts):** **`OBJECT#`** is a **`ComponentUUID`** tag; **`StandardObject`** is the merge/storage protocol (**`shortName`** only on pair JSON; Coyote fields stay on **`Meta::Object`**). WML shape **`<Object uuid=(id)><ShortName>label</ShortName></Object>`** (canonical **`OBJECT#...`** in memory). **Two parse contexts:** (1) **room-nested** under **`<Room>`** --- consumed into **`StandardRoom.objects[]`** for affordance wire (**I6**); asset **`StandardForm.validate()`** rejects non-empty **`objects`**. (2) **top-level** under **`<Asset>`** in **ephemeraWire** --- becomes **`StandardObject`** in **`StandardForm._components`** for merge/storage; as of iteration 10 ("object-character-render-hosts"), top-level **`StandardObject`** (including **`situations`**) is allowed in asset mode, and asset mode rejects only a defined **`render`** on it (same rule as Feature/Knowledge). **`schemaToWML`** prints **`uuid=(id)`** via **`stripTypedKey('OBJECT')`**.

**Ephemera-only:** **`Render`** under **`Room`** (DisplayName / Summary / Description) is stored on **`StandardRoom`** as **`render`** in JSON with the same shape as **`SituationRoomFacetPayloadType`** (literal **`displayName`**, **`summary`** and **`description`** as render-tree editable data), not three plain strings. Each field is tag-matched and independently optional (not positional, not a fixed triplet) --- `DisplayName` is validated non-empty only when present; a present-but-empty child collapses to absent on write.
...
- `'Edge'`: Area topology exit endpoints (**`From`** / **`To`** on **`ludicGraph.edges`**; subset **`Stub`** on target Room; distinct from room **`Exit`**)
