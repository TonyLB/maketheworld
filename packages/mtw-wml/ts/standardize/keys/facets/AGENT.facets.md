...
- **Parsing (tag-matched, not positional):** [`parseProseTripletChildren`](./situationRoom.ts) is the shared reader every embedder's `<Render>`/prose-facet consumer defers to --- it resolves `DisplayName` / `Summary` / `Description` independently by tag, in any combination, with no arity or ordering requirement. `Render.finalize` (`schema/converters/components.ts`) collapses a present-but-empty field to absent before this payload is built, so "absent" is the single canonical spelling of "no content" on the wire. Use **`payload.hasDisplayName()`** (was a node present at all) and **`payload.hasNonEmptyDisplayName()`** (non-empty after trim) to distinguish presence from validity --- validate non-emptiness **only when `hasDisplayName()` is true**; an absent DisplayName is valid and is the client's signal to fall back to `Unknown`.
