...
- **References**: Stored in `ReferenceList` types (and related structures such as **`SingleReference`**). Each reference collection (like `features`, `characters`) is maintained separately. **Room** does **not** serialize an **`examples`** list; use **Situation** facets and ephemera **`render`** for Room prose. **Feature** and **Knowledge** are migrating to **`situations`** facet lists (shared **`SituationRoomFacetPayload`** prose triplet, DEFAULT-only in v1); **`examples`** remains in current code until Phase 4 retires **`<Example>`** (see [`taskPlanning/.../AGENT.featureKnowledgeExamples.planning.md`](../../../../../taskPlanning/packages/mtw-wml/standardize/AGENT.featureKnowledgeExamples.planning.md)).
