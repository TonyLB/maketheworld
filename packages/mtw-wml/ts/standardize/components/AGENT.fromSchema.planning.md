...
7. **Integrate two-remainder pipeline with processComponents**
   ...
   - **Two remainders**:
     ...
     - **Return remainder**: Starts empty. Only ReferenceList and (when added) FacetList consumers add to it. For **ReferenceList**: each matched tag (e.g. Feature, Example, Room, Message, Mark under Lens) is added in full so `processComponents` can recurse and create the child component. For **FacetList** (e.g. Position under Map): each matched "Room with Position" yields facet data for the FacetList and a **Room node without Position** (same node, children = remainder after stripping Position) added to the return remainder so `processComponents` builds `StandardRoom` from the rest. Literal/Render/Simple consumers never add to the return remainder.
...
9. **Wire return remainder into processComponents** — **Done (February 2026)**
   - **ReferenceList and facet consumers contribute return remainder:** ReferenceList consumers (Feature, Example, Lens, Room, Message, Moment, etc.) add their matched component tags in full to `returnRemainderAddition` so `processComponents` can recurse and create the child components. Facet-list consumers (`StandardizeConsumerFacetListPosition`, `StandardizeConsumerFacetListMark`) add cleaned component nodes (Rooms without `Position`, Marks without `Match`) to the same channel.
   - **ProcessComponents recurses via factory remainder:** `standardComponentFactory` now returns `{ component, remainder }` for schema-based construction, where `remainder` is the payload’s aggregated return remainder from `fromSchema`. `processComponents` invokes `standardComponentFactory(item)` once per component tag and recurses into `remainder` instead of `item.children`, while preserving existing behavior for Remove/Replace handling, top-level detection, and `referenceCollection` building.
