...
2. **Define a minimal pipeline contract**
   ...
   - Decide where the pipeline runs: inside each payload’s `fromSchema`, or in a shared helper that takes “steps” and “payload” and is called from `fromSchema`. Prefer a pattern that can be reused across Room, Feature, Lens, etc.
