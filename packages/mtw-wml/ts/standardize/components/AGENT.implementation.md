...
- **fromSchema**: `ShortName` (required — schema `finalize` still enforces exactly one), **`StandardizeConsumerFacetListSituation`**, `Render` (`StandardizeConsumerSimple`, same DisplayName/Summary/Description-triplet rule as Feature), inline `ref={0}` children. Schema-layer `typeCheckContents`/`finalize` (`schema/converters/components.ts`) admit **`ShortName`**, **`Situation`**, and (as of iteration 10) **`Replace`/`Remove`** children; **`Render`**'s own parent whitelist (`Render.initialize`) does **not** yet include Object, so `<Render>` cannot be authored under `<Object>` via WML text — the `_render`/`render` field exists for JSON/ephemera-wire construction (mirroring Feature/Knowledge) ahead of that whitelist change, which is deferred to when Object's ephemera-wire render path actually lands.
...
- **Schema layer**: `Character`'s converter has no `typeCheckContents`/`finalize` (unrestricted at parse time already), so no schema-layer change was needed to admit `<Situation>`. `Render`'s parent whitelist does not include Character — same deferred state as Object.
