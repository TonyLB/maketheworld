...
### **StandardRoom** 🟢
- **Purpose**: Represents rooms with exits, situation facets, lens, Feature containment via **`ludicGraph`**, guidance, characters, and optional ephemera **`render`** / **`objects`**
...
- **Reference Properties**: **`lens`** (`SingleReference`); **`ludicGraph.nodes`** (`ReferenceList`, Feature-only for Room today) via [`StandardLudicGraph`](./ludicGraph.ts) — same field/type **Area** uses (Area is no longer the only component with a `ludicGraph`; Room's `ludicGraph.edges` stays structurally empty, it never gets an `Exit` consumer); **`guidance`**, **`characters`** (`ReferenceList`). **No** serialized **`examples`** field on **`StandardRoomData`**. Room prose is **Situation** facets and optional ephemera **`render`**.
...
- **Reference Properties**: Heterogeneous **`ludicGraph.nodes`** (`ReferenceList`) with tags `Area`, `Room`, `Feature`, `Character`. **`ludicGraph.edges`**: [`ExitEdgeList`](../keys/edges/exitEdge.ts) (v1 **Exit** member; uuid-keyed merge). See [`../keys/AGENT.referenceList.md`](../keys/AGENT.referenceList.md) and [`../keys/edges/AGENT.edges.md`](../keys/edges/AGENT.edges.md).
...
- **`referencedKeys()`**: Participant refs from `ludicGraph.nodes` as **Direct** / **Dependency**; **`From`** / **`To`** endpoint refs from `ludicGraph.edges` as **`Edge`** (non-structural; subset cascade **`connectionType: 'Edge'`** -> target Room **`Stub`**). See [`standardForm.subset.test.ts`](../integration/standardForm.subset.test.ts) (area edge cascade).
...
- **StandardRoom**: Buckets Lens, Feature (into `ludicGraph.nodes`), Guidance, Character (`Example` is **not** a reference-list bucket on Room; inline **`ref={0}`** Examples are tracked separately for schema and subset; see **StandardRoom** section above)
...
**Example**: `StandardRoom` (multiple **`ReferenceList`** buckets: `ludicGraph.nodes` (Feature), `guidance`, `characters`, plus **`lens`**, **`situations`**, and optional **`render`**; no persisted **`examples`** list; see **StandardRoom** at top of this file)
