...
- **`referencedKeys()`**: Participant refs from `positionGraph.nodes` as **Direct** / **Dependency** (edge endpoint **`Edge`** refs deferred to D7 slice).
...
- **Follow-on (M1 remaining):** D4 endpoint-in-`nodes` validation; D7 **`referenceType: 'Edge'`** subset cascade. Ephemera / **RoomAffordances** projection in Milestone 3+ ([`AGENT.areaTopologyExits.planning.md`](../../../../taskPlanning/packages/mtw-wml/AGENT.areaTopologyExits.planning.md)).
