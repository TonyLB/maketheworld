...
- **`ludicGraph`**: Object carries `_ludicGraph` (`StandardLudicGraph`, aligned tagged-union shape — see **StandardArea** below), the same accessor pattern as Area/Room, and Object-in-Object nesting is authorable (its schema converter's context guard and `typeCheckContents` were widened for this). This is a **scoping projection**, not a display one: it says what the player *may know* is nested inside the object (a subgraph of a graph), decided server-side; it says nothing about *how* that's rendered — that's a separate client-side display projection over the returned subgraph, never collapsed into this field.
...
- **fromSchema**: ShortName, Pronouns, DisplayName, Image, **`StandardizeConsumerFacetListSituation`**, `Render` (`StandardizeConsumerSimple`; children are tag-matched and each independently optional — see **StandardObject**'s `fromSchema` bullet above for the full rule). No **`StandardizeConsumerInline()`** in the pipeline (pre-existing; unrelated to this addition) — unconsumed non-Situation/Render child tags still fail parse.
- **`ludicGraph`**: Character carries `_ludicGraph` (`StandardLudicGraph`, same aligned shape as Area/Room/Object/Feature — see **StandardArea** below), folded in alongside Object/Feature since it is structurally identical and ephemera already had the precedent (`EphemeraMetaCharacter.ludicGraph`). Character has no `equals` override by design (relies on the base class's `deepEqual(toJSON())`), and `ludicGraph` needed no special-casing there since it is already in `toJSON()`. The self-reference/`rootId` guard (`assertRootIdConsistency`) is **not** ported to Character — that guard is Area's inversion of a pre-existing guard; Room/Feature/Object follow Room's precedent (no such guard), not Area's.
...
### **StandardRoom** 🟢
- **Purpose**: Represents rooms with exits, situation facets, lens, Feature/Object membership via **`ludicGraph`**, guidance, characters, and optional ephemera **`render`**
...
- **Reference Properties**: **`lens`** (`SingleReference`); **`ludicGraph.nodes`** via [`StandardLudicGraph`](./ludicGraph.ts) — same aligned tagged-union shape **Area** uses (see **StandardArea** below for the field-by-field description). **No longer Feature-only**: Room nests both `Feature` and `Object` nodes (Object landed in Slice 5a of the alignment work, with its own `<Object>` `StandardizeConsumerReferenceList`, mirroring `Feature`'s). `roomFeaturesListAccessor` (Workbench) filters `ludicGraph.nodesByTag('Feature')`, non-root, on read and writes back through `withComponentRefs` rather than reading/writing the raw list, so it no longer clobbers Character/Object/root/Presence nodes already on the graph. Room's `ludicGraph.edges` stays structurally empty in practice — it has the `<Exit>` consumer available on the shared type but Area authors topology, not Room; **`guidance`**, **`characters`** (`ReferenceList`). **No** serialized **`examples`** field on **`StandardRoomData`**. Room prose is **Situation** facets and optional ephemera **`render`**.
...
- **Ephemera wire (room contents)**: **`StandardRoomObjectData`**/the flat **`_objects`** field no longer exist. Room's contents line is derived server-side from the ludicGraph itself: [`roomHeaderPhaseC.ts`](../../../../../charcoal-client/src/slices/messages/roomHeaderPhaseC.ts) reads `ludicGraph.nodesByTag('Object')` (plus sibling `StandardObjectData` rows shipped alongside, mirroring the existing `characters` pattern) rather than a separate `objects` projection — one mechanism instead of two. Asset **`StandardForm.validate()`** still blocks authored Room objects via `assetWirePolicy.ts`'s `ludicGraph.nodesByTag('Object')` check (LG-7's `not yet` gate — see [`AGENT.md`](../AGENT.md)'s asset-wire-policy section — replacing the old objects-array check, same enforcement style as `exits`/`render`: parses in every mode, blocked only in asset mode).
- **fromSchema**: Uses the process-and-remainder pipeline. Consumers include ShortName, Exit, Lens, Feature (routed into **`ludicGraph.nodes`**), Object (routed into **`ludicGraph.nodes`**, its own `<Object>` records graph membership while `processComponents`'s `returnRemainderAddition` mechanism independently promotes the same tag to a real top-level `StandardObject`), Situation (facet list), Guidance, Character, Position (no-op), Grant, DisplayName (no-ops for backward compatibility), plus **`Render`**. Unconsumed **`<Example>`** (tag removed) fails parse. See [fromSchema: process-and-remainder pipeline](#fromschema-process-and-remainder-pipeline) below.
...
- **`ludicGraph`**: Feature carries `_ludicGraph` (`StandardLudicGraph`, same aligned shape as Area/Room/Object — see **StandardArea** below), including Feature-in-Feature nesting (a member node on the parent's `ludicGraph.nodes`; the relational `PartOf`/`In` edge kinds are typed but have no WML surface tag or author path yet). Same scoping-projection rule as **StandardObject** above: `ludicGraph` says what's nested, server-side and knowledge-gated, never how it's displayed.
...
### **StandardArea** 🟢
- **Purpose**: Large spatial regions (districts, biomes, building complexes, etc.). v1 models **participation in space** via participant references and **topology edges** on `ludicGraph`.
- **Content Properties**: `shortName` (`StandardLiteral`); **`ludicGraph`** via [`StandardLudicGraph`](./ludicGraph.ts) (see [`dataTypes/ludicGraph.ts`](./dataTypes/ludicGraph.ts)).
- **Reference Properties**: **`ludicGraph.nodes`** is a tagged union (`LudicGraphNodeListData`, LG-8), aligned field-for-field to ephemera's `EphemeraLudicGraphNode`: a `ReferenceList` of component-reference nodes (tags `Area`, `Room`, `Feature`, `Character`, `Object` --- `LUDIC_GRAPH_NODE_TAGS`) plus a presence-structure-node arm (typed and unit-exercised; no WML surface tag authors one yet). **The host's own node is required in `nodes` as the designated root** (`rootId`, present-in-`nodes`) --- this **inverts** the older self-reference guard, which used to forbid exactly that; `assertRootIdConsistency` asserts the positive invariant now. `referenceKeys()` derives `Direct` references from the **non-root, non-presence** component nodes only --- the root is excluded because it is the host itself, and a presence node references no component. **Schema rendering follows the same rule** (fixed 2026-09-22, `StandardLudicGraph.excludeRoot`/`nonRootComponentRefs`): every host class's `nestedSchema()` (the path `StandardForm.schema` actually exercises for top-level components) and `.schema()` strip the root out of `nodes` before rendering children --- a self-referencing `<Object uuid=(id)/>` inside its own `<Object uuid=(id)>` is both semantically empty and structurally invalid on re-parse (a bare reference has no `ShortName`, which every host tag's converter requires unconditionally). Before this fix, `toWireLudicGraphFull`'s root-inclusive projection (correct for the ephemera-side data) produced exactly that once serialized --- latent in the shipped Room case (`lambda/ephemera/internalCache/affordanceRoomDeliverable.ts`) until something finally re-parsed a delivered `ludicGraph` (`lambda/ephemera`'s `AGENT.nestedObjectLook.planning.md`, Phase 1). **`ludicGraph.edges`** is a heterogeneous `LudicEdgeList` ([`../keys/edges/ludicEdge.ts`](../keys/edges/ludicEdge.ts)) over Topology (`Navigation` --- the `<Exit>` surface tag, plus the unauthored `Bearing`), Membership (`In`/`On`/`PartOf`), and Peer (`Under`/`Against`/`Custom`) kinds --- `kind` is the sole discriminant, no `category` field restates it; `<Exit>` still parses to `kind: 'Navigation'` and is the only kind with a WML surface tag and author path. See [`../keys/AGENT.referenceList.md`](../keys/AGENT.referenceList.md) and [`../keys/edges/AGENT.edges.md`](../keys/edges/AGENT.edges.md).
- **JSON**: `ludicGraph: { nodes?, edges? }`; omit `ludicGraph` when both are empty (omission-over-empty). **No default root node** — a host with no participant refs and no edges serializes with `ludicGraph` absent entirely (same rule applies to **StandardRoom**'s `ludicGraph.nodes`). This is an asset-side/WML-layer convention only: the **ephemera-side** `EphemeraLudicGraph` (`lambda/ephemera/dataSource/positions/`) has the opposite invariant — every graph always carries its own root node in `nodes`, even for a childless host (concepts clause 3, [`AGENT.concepts.md`](../../../../../lambda/ephemera/dataSource/positions/AGENT.concepts.md#wholes-parts-and-ports)). Don't port that expectation onto `StandardLudicGraph`; the two layers deliberately disagree here.
...
- **fromSchema**: Four `StandardizeConsumerReferenceList` consumers append into `ludicGraph.nodes`; **`StandardizeConsumerSimple`** for **`Exit`** appends into **`ludicGraph.edges`** with **area exit endpoint tags** asset-mode validation (reject **`to=`**, require **`uuid`** + endpoint children). Strict remainder for unknown tags. **Self-reference** in `nodes` throws. Tests: [`area.test.ts`](./area.test.ts), [`../keys/edges/exitEdge.test.ts`](../keys/edges/exitEdge.test.ts).
- **`referencedKeys()`**: Non-root, non-presence participant refs from `ludicGraph.nodes` as **Direct** only (LG-8: the inert `Dependency` duplicate was dropped in the alignment work — nothing consumed it); **`From`** / **`To`** endpoint refs from `ludicGraph.edges` as **`Edge`** (non-structural; subset cascade **`connectionType: 'Edge'`** -> target Room **`Stub`**). See [`standardForm.subset.test.ts`](../integration/standardForm.subset.test.ts) (area edge cascade).
- **Participant endpoint rule**: When both endpoints resolve, at least one must match a ref in `ludicGraph.nodes` (`sameKey`); portal edges allowed. **Not** a standardize hard error --- use [`areaTopologyValidation.ts`](./areaTopologyValidation.ts) for warnings/lint. Incomplete edges are valid in storage. **Semantic filter:** [`projectRoomExits`](../projection/projectRoomExits.ts) skips edges that cannot produce a facet. See [`../keys/edges/AGENT.edges.md`](../keys/edges/AGENT.edges.md#incomplete-edges-and-projection).
...
### Provenance is orthogonal to component kind

**Do not build kind-specific provenance assumptions into a facet shape.** Whether a component's prose was *authored* by a human or *generated* by a model is a property of the individual case, never of the tag. The current split is transitional and already has counterexamples in both directions: Objects are improvisation-spawned today but are destined for authoring, and Characters are authored today but can already be generated (Guest characters, see [`lambda/ephemera/guestCharacter/AGENT.md`](../../../../../lambda/ephemera/guestCharacter/AGENT.md)). So `situations`/`render` are deliberately the *same* `SituationProseFacetList`/`SituationProseFacetPayload` shape on Room, Feature, Knowledge, Object and Character --- only the *source* of the prose differs per case, and the facet must not encode which source it came from.

Established as a design decision in iteration 10 of the object-manipulation ladder (2026-08-02), when Object and Character gained facet support. The practical test: if a proposed field or branch would answer "was this authored or generated?" from the component's *tag*, it is wrong.

A related distinction worth keeping straight, since it is easy to collapse: **spawn-time generation and render-time generation are different mechanisms.** Spawn-time generation writes prose into the improvisation pair row, which then participates in merge and hydrates through the ordinary authored-catalog path --- the content is authored-*equivalent* by render time. Render-time generation (`allowGeneration`, the path Room uses for non-authored mark states) is untouched by any of this and remains room-only.
...
- **StandardRoom**: Buckets Lens, Feature and Object (both into `ludicGraph.nodes`), Guidance, Character (`Example` is **not** a reference-list bucket on Room; inline **`ref={0}`** Examples are tracked separately for schema and subset; see **StandardRoom** section above)
...
**Area:** mtw-base `SchemaAreaTag` / `isSchemaArea` shipped; **`StandardArea`** and **`StandardLudicGraph`** implemented — see [**StandardArea**](#standardarea-) above and [`schema/converters/components.ts`](../../schema/converters/components.ts) (`PrefixKey` `'AREA'` in mtw-utilities).
...
**Example**: `StandardRoom` (multiple **`ReferenceList`** buckets: `ludicGraph.nodes` (Feature and Object), `guidance`, `characters`, plus **`lens`**, **`situations`**, and optional **`render`**; no persisted **`examples`** list; see **StandardRoom** at top of this file)
