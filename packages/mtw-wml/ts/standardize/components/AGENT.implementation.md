...
- **Participant endpoint rule**: Each Exit in `positionGraph.edges` must have at least one endpoint matching a ref in `positionGraph.nodes` (`sameKey`). Portal edges (one endpoint inside, one outside) are allowed. Enforced on **`fromSchema`** (strict), **`merge`** (strict), and **`fromJSON`** when local **`nodes`** are present (overlay layers with edges-only skip until merge). See [`../keys/edges/AGENT.edges.md`](../keys/edges/AGENT.edges.md#topology-invariants).
