...
- **Follow-on (out of v1 scope)**: graph **edges** and coordinates; general Area **cycle** detection (only self-reference is enforced today); ephemera / **RoomAffordances** play impact; Workbench heterogeneous **ReferenceList** editor for `nodes`. **M1 (area topology exits):** mtw-base + mtw-wml **schema parse/print** for D29 Exit shape (`uuid=` + **From** / **To** / **Forward** / **Back**, layered **Replace** on endpoints) is done; **`positionGraph.edges`** standardization and dual-read Standardize behavior remain pending ([`AGENT.areaTopologyExits.planning.md`](../../../../taskPlanning/packages/mtw-wml/AGENT.areaTopologyExits.planning.md)).
