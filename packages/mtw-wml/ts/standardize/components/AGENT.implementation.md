...
**Area (2026-05-28):** Step 1 is complete in mtw-base (`SchemaAreaTag`, `isSchemaArea`, `SchemaWithKey`, asset-legal + importable, `AREA#` via `isSchemaComponentUUID`). **`StandardPositionGraph`** is implemented in [`positionGraph.ts`](./positionGraph.ts) with data type [`dataTypes/positionGraph.ts`](./dataTypes/positionGraph.ts): JSON shape `{ nodes?: ReferenceListData }` (omit via `toJSON()` when empty); `merge` / `diff` / `equals` delegate to `_nodes` `ReferenceList` (**P2**); `nodesByTag()` filters heterogeneous participants (`Area`, `Room`, `Feature`, `Character`) for WML serde. WML converters, `PrefixKey` `'AREA'`, `StandardArea`, and factory wiring are follow-on (see [`taskPlanning/packages/mtw-wml/AGENT.areaTag.planning.md`](../../../../taskPlanning/packages/mtw-wml/AGENT.areaTag.planning.md)).
