...
describe('StandardKey', () => {
    it('should return a clone for format "both"', () => {
        const refData: StandardReferenceData = { key: 'test', universalKey: 'ROOM#1234' }
    ...
    it('should not throw if universalKey is missing for format "universalKey"', () => {
        const refData: StandardReferenceData = { key: 'test' }
        const key = new StandardKey(refData)
        const formatted = key.toFormat('universal')
        expect(formatted.key).toBe('test')
        expect(formatted.tag).toBeUndefined() // tag cannot be derived without universalKey
...
describe('StandardReference', () => {
    ...
    it('should construct StandardReference from StandardReferenceData', () => {
        const testReferenceData: StandardReferenceData = { key: 'test' }
    ...
    it('should correctly judge equality when only key specified', () => {
        ...
        expect(testReference.equal(new StandardReference({ key: 'test' }))).toBe(true)
    })
    ...
    it('should correctly judge equality when only universalKey specified', () => {
        ...
        const testReference = new StandardReference(testReferenceData)
        expect(testReference.equal(new StandardReference({ universalKey: 'ROOM#1234' }))).toBe(true)
    })
    ...
    it('should correct judge inequality when key differs', () => {
        ...
        const testReference = new StandardReference(testReferenceData)
        expect(testReference.equal(new StandardReference({ key: 'test2', universalKey: 'ROOM#1234' }))).toBe(false)
    })
    ...
    it('should correct judge inequality when universalKey differs', () => {
        ...
        const testReference = new StandardReference(testReferenceData)
        expect(testReference.equal(new StandardReference({ key: 'test', universalKey: 'ROOM#5678' }))).toBe(false)
    })
    ...
    it('should correctly judge inequality when tags differ', () => {
        ...
        const testReference = new StandardReference(testReferenceData)
        expect(testReference.equal(new StandardReference({ key: 'test', universalKey: 'EXAMPLE#1234' }))).toBe(false)
    })
    ...
    it('should correctly lookup keys in reference callback', () => {
        ...
        expect(testSimple.lookup(callback).toJSON()).toEqual({ key: 'room1', universalKey: 'ROOM#Room1'})
        const testRemove = new StandardReference('<Remove><Room key=(room2) /></Remove>')
        expect(testRemove.lookup(callback).toJSON()).toEqual({ tag: 'Remove', match: { key: 'room2', universalKey: 'ROOM#Room2' } })
        ...
        expect(testReplace.lookup(callback).toJSON()).toEqual({
            tag: 'Replace',
            match: { key: 'room2', universalKey: 'ROOM#Room2' },
            payload: { key: 'room3', universalKey: 'ROOM#Room3' }
        })
    })
        ...
        const testSimple = new StandardReference('<Room key=(room1) />')
        expect(testSimple.lookup(keys).toJSON()).toEqual({ key: 'room1', universalKey: 'ROOM#Room1'})
        const testRemove = new StandardReference('<Remove><Room key=(room2) /></Remove>')
        expect(testRemove.lookup(keys).toJSON()).toEqual({ tag: 'Remove', match: { key: 'room2', universalKey: 'ROOM#Room2' } })
        const testReplace = new StandardReference('<Replace><Room key=(room2) /></Replace><With><Room key=(room3) /></With>')
    ...
    it('should correctly format a simple reference', () => {
        ...
        expect(testSimple.toFormat('both').toJSON()).toEqual({ key: 'room1', universalKey: 'ROOM#Room1' })
        expect(testSimple.toFormat('key').toJSON()).toEqual({ key: 'room1' })
    ...
    it('should correctly format a remove reference', () => {
        ...
        expect(testSimple.toFormat('both').toJSON()).toEqual({ tag: 'Remove', match: { key: 'room1', universalKey: 'ROOM#Room1' } })
        expect(testSimple.toFormat('key').toJSON()).toEqual({ tag: 'Remove', match: { key: 'room1' } })
    ...
    it('should correctly format a replace reference', () => {
        ...
        expect(testSimple.toFormat('both').toJSON()).toEqual({
            tag: 'Replace',
            match: { key: 'room1', universalKey: 'ROOM#Room1' },
            payload: { key: 'room2', universalKey: 'ROOM#Room2' }
        })
...
describe('ReferenceList', () => {
    ...
    it('should correctly format references to both', () => {
        ...
        expect(testList.toFormat('both').toJSON()).toEqual([
            { key: 'room1', universalKey: 'ROOM#Room1' },
            { key: 'room2', universalKey: 'ROOM#Room2' },
            { key: 'room3', universalKey: 'ROOM#Room3' }
        ])
    })
