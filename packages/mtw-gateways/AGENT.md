...
| **Component topology (room exit projection)** | N/A (compute-only; composes **`ComponentAggregateMergedCache`** + **`projectRoomExits`** in **`mtw-wml`**). Steady-state invalidations from [`lambda/assets/componentTopology/`](../../lambda/assets/componentTopology/) (`mtw.assets.componentTopology`). | **Primary (M4 wiring):** register **`createComponentTopologyCacheHandler({ ComponentAggregate })`** on **`internalCache.ComponentTopology`** (Ephemera; diagnostics future). **Consumers:** planned **`mtw.ephemera.affordanceCache`** hydrate-on-demand via **`internalCache.ComponentTopology.get(...)`** (not on invalidation receive). | [**Component topology read surfaces**](#component-topology-read-surfaces-primary-vs-secondary) below. **`ProjectedRoomTopology`**; **`filterAreaEdgeReferrers`**. Initiative: [`AGENT.areaTopologyExits.planning.md`](../../taskPlanning/packages/mtw-wml/AGENT.areaTopologyExits.planning.md). Deep import: `@tonylb/mtw-gateways/ts/assets/components/componentTopology`. |
...
**Primary (shipped):** **`ComponentTopologyMergedCache`** / **`createComponentTopologyCacheHandler({ ComponentAggregate })`** in [`ts/assets/components/componentTopology/factory.ts`](ts/assets/components/componentTopology/factory.ts). Keyed by **`roomUniversalKey` + `computePerspectiveKey(mergeParticipationOrder)`** (see **`componentTopologyPerspectiveCacheKey`** in [`keys.ts`](ts/assets/components/componentTopology/keys.ts)). Ephemera registers on **`internalCache.ComponentTopology`** (Milestone 4); **`mtw.ephemera.affordanceCache`** calls **`get(...)`** inside hydrate-on-demand preflight (**`ensureAffordanceTopology`**, not on **`TopologyInvalidated`** receive).
