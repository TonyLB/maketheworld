...
| **Positions play graph (slice 1 flat projection; slice 2 schema landed)** | [`lambda/ephemera/dataSource/positions/membership/`](../../lambda/ephemera/dataSource/positions/membership/) membership persistence API. | **Primary:** **`createPositionsCacheHandler(ephemeraDB)`** on **`internalCache.Positions`** (Ephemera via **`PositionsData`**). **Consumers:** **`AffordanceRoomDeliverable.get`** roster via **`getRoomRoster`**; navigate parse + membership pre-read via **`getMembershipContainers`**. | Module doc: [`ts/ephemera/positions/AGENT.md`](ts/ephemera/positions/AGENT.md) (**Storage schema (slice 2)**). Deep import: `@tonylb/mtw-gateways/ts/ephemera/positions`. Slice 2 factory backing swap pending; types + adjacency query helpers shipped. |
