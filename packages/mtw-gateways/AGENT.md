...
| --- | --- | --- | --- |
| **Component data (pair-addressed reads)** | [`lambda/assets/dataSource/caching/`](../../lambda/assets/dataSource/caching/) ([`cacheAsset`](../../lambda/assets/dataSource/caching/cacheAsset.ts) maintains universal-key component rows in `assetDB`). | Ephemera, assets, and diagnostics: **`internalCache.ComponentData`** via **`createComponentDataCacheHandler`**. On ephemera, **`ComponentData`** is an **internalCache-only** composite router (assetDB + improvisation); assets/diagnostics unchanged. | **Normative:** [`componentData/`](ts/assets/components/componentData/index.ts) --- **`createComponentDataCacheHandler`** (pair **`getItems`**), **`authoritativeFromParticipationOrder`**. Pair persisted-field shape is shared via **`ComponentPairPersistedFields`** / **`EphemeraDbGetItemsComponentRow`** (ephemera improvisation rows reuse the same merge-body fields; **table routing** is ephemera **`internalCache`** composite-handler concern --- not a separate gateway row). **Maintenance-only partition scan:** subpath [`componentData/exhaustiveScan`](ts/assets/components/componentData/exhaustiveScan.ts) + [`exhaustiveScanCache`](ts/assets/components/componentData/exhaustiveScanCache.ts) (not barrel-exported). Deep import: `@tonylb/mtw-gateways/ts/assets/components/componentData`. |
...
| **Positions play graph (topology + adjacency)** | [`lambda/ephemera/dataSource/positions/membership/`](../../lambda/ephemera/dataSource/positions/membership/) membership persistence API. | **Primary:** **`createPositionsCacheHandler(ephemeraDB)`** on **`internalCache.Positions`**. **Consumers:** navigate parse + membership pre-read via **`getMembershipContainers`**; roster **presentation** via ephemera **`getRoomCharacterList`** + **`AffordanceRoomDeliverable`** (not gateway authority). | Module doc: [`ts/ephemera/positions/AGENT.md`](ts/ephemera/positions/AGENT.md). Graph roles: [`positions/AGENT.concepts.md`](../../lambda/ephemera/dataSource/positions/AGENT.concepts.md#graph-roles-shared-shape-different-authority). Deep import: `@tonylb/mtw-gateways/ts/ephemera/positions`. Topology + adjacency read only. |
| **Improvisation component data (ephemeraDB pair rows)** | [`lambda/ephemera/dataSource/objects/`](../../lambda/ephemera/dataSource/objects/) improvisation persistence coordinators. | **Primary:** **`createImprovisationComponentDataCacheHandler(ephemeraDB)`** on **`internalCache.ImprovisationComponentData`**. **Consumers:** ephemera **`internalCache`** composite **`ComponentData`** router; aggregate merge when **`ASSET#IMPROVISATION`** is last in participation order. | Module doc: [`ts/ephemera/improvisation/AGENT.md`](ts/ephemera/improvisation/AGENT.md). Pair merge-body fields shared with **`componentData`**; **`Meta::Object`** play meta is **`internalCache.ObjectEphemeraMeta`** (lambda-local). Composite **`ComponentData`** routing stays on ephemera **`internalCache`**, not this package. Deep import: `@tonylb/mtw-gateways/ts/ephemera/improvisation`. |
| **Object embedding (`EMBEDDING#IMPROMPTU` adjacency rows)** | [`lambda/ephemera/dataSource/objects/`](../../lambda/ephemera/dataSource/objects/) improvisation persistence coordinators (spawn/update **`Put`**; delete **`Delete`**). | **Primary:** **`createObjectEmbeddingCacheHandler(ephemeraDB)`** on **`internalCache.ObjectEmbedding`**. **Consumers:** ephemera actions catalog ingress ([`handleParseRequested`](../../lambda/ephemera/dataSource/actions/index.ts) via [`attachEmbeddingsToCatalogEntries`](../../lambda/ephemera/dataSource/actions/attachEmbeddingsToCatalogEntries.ts)); identity embedding fast path via pre-attached catalog vectors. | Module doc: [`ts/ephemera/objectEmbedding/AGENT.md`](ts/ephemera/objectEmbedding/AGENT.md). v1 reads **`EMBEDDING#IMPROMPTU`** only; handler name is scope-neutral for future authored-object embedding rows. Memo **`set`** / **`invalidate`** via [`invalidateImprovisationObjectCaches.ts`](../../lambda/ephemera/dataSource/objects/invalidateImprovisationObjectCaches.ts). Deep import: `@tonylb/mtw-gateways/ts/ephemera/objectEmbedding`. |
...
**Cache-host gate:** **`isCacheHostEphemeraId`** in [`componentExamples/input.ts`](ts/assets/components/componentExamples/input.ts) admits **`ROOM#`, `FEATURE#`, `KNOWLEDGE#`, `OBJECT#`, and `CHARACTER#`**; **`validateAssembleComponentExamplesInput`** throws on anything else. **`isCacheHostWithSituationFacets`** in [`perspectives.ts`](ts/assets/components/componentExamples/perspectives.ts) mirrors that set at the `StandardComponent` level (`StandardRoom | StandardFeature | StandardKnowledge | StandardObject | StandardCharacter`) --- widen both in lockstep if a new kind joins. `resolveRoomLensMarkDefaults` defaults to `isEphemeraRoomId(hostUniversalKey)` (**A4**), so Object/Character join Feature/Knowledge in the no-lens-defaults group automatically.
