...
| **Render cache (`CACHE#` + `Cache::` catalog rows)** | [`lambda/ephemera/dataSource/renderCache/`](../../lambda/ephemera/dataSource/renderCache/) (**`mtw.ephemera.renderCache`** DataSource). | **Primary:** **`createRenderCacheCacheHandler(ephemeraDB)`** on **`internalCache.RenderCache`** (Ephemera via **`RenderCacheData`** + **`getExactMatch`**; diagnostics get-only). | [`ts/ephemera/renderCache`](ts/ephemera/renderCache/index.ts): **`queryCacheRowsForComponent`**, **`queryCatalogRowsForComponent`**, **`fetchCatalogRow`**; catalog guards; **`classifyAuthoredCatalogDrift`**; **`RenderCacheCacheHandler`** with **`getCacheRows`**, **`getCatalogRows`**, **`getCatalogRow`**, memo **`set`** / **`deleteCacheRecords`** / **`invalidate`** (no Dynamo in memo APIs). Initiative: [`AGENT.onDemandAuthoredExamples.planning.md`](../../taskPlanning/lambda/ephemera/dataSource/renderCache/AGENT.onDemandAuthoredExamples.planning.md). Deep import: `@tonylb/mtw-gateways/ts/ephemera/renderCache`. |
