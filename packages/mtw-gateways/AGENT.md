...
| **Positions play graph (slice 2 graph + adjacency)** | [`lambda/ephemera/dataSource/positions/membership/`](../../lambda/ephemera/dataSource/positions/membership/) membership persistence API. | **Primary:** **`createPositionsCacheHandler(ephemeraDB)`** on **`internalCache.Positions`** (Ephemera via **`PositionsData`**). **Consumers:** **`AffordanceRoomDeliverable.get`** roster via ephemera **`getRoomRoster`** (hydrate from **`CharacterMeta`** + **`CharacterSessions`**); navigate parse + membership pre-read via **`getMembershipContainers`**. | Module doc: [`ts/ephemera/positions/AGENT.md`](ts/ephemera/positions/AGENT.md) (**Slice 2 backing**). Deep import: `@tonylb/mtw-gateways/ts/ephemera/positions`. Factory reads stored `positionGraph` topology + adjacency (transitional fallbacks until S2-6). |
