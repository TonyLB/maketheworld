...
| **Positions play graph (slice 1 flat projection)** | [`lambda/ephemera/dataSource/positions/membership/`](../../lambda/ephemera/dataSource/positions/membership/) membership persistence API. | **Primary:** **`createPositionsCacheHandler(ephemeraDB)`** on **`internalCache.Positions`** (Ephemera via **`PositionsData`**). **Consumers:** **`AffordanceRoomDeliverable.get`** roster via **`getRoomRoster`**. | Module doc: [`ts/ephemera/positions/AGENT.md`](ts/ephemera/positions/AGENT.md). Deep import: `@tonylb/mtw-gateways/ts/ephemera/positions`. Slice 2 swaps backing read to stored **`Meta::Room.positionGraph`**. |
