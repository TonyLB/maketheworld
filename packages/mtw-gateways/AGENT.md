...
| **Component aggregate (merge assembly)** | Composed on read from authoritative rows in [`lambda/assets/dataSource/caching/`](../../lambda/assets/dataSource/caching/) (per-asset component bodies) and [`lambda/assets/dataSource/components/verticals/`](../../lambda/assets/dataSource/components/verticals/) (`Meta::Import` hops). No separate Dynamo projection for merged blobs in v1. | (Planned) assets [`internalCache` aggregate handler](../../lambda/assets/internalCache/AGENT.md) (`DeferredCache` over sibling **`ComponentData`** / **`ComponentVerticals`**); future [`fetchImportDefaults`](../../lambda/assets/fetchImportDefaults/index.ts). | **Compute-only** [`aggregate/`](ts/assets/components/aggregate/index.ts): **`createComponentAggregateGateway(slice)`** returns **`{ gateway }`** where **`ComponentAggregateInternalCacheSlice`** names **`ComponentData`** and **`ComponentVerticals`** like assets **`internalCache`**; **`createAggregateGateway(deps)`** remains for **`AggregateGatewayDeps`** / **`ImportVerticalConsistencyAnalyzerDeps`** field names. Pure merge in [`assemble.ts`](ts/assets/components/aggregate/assemble.ts). Initiative: [`AGENT.componentAggregate.planning.md`](../../taskPlanning/lambda/assets/AGENT.componentAggregate.planning.md). Deep import: `@tonylb/mtw-gateways/ts/assets/components/aggregate`. |
...
| **`factory.ts`** | **`createComponentAggregateGateway(slice)`** returning a **`{ gateway }`** bundle (extensible); **`createAggregateGateway(deps)`** where analyzer-shaped **`deps`** is still supported. |
