...
- **Optional gateway factories**: `createXGateway(deps)` returning an object (`get`, `getAcrossAssets`, etc.) where `deps` injects either a **data-store** slice (projection-read gateways; e.g. `assetDB`) or **loader ports** (compute-only gateways; see [**Projection-read vs compute-only gateways**](#projection-read-vs-compute-only-gateways)) so each lambda wires the gateway into its own per-invocation `InternalCache` once.
