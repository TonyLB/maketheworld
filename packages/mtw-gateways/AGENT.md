...
- **No lambda fetch/cache orchestration exported from here.** Consistency-style objects (see [**Consistency analyzers: contract vs composition**](#consistency-analyzers-contract-vs-composition) below) declare **structural** injected interfaces only; **how** each lambda satisfies them---one Dynamo **`Query`** vs several, **`DeferredCache`**, promise memoization, intentional duplicate reads---is **caller-owned**. Do not ship shared wiring factories from **`mtw-gateways`** that encode one lambda's preferred strategy as the canonical recipe (that blurs boundaries even when imports remain acyclic).
...
**Shipped exports** (component verticals module): **`deriveRawImportVerticalHopsFromComponents`**, **`salvageImportVerticalHops`**, **`RawImportVerticalHop`** ([`ts/assets/components/verticals/salvage.ts`](ts/assets/components/verticals/salvage.ts)). **`META_IMPORT_PREFIX`** ([`keys.ts`](ts/assets/components/verticals/keys.ts)) is the stable `Meta::Import::...` prefix string.
...
**Consistency analyzer (import vertical prototype):** **`ImportVerticalConsistencyAnalyzer`** ([`consistency.ts`](ts/assets/components/verticals/consistency.ts)) exposes **`async check(universalKey)`** and getters reading stored **findings** (classification, expected vs existing category sets, **`categoriesToAdd`** / **`metaRowsToDelete`** repair intent). Dependencies are **two narrow structural interfaces** only---**`ImportVerticalAuthoritativeComponentDataLoader`** (**`get(ComponentIds)`** returning **`ImportVerticalAuthoritativeComponentData`**, same contract as assets **`internalCache.ComponentData`**) and **`ImportVerticalMetaImportProjectionLoader`** (projected **`Meta::Import::...`** rows). Each lambda passes implementations that match those shapes (**`assetDB`** wrappers, cache-aligned surfaces, tests); **`InternalCache`** types never appear in this package. The class runs the same **derive / salvage / `metaImportDataCategory`** pipeline as heal and diagnostics; it does **not** call Dynamo or mutate caches.
...
- **In each lambda (composition):** constructing **`ImportVerticalConsistencyAnalyzerDeps`**, choosing **`assetDB`** vs **`InternalCache`** vs both, and any memoization so **`check()`** (which awaits **both** loaders in parallel) does not accidentally duplicate work unless that is an explicit, documented trade. Reader-facing **`internalCache` / `AGENT.md`** files describe those choices per lambda.
- **Anti-pattern:** exporting a helper from **`mtw-gateways`** that bakes in one deployment's Dynamo layout or cache layering as the default wiring. That does not reverse TypeScript imports by itself, but it **does** let this package dictate lambda internals by convention---keep composition at the call site.
