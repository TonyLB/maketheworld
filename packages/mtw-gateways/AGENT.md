...
| --- | --- | --- | --- |
| **Component data (pair-addressed reads)** | [`lambda/assets/dataSource/caching/`](../../lambda/assets/dataSource/caching/) ([`cacheAsset`](../../lambda/assets/dataSource/caching/cacheAsset.ts) maintains universal-key component rows in `assetDB`). | Ephemera, assets, and diagnostics: **`internalCache.ComponentData`** via **`createComponentDataCacheHandler`**. Diagnostics may still use deprecated shim imports until slice 269 ([`AGENT.componentDataConsolidation.planning.md`](../../taskPlanning/packages/mtw-gateways/AGENT.componentDataConsolidation.planning.md)). | **Normative:** [`componentData/`](ts/assets/components/componentData/index.ts) --- **`createComponentDataCacheHandler`** (pair **`getItems`**), **`authoritativeFromParticipationOrder`**. **Maintenance-only partition scan:** subpath [`componentData/exhaustiveScan`](ts/assets/components/componentData/exhaustiveScan.ts) + [`exhaustiveScanCache`](ts/assets/components/componentData/exhaustiveScanCache.ts) (not barrel-exported). **`assetMeta/`** is a deprecated re-export shim; prefer `@tonylb/mtw-gateways/ts/assets/components/componentData`. |
...
| **Primary hot-path API** | **`getAcrossAssets(universalKey, explicitAssetStack)`** --- stack from **`RoomAssets`**, character viewpoint, or canon participation order | **`authoritativeFromParticipationOrder`** via aggregate slice; pair **`get`** / **`getAcrossAssets`** for mirroring and bounded reads |
