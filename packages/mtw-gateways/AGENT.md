...
| **Positions play graph (topology + adjacency)** | [`lambda/ephemera/dataSource/positions/membership/`](../../lambda/ephemera/dataSource/positions/membership/) membership persistence API. | **Primary:** **`createPositionsCacheHandler(ephemeraDB)`** on **`internalCache.Positions`** (Ephemera via **`PositionsData`**). **Consumers:** navigate parse + membership pre-read via **`getMembershipContainers`**; roster **presentation** via ephemera **`getRoomRoster`** + **`hydrateRoomRoster`** + **`AffordanceRoomDeliverable`** (not gateway authority). | Module doc: [`ts/ephemera/positions/AGENT.md`](ts/ephemera/positions/AGENT.md). Graph roles: [`positions/AGENT.concepts.md`](../../lambda/ephemera/dataSource/positions/AGENT.concepts.md#graph-roles-shared-shape-different-authority). Deep import: `@tonylb/mtw-gateways/ts/ephemera/positions`. Topology + adjacency read only (**S2-6**). |
