...
4. **Understand Core Integration Points**
   ...
   - **Key Insight**: CoreExternalFormat is `{ header, update }` only (no duplicated top-level envelope fields). Wire format splits extended fields into `extendedHeader`; consumers always read full `header` after deserialize.
