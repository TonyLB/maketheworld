...
- **personalAssets derives base** from wmlDataSource via `getWMLBase(state, assetId)`; base is injected by `augmentPublicDataForSelect` for selectors and by the `updateStandard` thunk for the reducer.
