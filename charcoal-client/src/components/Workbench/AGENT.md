...
- **Consistency layer (M1 shipped; M2 flush wiring pending)**: **`materializeComponent`**, **`normalizeWorkbenchDraft`**, **`previewOrphanClosure`**, and **`isReferencedInAssetLayer`** on the **local** form (`_topLevel` union `referencedBy` for orphan GC); unit tests in `foundations/consistency/`; see [foundations/consistency/AGENT.md](./foundations/consistency/AGENT.md)
