...
- **Consistency layer (in progress)**: **`materializeComponent`**, **`normalizeWorkbenchDraft`**, **`previewOrphanClosure`**, and **`isReferencedInAssetLayer`** on the **local** form (`_topLevel` union `referencedBy` for orphan GC); see [foundations/consistency/AGENT.md](./foundations/consistency/AGENT.md)
