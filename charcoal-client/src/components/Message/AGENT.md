...
- **Nested contents (nestedObjectLook Phase 3, shipped):** the Object branch also renders a "Contents: ..." line below the description, one level deep, whenever the looked-at object hosts anything (`On`/`In`/`PartOf`, kind-agnostic --- no filtering by relation kind). Source: `component.ludicGraph.nonRootComponentRefs` (`StandardLudicGraph`, `packages/mtw-wml`), which already excludes the root and any `Presence` arm. Each hosted ref's name is resolved via `parsedWML.byUniversalId[ref.universalKey]` (the sibling stub component the server writes into the same `StandardForm`, see [`lambda/ephemera/dataSource/perception/AGENT.md`](../../../../lambda/ephemera/dataSource/perception/AGENT.md#correlated-object-description-policy)) and `componentDisplayLabel` (`lib/componentDisplayLabel.ts`) --- the generic, already-existing label resolver, not a hand-rolled per-tag branch. A hosted ref with no resolvable stub/label is dropped silently, matching the server's own "no nameless reference" rule; absent or empty contents render nothing, no `"Contents: (none)"` placeholder. The Oxford-style join (`"X, Y, and Z"`) is shared with `RoomDescription`'s own floor-objects "Contents:" line via `formatContentsLine` (`slices/messages/roomHeaderContents.ts`); the Object-specific resolver is `formatObjectContentsLine` in the same file.
