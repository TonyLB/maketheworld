...
**Classification helpers:** The per-partition status (`aligned` / `missing` / `orphan` / `stale`) is computed inside the analyzer and surfaced via **`getClassification()`** / `findings.classification`; there is no shared `classifyImportVerticalSets` export. Asset-level **rollup across partitions** (e.g. for diagnostics emission) is owned by the consumer that needs it---see [`lambda/diagnostics/componentVerticalMisalignmentSweep/classification.ts`](../../../lambda/diagnostics/componentVerticalMisalignmentSweep/classification.ts) for the pattern.
...
| **Row typing** | Authoritative analyzer injection uses **`ImportVerticalAuthoritativeComponentData`** (parsed **`byAssets`** per universal id, aligned with [`lambda/assets/internalCache/componentData.ts`](../../../lambda/assets/internalCache/componentData.ts)). Universal-partition **NDJSON query rows** (**`StandardComponentData` + `AssetId` + `DataCategory`**) remain for [`componentRowsFromUniversalPartitionLines`](../../../packages/mtw-gateways/ts/assets/components/assetMeta/dynamoStandardComponents.ts) and adapters that build the cache shape via **`authoritativeComponentDataFromUniversalPartitionRows`**. |
...
| **Classification and lambda-local config** | The **per-partition** taxonomy (`aligned` / `missing` / `orphan` / `stale`) is now an analyzer-internal computation surfaced via **`getClassification()`** / `findings.classification` ([`consistency/index.ts`](../../../packages/mtw-gateways/ts/assets/components/verticals/consistency/index.ts)). The asset-level **rollup** is owned by the diagnostics consumer ([`lambda/diagnostics/componentVerticalMisalignmentSweep/classification.ts`](../../../lambda/diagnostics/componentVerticalMisalignmentSweep/classification.ts)) since the analyzer runs per universal key and the aggregation rule is diagnostics-shaped. |
...
- [X] Place taxonomy where the **call graph** wants it: per-partition status is now an analyzer-internal computation (no `classifyImportVerticalSets` export); asset-level rollup lives next to its sole consumer in **`lambda/diagnostics/componentVerticalMisalignmentSweep/classification.ts`**.
