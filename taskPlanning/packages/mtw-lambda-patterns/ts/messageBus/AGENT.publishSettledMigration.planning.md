...
**Status:** In progress (P5). Q1-Q9 are locked (P0.5 complete; Q9 **`deliveryMode`**: deferred **only** character-move). **Phase P1** complete. **P2a** complete. **P2b** ephemera + assets + **wml** + **connections** + **diagnostics** complete. **P3** complete. **P4** complete (including **P4 ephemera closeout grep** -- zero production `messageBus.send` in `lambda/ephemera/`). **P5 assets** complete (**ASSETS closeout grep**). **P5 wml** complete (**WML closeout grep**). **P5 connections** complete (**CONNECTIONS closeout grep**; cognitoEvent boundary folded in). **P5 diagnostics** complete (**DIAGNOSTICS closeout grep**). Next step: **P5 global grep gate** (see Recommended order).
...
| FETCH-EPH | Shielding | State convergence | Consumer overwrite by `CharacterId` | **Yes** (wasteful) | **Migrated (P4 Easy / Low)** |
| EPH-UPDATE | Contract packaging (wire) | State convergence | Client `updates.forEach`; keyed Redux | **Yes** (extra frames) | **Migrated (P4 Easy / Low)**; optional session coalesce is optimization |
...
| MAP-SUB | Contract packaging | Response shape | `extractReturnValue` overwrites keys; does not union `maps[]` | **No** when multiple bus messages share one invocation | **Migrated (P4):** one ReturnValue per handler invocation; ingress 1:1 per API op |
| DIAG-DEDUP | Shielding | Side-effect once | Sweep emits new EventBridge rows per run | **No** | **Migrated (P5 diagnostics):** producer `tryClaim` by `dedupeKey` across invocation via `intakeDeduper` + `onClear`; outbound `publish` |
...
| COYOTE-PLAN | [`handleAwaitRoadRunnerForPlanOutcome.ts`](../../../../../lambda/ephemera/dataSource/coyoteGame/handlers/handleAwaitRoadRunnerForPlanOutcome.ts) | coyoteGame DS **20** -> PublishMessage **15** + lane flush | Medium (**Migrated P4 COYOTE-PLAN**) |
...
| SELF-HEAL | [`selfHealing/roomOccupancyDriftFinding.ts`](../../../../../lambda/ephemera/dataSource/selfHealing/roomOccupancyDriftFinding.ts) | ephemera DS **5** -> RoomUpdate **6** / CheckLocation **3** | Low (**Migrated P4 SELF-HEALING**) |
| FETCH-EPH | [`fetchEphemera/index.ts`](../../../../../lambda/ephemera/fetchEphemera/index.ts) | FetchPlayerEphemera **2** -> EphemeraUpdate **10** | Low (**Migrated P4 Easy / Low**) |
...
| MOVE-CHAR | `moveCharacter/index.ts` | Q4 | **High** | Tier 1C; multi-tier sends | **Migrated (P4):** all outbounds `publish`; fallback WorldMessage `deliveryMode: 'deferred'`; primary path register-first + `characterMoveDelivery`; boundary drain only |
...
| ROOM-AFFORD | `roomUpdate` -> affordance pipeline | Q4 | **High** | Tier 1C; prio 4->5 edge | **Migrated (P4):** affordance subgraph P3; `roomUpdate` handler delegates to `sendAffordanceRefreshRequestedForRoom` -> `publish`; disconnect/move/positions RoomUpdate producers now `publish`; passThrough integration gate passes |
| COMP-KICK | `ephemera/dataSource/index.ts` component kick | Q4 | **High** | Tier 1C StreamingEvent chain | **Migrated (P4):** component kick + Canon* ingress -> `publish`; passThrough tests |
...
| DISCONNECT-CHAIN | `disconnectMessage` (Unregister + Disconnect) | Q4 | **Medium** | Tier 1C prio 1 -> 15, 6 | **Migrated (P4):** all outbounds `publish`; injected bus in `atomicallyRemoveCharacterAdjacency`; immediate WorldMessage; boundary drain only |
...
| CHECK-LOC | `checkLocation/index.ts` | Q5 | **High** | Shielding: character dedup; side-effect-once downstream (`moveCharacter`) | **Migrated (P4):** producer coalesce + outbound `publish`; CheckLocation ingress from positions + self-healing now `publish` |
| SELF-HEAL | `selfHealing/roomOccupancyDriftFinding.ts` | Q5 | **Low** | Tier 1C DS -> RoomUpdate / CheckLocation | **Migrated (P4 SELF-HEALING):** `RoomUpdate` + `CheckLocation` -> `publish`; CHECK-LOC ingress chain complete; boundary drain only |
...
| EPH-UPDATE | `ephemeraUpdate/index.ts` | Q5 | **Low** | State convergence: client keyed by `CharacterId` | **Migrated (P4 Easy / Low):** subscriber only; producers `publish`; no cross-invocation session coalesce (extra frames OK) |
| STATE-CHANGE | `state/handleApiStateChange.ts` + `state/index.ts` | Q5 | **Low** | ReturnValue via collector; State Changed outbound to renderOrch | **Migrated (P4 Easy / Low):** ReturnValue `publish`; P2b `outboundBusDelivery: 'publish'` on `mtw.ephemera.state` |
| MAP-SUB | `mapSubscription/index.ts` | Q5 | **Medium** | Contract: `extractReturnValue` spread overwrites `maps` | **Migrated (P4):** `ReturnValue` -> `publish`; one ReturnValue per handler invocation (ingress 1:1); batch aggregation in handler unchanged |
...
| DIAG-DEDUP | `diagnostics/dataSource/index.ts` | Q5 | **Medium** | Shielding: side-effect-once (`staleSessionSweep`) | **Migrated (P5 diagnostics):** producer `tryClaim` by `dedupeKey` across invocation (`intakeDeduper`); ingress + ReturnValue/Error `publish`; boundary `flushAndSettle` + returnValue collector |
...
| FETCH-EPH | `fetchEphemera/index.ts` | Q5 | **Low** | State convergence: full snapshot overwrite | **Migrated (P4 Easy / Low):** `EphemeraUpdate` outbound `publish`; FETCH-EPH chain publish-native |
...
- [ ] Phase P2 - `DataSource` port and outbound path (piecewise per Q7; closeout per Q2)
  ...
  - [X] P2b -- per DataSource: set `outboundBusDelivery: 'publish'` with that directory's lane/send atomic migration (coordinate with P3/P4/P5); update that DS's package/lambda tests to assert `publish`. **Done (ephemera):** `mtw.ephemera.coyoteGame` (Coyote hypothesis P3 slice), `mtw.ephemera.renderOrchestration` (render orchestration P3 slice), `mtw.ephemera.affordanceOrchestration` (affordance orchestration P3 slice), `mtw.ephemera.actions` (ACTIONS-PARSE P4 slice), `mtw.ephemera.perception` (**COMP-KICK + PERCEPTION** P4 slice), `mtw.ephemera.renderCache` (**RENDER-CACHE** P4 slice), `mtw.ephemera.affordanceCache` (**AFFORDANCE-CACHE** P4 slice), `mtw.ephemera.state` (**Easy / Low rows** P4 slice), `mtw.ephemera.objects` + `mtw.ephemera.thinking.scheduling` (**P4 ephemera closeout grep**). **Done (assets P5):** `mtw.assets`, `mtw.assets.contentHeaders`, `mtw.assets.library`, `mtw.assets.players`, `mtw.assets.characters`, `mtw.assets.componentTopology`, `mtw.assets.componentExamples`, `mtw.assets.components.verticals`. **Done (wml P5):** `mtw.wml`. **Done (connections P5):** `mtw.connections`, `mtw.connections.characters`. **Done (diagnostics P5):** `mtw.diagnostics`.
...
- [X] Phase P4 - remaining ephemera `send` sites (piecewise **atomic units** per Q2; bucket-1 fix axes land **inside** the slice that owns the row -- not a separate pass after all DS work)
  ...
  - [X] **MOVE-CHAR** (Q4 **High**; atomic unit): [`moveCharacter/index.ts`](../../../../../lambda/ephemera/moveCharacter/index.ts) multi-tier send graph; **`deliveryMode: 'deferred'`** on move-related `PublishMessage`s. **Migrated:** all `EphemeraUpdate` / `PublishMessage` / `RoomUpdate` / `Perception` / `MapUpdate` outbounds -> `publish`; fallback WorldMessage leave/arrive `deliveryMode: 'deferred'`; no producer-side drain. **Tests:** `moveCharacter/index.test.ts` (8 tests); `checkLocation/index.test.ts` + passThrough integration gates pass.
  - [X] **ROOM-AFFORD + disconnect chains** (Q4 Medium/**High**): [`roomUpdate/index.ts`](../../../../../lambda/ephemera/roomUpdate/index.ts) + [`disconnectMessage/index.ts`](../../../../../lambda/ephemera/disconnectMessage/index.ts) (DISCONNECT-UNREG, DISCONNECT-CHAR); affordance fan-out already P3 -- migrate remaining `send` on RoomUpdate -> affordance kick paths. **Migrated:** `disconnectMessage` all outbounds `publish`; injected bus in `atomicallyRemoveCharacterAdjacency` (no singleton sends); `roomUpdate` handler unchanged (already delegates to P3 affordance kick); immediate WorldMessage (no `deliveryMode: 'deferred'`). **Tests:** `disconnectMessage/index.test.ts` (5 tests), `roomUpdate/index.test.ts`, `passThroughAffordanceOrchestrationToCache.integration.test.ts`, `app.test.ts`.
  - [X] **Easy / Low rows** (bucket-1 migrate freely): [`fetchEphemera/index.ts`](../../../../../lambda/ephemera/fetchEphemera/index.ts) (FETCH-EPH), [`ephemeraUpdate/index.ts`](../../../../../lambda/ephemera/ephemeraUpdate/index.ts) (EPH-UPDATE), [`state/handleApiStateChange.ts`](../../../../../lambda/ephemera/dataSource/state/handleApiStateChange.ts) where triage allows; **P2b** per DS as needed. **Migrated:** FETCH-EPH `EphemeraUpdate` outbound `publish`; EPH-UPDATE subscriber validated (no handler change; per-publish `payloads: [single]` acceptable per Q5); `handleApiStateChange` ReturnValue `publish`; `mtw.ephemera.state` `outboundBusDelivery: 'publish'`. **Tests:** `fetchEphemera/index.test.ts`, `ephemeraUpdate/index.test.ts`, `handleApiStateChange.test.ts`, `state/index.test.ts`, `app.test.ts`, passThrough integration gates.
  - [X] **SELF-HEALING** (after **CHECK-LOC**): [`selfHealing/roomOccupancyDriftFinding.ts`](../../../../../lambda/ephemera/dataSource/selfHealing/roomOccupancyDriftFinding.ts) (`RoomUpdate`, `CheckLocation` sends). **Tests:** selfHealing tests if present. **Migrated:** `RoomUpdate` + `CheckLocation` outbounds -> `publish`; CHECK-LOC ingress chain complete; no `outboundBusDelivery` flip (direct `messageBus`); boundary drain only. **Tests:** `roomOccupancyDriftFinding.test.ts`, `index.test.ts`, `checkLocation/index.test.ts`, `passThroughAffordanceOrchestrationToCache.integration.test.ts`, `app.test.ts`.
  - [X] **MAP-SUB** (bucket-1 **No**; Q5 Medium): [`mapSubscription/index.ts`](../../../../../lambda/ephemera/mapSubscription/index.ts) -- contract / `extractReturnValue` merge policy if multi-send per invocation; migrate or keep one bus message per API op. **Tests:** mapSubscription tests. **Migrated:** all `ReturnValue` outbounds -> `publish`; one ReturnValue per handler invocation (ingress 1:1 per API op); handler batch aggregation unchanged; no collector merge change. **Tests:** `mapSubscription/index.test.ts` (3 tests), `app.test.ts`.
  - [X] **COYOTE-PLAN** (Q4 Medium): [`handleAwaitRoadRunnerForPlanOutcome.ts`](../../../../../lambda/ephemera/dataSource/coyoteGame/handlers/handleAwaitRoadRunnerForPlanOutcome.ts) remaining `PublishMessage` `send`s (coyoteGame outbound already `publish`). **Tests:** `handleAwaitRoadRunnerForPlanOutcome.test.ts`. **Migrated:** mirror COYOTE-LANE / hypothesis pattern; `publish` Generating + terminal with `createdTime` revision pair; drop `outcomeLane` + scoped `flush`; linearized handler; immediate wire (no `deliveryMode: 'deferred'`); boundary drain only. **Tests:** `handleAwaitRoadRunnerForPlanOutcome.test.ts`, passThrough integration gates.
  - [X] **P4 ephemera closeout grep:** `rg 'messageBus\.send\(' lambda/ephemera/ --glob '*.ts' | rg -v '\.test\.ts'` zero production hits (tests/harnesses may remain until P6); complete remaining **P2b** `outboundBusDelivery: 'publish'` flips for ephemera DataSources touched above. **Migrated:** `dataSource/index.ts` Canon* ingress -> `publish`; `materializeRoomStateRender` terminal wire -> `publish`; removed commented legacy `send` blocks in `app.ts` and `moveCharacter/index.ts`; P2b `outboundBusDelivery: 'publish'` on `mtw.ephemera.objects` and `mtw.ephemera.thinking.scheduling`. **Tests:** `index.test.ts`, `objects/index.test.ts`, `thinking/scheduling/index.test.ts`, `materialize.test.ts`, `conversations.test.ts`, `app.test.ts`, passThrough integration gates.

- [ ] Phase P5 - other lambdas (one Plan-mode run per top-level slice; boundary `flush()` -> `flushAndSettle()` lands **inside** each slice, not as a separate pass)

  P5 is smaller than ephemera P4: ~35 production `messageBus.send` sites across five lambdas (baseline grep), mostly mechanical `publish` + **P2b** DataSource flips. No ephemera-grade pass-through integration gates; triage rows **CONTENT-AGG** (Low), **PLAYER-REDUCE** (Low), **DIAG-DEDUP** (Medium), **WML-DUP-SUB** (Medium), **promoteToCanon** lane atomic unit (Q2).

  - [X] **ASSETS** (single slice; ~19 production `send`s): [`lambda/assets/`](../../../../../lambda/assets/)
    - [X] [`app.ts`](../../../../../lambda/assets/app.ts): WebSocket/API ingress `send` -> `publish`; all boundary `flush()` -> `flushAndSettle()` (7 sites).
    - [X] Legacy handlers: [`fetch/index.ts`](../../../../../lambda/assets/fetch/index.ts), [`upload/index.ts`](../../../../../lambda/assets/upload/index.ts), [`library/index.ts`](../../../../../lambda/assets/library/index.ts) (`ReturnValue` / library flows), [`contentHeaders/index.ts`](../../../../../lambda/assets/contentHeaders/index.ts) (**CONTENT-AGG**; consumer merge OK; note remove/update ordering in same burst).
    - [X] [`dataSource/index.ts`](../../../../../lambda/assets/dataSource/index.ts): direct `messageBus` in `receiveEvents` / event processors (`Error`, zone/canon side effects).
    - [X] **P2b** `outboundBusDelivery: 'publish'` on all assets DataSources with `streamEvent` outbounds: `mtw.assets`, `contentHeaders`, `library`, `players`, `characters`, `componentTopology`, `componentExamples`, `components.verticals` (see Tier 1A table). **`player/update.ts`** (**PLAYER-REDUCE**): migrate freely; optional per-player coalesce is optimization only.
    - [X] **Boundary assembly:** [`returnValue/collector.ts`](../../../../../lambda/assets/returnValue/collector.ts) ReturnValue + Error collectors (priority **16**); `extractReturnValue` reads collectors only.
  ...
  - [X] **DIAGNOSTICS** (separate slice; **DIAG-DEDUP** design): [`lambda/diagnostics/`](../../../../../lambda/diagnostics/)
    - [X] [`dataSource/index.ts`](../../../../../lambda/diagnostics/dataSource/index.ts): **producer** dedup by `dedupeKey` across all events in one invocation (not only within one `receiveEvents` batch); then `send` -> `publish` for sweep / EventBridge side effects.
    - [X] [`ingress.ts`](../../../../../lambda/diagnostics/ingress.ts): ingress `send` -> `publish`.
    - [X] [`app.ts`](../../../../../lambda/diagnostics/app.ts): boundary `flush()` -> `flushAndSettle()`.
    - [X] **P2b** `outboundBusDelivery: 'publish'` on `mtw.diagnostics` if applicable.
    - [X] **Boundary assembly:** [`returnValue/collector.ts`](../../../../../lambda/diagnostics/returnValue/collector.ts) ReturnValue + Error collectors (priority **16**); `extractReturnValue` reads collectors only.
    - [X] **DIAGNOSTICS closeout grep:** `rg 'messageBus\.send\(' lambda/diagnostics/ --glob '*.ts' | rg -v '\.test\.ts'` zero production hits.
    - [X] **Tests:** `returnValue/collector.test.ts`, `dataSource/index.test.ts`, `app.test.ts` (26 tests); package `ts/dataSource/index.test.ts` + `ts/messageBus/index.test.ts` baseline.

  - [X] **CONNECTIONS** (small slice): [`lambda/connections/`](../../../../../lambda/connections/)
    - [X] [`ingress.ts`](../../../../../lambda/connections/ingress.ts), [`dataSource/apiConnections.ts`](../../../../../lambda/connections/dataSource/apiConnections.ts), [`dataSource/index.ts`](../../../../../lambda/connections/dataSource/index.ts): `send` -> `publish`.
    - [X] [`app.ts`](../../../../../lambda/connections/app.ts): boundary `flush()` -> `flushAndSettle()`.
    - [X] **P2b** on `mtw.connections` + `mtw.connections.characters` DataSources.
    - [X] **Boundary assembly:** [`returnValue/collector.ts`](../../../../../lambda/connections/returnValue/collector.ts) ReturnValue + Error collectors (priority **16**); `extractReturnValue` reads collectors only.
    - [X] **CONNECTIONS closeout grep:** `rg 'messageBus\.send\(' lambda/connections/ --glob '*.ts' | rg -v '\.test\.ts'` zero production hits.
    - [X] **Tests:** `returnValue/collector.test.ts`, `app.test.ts` (40 tests); package `ts/dataSource/index.test.ts` + `ts/messageBus/index.test.ts` baseline.

  - [X] **COGNITOEVENT** (boundary only; folded into **CONNECTIONS** pass): [`lambda/cognitoEvent/app.ts`](../../../../../lambda/cognitoEvent/app.ts) `flush()` -> `flushAndSettle()` (no production `messageBus.send` today).

  - [ ] **P5 global grep gate:** `rg 'messageBus\.send\(' lambda/ --glob '*.ts' | rg -v '\.test\.ts'` zero production hits (tests/harnesses may remain until P6).
...
### P5 (per-lambda slices)

After each P5 slice, confirm that lambda's production `send` backlog is zero:

```bash
rg 'messageBus\.send\(' lambda/assets/ --glob '*.ts' | rg -v '\.test\.ts' || true
rg 'messageBus\.send\(' lambda/wml/ --glob '*.ts' | rg -v '\.test\.ts' || true
rg 'messageBus\.send\(' lambda/diagnostics/ --glob '*.ts' | rg -v '\.test\.ts' || true
rg 'messageBus\.send\(' lambda/connections/ --glob '*.ts' | rg -v '\.test\.ts' || true
```

From `packages/mtw-lambda-patterns/` after assets/wml DataSource touches:
...
Run lambda-local tests from each `lambda/<name>/` package for touched paths (no repo-wide P5 integration gate equivalent to ephemera pass-through tests).
...
| DataSource port migration (P2) | P2a done; P2b ephemera + assets + wml + connections complete; diagnostics pending (P5) |
...
| Other lambdas (P5) | Assets + **wml** + **connections** + **cognitoEvent** done (**CONNECTIONS closeout grep**); next: **diagnostics** (see Recommended order) |
