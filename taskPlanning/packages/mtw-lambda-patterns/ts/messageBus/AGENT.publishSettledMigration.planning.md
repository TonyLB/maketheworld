...
| Remaining ephemera migration (P4) | In progress (**PUBLISH-MSG** + **CHECK-LOC** + **WebSocket / api.ephemera ingress** + **ACTIONS-PARSE** + **POSITIONS** + **COMP-KICK + PERCEPTION** + **RENDER-CACHE** + **AFFORDANCE-CACHE** + **MOVE-CHAR** done; next: **ROOM-AFFORD + disconnect chains** -- see **Recommended order** sub-slices) |
