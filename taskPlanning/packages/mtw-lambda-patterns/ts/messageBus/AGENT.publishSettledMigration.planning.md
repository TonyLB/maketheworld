...
| Remaining ephemera migration (P4) | In progress (**PUBLISH-MSG** + **CHECK-LOC** + **WebSocket / api.ephemera ingress** + **ACTIONS-PARSE** + **POSITIONS** + **COMP-KICK + PERCEPTION** + **RENDER-CACHE** + **AFFORDANCE-CACHE** + **MOVE-CHAR** + **ROOM-AFFORD + disconnect chains** + **Easy / Low rows** + **SELF-HEALING** done; next: **MAP-SUB** -- see **Recommended order** sub-slices) |
