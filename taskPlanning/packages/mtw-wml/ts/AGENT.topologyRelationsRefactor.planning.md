...
**Status:** Phases 1-3 complete; Phase 4 **blocked** on [`AGENT.referencedByCacheDecacheRefactor.planning.md`](./AGENT.referencedByCacheDecacheRefactor.planning.md) Phases 2-5 (Coyote topology authored in overlay asset; smoke-test failed on thin/clobbered room rows from inverse `referencedBy` pass). **Prerequisite shipped:** referencedBy plan **Phase 1** (`referencesFromExitEndpoint` + Area `referencedKeys` / `assureComponents` coverage). **Next:** complete referencedBy cache refactor Phases 2-5, then smoke-test, durable docs cleanup, delete this plan. Steady-state invariant names live in [`AGENT.edges.md`](../../../packages/mtw-wml/ts/standardize/keys/edges/AGENT.edges.md#topology-invariants).
