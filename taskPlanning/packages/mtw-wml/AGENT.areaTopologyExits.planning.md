...
**Status:** In progress. **Milestone 0 (decisions)** complete; **Milestone 1** in progress (dual-read guard done; edge pattern next).

**Next step:** Continue **Milestone 1** (**Edge pattern**, `positionGraph.edges`) per [Recommended order](#recommended-order); **M2** may proceed in parallel once gated.
...
| M1 WML + StandardArea edges | In progress (dual-read guard done; edge pattern next) |
