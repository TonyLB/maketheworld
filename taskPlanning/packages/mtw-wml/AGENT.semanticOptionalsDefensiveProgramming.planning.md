...
- [X] Implement **`defaultedEquals`** in an appropriate shared module (likely under `packages/mtw-wml` or `packages/mtw-base`, per team preference), then roll it out across optional semantic-content fields with focused tests and call-site updates.
  - [X] Define and export a generic helper contract for optional values with **`isEmpty`** + **`equals`** semantics (treat **`undefined`** and semantic-empty as the same vacuous class).
