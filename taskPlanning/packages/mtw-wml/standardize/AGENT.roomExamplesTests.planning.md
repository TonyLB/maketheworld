...
- **For this test-refactor initiative:** No separate choice between omit vs read-only shim. **Leave** Room `examples` in fixtures and expectations **until** [`AGENT.roomExamplesLegacy.planning.md`](AGENT.roomExamplesLegacy.planning.md) **Gate D** removes the field from `StandardRoom` / `StandardRoomData` and consumers; then tests follow the implementation. **`charcoal-client`:** Phase 5 removed direct **`room.examples`** assertions (Maps stub test); remaining client suites do not depend on that accessor pending **D2**.
...
- **Post Phase 4 `lambda/ephemera` partial (2026-04-22):** [`componentStackMerge.test.ts`](../../../../lambda/ephemera/internalCache/componentStackMerge.test.ts) no longer uses bare `examples: []` on Room stubs. [`componentAssetMeta.test.ts`](../../../../lambda/ephemera/internalCache/componentAssetMeta.test.ts) uses **`FEATURE#...`** for per-asset **example** list scenarios; **`examples:`** literals remain on Feature mocks and in [`examples.test.ts`](../../../../lambda/ephemera/internalCache/examples.test.ts) (**D3**). Re-run cmd **3** before the next freeze if you need exact counts.
- **Post Phase 5 `charcoal-client` (2026-04-22):** Inventory patterns **cmd 1** (`\.examples\b`), **cmd 3** (`examples:\s*\[|examples:\s*\{`), and multiline **cmd 2** (`<Room...<Example`) under `charcoal-client` `*.{test.ts,test.tsx}` yield **no matches** (also extended to [`RoomDescription.test.tsx`](../../../../charcoal-client/src/components/Message/RoomDescription.test.tsx) skipped blocks and [`buildGenerationContextSubset.test.ts`](../../../../charcoal-client/src/lib/buildGenerationContextSubset.test.ts)).
...
| File | Kind | Notes |
| --- | --- | --- |
| [`src/components/Maps/Controller/index.test.tsx`](../../../../charcoal-client/src/components/Maps/Controller/index.test.tsx) | Migrated (Phase 5, **2026-04-22**) | Stub test uses **schemaToWML** exclusions for Example/Feature; no `room.examples` assertion |
| [`src/components/Workbench/foundations/LayeredContext/layeredContextUtils.test.ts`](../../../../charcoal-client/src/components/Workbench/foundations/LayeredContext/layeredContextUtils.test.ts) | Migrated (Phase 5, **2026-04-22**) | Nested Room `Example` removed from fixture; Room vs **EXAMPLE#** negative path unchanged (asset-level Example) |
| [`src/slices/personalAssets/reducers.test.ts`](../../../../charcoal-client/src/slices/personalAssets/reducers.test.ts) | Migrated (Phase 5, **2026-04-22**) | Room prose via **`<Situation uuid=(DEFAULT)>`**; mutations use **`StandardSituationRoomFacet`** / **`SituationRoomFacetList`** |
| [`src/lib/buildGenerationContextSubset.test.ts`](../../../../charcoal-client/src/lib/buildGenerationContextSubset.test.ts) | Migrated (Phase 5, **2026-04-22**) | Removed redundant nested Room `Example` from exclusion fixture |
| [`src/components/Message/RoomDescription.test.tsx`](../../../../charcoal-client/src/components/Message/RoomDescription.test.tsx) | Migrated (Phase 5, **2026-04-22**) | Defaults test uses minimal Room; skipped **Merged behavior** fixtures use **Situation** instead of Room-nested Example |
