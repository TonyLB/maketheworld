...
- [ ] **Slice 0 --- the portal sub-question, and survey before building.** No code. **LG-4, LG-8, LG-9 and LG-10 are all decided; no whole row is open.** What remains is one sub-question and one measurement.
  - [ ] **LG-10's portal sub-question.** Walk a portal exit (one endpoint inside the Area, one outside) against a relational edge crossing a host boundary. **The prediction to test:** if `Topology` edges are edges in the same sense, the portal should decompose into legs sharing a `chainId` while carrying the authored `uuid` as their common `edgeId`. If it does not, record *why the Area seam differs* rather than letting the gap pass unnamed.
  - [ ] **Survey `referenceKeys` consumers.** The dependency graph feeds asset compilation, and LG-8 added **two** exclusions (presence nodes, and the root). **This is the one thing most likely to complicate Slice 1**; do it before writing code, not during.
  - [ ] Deliverable: the target `StandardLudicGraphData` written out explicitly, field by field, in the LG-8 row --- nodes as a tagged union, `edges` as one kind-discriminated list, `ports`, `rootId`.
...
| Slice | State | Notes |
| --- | --- | --- |
| 0. Portal sub-question + survey | Not started | All rows LG-1/2/3/4/5/8/9/10 disposed 2026-09-20; only the portal-crossing sub-question and the `referenceKeys` measurement remain |
...
- **The blast radius of `referenceKeys` is the largest unknown in the plan, and LG-8 being decided does not shrink it.** The node list feeds asset dependency resolution; a wrong filter is a silent compilation-ordering bug rather than a type error, and the decision now adds a *second* exclusion (the root) on top of presence. Slice 0's survey covers it. **The decision settled what the shape is, not what depends on it** --- do not read a Decided row as a measured risk.
