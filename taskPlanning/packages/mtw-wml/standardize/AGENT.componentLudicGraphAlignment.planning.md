...
**This is what keeps the alignment bounded.** Aligning the type contract makes presence *representable* in WML; it does not make it *authored*. **Revised 2026-09-21:** for LG-7 the plan's original framing still holds --- type permits, lint forbids, via a clause in `assetWirePolicy.ts`. For LG-6 it does not: there is no lint clause, only the absence of any WML surface tag to author a presence node with. Type permits, and *nothing yet writes one* --- see [Why merge does not force LG-6](#why-merge-does-not-force-lg-6), which is the check that establishes this rather than assuming it.
...
**So WML graph merge sees authored content exclusively, and today that is because nothing authors a presence node, not because a lint forbids it.** **Revised 2026-09-21:** the presence-not-authorable clause described in early drafts of this section was never shipped --- the author decided against it once Slice 2 reached implementation, on the grounds that WML authoring is **definitely** going to gain a way to express presence, and a temporary rejection would only have to be undone later for no safety gained today (there is no WML surface tag for `Presence` regardless, so the only way to trigger this path at all is hand-authored JSON). The merge-identity hazard the old clause was guarding against does not disappear; it is simply deferred to whichever future authoring surface mints presence nodes, which will have to solve it with a stable identity scheme rather than a minted uuid --- see the worked `Exit`/`edgeId` precedent below --- before it can ship, not before Slice 2 can.
...
**What remains genuinely open** is narrower than the fork below: whether WML should be *able* to express presence directly (Design B), not whether something derives it today. **Revised 2026-09-21: this question is not being kept shut by a lint clause** --- Slice 2 decided against adding one, since Design B is coming regardless and a temporary block would only be removed again. It stays practically shut for now only because no WML surface tag authors a `Presence` node yet; the rest of this section is background for whichever future slice adds one, not an argument for permanent prohibition.
...
| Asset-mode lint | none (2026-09-21: no clause added --- see below) | none needed (never had one) |
...
**Re-graded three times.** First (2026-09-20) from blocker to non-blocker: it gated the old Slices 1 and 2 when the question was whether presence belonged in the wire shape at all, and alignment answers that structurally (presence is in the type because it is in ephemera's type). **Then (2026-09-20) from Open to Decided**, on finding the shipped cache-time derivation described at the head of this section --- the fork was being argued as though neither side existed, when one side is running in production. **Then (2026-09-21), the planned enforcement mechanism was dropped.** Slice 2 was going to ship an asset-mode lint clause rejecting any authored presence node, reasoning that it kept WML's merge paths from ever seeing one "while the lint holds." The author rejected that at implementation time: WML authoring **is** going to gain a way to express presence, so a clause that exists only to be deleted again later is net-negative work, not safety. What's actually true today is narrower and still sufficient --- [Why merge does not force LG-6](#why-merge-does-not-force-lg-6) --- nothing in WML's merge paths can see a presence node **because nothing authors one**, not because a lint intercepts one. The merge-identity hazard this was protecting against is real but belongs to whatever authoring surface ships it, not to this slice.
...
| **LG-6** | **Does authoring express presence, or does ephemera derive it?** See [Authored nesting implies presence](#authored-nesting-implies-presence-----and-that-is-the-fork-lg-6). | --- | **Decided (2026-09-20): ephemera derives it today --- Design A, already shipped.** [`containmentPopulationSteps.ts`](../../../../lambda/ephemera/dataSource/positions/manipulation/containment/containmentPopulationSteps.ts) does cache-time containment authoring for **Feature-in-Room** (and Room-in-Area, Feature-in-Feature), minting node membership, a `PartOf` containment edge and the presence binding as three separately-idempotent steps. Authored nesting yields **`PartOf`, never `In`**. **Revised 2026-09-21: no asset-mode lint clause was added.** Design A remains the only thing that mints presence *today*, but that's because nothing authors it, not because a lint blocks it --- the author decided against ever shipping the planned rejection clause, since WML authoring **will** gain a presence-authoring path (Design B is a "when," not an "if"), and a block that exists solely to be removed later isn't worth writing. The arity-2 merge-identity hazard is real and still unsolved, but it is now explicitly the responsibility of whatever future slice adds an authoring surface, not of this one. *(Superseded framings: first, argued as an open fork between two designs when one was already in production; second, planned a permanent lint clause where none was needed or wanted.)* |
...
| **LG-3** | `_characters` comes from the roster, not the graph, and the graph *also* holds `CHARACTER#` nodes. | --- | **Decided (2026-09-20): out of scope, forwarded.** A two-sources-of-truth problem with its own evidence ([`roomOccupancyDriftSweep`](../../../../lambda/diagnostics/roomOccupancyDriftSweep/classification.ts), whose named failure mode is "ghost on graph"); resolving it means deciding whether roster or graph is authoritative --- an ephemera question, not a WML one. Do **not** fold it into Slice 5. **Hand-off landed in Slice 5b (2026-09-21):** the premise narrows on inspection --- at the ephemera-lambda level `getRoomCharacterList` already derives roster ids from `ludicGraph.characterIds` (graph is sole authority per `AGENT.concepts.md`'s "Room play graph + adjacency reverse index"), so the live two-sources gap is at the WML authoring layer only (`StandardRoom._characters` vs. graph `CHARACTER#` nodes). Finding recorded in `AGENT.concepts.md`; forwarded as **PR-19** in [`AGENT.presence.planning.md`](../../../../lambda/ephemera/dataSource/positions/AGENT.presence.planning.md)'s Open decisions table, cross-linked to PR-6. Not resolved here, per this row's own instruction. |
| **LG-11** | **Found during Slice 4 implementation (2026-09-21): can a wire relational edge's `from`/`to` name a port-qualified terminal or a bare presence-node id, or only a bare component reference?** Stored relational edges land on all three forms (`AGENT.ludicNetwork.md` sections 2 and 4: a crossing-port address, a presence binding's exterior address, and --- per PR-15/PN-6 clause (c) --- a bare `PRESENCE#<uuid>` reached from inside its own graph). Slice 1's `StandardLudicRelationalEdgeData.from`/`to` admitted only `StandardReferenceData`, a bare component reference --- confirmed by grep that no port-qualified concept existed anywhere in `packages/mtw-wml/ts/standardize/keys/edges/`. | --- | **Decided with the user (2026-09-21): widen the type now, in Slice 4, rather than deferring behind a gate.** The alternative would leave the totality round-trip test unable to assert what it claims to assert ("nothing should be dropped"). `StandardLudicTerminalData` (`.../keys/edges/dataTypes/ludicEdge.ts`) is now a three-arm union --- bare component reference, `{ owner, port }`, `{ presence }` --- mirroring `EphemeraLudicTerminalId` exactly; `StandardLudicRelationalEdge`'s comparison/format/lookup methods (`.../keys/edges/ludicEdge.ts`) branch on the arm. Scoped to the Membership/Peer/Bearing arm only --- `StandardLudicNavigationEdge`/`StandardExitEdge` untouched, since Exit endpoints are always component-to-component. |
  ...
  - [X] Deliverable: the target `StandardLudicGraphData`, field by field, mirroring `EphemeraLudicGraphData` ([`ephemeraMeta.ts:660-669`](../../../../packages/mtw-interfaces/ts/ephemeraMeta.ts#L660)):
    ...
    type StandardLudicGraphData = {
        ...
        nodes?: LudicGraphNodeListData      // LG-8: tagged union, mirrors EphemeraLudicGraphNode.
                                             ...
                                             //     -- typed and unit-exercised; no WML surface tag authors one yet,
                                             //     and asset mode does not lint against it either (revised 2026-09-21)
...
- [X] **Slice 1 --- the aligned type and its existing consumers.** **Atomic by necessity:** changing `StandardLudicGraphData` breaks Area and Room at compile time, so they cannot land separately. Sequence within the slice:
  - [X] `dataTypes/ludicGraph.ts` --- the target shape from Slice 0: nodes per LG-8, edges per LG-9, `ports`, `rootId`, with typeguards. Mirror ephemera's **accessor-as-tag-filter** idiom rather than inventing a parallel one. **Implementation note:** `nodes` stays one JSON field (full parity with `EphemeraLudicGraphNode`), but the runtime `StandardLudicGraph` composes it from two internal arms --- the pre-existing `ReferenceList` mechanism for component nodes (untouched, reused via a new `LudicGraphNodeList` wrapper's `.componentRefs`/`.withComponentRefs`) and a small new presence-node arm. This kept every existing component-node behavior byte-identical while adding presence as pure addition, at the cost of a `.componentRefs` indirection at every call site that used to read `.nodes` directly as a `ReferenceList`.
  - [X] `StandardLudicGraph` ([`components/ludicGraph.ts`](../../../../packages/mtw-wml/ts/standardize/components/ludicGraph.ts)) --- `toJSON`/`fromJSON`/`merge`/`diff` over the new shape, plus `rootId`/`ports` fields and a `presenceNodes` getter.
  - [X] **The `edges` union (LG-9)** --- one new heterogeneous list (`LudicEdgeList` in [`keys/edges/ludicEdge.ts`](../../../../packages/mtw-wml/ts/standardize/keys/edges/ludicEdge.ts), data types in [`keys/edges/dataTypes/ludicEdge.ts`](../../../../packages/mtw-wml/ts/standardize/keys/edges/dataTypes/ludicEdge.ts)) over Topology (`Navigation`, `Bearing`), Membership (`In`/`On`/`PartOf`) and Peer (`Under`/`Against`/`Custom`) kinds, `kind` as the sole discriminant, no `category` field. **A new heterogeneous `EdgeList`; `ExitEdgeList`/`StandardExitEdge` are untouched and unwidened** --- `StandardLudicNavigationEdge` wraps `StandardExitEdge` internally and translates only the outer discriminant (`tag: 'Exit'` (stored) -> `kind: 'Navigation'`), reusing its Replace/Remove-editable endpoints, uuid identity, and `<Exit>` schema parsing verbatim. Membership/Peer/Bearing kinds are typed and unit-exercised (round-trip tested) but have no WML surface tag or author path this slice, per the lift rule -- their endpoints are plain `StandardReferenceData`, not the Exit-specific editable wrapper. Identity per LG-10: Navigation's `uuid` plays the `edgeId` role; other kinds carry `edgeId`/`chainId` as optional base fields with structural (kind+endpoints+label) `sameKey`, since nothing mints an author identity for them yet.
  - [X] **`referenceKeys` correctness (LG-8)** --- `Area`/`Room` now derive `Direct` references from non-root component nodes only; the `Dependency` duplicate is dropped entirely (Slice 0's survey found no live consumer of it). Both exclusions (presence, root) are unit-tested directly.
  - [X] **Invert the self-reference guard.** `assertNoSelfAreaReference` replaced by `assertRootIdConsistency`: a no-op when `rootId` is absent (nothing populates it yet -- that's Slice 4), and when present, asserts it equals the component's own identity and names a node actually in `nodes`. The case the old guard caught (accidental self-reference) stays caught, now conditioned on `rootId` being in use.
  - [X] Migrated [`area.ts`](../../../../packages/mtw-wml/ts/standardize/components/area.ts) and [`room.ts`](../../../../packages/mtw-wml/ts/standardize/components/room.ts) to the new shape.
  - [X] **LG-4** --- `roomFeaturesListAccessor` ([`roomReferenceListAccessors.ts`](../../../../../charcoal-client/src/components/Workbench/RoomEdit/roomReferenceListAccessors.ts)) filters to `Feature`-tagged, non-root nodes on read and merges back through `withComponentRefs` on write, so it no longer clobbers Character/Object/root/Presence nodes already on the graph.
  - [X] Round-trip test: every node tag (including `Presence`, both cover arms), every edge kind, `rootId`, and `ports` serialise and re-parse through `fromJSON`/`toJSON` without loss ([`ludicGraph.test.ts`](../../../../packages/mtw-wml/ts/standardize/components/ludicGraph.test.ts)'s "alignment round-trip" suite).
  - [X] **Consumer migration beyond area.ts/room.ts, found by running the plan's own Verification commands** (not originally itemized, but required by "atomic by necessity"): `lambda/ephemera`'s `dataSource/index.ts` (duck-typed `.nodes.payload[].universalKey` read -- fixed by giving `LudicGraphComponentNode` a `.key`/`.universalKey` passthrough), `dataSource/positions/ludicGraph/{index,baseClasses}.ts` and `dataSource/actions/enrich/objectManipulation/membershipObservation.ts` (three `new StandardExitEdge(rawPlayEnvelopeEdge)` guards, now `new StandardLudicNavigationEdge(...)`, since `PlayLudicGraph = StandardLudicGraphData`), and `charcoal-client`'s Area Workbench editors (`areaEditMutations.ts` and its three consumer components), which construct and merge `<Exit>` edges directly against `StandardArea.ludicGraph.edges`.

**Verification run for this slice (2026-09-21):** `packages/mtw-wml` full suite, `lambda/ephemera` full suite, `lambda/assets` full suite, `lambda/wml` full suite, `charcoal-client` Workbench suite (vitest) -- all green; `charcoal-client`'s `tsc --noEmit` error set is byte-identical before/after (14 pre-existing errors, none touching this slice's files).
- [X] **Slice 2 --- the doc, and the lint clause that survived.** Revised 2026-09-21: only LG-7 keeps an asset-mode lint entry (and its code lands in Slice 5, not here); LG-6's presence check was decided against rather than shipped, since WML authoring is going to gain a presence-authoring path and a temporary rejection would only be removed again. Both decisions' rationale lands in durable docs this slice regardless, so neither is re-argued from scratch later.
  - [X] Durable entries in [`standardize/AGENT.md`](../../../../packages/mtw-wml/ts/standardize/AGENT.md)'s asset-wire-policy section: the authored-objects `not yet` (LG-7 --- intent, what it waits on, the complications list) and the presence-is-un-linted note (LG-6 --- why the type permits presence and no lint forbids authoring it, even though nothing authors it yet).
  - [X] **No presence-not-authorable clause added to [`assetWirePolicy.ts`](../../../../packages/mtw-wml/ts/standardize/assetWirePolicy.ts).** Decided against, not merely deferred: [`containmentPopulationSteps`](../../../../lambda/ephemera/dataSource/positions/manipulation/containment/containmentPopulationSteps.ts) is the sole minter of presence bindings *today* only because no WML surface tag authors one, not because a lint would catch it. The arity-2 merge-identity hazard a lint would have guarded against is real but is deferred to whichever future slice adds a presence-authoring surface --- it will need a stable identity scheme (not a minted uuid) to solve it, same shape as `Exit`'s `edgeId`.
  - [X] Test that an authored presence node is **allowed** (not rejected) on both asset-mode and `ephemeraWire` `StandardForm`s, as a regression guard against reintroducing the block.
- [X] **Slice 3 --- `_ludicGraph` on `StandardObject`, `StandardFeature`, and `StandardCharacter`.** Straightforward once the type is aligned. Character was folded in at implementation time (see the resolved "Known unknowns" entry below): it is structurally identical to Object/Feature and the ephemera precedent (`EphemeraMetaCharacter.ludicGraph`) already existed, so the cost of doing all three together was the cost of one mechanical pattern applied three times, not three separate efforts.
  - [X] Field, constructor/clone branches, `toJSON`/`fromJSON`, `merge`, `diff` (generic, via `invert`/`merge` on the base class), `referencedKeys`, `isEmpty`, `invert`, `remapReferences`, `withChild`, `assureReferences`, `removeReferences`, `schema`/`nestedSchema` (organization-aware, mirroring [`area.ts:182-193`](../../../../packages/mtw-wml/ts/standardize/components/area.ts#L182)), `ludicGraph` accessor --- mirroring [`area.ts:41-61`](../../../../packages/mtw-wml/ts/standardize/components/area.ts#L41-L61), the smaller precedent, in `object.ts`, `feature.ts`, `character.ts`. `equals` was extended for Object/Feature (both already override it); Character has no `equals` override by design (relies on the base class's `deepEqual(toJSON())`) and needed none, since `ludicGraph` is now in `toJSON()`. The self-reference/`rootId` guard (`assertRootIdConsistency`) was deliberately **not** ported: it is Area's inversion of a pre-existing guard, Room never had one and Slice 1 didn't add it there either, and `rootId` isn't populated by any writer until Slice 4 --- Object/Feature/Character follow Room's precedent, not Area's.
  - [X] Data type + typeguard in `dataTypes/object.ts`, `dataTypes/feature.ts`, and `dataTypes/character.ts`.
  - [X] Schema converter: `<Object>` previously threw unless a Room or Asset was in the context stack, and type-checked children against `ShortName | Situation | Render | Replace | Remove` ([`components.ts:327-349`](../../../../packages/mtw-wml/ts/schema/converters/components.ts#L327-L349)). Widened the context guard to add Object-in-Object, and `typeCheckContents` to add the five `LUDIC_GRAPH_NODE_TAGS` members. Room/Feature/Area/Character needed no schema-converter change --- they have no `typeCheckContents`/context guard at that layer, so the standardize-layer consumers alone were sufficient (confirmed: Feature-in-Feature and Character-in-Feature both parse; Object nesting is gated to Room/Asset/Object context only, so e.g. Feature-in-Object is not authorable this slice).
  - [X] Round-trip tests: `object.test.ts`, `feature.test.ts`, `character.test.ts` each gained a `ludicGraph (Slice 3)` suite --- JSON round-trip, a nesting-authoring case (Object-in-Object, Feature-in-Feature, Character-in-Feature), merge/equals/diff detecting a graph-node addition, `isEmpty` with only a graph node, and `assureReferences`/`removeReferences` bucketing. Full `packages/mtw-wml` suite green (137/138 suites, 1 pre-existing skip); `lambda/ephemera` `tsc --noEmit` clean against the widened types; no consumers of `StandardObjectPayload`/`StandardFeaturePayload`/`StandardCharacterPayload` exist outside `packages/mtw-wml`.
- [X] **Slice 4 --- the stored-to-wire projection.** One shared pair of functions (`toWireLudicGraph`/`fromWireLudicGraph`, `lambda/ephemera/dataSource/positions/ludicGraph/wireProjection.ts`), sub-graphed by binding, **taking a binding parameter even while arity is 1** --- the lift rule's central case, and the one thing that is expensive to retrofit.
  - [X] Call the real arity-general primitives (`nodesFromPresenceBinding`, `subGraphFromNodes`), never a hand-rolled node walk.
  - [X] **Assert totality:** a round-trip test that a stored payload projects to WML and back with every field intact. Under alignment nothing should be dropped --- if something is, the alignment is incomplete and that test is where it surfaces. (`wireProjection.test.ts`'s "totality round-trip" suite --- one of every node tag, both cover arms, every Membership/Peer edge kind, `edgeId`/`chainId`, a port-qualified edge terminal, a bare-presence-id edge terminal, and a non-empty `ports` list, deep-equal after the round trip.)
  - [X] A rope-shaped test that a second binding's nodes do **not** cross, even though nothing mints one yet. (`wireProjection.test.ts`'s "rope-shaped second-binding test.")
  - [X] **LG-11 (found during this slice, not itemized when it was written): widen `StandardLudicRelationalEdgeData.from`/`to`.** Stored relational edges can land on a port-qualified terminal (a crossing port, or a presence binding's exterior address) or directly on a bare presence-node id (PR-15/PN-6 clause (c)) --- neither was representable in the Slice-1 type, which admitted only a bare `StandardReferenceData` component reference. Left alone, the projection would have silently dropped the port/presence half of such a terminal, reintroducing the exact failure mode alignment exists to retire. Widened `StandardLudicTerminalData` (`packages/mtw-wml/ts/standardize/keys/edges/dataTypes/ludicEdge.ts`) to a three-arm union mirroring `EphemeraLudicTerminalId`, and `StandardLudicRelationalEdge`'s `sameKey`/`toJSON`/`toFormat`/`lookup` (`.../keys/edges/ludicEdge.ts`) to a terminal-aware branch. No schema/converter or `StandardLudicNavigationEdge`/`StandardExitEdge` change --- Exit endpoints stay component-to-component, and nothing authors these kinds regardless. See the Open decisions table for the row.
- [X] **Slice 5 --- Room realignment.** Two differently-shaped items; do not treat as one.
  - [X] **5a --- retire `_objects` (LG-2).** Strictly sequenced, because the room header breaks if the order slips: **(i)** Room's wire carries the scoped subgraph; **(ii)** [`roomHeaderPhaseC.ts`](../../../../charcoal-client/src/slices/messages/roomHeaderPhaseC.ts) derives its contents line from graph nodes instead of `objects`, with the existing `Contents: a, b, and c` output unchanged as the regression guard; **(iii)** only then delete `StandardRoomObjectData`, `_objects`, the `Object` `StandardizeConsumer` in [`room.ts:129`](../../../../packages/mtw-wml/ts/standardize/components/room.ts#L129), both schema-emit sites ([`:235`](../../../../packages/mtw-wml/ts/standardize/components/room.ts#L235), [`:300`](../../../../packages/mtw-wml/ts/standardize/components/room.ts#L300)), and the `objects` clause in `assetWirePolicy.ts` --- **replacing** it with the LG-7 `not yet` gate whose doc landed in Slice 2. Do not simply delete it.
  - [X] **5b --- `characters`: hand off, do not fix here (LG-3).** Write the two-sources-of-truth finding and the drift-sweep evidence into a durable home; open a row wherever the roster/graph authority question belongs.
  - [X] **`exits`: no action.** Genuine derivation (one bidirectional Area edge -> a per-room directional view). Recorded so a later reader does not "finish the job" by retiring it alongside `objects`.
...
| 1. Aligned type + existing consumers | Done | `referenceKeys` and guard inversion landed as scoped; the actual silent-failure risk turned out to be downstream duck-typed consumers (ephemera's play-envelope edge parsing, Workbench Area edge editing), found by running the slice's own Verification commands and fixed in the same commit |
| 2. Lint clauses, doc first | Done | Revised 2026-09-21: no presence-not-authorable clause was added to `assetWirePolicy.ts` -- decided against, since WML authoring will definitely gain a presence-authoring path and a temporary block would only be removed again. LG-6 and LG-7 doc entries both landed in `standardize/AGENT.md`'s asset-wire-policy section; LG-7's `Room.objects` clause is the only lint entry this slice touches conceptually (its code still lands in Slice 5). Regression tests assert presence nodes are *allowed*, not rejected, in both asset and ephemeraWire modes. The arity-2 merge-identity hazard LG-6 raised is deferred to whichever future slice adds presence authoring. |
| 3. Object/Feature/Character field | Done | Character folded in (mechanically identical, ephemera precedent already existed); only `<Object>`'s schema converter needed widening (context guard + `typeCheckContents`), Room/Feature/Area/Character had none to widen; `assertRootIdConsistency` deliberately not ported (Room precedent, not Area's) |
| 4. Stored -> wire projection | Done | `wireProjection.ts` (+ test) added to `lambda/ephemera/dataSource/positions/ludicGraph/`; totality round-trip and rope-shaped second-binding tests both pass. Mid-slice finding (LG-11): stored edge terminals can be port-qualified or a bare presence id, which Slice 1's edge type couldn't represent -- widened `StandardLudicTerminalData` (three-arm union) and `StandardLudicRelationalEdge`'s comparison/format/lookup methods to close it, same shape as row 1's downstream-consumer discovery |
| 5a. Retire `_objects` | Done | `toWireLudicGraphFull` (un-cut projection, no presence binding to cut for a host that is never a hosted thing) added to `wireProjection.ts`; `affordanceRoomDeliverable.ts` now ships `ludicGraph` plus sibling `StandardObjectData` rows, mirroring the existing `characters` pattern; `roomHeaderPhaseC.ts` reads `ludicGraph.nodesByTag('Object')` + `byUniversalId` via the existing `componentDisplayLabel` accessor, same rendered output preserved as the regression guard; `_objects`/`StandardRoomObjectData` deleted from `mtw-wml`. **Revised while restoring WML-text test coverage:** a nested `<Object>` under `<Room>` now gets a real WML surface (a new `StandardizeConsumerReferenceList` for tag `Object`, mirroring `Feature`, merge-safe against `withLudicGraphNodes`'s replace semantics) that both records graph membership and lets `processComponents`'s existing `returnRemainderAddition` mechanism independently promote the same tag to a real top-level `StandardObject` component -- `assureReferences`/`withChild` extended to match. `assetWirePolicy.ts`'s Room clause replaced (not deleted) with a check on `ludicGraph.nodesByTag('Object')`, enforced the same way as `exits`/`render` (parses in every mode, blocked only in asset mode) rather than via absence of a surface tag |
| 5b. `characters` | Done | Hand-off only (LG-3); do not fix here. Corrected the premise while writing it up: at the ephemera-lambda level `getRoomCharacterList` already derives roster ids from `ludicGraph.characterIds` (graph is sole authority per `AGENT.concepts.md`), so the real two-sources gap is narrower and lives at the WML authoring layer (`StandardRoom._characters` vs. graph `CHARACTER#` nodes). Finding recorded in `AGENT.concepts.md`'s "Room play graph + adjacency reverse index"; forwarded as PR-19 in `AGENT.presence.planning.md`'s Open decisions table, cross-linked to PR-6 |
...
- ~~`StandardCharacter` also hosts a graph in storage and is **not** in this plan's title. Under alignment it is a Slice 3 sibling costing almost nothing; whether to include it is worth one sentence of discussion before Slice 3.~~ **Resolved (2026-09-21): included in Slice 3.** Confirmed with the author before implementing --- `EphemeraMetaCharacter.ludicGraph` already existed as the ephemera precedent, and Character's WML shape (shortName/situations/render, no graph) was identical to Object/Feature's pre-Slice-3 shape, so it was the same mechanical pattern a third time rather than separate work.
