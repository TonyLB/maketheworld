...
- [X] Implement **`StandardForm.equals(incoming, options?)`** with **`optimizeByUniversalKey`** (**Decisions locked**); default behavior = full comparison unless optimization explicitly enabled. (Shipped in `ts/standardize/index.ts`; optimization path only applies when both forms have complete, unique universal keys and otherwise falls back to full comparison.)
