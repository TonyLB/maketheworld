...
- [ ] Staged deprecation and removal of `StandardRoom.examples` (**Room-only**; `StandardFeature` / `StandardKnowledge` `examples` stay in scope of [`AGENT.featureKnowledgeExamples.planning.md`](AGENT.featureKnowledgeExamples.planning.md)). Runtime Room paths are already off `examples`; remaining work is model, schema, stored data, authoring, and tests.
  ...
  - [X] **Gate B (authoring):** Confirm no authoring UI path still writes new Room example refs; fix stragglers or record explicit defer with owner if any remain.
    ...
    - **Defer (Gate C):** Merging inherited/base WML or external assets may still contain non-empty legacy Room `examples`; no Charcoal UI affordance adds them in normal flows. Normalize or migrate stored data under **Gate C** when that gate is executed.
...
- **Code trace:** Top-level Room add uses `standardComponentFactory` (empty `_examples`). `updateStandard` applies diffs via `StandardForm.diff`/`merge` with no Room-example-specific logic. `addImportToDraft` uses factory for absent keys (empty examples on new stub); re-import of existing draft clones current payload (legacy data possible — **Gate C**).
