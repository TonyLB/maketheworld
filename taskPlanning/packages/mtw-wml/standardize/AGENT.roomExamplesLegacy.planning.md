...
Status: in progress. Next step: **Recommended order** -- **Gate D (code removal)** under staged removal of `StandardRoom.examples` (Room-only). **Gates A–C** complete for current corpus (Gate C: very small data set, manually confirmed no Room `examples`). Durable guidance linked below. Feature/Knowledge defer sites: **Out-of-scope Feature/Knowledge defer registry** and [`AGENT.featureKnowledgeExamples.planning.md`](AGENT.featureKnowledgeExamples.planning.md).
...
| 4 | Update tests/docs and re-baseline inventory | In progress | `componentRender` slice updated tests and [`componentRender.AGENT.md`](../../../../lambda/ephemera/internalCache/componentRender.AGENT.md); verification run 2026-04-22 (unit tests, bundle gates, inventory; package-wide `tsc` debt noted); **Gate A** doc/API sweep (2026-04-22); **Gate B** authoring audit (2026-04-22); **Gate C** stored-data inventory (2026-04-22, no Room examples in corpus) |
| 5 | Decide deprecation gate for `StandardRoom.examples` | Complete | **Gate D** landed **2026-04-22** (types, tests, D3/D4 companions); Feature/Knowledge `examples` unchanged |
...
- [X] Staged deprecation and removal of `StandardRoom.examples` (**Room-only**; `StandardFeature` / `StandardKnowledge` `examples` stay in scope of [`AGENT.featureKnowledgeExamples.planning.md`](AGENT.featureKnowledgeExamples.planning.md)). Runtime Room paths are already off `examples`; remaining work is model, schema, stored data, authoring, and tests.
  ...
  - [X] **Gate B (authoring):** Confirm no authoring UI path still writes new Room example refs; fix stragglers or record explicit defer with owner if any remain.
    - **Durable guidance:** Workbench authoring patterns in [`charcoal-client/src/components/Workbench/AGENT.md`](../../../../charcoal-client/src/components/Workbench/AGENT.md) (Reference List Editing: Room uses Situation/`render`, not `_examples`; code sample uses **Feature** for `examples` edits).
    - **Stored data (see Gate C):** Charcoal UI does not add Room `examples` in normal flows. Inherited or imported WML could still carry legacy Room `examples` in other corpora; **Gate C** records inventory outcome for this deployment.
  - [X] **Gate C (data):** Inventory real assets / WML for non-empty Room example children or serialized `examples` refs; define migration (e.g. fold prose into default situation or render target); run or schedule migration slices; re-run targeted tests and inventory after migration.
    - **Inventory (2026-04-22):** Existing real-asset corpus is **very small**; plan maintainer **manually confirmed** no non-empty Room `examples` / nested Room `<Example>` payloads in that data.
    - **Migration:** **Not required** for the current corpus (nothing to fold). If the asset base grows or imports legacy packs, **re-run** a Room-`examples` inventory before treating Gate C as evergreen-complete; use the migration patterns already described in durable docs ([`packages/mtw-wml/ts/standardize/AGENT.md`](../../../../packages/mtw-wml/ts/standardize/AGENT.md) **Room prose migration**).
  - [X] **Gate D (code removal):** After Gate C and test updates, remove `_examples` / `examples` from [`StandardRoom`](../../../../packages/mtw-wml/ts/standardize/components/room.ts) and [`StandardRoomData`](../../../../packages/mtw-wml/ts/standardize/components/dataTypes/room.ts) (`isStandardRoomData`, merge/diff/schema consumers); align schema and WML tests (e.g. [`packages/mtw-wml/ts/schema/index.test.ts`](../../../../packages/mtw-wml/ts/schema/index.test.ts), [`packages/mtw-wml/ts/standardize/components/room.test.ts`](../../../../packages/mtw-wml/ts/standardize/components/room.test.ts), baseline **test-only references** list in this plan). **Legacy `<Room>...<Example>` WML:** breaking policy is **D1** in [`AGENT.roomExamplesTests.planning.md`](AGENT.roomExamplesTests.planning.md) (**ignore at parse**: Room does not consume nested Example; consume/remainder reports error; no migration fold). **Assets:** remove **`Room`** from **`EXAMPLE_PARENT_TAGS`** / **`EXAMPLE_ASSOCIATED_TAGS`** in [`lambda/assets/componentExamples/exampleAssociatedFilter.ts`](../../../../lambda/assets/componentExamples/exampleAssociatedFilter.ts) and tests (**D4**, same companion doc). **Ephemera:** **`ExamplesData`** narrowed to Feature/Knowledge ids (**D3**). **Slice update:** **2026-04-22**.
...
### Gate B slice (2026-04-22): authoring UI audit (Room `examples`)

- **Static audit:** `rg "_payload\\._examples" charcoal-client --glob "*.{ts,tsx}"` hits only `FeatureEditor.tsx`, `KnowledgeEditor.tsx` (plus [buildGenerationContextSubset.ts](../../../../charcoal-client/src/lib/buildGenerationContextSubset.ts), which clears room `_examples` for generation context, not adds). `RoomEditor.tsx` / `FeatureListEditor.tsx` use `ReferenceListEditor` for **Guidance** / **Feature** only, not `Example` on Room.
- **Code trace:** Top-level Room add uses `standardComponentFactory` (empty `_examples`). `updateStandard` applies diffs via `StandardForm.diff`/`merge` with no Room-example-specific logic. `addImportToDraft` uses factory for absent keys (empty examples on new stub); re-import of existing draft clones current payload (legacy WML from other sources could still carry Room `examples`; **Gate C slice** below — current corpus clean).
- **Verification:** `npx vitest run src/slices/personalAssets/requestLLMGeneration.test.ts src/slices/personalAssets/addImportToDraft.test.ts` from `charcoal-client/` (9 tests) **PASS**.
- **Straggler:** Updated misleading Workbench doc sample that showed mutating `StandardRoom._payload._examples`; see [`charcoal-client/src/components/Workbench/AGENT.md`](../../../../charcoal-client/src/components/Workbench/AGENT.md).

### Gate C slice (2026-04-22): stored assets / WML (Room `examples`)

- **Scope:** Real deployed or author-held assets / WML (not unit-test fixtures).
- **Finding:** Corpus is **very small**; plan maintainer **manually confirmed** there are **no** non-empty Room `examples` reference lists and no reliance on nested Room `<Example>` for display prose in that data.
- **Migration:** **None executed** — nothing to fold into default Situation or `render`. Broader environments should re-run inventory before assuming the same conclusion.
- **Tests / inventory:** No migration slice required; optional future `rg` / DB queries remain documented under **Gate C** checklist in **Recommended order** for operators who onboard legacy packs later.
