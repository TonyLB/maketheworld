...
| 5 | Optional: `FacetListSessionEditor` or `onAssociateReference` on [`FacetListEditorGeneric`](../../../../charcoal-client/src/components/Workbench/foundations/FacetList/FacetListEditorGeneric.tsx) | [ ] |
...
- [ ] **Phase 2 --- Lens mark facets (LensDetail session)**
  ...
  - [ ] Facet remove/payload: confirm still routed through `onFacetsChange` only (no new `updateStandard` on edit path).
  - [ ] Add or extend tests under `LensMarkFacetsEditor/` or LensDetail session harness.
- [ ] **Phase 3 --- Guidance mark facets (GuidanceEditor session)**
  - [ ] Same as Phase 2 for [`MarkFacetsEditor`](../../../../charcoal-client/src/components/Workbench/MarkFacetsEditor/MarkFacetsEditor.tsx) when parent supplies `onChange` -> `updateComponent` ([`GuidanceEditorBody`](../../../../charcoal-client/src/components/Workbench/GuidanceEdit/GuidanceEditor.tsx)).
  - [ ] Tests: Guidance session + mark create does not call `updateStandard` before flush (mirror GuidanceEditor.test patterns).
- [ ] **Phase 4 --- Room non-DEFAULT Situations list**
  - [ ] Move situation handlers inside **`WorkbenchComponentProvider`** scope; build list items from **`working.situations`**, not committed `room` from `standardForm` (**P4**).
  - [ ] **Remove:** `updateComponent` disassociate at `room.situations` site only; add **`confirmOrphanClosureBeforeComponentDisassociate`** when removing a Situation ref may orphan a non-empty body (**P3**).
  - [ ] **Create:** `materializeComponentInAsset` for Situation body; `updateComponent` to add `StandardSituationRoomFacet` on **`working`**.
  - [ ] **`_topLevel` on create:** today create also `assureItem`s on `draft._topLevel` in one `update`. Decide and implement explicitly:
    - **Option A (preferred for consistency):** after materialize, `updateAssetMeta` on asset-meta **`working.topLevel`** if asset root session is available from Room screen (may require provider nesting or a narrow helper); **or**
    - **Option B:** document that new Situations are only nested under Room until author adds to TopLevel manually; drop eager `_topLevel` from create path.
  - [ ] **Reference existing:** `onAssociateReference` pattern (facet associate on `working`), not `updateStandard`.
  - [ ] Consider extracting `roomSituationsFacetAccessor` + thin wrapper (like `ReferenceListSessionEditor`) to avoid duplicating `ReferenceListEditorGeneric` wiring.
  - [ ] RoomEditor tests for situations remove/create (session + flush + normalize).
...
[`materializeComponent`](../../../../charcoal-client/src/components/Workbench/foundations/consistency/materializeComponent.ts) ensures **`byUniversalId`** only. It does **not** add `_topLevel`. Room Situation create today bundles top-level registration; Phase 4 must make that an explicit product decision (see Phase 4 checkbox).
