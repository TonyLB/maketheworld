...
| MS-4 | **Should live markers (`BD-*`, `RD-*`, `LP4a`) get the same delete-in-place treatment as dangling ones?** Resolved 2026-09-06 for the dangling half: every occurrence of every dangling `PB-*` letter and `LP4h` had its rule already stated in self-contained prose, so option (c) (delete the marker, keep the prose) applied uniformly --- no case needed (a) inline-the-rule or (b) link-to-contract. What's still open is whether *live* markers, which aren't a subtraction question, should be swept the same way for consistency once the comment register is settled. | 4 | Open |
...
| Phase | Status |
| --- | --- |
| 1. Subtract known-dead | 1a/1b/1c done (2026-09-06). 1d next --- executes CD3 |
