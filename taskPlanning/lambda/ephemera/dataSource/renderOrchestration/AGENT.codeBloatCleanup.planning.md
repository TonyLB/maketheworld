...
- [X] Shift `GenerationContext` lookup ownership to generation execution path (instead of upstream orchestration pre-assembly).
  - [X] Investigate/decide target ownership boundary: generation-path lookup is sufficient for context sourcing/encoding/validation; do not keep per-request override or ingress-fallback semantics for `generationContextWml`.
  - [X] Implement generation-path lookup behavior so slow-path generation sources context directly from `internalCache.GenerationContext` using room/perspective inputs.
  - [X] Remove or simplify upstream orchestration-only context pre-assembly where no longer required (including look path helper boundaries).
  - [X] Update/add tests for ownership shift (unit + orchestration integration) and refresh relevant docs/task notes.
...
- 2026-04-25 (slice lines 34-38): Updated `dataSource/renderOrchestration/generateRoomPreview.ts` to own generation-context lookup/encoding via `internalCache.GenerationContext`, removed look-path orchestration pre-assembly in `dataSource/renderOrchestration/handleLookCommandRequestedForRenderOrchestration.ts`, and aligned `dataSource/renderOrchestration/findRender.ts` + `dataSource/renderOrchestration/baseClasses.ts` + `dataSource/renderOrchestration/requestIntake.ts` so slow-path generation no longer depends on resolve-level `generationContextWml`; refreshed related tests in `dataSource/renderOrchestration/handleLookCommandRequestedForRenderOrchestration.test.ts`, `dataSource/renderOrchestration/generateRoomPreview.test.ts`, `dataSource/renderOrchestration/findRender.test.ts`, and `dataSource/renderOrchestration/orchestrationHandler.test.ts`.
  - `npm --prefix "/Users/anthonylower-basch/Code/maketheworld/lambda/ephemera" run test -- dataSource/renderOrchestration/handleLookCommandRequestedForRenderOrchestration.test.ts dataSource/renderOrchestration/generateRoomPreview.test.ts dataSource/renderOrchestration/findRender.test.ts dataSource/renderOrchestration/orchestrationHandler.test.ts` (pass: 4 suites, 25 tests).
