...
**Status:** In progress. StableKey and compatibility choices below are locked; materialization contract is recorded in [`hypothesis/AGENT.md`](../../../../../../../../../lambda/ephemera/dataSource/coyoteGame/generators/pipelines/hypothesis/AGENT.md#materialized-affordance-rows-synthetic-stablekey); synthetic member `stableKey` validation and tests are in [`parsePlanSelectOutput.ts`](../../../../../../../../../lambda/ephemera/dataSource/coyoteGame/generators/pipelines/hypothesis/planSelect/parsePlanSelectOutput.ts); grammar shared via [`coyotePlanAffinities.ts`](../../../../../../../../../packages/mtw-interfaces/ts/coyotePlanAffinities.ts); plan-select prompt copy documents optional materialization in [`buildPlanSelectPrompt.ts`](../../../../../../../../../lambda/ephemera/dataSource/coyoteGame/generators/pipelines/hypothesis/planSelect/buildPlanSelectPrompt.ts); fixture-01 phase-plan harness inject carries `selectedCandidate` with **`affordance:coyote`** ([`coyoteEngineTestFixtures.ts`](../../../../../../../../../lambda/ephemera/dataSource/coyoteGame/generators/testHarness/coyoteEngineTestFixtures.ts), [`coyoteEngineTestFixtures.test.ts`](../../../../../../../../../lambda/ephemera/dataSource/coyoteGame/generators/testHarness/coyoteEngineTestFixtures.test.ts)); next step is **Close out** (migrate lasting notes; retire this file per [`taskPlanning/AGENT.md`](../../../../../../../../AGENT.md)).
...
- [X] **Downstream read paths:** update narrative beat (and any other consumer) so synthetic stableKeys do not break grounding or duplicate real props.
- [X] **Prompt:** update [`buildPlanSelectPrompt.ts`](../../../../../../../../../lambda/ephemera/dataSource/coyoteGame/generators/pipelines/hypothesis/planSelect/buildPlanSelectPrompt.ts) and snapshot tests in [`buildPlanSelectPrompt.test.ts`](../../../../../../../../../lambda/ephemera/dataSource/coyoteGame/generators/pipelines/hypothesis/planSelect/buildPlanSelectPrompt.test.ts).
- [X] **Harness / fixtures:** extend [`coyoteEngineTestFixtures.ts`](../../../../../../../../../lambda/ephemera/dataSource/coyoteGame/generators/testHarness/coyoteEngineTestFixtures.ts) or related tests when inject paths need new fields.
...
| Parser + tests | Member-row `stableKey`: trim; if prefixed with `affordance:`, non-empty suffix matching `[A-Za-z0-9_-]+`; exports `MATERIALIZED_AFFORDANCE_STABLE_KEY_PREFIX`, `isValidMaterializedAffordanceStableKey`; tests for accept / reject / trim |
| Downstream consumers | `validateCoyotePhasePlan` accepts `affordance:` in `stableKeysUsed` / `derivedFrom`; setting-only virtual cap treats materialized tokens as grounding; [`formatPhasePlanForOutcomePrompt`](../../../../../../../../../lambda/ephemera/dataSource/coyoteGame/generators/pipelines/outcome/formatPhasePlanForOutcomePrompt.ts) labels materialized keys; narrative beat / [`narrativePromptShared`](../../../../../../../../../lambda/ephemera/dataSource/coyoteGame/generators/pipelines/hypothesis/narrativePromptShared.ts) contract lines updated |
| Prompt + tests | Optional materialized affordance handoff section; `MATERIALIZED_AFFORDANCE_STABLE_KEY_PREFIX` in prompt strings; anchored-on-winner / additive member copy rules; rubric and task trace-to-input lines; [`buildPlanSelectPrompt.test.ts`](../../../../../../../../../lambda/ephemera/dataSource/coyoteGame/generators/pipelines/hypothesis/planSelect/buildPlanSelectPrompt.test.ts) assertions for prefix and no `full copy` / `complete copy` |
| Harness / integration | fixture-01 `FIXTURE_01_PHASE_PLAN_HANDOFF`: `selectedCandidate` with Contraption (`rocket-0`) plus Finishing Move materialized row `affordance:coyote` (room `STRAIGHTAWAY`); tests in [`coyoteEngineTestFixtures.test.ts`](../../../../../../../../../lambda/ephemera/dataSource/coyoteGame/generators/testHarness/coyoteEngineTestFixtures.test.ts) |
