...
**Status:** In progress -- **Option A is locked** (two Bedrock hops after combine). **Intent row + harness polish** (**Recommended order** line **282**) has shipped. Next actionable steps: token tuning from **`runCoyoteEngineTestHarness`** metrics; update durable **`AGENT.md`** (**line 283**). **Out of scope for this task plan:** data-driven migration toward **Option B** or **Option C** (see **Pipeline topology** below and **Recommended order**).
...
| Persistence + harness diagnostics | **Done (line 282):** [`CoyoteGameIntentRecord`](../../../../../lambda/ephemera/internalCache/coyoteGame.ts) matches **Decided: intent record shape** (**`phasePlan`**, **`walkthrough`**, no durable **`sceneAnalysis`**; legacy Dynamo **`sceneAnalysis`** maps to **`walkthrough`** on read); degraded persistence when **`phasePlan`** validation fails (**Decided: structured validation failure**). Harness concatenates **`selectionBody`**, **`phasePlanJson`** (+ **`phasePlanValidationReason`** when JSON absent), **`walkthrough`**, and usage lines ([`runCoyoteEngineTestHarness`](../../../../../lambda/ephemera/dataSource/coyoteGame/runCoyoteEngineTestHarness.ts)); pipeline **`GenerateHypothesisPipelineResult`** unchanged. |
...
- **Persistence (implementation):** **Shipped (line 282).** Remaining doc drift: update [`AGENT.md`](../../../../../lambda/ephemera/dataSource/coyoteGame/AGENT.md) (**line 283**) so steady-state docs describe **`walkthrough`** / **`phasePlan`** instead of **`sceneAnalysis`** on intent.
