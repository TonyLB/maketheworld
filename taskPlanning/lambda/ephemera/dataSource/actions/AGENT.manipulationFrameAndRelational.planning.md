...
- **`resolveComponent` I/O shape (FT-4, 2026-07-08):** the fault-tolerant gateway decided the span-resolution artifact shape C1 consumes. **Input** to `resolveComponent` is a **candidate pool** (evidence): `candidates[]`, each `{ id, label, relevance fields, locus }`, where **`locus`** is a structured "where is it now?" union (`room` \| `heldByActor` \| `heldByOtherCharacter` \| `withinObject`; deterministic membership evidence, room/held in v1). The input carries **no `status`** --- structural emptiness is the empty array. The **verdict** (`resolved` \| `consult` \| `error`) is `resolveComponent`'s **output**, decided at the single **FT-5** commit point --- not a field on the input. **Locked as two types (2026-07-08):** C1 models `SpanCandidatePool` (input) + `SpanResolutionOutcome` (output verdict) separately (a single combined type is rejected). Detail: [`AGENT.faultTolerantObjectManipulation.planning.md`](AGENT.faultTolerantObjectManipulation.planning.md) (**FT-4 decisions so far**).
