...
- [ ] **C1. Plan IR types + registry**
  ...
  - [ ] **LLM joint `(identity, plan)` proposer** (deferred from FT-3.2): open-language fallback when closed-world fast-path fails; emits N ranked tuples for FT-5 selection; owns semantic grey-band Abstain/Consult beyond numeric floors. See fault-tolerant plan **FT-3.2 decisions so far**.
...
| Fault-tolerant gateway ([`AGENT.faultTolerantObjectManipulation.planning.md`](AGENT.faultTolerantObjectManipulation.planning.md)) | In progress (**FT-4.1 e2e + FT-4.2 durable docs shipped 2026-07-10**; FT-4.3 Gateway sign-off still open; Phase C blocked) |
...
- **FT-2.1 steady-path identity (2026-07-09):** production identity emits `SpanCandidatePool` via [`resolveCatalogSpanToPool`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/resolveCatalogSpanToPool.ts); bridge [`selectSingleSpanFromPool`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/selectSingleSpanFromPool.ts) (`T_JOINT_*`). C1 handoff shape is **live on production path**.
- **FT-2.2 membership tuple selector (2026-07-10):** membership propose-N + FT-5 legality-gated selector + single-step dry-run + existence guard live ([`selectMembershipFromPool`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/selectMembershipFromPool.ts)). **FT-3.1 (2026-07-10):** membership Consult egress + actions OOC handler wired. **FT-3.2 (2026-07-10):** product **`Abstain`** + grey-band egress on deterministic path; **LLM joint proposer + complexity/frame-extract retirement deferred to Phase C (C1/C4)** --- gateway no longer blocked on an FT-layer LLM hop.
- **FT-3.3 relational selector (2026-07-10):** relational path cut over to propose-N + shared FT-5 selector ([`selectRelationalFromPools`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/selectRelationalFromPools.ts)); Consult/Abstain egress from [`compileRelational`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/compileRelational.ts); bridge retired from production. Gateway exit still blocked on FT-4 (e2e + durable docs + sign-off). Phase C remains blocked.
- **FT-4.1 / FT-4.2 (2026-07-10):** membership parseCommand e2e Consult matrix + dissolveRelation regression; durable trust posture + embeddingMatch v2 steady-state + Consult egress docs graduated. **FT-4.3 Gateway sign-off still open** --- Phase C remains blocked.
- **FT-1.2 pool merge (2026-07-09):** [`buildSpanCandidatePool`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/embeddingMatch/buildSpanCandidatePool.ts) emits `SpanCandidatePool` (rank-all, embed + lexical RMS joint relevance, gap-trim shortlist). **Superseded for routing by FT-2.1 note above.**
- **FT-1.1.5 lexical combine (2026-07-09):** flank geometry now combines via `tanhCenteredFlankScore` (edit gate * sigmoid(bias + sum tanh evidence)) in [`relevanceCombine.ts`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/embeddingMatch/relevanceCombine.ts); provisional constants in [`thresholds.ts`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/embeddingMatch/thresholds.ts). Identity corpus lexical pass confirms ordering invariants; tanh combine spreads more middle-band mass vs multiplicative v1 on wrapper cases.
- **FT-1.3.2-1.3.6 calibrated lexical combine (2026-07-09):** locked coverage bias, ratio-invariant flank channels, `w_adjoined=3.0`, `w_remote=0.4`, `biasMax=1.5`; `a/axe` lex < `T_JOINT_ABS`. **FT-1.3.1 admissibility gate retired (2026-07-09).** Gateway exit unchanged --- blocked on FT-2/FT-3/FT-5.
- **FT-1.3 calibration (2026-07-09):** locked FT-8/FT-1.2 constants + proposed FT-5 `T_JOINT_*` in [`thresholds.ts`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/embeddingMatch/thresholds.ts); pool snapshot [`embedding-identity-pool-v1-2026-07-09.json`](../../../../../lambda/ephemera/calibration/objectMatch/snapshots/embedding-identity-pool-v1-2026-07-09.json). **FT-1.3.1:** admissibility gate retired. Gateway exit still blocked (FT-2 identity wiring, FT-3 Consult, FT-5 selector implementation).
...
- **FT-1.4 leading-article normalization (2026-07-09):** classify-only QoL shipped in sibling plan --- [`peelLeadingArticleWhenTail`](../../../../../lambda/ephemera/dataSource/actions/discriminateIntent/peelLeadingArticleWhenTail.ts) aligns deterministic + LLM span parsing with prompt stripping rules. **Does not unblock Phase C** (gateway exit still blocked on FT-2/FT-3/FT-5).
- **FT-1.3.1 admissibility gate retired (2026-07-09):** short-span admissibility gate retired; lexical always on for non-empty spans. FT-5 auto-resolve consumes proposed `T_JOINT_*` on normalized joint relevance (not pool admission). Constants locked in [`thresholds.ts`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/embeddingMatch/thresholds.ts). Detail: [`AGENT.faultTolerantObjectManipulation.planning.md`](AGENT.faultTolerantObjectManipulation.planning.md) (**FT-8 decisions so far**). Legacy harness baseline retained for now; **C5** re-examines deletion after Phase C steady-state.
...
- **Legacy hops retire (FT-3, 2026-07-08; implementation home clarified FT-3.2, 2026-07-10):** the membership **complexity LLM** and the relational **frame extract** do **not** survive as distinct LLM steps --- **implementation lands in Phase C C4** (not FT-3.2). Complexity LLM -> **deterministic sandbox legality** + **shared validator** (BD-10 `defer`); its `verbClass`-agreement gap dissolves (membership `operationKind` is deterministic fallout once identity + host are pinned). Frame extract -> **net-new deterministic relational frame templating** (fast-path) + the **general joint proposer** (C1); the deterministic **normalizer** + **legality** tail is unchanged. **No narrow-scope middle LLM** ("tier-2" dissolves; the untemplated-simple-language case is a **classify** concern). Relational `operationKind` (BD-12) owner moves from a dedicated frame-extract hop to the **proposer / joint hop** --- this is the concrete content of **C4** ("deprecate ad-hoc stage graph"). **FT-3.2 shipped Abstain/Consult ownership on the deterministic path only.** Detail: [`AGENT.faultTolerantObjectManipulation.planning.md`](AGENT.faultTolerantObjectManipulation.planning.md) (**FT-3 decisions so far**, **FT-3.2 decisions so far**).
