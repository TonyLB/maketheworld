...
- [ ] **C1. Plan IR types + registry**
  ...
  - [ ] Define **`ParsePlanStep`** union and runtime primitive registry (transferMembership, establishRelation, dissolveRelation) --- the **grounded** shape the Synthesize-stage compiler emits. **`resolveComponent` is not a runtime primitive**; grounding is the selector verdict + existence/presence guard in the compiler tail, not an executable step.
