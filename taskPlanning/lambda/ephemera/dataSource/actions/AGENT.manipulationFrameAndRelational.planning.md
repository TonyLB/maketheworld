...
- **FT-1.3.2-1.3.6 calibrated lexical combine (2026-07-09):** locked coverage bias, ratio-invariant flank channels, `w_adjoined=3.0`, `w_remote=0.4`, `biasMax=1.5`; `a/axe` lex < `T_JOINT_ABS`. Admissibility gate **on**; FT-1.3.1 deferred post-merge. Gateway exit unchanged --- blocked on FT-2/FT-3/FT-5.
- **FT-1.3 calibration (2026-07-09):** locked FT-8/FT-1.2 constants + proposed FT-5 `T_JOINT_*` in [`thresholds.ts`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/embeddingMatch/thresholds.ts); pool snapshot [`embedding-identity-pool-v1-2026-07-09.json`](../../../../../lambda/ephemera/calibration/objectMatch/snapshots/embedding-identity-pool-v1-2026-07-09.json). Short-span admissibility **retained** (A/B retirement failed). Gateway exit still blocked (FT-2 identity wiring, FT-3 Consult, FT-5 selector implementation).
...
- **Pool relevance normalization (FT-8, 2026-07-08; calibrated FT-1.3 2026-07-09):** `SpanCandidatePool` relevance fields use absolute **`[0,1]`** per-signal scores before RMS joint merge --- embedding via two-point log map on cosine; lexical via Sellers alignment + edit gate * tanh-centered flank combine (FT-1.1.5 shipped 2026-07-09) with **catalog-derived short-span admissibility retained** (FT-1.3 A/B retirement failed). FT-5 auto-resolve consumes proposed `T_JOINT_*` on normalized joint relevance (not pool admission). Constants locked in [`thresholds.ts`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/embeddingMatch/thresholds.ts). Detail: [`AGENT.faultTolerantObjectManipulation.planning.md`](AGENT.faultTolerantObjectManipulation.planning.md) (**FT-8 decisions so far**).
