...
- [ ] **C1. Plan IR types + registry**
  ...
  - [ ] **`transferMembership` accepts an object set (BD-13)**, not only a single object; compiler builds the set via the sandbox's internal/boundary edge partition (transitive closure over **`carry`**-classified edges) so e.g. "get tray" (glass **`On`** tray) compiles to `dissolveRelation` + a set-valued `transferMembership` rather than deferring. Kernel (`applyObjectTakeHold`/`applyObjectDrop`) must recreate internal edges on the destination host graph within the same atomic `HostEffect` bundle. **Slice 1 of 2 done (2026-07-13; renamed 2026-07-13 for clarity):** the named blocker (`applyRelationalPatch`'s room-only guard) is resolved via a new, additive, host-agnostic persistence primitive --- [`positions/manipulation/relational/hostRelationalEdgeCarryTransactItems.ts`](../../../../../lambda/ephemera/dataSource/positions/manipulation/relational/hostRelationalEdgeCarryTransactItems.ts) (`HostRelationalEdgeCarry` type in `manipulation/types.ts`; originally named `HostRelationalEdgeRecreation`/`...RecreationTransactItems`, renamed once it was clear this is conceptually a **carry** --- an existing relationship persisting across a host move --- not a **create**, matching the `carry`-classified-edge vocabulary `interactionUnderTransfer.ts` already established), dispatching Room (`Meta::Room`/`fromRoomMeta`) vs. Character (`Meta::Character`/`fromCharacterMeta`) and reusing `EphemeraPositionGraph.addRelationalEdge` (host-agnostic, already used this way by the sandbox) rather than touching `applyRelationalPatch`/`HostRelationalPatch` at all --- that pathway stays intentionally Room-only per BD-6, **and stays deliberately separate even after BD-15/BD-16 generalize it beyond Room-only** (see BD-15 (4), resolved). **Remaining (slice 2, not done):** wire `applyObjectTakeHold`/`applyObjectDrop` to accept an object set, bundle this primitive's transact items into the **same** atomic `transactWrite` as the membership `HostEffect[]` bundle (today `applyHostEffects`/`applyHostRelationalPatch` are two separate `transactWrite` calls --- sequential use would violate BD-9), and update the `Object Take Hold`/`Object Drop` bus payloads from singular `objectId` to a set.
...
| BD-13 kernel work, slice 1 of 2: host-agnostic edge-recreation persistence | Done (2026-07-13) --- atomic multi-object transferMembership wiring remains |
