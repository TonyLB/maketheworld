...
- [ ] **C1. Plan IR types + registry**
  ...
  - [ ] **Extract Synthesize module** (mechanical, low-risk): move `interactionUnderTransfer.ts` / `computeCarryClosure` out of the sandbox's file family (`enrich/objectManipulation/`) into a Synthesize-named home; sandbox files (`sandboxState.ts` / `sandboxStep.ts` / `sandboxPlan.ts`) shrink to the pure-validation slice. No behavior change --- see [`AGENT.concepts.md`](../../../../../lambda/ephemera/dataSource/actions/AGENT.concepts.md) ("Current conflation").
  - [ ] **Define abstract-primitive types + Plan-stage compiler** (new foundational work, higher design risk --- refine as its own follow-on slice before starting): `ManipulationFrame` + verb/frame -> abstract primitive (`transferMembership` / `establishRelation` / `dissolveRelation` shape, referents like "wherever span X currently is" rather than resolved ids), with **zero KR access** --- testable without any graph fixture. Referent sub-language not yet designed; see [`AGENT.concepts.md`](../../../../../lambda/ephemera/dataSource/actions/AGENT.concepts.md) ("Abstract vs. grounded primitives").
  ...
  - [ ] **Synthesize-stage compiler**: abstract primitive + context (post-extraction Synthesize module + dry-run sandbox) -> grounded plan (usually length 1; sometimes 2+) or **`defer`** / **Error** per BD-10. (Was "deterministic compiler from `ManipulationFrame` + context -> plan" --- reworded 2026-07-12 to name the two stages explicitly.)
