...
- [ ] **C1. Plan IR types + registry**
  ...
  - [ ] **`transferMembership` accepts an object set (BD-13)**, not only a single object; compiler builds the set via the sandbox's internal/boundary edge partition (transitive closure over **`carry`**-classified edges) so e.g. "get tray" (glass **`On`** tray) compiles to `dissolveRelation` + a set-valued `transferMembership` rather than deferring. Kernel (`applyObjectTakeHold`/`applyObjectDrop`) must recreate internal edges on the destination host graph within the same atomic `HostEffect` bundle. **Slice 1 of 2 done (2026-07-13):** the named blocker (`applyRelationalPatch`'s room-only guard) is resolved via a new, additive, host-agnostic persistence primitive --- [`positions/manipulation/relational/hostRelationalEdgeRecreationTransactItems.ts`](../../../../../lambda/ephemera/dataSource/positions/manipulation/relational/hostRelationalEdgeRecreationTransactItems.ts) (`HostRelationalEdgeRecreation` type in `manipulation/types.ts`), dispatching Room (`Meta::Room`/`fromRoomMeta`) vs. Character (`Meta::Character`/`fromCharacterMeta`) and reusing `EphemeraPositionGraph.addRelationalEdge` (host-agnostic, already used this way by the sandbox) rather than touching `applyRelationalPatch`/`HostRelationalPatch` at all --- that pathway stays intentionally Room-only per BD-6. **Remaining (slice 2, not done):** wire `applyObjectTakeHold`/`applyObjectDrop` to accept an object set, bundle this primitive's transact items into the **same** atomic `transactWrite` as the membership `HostEffect[]` bundle (today `applyHostEffects`/`applyHostRelationalPatch` are two separate `transactWrite` calls --- sequential use would violate BD-9), and update the `Object Take Hold`/`Object Drop` bus payloads from singular `objectId` to a set.
  - [ ] **Synthesize-stage compiler**: ungrounded primitive + context (post-extraction Synthesize module + dry-run sandbox) -> grounded plan (usually length 1; sometimes 2+) or **`defer`** / **Error** per BD-10. (Was "deterministic compiler from `ManipulationFrame` + context -> plan" --- reworded 2026-07-12 to name the two stages explicitly.)
