...
- [ ] **C1. Plan IR types + registry**
  ...
  - [ ] **Define ungrounded position planning primitive types + Plan-stage compiler** (new foundational work, higher design risk --- refine as its own follow-on slice before starting): `ManipulationFrame` + verb/frame -> ungrounded primitive (`transferMembership` / `establishRelation` / `dissolveRelation` shape, referents like "wherever span X currently is" rather than resolved ids), with **zero KR access** --- testable without any graph fixture. Referent sub-language not yet designed; see [`AGENT.concepts.md`](../../../../../lambda/ephemera/dataSource/actions/AGENT.concepts.md) ("Ungrounded position planning primitives vs. grounded primitives").
