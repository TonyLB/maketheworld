...
This enables a cheaper **intent-classification** pass for most commands and a **second, Acme-only** Bedrock call that performs validation, Acme-style enrichment (normalized catalog **name**), and affinity evaluation on the **enriched** object (for example the beehive, not the raw "BEES!" line). A separate prose **description** for each object is **not** part of the game contract; prompts and persistence use **labels and affinities** only.
...
3. **Understand integration points**
   - **Intent + Acme delivery today**
     ...
     - [`actions/index.ts`](../../../../../lambda/ephemera/dataSource/actions/index.ts) — on `AcmeOrder`, emits stream + bus messages; [`publishedEvents.ts`](../../../../../lambda/ephemera/dataSource/actions/publishedEvents.ts) — **`AcmeOrderPublishedPayload`**: **`orders`** is **`AcmeOrderPublishedOrder[]`** (**`shortName`**, **`affinities`**, optional **`affinitiesFailed`**).
   - **Objects persistence**
...
- [X] Phase 1 - Schema and types (interfaces package + guards)
  - [X] Extend [`EphemeraMetaRoomObject`](../../../../../packages/mtw-interfaces/ts/ephemeraMeta.ts) with optional fields aligned to the contract: **`affinities`** (array of discriminated role possibilities with `aptness`; `entity_modification` variants include `target` + `mode`), and **`affinitiesFailed`** (or equivalent) when the enrich step did not produce validated affinities so downstream code can tell **missing** from **known-empty** from **unavailable due to LLM failure**.
...
- [X] Phase 3 - Acme enrich prompt and parsing (full prompt quality)
  - [X] Replace the Phase 2 placeholder: expand **`buildParseAcmeOrderEnrichPrompt`** so instructions include (1) full role vocabulary and **`affinities`** with **aptness**, (2) enforce no RPG wording, (3) drop possibilities below the aptness floor, (4) normalize player text to Acme register **`name`** with the full catalog rigor intended for production.
  - [X] Extend parser tests with fixtures (beehive, shovel, rope-style multi-role examples from the handoff).
  - [X] **Planned (see [Planned Stage B enrich output](#planned-stage-b-enrich-output)):** Intermediate **Markdown** output with **one entry per line item** and explicit **valid** vs **invalid** (with **Not a thing** / **Not tangible** / **Too large**) for chain-of-reasoning, then the existing JSON **`lines`** contract. **Implementation:** one assistant message with Markdown then a trailing json-tagged fenced block; **[`interpretAcmeOrderEnrichBody`](../../../../../lambda/ephemera/dataSource/actions/mergeAcmeOrderEnrich.ts)** uses **[`splitMarkdownReasoningAndJson`](../../../../../lambda/ephemera/llm/splitMarkdownReasoningAndJson.ts)** (plus **`extractJsonObjectText`** fallback) so only parsed JSON feeds merge.
...
- [X] Phase 4 - Persistence and bus payload
  - [X] Extend [`AcmeOrderPublishedPayload`](../../../../../lambda/ephemera/dataSource/actions/publishedEvents.ts) **in place** (same **`type: 'Acme Order'`**) so `mtw.ephemera.objects` receives **full enriched objects**: keep the property name **`orders`**, but change it from **`string[]`** to an **array of objects** — each element has **`shortName`** (= enriched catalog **`name`**), **`affinities`**, and **`affinitiesFailed`** when applicable. Update [`isAcmeOrderPublishedPayload`](../../../../../lambda/ephemera/dataSource/actions/publishedEvents.ts) accordingly.
...
- **No per-object prose `description`:** Catalog-style blurbs are **not** modeled in room objects, bus payloads, or Coyote prompts for this initiative; **shortName + affinities** carry the intended contract.
- **`AcmeOrderPublishedPayload` evolution:** **Extend** the existing payload type and event shape; do **not** introduce a separate versioned successor bus event for this initiative.
- **`orders` on the bus:** Keep the field name **`orders`**; replace **`string[]`** with an array of **objects** (one per persisted catalog line: **`shortName`**, **`affinities`**, optional **`affinitiesFailed`**) aligned with **`EphemeraMetaRoomObject`** fields other than **`uuid`** (assigned when merging).
...
**Status:** Implemented in prompt + **`interpretAcmeOrderEnrichBody`**. **[`buildParseAcmeOrderEnrichPrompt`](../../../../../lambda/ephemera/dataSource/actions/buildParseAcmeOrderEnrichPrompt.ts)** asks for Markdown chain-of-reason first, then a trailing json-tagged fence with the **`AcmeOrderEnrichModelResponse`** object only.

**Goal:** An explicit **intermediate Markdown output** so the model can show **chain-of-reasoning** before the machine-readable **`AcmeOrderEnrichModelResponse`** contract. Classification justification stays in Markdown; **`interpretAcmeOrderEnrichBody`** strips reasoning and parses JSON only (**[`splitMarkdownReasoningAndJson`](../../../../../lambda/ephemera/llm/splitMarkdownReasoningAndJson.ts)**).
...
**Final JSON:** Single source of truth for [`mergeAcmeOrderEnrich`](../../../../../lambda/ephemera/dataSource/actions/mergeAcmeOrderEnrich.ts) and downstream persistence. **Chosen approach:** one assistant message with Markdown then fenced JSON; parser extracts JSON via **`splitMarkdownReasoningAndJson`**.

**Tests:** [`buildParseAcmeOrderEnrichPrompt.test.ts`](../../../../../lambda/ephemera/dataSource/actions/buildParseAcmeOrderEnrichPrompt.test.ts) and [`mergeAcmeOrderEnrich.test.ts`](../../../../../lambda/ephemera/dataSource/actions/mergeAcmeOrderEnrich.test.ts) cover CoR + fence and fallbacks.
