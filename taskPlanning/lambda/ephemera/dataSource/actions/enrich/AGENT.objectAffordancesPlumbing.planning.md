...
**Status:** In progress. Phases A0-A3 complete and field naming was updated mid-development from `affordances` to `environmentAffordances`; next step is Phase A4 (verification and closeout).

Task-planning conventions: [`taskPlanning/AGENT.md`](../../../../AGENT.md).

## Purpose

Track a bounded plumbing task to introduce an optional `environmentAffordances?: string[]` channel on each `tropeAffinities` element and carry it through types, persistence, and prompt-ingest seams.
...
- Add `environmentAffordances?: string[]` to the relevant `tropeAffinities` element type contracts.
...
- Keep compatibility with existing records/fixtures that do not include `environmentAffordances`.
...
- New planning logic that actively consumes `environmentAffordances` for decision quality.
...
2. Validation and normalization policy for `environmentAffordances`:
...
4. Prompt plumbing boundary:
   - This task only threads `environmentAffordances` through prompt input seams plus object-cache persistence paths.
   - Prompt text/rendering remains behaviorally unchanged in this slice.
5. Compatibility and migration:
   - Mid-development strict-breaking rename: `affordances` -> `environmentAffordances`.
   - No historical backfill/migration is required.
   - Existing fixtures/data without `environmentAffordances` remain valid.
...
| `environmentAffordances` validation policy | Anthony + Agent | Allow omitted / `string[]` / explicit `[]`; reject non-array and non-string elements; ignore empty/whitespace-only strings at construction time. | 2026-04-29 |
...
| Mid-development field rename | Anthony + Agent | Rename trope-affinity field from `affordances` to `environmentAffordances` to reduce source-of-affordance ambiguity; strict-breaking policy rejects legacy key in validators/tests. | 2026-04-29 |
| Backfill/migration requirement | Anthony + Agent | No historical backfill/migration required; existing fixtures/data without `environmentAffordances` remain valid. | 2026-04-29 |
...
- Empty or whitespace-only environment-affordance entries should be filtered during normalization, then persistence should prefer omission over explicit empty arrays.
...
- [X] Phase A1 - add type and validation support
  - [X] Add optional `environmentAffordances?: string[]` to canonical interfaces/types.
  - [X] Update parser/validator schemas to accept missing or present `environmentAffordances`.
  - [X] Preserve strictness for non-array or non-string invalid values.
  - Locked implementation notes:
    - Canonical `CoyoteTropeAffinity` now includes optional `environmentAffordances?: string[]` in `packages/mtw-interfaces/ts/coyotePlanAffinities.ts`.
...
- [X] Phase A2 - persistence plumbing
  - [X] Ensure serialization/writes preserve optional `environmentAffordances` when present.
  - [X] Ensure hydration/reads expose `environmentAffordances` when present and remain compatible when absent.
  - [X] Add regression coverage for both present and absent cases.
  - Locked implementation notes:
    - Acme order write-path mapping/stream payload assertions now lock nested `tropeAffinities[].environmentAffordances` passthrough plus compatibility entries where `environmentAffordances` is absent.
    - Meta-room persistence snapshot coverage now locks `priorObjects`/`newObjects` preservation for both present and absent `tropeAffinities[].environmentAffordances`.
    - Room-object hydration/formatting coverage confirms read-path objects carrying `environmentAffordances` remain valid while rendered prompt text remains unchanged.
...
- [X] Phase A3 - prompt-ingest plumbing
  - [X] Thread `environmentAffordances` through prompt input structures where `tropeAffinities` already flows.
  - [X] Keep prompt content behavior unchanged (channel may remain unused textually).
  - [X] Add/update tests to prove no regressions in existing prompt assembly behavior.
  - Locked implementation notes:
    - Prompt-ingest seam contracts are already typed through `CoyoteRoomObjectsByRoom` / `EphemeraMetaRoomObject`, so optional `tropeAffinities[].environmentAffordances` now flows without additional production-path shape changes.
    - Prompt text/rendering remains unchanged: staged-object formatting continues to render trope labels from `trope`, `aptness`, and `narrowing` only.
    - Added prompt assembly regressions in plan-selection, phase-plan-hop, and outcome prompt builders that pass staged objects containing `tropeAffinities[].environmentAffordances` and assert no environment-affordance text leaks into prompt output.
    - Mid-development rename was applied as strict-breaking: canonical validators now reject legacy `tropeAffinities[].affordances`.
    - A3 targeted verification passed:
      - `npm --prefix lambda/ephemera run test -- --watchAll=false dataSource/actions/publishedEvents.test.ts`
      ...
      - `npm --prefix lambda/ephemera run test -- --watchAll=false dataSource/coyoteGame/generators/pipelines/hypothesis/buildHypothesisPhasePlanHopPromptParts.test.ts`
      - `npm --prefix lambda/ephemera run test -- --watchAll=false dataSource/coyoteGame/generators/pipelines/outcome/buildPlanOutcomePrompt.test.ts`
      - `npm --prefix lambda/ephemera run test -- --watchAll=false dataSource/coyoteGame/utilities/coyoteRoomObjectSnapshot.test.ts`
...
| Prompt-ingest plumbing landed | Done |
| Field rename to `environmentAffordances` propagated | Done |
