...
2. Read [`AGENT.classifyPlanGeneralization.planning.md`](AGENT.classifyPlanGeneralization.planning.md)'s CPG-5 row and its "Carried-in findings from BD-22" section --- the two concrete gaps named there (`RoomInPlayObjectCatalogEntry` is `EphemeraObjectId`-only; `EphemeraObjectId`/`EphemeraCharacterId` are distinct branded types) are exactly what Phase 2 below has to close.
...
- [ ] **Phase 2. Resolve CPG-5 concretely for this one case.**
  ...
  - [ ] Close the two gaps named in iteration 7's carried-in BD-22 findings: `RoomInPlayObjectCatalogEntry` is `EphemeraObjectId`-only and never scans Character nodes; `EphemeraObjectId`/`EphemeraCharacterId` are distinct branded types throughout. Resolve only as far as this iteration's concrete referent needs require --- do not build a fully general cross-family `Identify` abstraction speculatively.
...
| --- | --- | --- | --- |
| PK-1 | Exact shape/location of the new `describe` instruction kind (widen `KernelStep` vs. sibling type) | Phase 1 | Open |
