...
- **Shipped path (event-driven look):** `LookRoom` in actions **`streamEvent`** **`Look Command Requested`**; **`mtw.ephemera.renderOrchestration`** subscribes (**[`subscribedEvents.ts`](../../../../lambda/ephemera/dataSource/renderOrchestration/subscribedEvents.ts)**), runs **`Perception Thread Registered`** on **`lookCommandPerceptionThreadLaneId`**, **`await messageBus.flush(thatLane)`**, then **`sendRenderRequested(..., { useDefaultMessageBusLane: true })`**; terminal **`app`** **`messageBus.flush()`** drains the default-lane **Render Requested** and orchestration outbounds. Shared prep: [`prepareFullRoomDescriptionRenderForCharacter`](../../../../lambda/ephemera/dataSource/actions/requestFullRoomDescriptionForCharacter.ts) (used by the handler and by legacy direct **`requestFullRoomDescriptionForCharacter`**).
- **Helper path (not used for in-room `LookRoom` in production):** **`requestFullRoomDescriptionForCharacter`** still enqueues default-lane perception + **named** **`renderOrchestration:*` `sendRenderRequested`** for other callers.
...
| Event-driven look: actions outbound, render orchestration subscribe, lane-ordered `PerceptionThreads` then default-lane `Render Request` | Done: [`subscribedEvents.ts`](../../../../lambda/ephemera/dataSource/renderOrchestration/subscribedEvents.ts), [`handleLookCommandRequestedForRenderOrchestration.ts`](../../../../lambda/ephemera/dataSource/renderOrchestration/handleLookCommandRequestedForRenderOrchestration.ts), [`prepareFullRoomDescriptionRenderForCharacter`](../../../../lambda/ephemera/dataSource/actions/requestFullRoomDescriptionForCharacter.ts) |
| Durable doc touch-up (`dataSource/actions/AGENT.md`, `parse` pointer; post-next-phase render + actions `AGENT.md` ingress) | Done (AGENT + [`app.ts`](../../../../lambda/ephemera/app.ts) + [`messageBus/AGENT.md`](../../../../lambda/ephemera/messageBus/AGENT.md)) |
...
- [X] **Next phase --- emit from actions:** In [`index.ts`](../../../../lambda/ephemera/dataSource/actions/index.ts), for **`LookRoom`**, **`streamEvent`** the new outbound in-room (and stop or feature-flag the direct **`requestFullRoomDescriptionForCharacter`** call so there is a single code path to production). Keep **OOC** and **paraphrase** tests aligned.
- [X] **Next phase --- render orchestration subscription:** Extend [`isRenderOrchestrationSubscribedEnvelope`](../../../../lambda/ephemera/dataSource/renderOrchestration/subscribedEvents.ts) and related types so **`dataSourceKey: 'mtw.ephemera.actions'`** + the new header **`type`** are part of the union; **`RenderOrchestrationSubscribedContent`** extends with **`LookCommandRequestedPublishedPayload`** in [`subscribedEvents.ts`](../../../../lambda/ephemera/dataSource/renderOrchestration/subscribedEvents.ts) (ingress wiring in [`index.ts`](../../../../lambda/ephemera/dataSource/renderOrchestration/index.ts)).
- [X] **Next phase --- handler and ordering:** In the new branch, **`sendPerceptionThreadRegistered`** on **`lookCommandPerceptionThreadLaneId`**, **`await messageBus.flush(thatLaneId)`**, then **`sendRenderRequested(..., { useDefaultMessageBusLane: true })`** (default-lane **Render Requested**; no **`renderOrchestration:*` flush** for that kick). See [`handleLookCommandRequestedForRenderOrchestration.ts`](../../../../lambda/ephemera/dataSource/renderOrchestration/handleLookCommandRequestedForRenderOrchestration.ts).
- [X] **Next phase --- app flush and prod checks:** Document in [`app.ts`](../../../../lambda/ephemera/app.ts) and [`messageBus/AGENT.md`](../../../../lambda/ephemera/messageBus/AGENT.md) that only the **perception** lane needs an explicit **`flush(laneId)`** for `look`; default-lane drain at Lambda exit covers the **Render Requested**. Jest: `handleLookCommandRequestedForRenderOrchestration.test.ts`, `subscribedEvents.test.ts`, `index.test.ts` (render orchestration). **Prod / CloudWatch / client `RoomDescription`:** manual re-verify when deploying.
- [X] **Next phase --- document:** Update [`dataSource/renderOrchestration/AGENT.md`](../../../../lambda/ephemera/dataSource/renderOrchestration/AGENT.md) and [`dataSource/actions/AGENT.md`](../../../../lambda/ephemera/dataSource/actions/AGENT.md) with the sibling-event contract; **Progress** and **Durable doc** rows updated in this file.
...
- `LookCommandRequestedPublishedPayload` / **`Look Command Requested`** + **`isLookCommandRequestedPublishedPayload`** in `publishedEvents.ts` (Jest: `dataSource/actions/publishedEvents.test.ts`). In-room `Look Room` in `index.test.ts` via **`streamEvent`**. **`isRenderOrchestrationSubscribedEnvelope`**, **`isLookCommandRequestedActionsEnvelope`**, **`lookCommandPerceptionThreadLaneId`**, and perception-only **`flush`** in `dataSource/renderOrchestration/*.test.ts` and `handleLookCommandRequestedForRenderOrchestration.test.ts`.
