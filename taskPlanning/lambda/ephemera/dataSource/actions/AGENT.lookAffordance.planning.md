...
| Event-driven look: actions outbound, render orchestration subscribe, lane-ordered `PerceptionThreads` then `Render Request` | In progress: type + emit in [`index.ts`](../../../../lambda/ephemera/dataSource/actions/index.ts) (`Look Command Requested` only in-room; no direct `requestFullRoomDescriptionForCharacter`); subscribe + ordering in [Recommended order](#recommended-order) |
...
- `LookCommandRequestedPublishedPayload` / **`Look Command Requested`** + **`isLookCommandRequestedPublishedPayload`** in `publishedEvents.ts` (Jest: `dataSource/actions/publishedEvents.test.ts`). In-room `Look Room` in `index.test.ts` via **`streamEvent`**. Remaining next phase: `isRenderOrchestrationSubscribedEnvelope` + perception **lane** `flush` in logs/tests.
