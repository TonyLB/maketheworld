...
**Status:** Scoped through conversation 2026-08-02. Phase 1 (mtw-wml situation facets), Phase 2 (mtw-gateways host-gate widening), Phase 3 (Character as a render-cache host, `lambda/ephemera`), and Phase 4 (retire the Object stub) all shipped 2026-08-02. Phase 5 not started.
...
- [X] **Phase 4. Retire the Object stub.** Done 2026-08-02.
  - [X] Once Phases 1--2 land, Object rides the real `ensureAuthoredCatalog`; delete `ensureObjectShortNameCacheRecord.ts` and its injection at `orchestrateRenderRequest`'s `ensureAuthoredCatalog` seam. Done 2026-08-02 --- **design correction found during implementation:** a literal stub-delete-only swap would have regressed every Object to a blank/nameless render, since the real `ensureAuthoredCatalog` only pulls from an authored `SITUATION#DEFAULT` `<Example>` facet, which no existing Object has (Phase 1 just added the WML support; spawn-time generation is the still-separate Phase 5). Per user decision, kept a **live shortName fallback in the WML builder** instead of a literal delete: added `resolveObjectShortName` to [`objects/objectShortName.ts`](../../../../../lambda/ephemera/dataSource/objects/objectShortName.ts) (composing the existing `shortNameFromMergedAggregate` → improvisation-pair-row fallback chain already duplicated by `roomObjectCatalogForCharacter.ts`/`heldInventoryCatalogForCharacter.ts` --- this is the fourth call site of an established pattern, not a new one), and threaded it through `objectRenderWmlFromCacheRecord.ts`'s new `fallbackShortName` param, resolved at `orchestrate.ts`'s `handleObjectRenderPertains` call site. `handleLookCommandRequestedForRenderOrchestration.ts`'s Object branch is now structurally identical to Feature/Knowledge/Character's (no override).
  - [X] Confirm the U+2060 placeholder in [`objectRenderWmlFromCacheRecord.ts`](../../../../../lambda/ephemera/dataSource/perception/objectRenderWmlFromCacheRecord.ts) is still needed (it guards `<Object>`'s non-empty-`ShortName` content-model rule, which is independent of prose) --- confirmed, kept: it's now the final fallback after both the facet's own name and the live shortName resolve to nothing.
  - [X] Update the durable Object-description policy in [`perception/AGENT.md`](../../../../../lambda/ephemera/dataSource/perception/AGENT.md#correlated-object-description-policy), which currently documents the stub as deliberate. Done 2026-08-02 --- rewritten to describe the real `ensureAuthoredCatalog` path plus the WML-builder shortName fallback. Note: the client (`ComponentDescription.tsx`) still only reads `component.shortName` for Object (never `.render`/`.situations`), so piping real facet *description* prose through to the client remains separate, deferred work --- not attempted here.
...
| Phase 4 (retire the Object stub) | Done (2026-08-02) |
