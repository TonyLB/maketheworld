...
- [X] **Types:** **`stableKey?: string`** on [`AcmeOrderPublishedOrder`](../../../../../lambda/ephemera/dataSource/actions/publishedEvents.ts) and [`EphemeraMetaRoomObject`](../../../../../packages/mtw-interfaces/ts/ephemeraMeta.ts); guards [**`isAcmeOrderPublishedOrder`**](../../../../../lambda/ephemera/dataSource/actions/publishedEvents.ts), [**`isEphemeraMetaRoomObject`**](../../../../../packages/mtw-interfaces/ts/ephemeraMeta.ts) (present value must be non-empty after trim). **Persisted objects** stay optional for legacy; **bus** **`AcmeOrderPublishedOrder.stableKey`** optional until **`index.ts`** wiring — then flip to **`stableKey: string`** required. Step B / **`coyotePlanAffinities`** unchanged (already emits **`stableKey`**). Jest: **`publishedEvents.test.ts`**, **`isEphemeraMetaRoomObject`** cases in **`coyotePlanAffinities.test.ts`**.
