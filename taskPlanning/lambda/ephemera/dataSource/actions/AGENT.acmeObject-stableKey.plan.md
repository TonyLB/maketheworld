...
1. **LLM-first (semantic):** **Before** relying on numeric repair alone, steer the Acme Step B enrich **LLM** to propose **`stableKey`** candidates that **semantically disambiguate** when possible (e.g. **`rocket-high-powered`** rather than opaque duplicates) --- the model has the richest access to catalog gloss and intent. That step may be **non-deterministic** across calls; proposals may still collide with each other, with **Coyote-wide** occupancy, or with **charset** rules.
...
1. **Phase 1 (LLM) wire format:** Field name and JSON shape for model-proposed **`stableKey`** in Step B output; whether the enrich prompt includes the **Coyote-wide occupied-key list**. Must align with **Disambiguation phases**; implementation detail only.
...
- [ ] **Design note:** Reference **Disambiguation phases**, **Deterministic numeric repair**, and **Where deterministic enforcement runs** from Purpose; document Phase 1 prompt/JSON choices from **Unknowns** in this section or module docstrings.
- [ ] **Step B enrich:** Extend [`buildParseAcmeOrderEnrichPrompt`](../../../../../lambda/ephemera/dataSource/actions/buildParseAcmeOrderEnrichPrompt.ts) / JSON contract so the model **proposes** **`stableKey`** per line (semantic disambiguation first); include Coyote-wide occupied keys in prompt **if** product chooses (optional).
