...
**Design note:** The behavioral contract for **`stableKey`** is specified under Purpose in **Disambiguation phases**, **Deterministic numeric repair**, **Where deterministic enforcement runs**, and **Phase 1 Step B enrich contract**. Implementation should link or summarize those headings from module docstrings on touchpoints ([`buildParseAcmeOrderEnrichPrompt`](../../../../../lambda/ephemera/dataSource/actions/buildParseAcmeOrderEnrichPrompt.ts), [`finalizeStableKeysDeterministic.ts`](../../../../../lambda/ephemera/dataSource/actions/finalizeStableKeysDeterministic.ts), [`index.ts`](../../../../../lambda/ephemera/dataSource/actions/index.ts)) when wiring; durable narrative belongs in AGENT.md updates per the **Durable docs** checklist item when shipped.
...
- [X] **Deterministic enforcement:** [`finalizeStableKeysDeterministic.ts`](../../../../../lambda/ephemera/dataSource/actions/finalizeStableKeysDeterministic.ts) — **pure** `finalizeStableKeysDeterministic` (and shared [`normalizeStableKeyCharset`](../../../../../packages/mtw-interfaces/ts/coyotePlanAffinities.ts) / `defaultStableKeyProposal` in **`coyotePlanAffinities`**); inputs = LLM-proposed **`stableKey`** per line + occupied set + batch line order; output = final keys per **Deterministic numeric repair** and charset / **`constructed-`** rules; legacy rows without **`stableKey`** contribute nothing to the occupied set. Jest: `finalizeStableKeysDeterministic.test.ts`.
- [X] **Types:** **`stableKey: string`** required on [`AcmeOrderPublishedOrder`](../../../../../lambda/ephemera/dataSource/actions/publishedEvents.ts) after **`index.ts`** wiring; **`stableKey?: string`** on [`EphemeraMetaRoomObject`](../../../../../packages/mtw-interfaces/ts/ephemeraMeta.ts) for legacy rows; guards [**`isAcmeOrderPublishedOrder`**](../../../../../lambda/ephemera/dataSource/actions/publishedEvents.ts), [**`isEphemeraMetaRoomObject`**](../../../../../packages/mtw-interfaces/ts/ephemeraMeta.ts) (present value must be non-empty after trim). Step B / **`coyotePlanAffinities`** unchanged (already emits **`stableKey`**). Jest: **`publishedEvents.test.ts`**, **`isEphemeraMetaRoomObject`** cases in **`coyotePlanAffinities.test.ts`**.
- [X] **Wire [`index.ts`](../../../../../lambda/ephemera/dataSource/actions/index.ts):** Thin wrapper after Step B merge per **Where deterministic enforcement runs** --- gather Coyote-wide occupancy, call **`finalizeStableKeysDeterministic`**, attach **final** keys before [`streamEvent`](../../../../../lambda/ephemera/dataSource/actions/index.ts) **`Acme Order`** and [`handleAcmeOrderAddObjects`](../../../../../lambda/ephemera/dataSource/objects/handleApiObjectsChange.ts) (new objects still persist only to the character's **current** room).
    - **Shipped:** [`collectCoyoteOccupiedStableKeys.ts`](../../../../../lambda/ephemera/dataSource/actions/collectCoyoteOccupiedStableKeys.ts) builds the occupied set from **`Meta::Room.objects`** across **`CoyoteGame.gameRooms`**; [`index.ts`](../../../../../lambda/ephemera/dataSource/actions/index.ts) prefetches once per parse, passes **`occupiedStableKeys`** into **`parseCommand`** (Step B enrich) and **`finalizeStableKeysDeterministic`**, then publishes **`stableKey: string`** on each bus line; [`handleAcmeOrderAddObjects`](../../../../../lambda/ephemera/dataSource/objects/handleApiObjectsChange.ts) maps **`stableKey`** onto **`add`** payloads (merge layer unchanged).
- [X] **Persist path:** Ensure **`mergePersistMetaRoomObjects`** stores **`stableKey`** on new rows; extend [`handleAcmeOrderAddObjects`](../../../../../lambda/ephemera/dataSource/objects/handleApiObjectsChange.ts) mapping from published order to **`add`** payload.
    - **Shipped:** **`mergeMetaRoomObjects`** persists each **`add`** row as a full **`EphemeraMetaRoomObject`** (optional **`stableKey`** included); [`handleAcmeOrderAddObjects`](../../../../../lambda/ephemera/dataSource/objects/handleApiObjectsChange.ts) maps **`stableKey`** from the bus; Jest: **`mergePersistMetaRoomObjects.test.ts`** (**`stableKey`** append / upsert cases).
- [X] **Tests:** Unit tests for allocator + [`mergeAcmeOrderEnrich.test.ts`](../../../../../lambda/ephemera/dataSource/actions/mergeAcmeOrderEnrich.test.ts) / [`handleApiObjectsChange.test.ts`](../../../../../lambda/ephemera/dataSource/objects/handleApiObjectsChange.test.ts) / [`index.test.ts`](../../../../../lambda/ephemera/dataSource/actions/index.test.ts) as touched.
    - **Shipped:** Allocator / occupancy covered by **`finalizeStableKeysDeterministic.test.ts`**, **`collectCoyoteOccupiedStableKeys.test.ts`**, **`parseCommand.test.ts`** (**`occupiedStableKeys`** → enrich); **`mergeAcmeOrderEnrich.test.ts`** asserts **`stableKey`** on interpreted lines and multi-line **`finalizeAcmeOrderFromStepB`**; **`handleApiObjectsChange.test.ts`** persists **`stableKey`**; **`index.test.ts`** wires prefetch → **`parseCommand`** (**`occupiedStableKeys`**) plus finalized bus keys / collision repair.
...
- Deterministic keys: `npx jest dataSource/actions/finalizeStableKeysDeterministic.test.ts`
- Published-order guards: `npx jest dataSource/actions/publishedEvents.test.ts`
