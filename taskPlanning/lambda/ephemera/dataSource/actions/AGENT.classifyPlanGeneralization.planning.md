...
- [ ] **`LookRoom`-paraphrase LLM fallback (Sub-iteration 2's deliberately-unclosed regression, folded back in 2026-07-31).** Bare `look`/`l` paraphrases (e.g. "peruse the room") have no closed lexicon and no text-parse path today --- Sub-iteration 2 shipped command-plan dispatch for five of six non-object-manipulation families but explicitly excluded this one as LLM-fallback territory, not template territory. Named again during iteration 9 (tracked there as PK-5; that plan shipped and retired 2026-08-01) as explicitly *separate* from that iteration's object-directed-look work ("what happens once Plan has produced a grounded read instruction," not "classify/Plan doesn't recognize the command at all") --- folded back into this doc, its original home, once iteration 9 retired. No concrete design started; open-ended phrasing like this needs a real LLM fallback, not a hardcoded paraphrase lexicon --- do not attempt to close this by enumerating more skeleton templates.
- **CPG-5 is not tracked here anymore.** Revived 2026-07-24 as iteration 9, which shipped and retired 2026-08-01 --- its type-level half (`EphemeraThingId` through Grounding/Identify) landed and graduated to [`actions/AGENT.concepts.md`](../../../../../lambda/ephemera/dataSource/actions/AGENT.concepts.md); the still-open remainder (character-inclusive referent catalog population) is not folded back here either --- see iteration 10 in [`AGENT.objectManipulationIterations.planning.md`](AGENT.objectManipulationIterations.planning.md).
