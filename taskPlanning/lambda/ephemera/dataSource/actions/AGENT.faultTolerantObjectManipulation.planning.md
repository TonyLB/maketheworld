...
- [ ] **FT-1.4 Leading-article span normalization (deferred QoL --- not Gateway-blocking)**
  ...
  - [ ] Wire deterministic classify [`deterministicChecks.ts`](../../../../../lambda/ephemera/dataSource/actions/discriminateIntent/deterministicChecks.ts) (replaces `the`-only `stripLeadingArticle`).
  - [ ] Optional defensive pass on LLM-parsed manipulation / Acme spans in [`intentClassification.ts`](../../../../../lambda/ephemera/dataSource/actions/discriminateIntent/intentClassification.ts) for consistency with prompt stripping rules.
  - [ ] Tests: peel-when-tail vs leave-alone cases; **not** pool-builder or embed-skip assertions.
...
| FT-1 candidate pool (embedding + lexical) | **FT-1.3.1 admissibility gate retired (2026-07-09)** --- lexical always on; legacy gate in harness only; canonical snapshot `embedding-identity-pool-v1-2026-07-09-bias-sweep.json`; production `identityStage` still v1 until FT-2 |
