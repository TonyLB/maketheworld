...
**Still pending (implementation, not decision):** FT-2.2 tuple selector + joint proposer; Consult wire (FT-3); existence/presence guard. **FT-2.1 shipped (2026-07-09):** pool emission + bridge selector; identity LLM + v1 terminal embed resolve retired from production.
...
- [ ] **FT-2** decided --- identity LLM role in steady path documented (**decided (e) 2026-07-08**; **FT-2.1 shipped 2026-07-09**: pool emission + bridge selector; identity LLM retired from production; **FT-2.2** tuple selector + validator pending).
...
- [ ] **FT-2.2 Selector + validator integration**
  - [ ] Wire FT-5 selector + shared validator against pool relevance (FT-1) and legality (FT-3 dry-run sandbox); no standalone identity validation hop.
  - [ ] Tests: illegal-if-wrong tuple rejected by selector; thin-margin -> Consult; absent-object grey band -> Abstain per FT-2.1 calibration gate.
...
| FT-2 identity tier + recovery | **FT-2.1 shipped (2026-07-09)**: pool emission + bridge selector on golden path; identity LLM removed from production; **FT-2.2** (tuple selector + validator) pending |
