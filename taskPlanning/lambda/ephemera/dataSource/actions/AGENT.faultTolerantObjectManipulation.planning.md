...
| --- | --- | --- | --- |
| **FT-1** | **Candidate pool construction** --- How do **embeddings** (cosine rank on **`EMBEDDING#IMPROMPTU`**, optional enriched index) combine with **lexical similarity** scans (token overlap, edit distance, normalized shortName) to produce a **ranked candidate pool** per span with **confidence ratings** (absolute score, margin, eligible count, source tags)? Merge strategy: union+rerank, conjunctive gate, or staged shortlist? Top-N and floor constants --- recalibrate or inherit [`thresholds.ts`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/embeddingMatch/thresholds.ts)? | FT-2, FT-3, gateway | Open |
...
- Lexical scan on **normalized shortName** only, or also description when enriched index ships?
- Weight embedding vs lexical when both rank the same wrong object (unary catalog trap)?
- Reject [`embeddingMatch/AGENT.md`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/embeddingMatch/AGENT.md) **lexical backstop** for v1 --- still rejected, or revived as **pool builder** not terminal gate?
...
- [ ] **FT-1** decided --- candidate pool + confidence contract documented and tested.
...
- [ ] **FT-1.3 Calibration pass**
  ...
  - [ ] Lock or explicitly defer threshold constants with provenance comment.
...
| FT-1 candidate pool (embedding + lexical) | Not started |
