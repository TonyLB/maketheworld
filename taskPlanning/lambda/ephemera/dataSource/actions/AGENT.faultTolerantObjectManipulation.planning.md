...
**Status:** In progress (FT-3.2 Abstain ownership shipped 2026-07-10; LLM joint proposer deferred to Phase C). **Gateway** for Phase C of [`AGENT.manipulationFrameAndRelational.planning.md`](AGENT.manipulationFrameAndRelational.planning.md) --- do not begin Plan IR / compiler work until **Gateway exit** below is satisfied.
...
- [ ] **FT-2** decided --- identity LLM role in steady path documented (**decided (e) 2026-07-08**; **FT-2.1 shipped 2026-07-09**; **FT-2.2 shipped 2026-07-10**: membership tuple selector + single-step dry-run + existence guard; **FT-3.1 Consult egress shipped 2026-07-10**; relational bridge retained).
- [ ] **FT-3** decided --- Abstain vs Consult wire types + owner stages documented (**FT-3.1 Consult + FT-3.2 Abstain shipped 2026-07-10**; hop retirement / LLM joint proposer **deferred to Phase C C1/C4** with written rationale --- see **FT-3.2 decisions so far**; FT-3.3 relational selector still open).
...
- [ ] **FT-3.3 Selector + existence-guard rules (foundation for C1)**
  - [ ] Deterministic **selector**: N `(identity, plan)` candidates -> auto-resolved tuple | consult | error per **FT-5** policy; per-span **existence/presence guard** (referential integrity) on the chosen tuple. (`resolveComponent` retired as a standalone primitive --- these are selector-tail facets, not a runtime Plan IR step.) **Partial (FT-2.2):** membership path shipped; relational cutover + C1 packaging still here.
  - [ ] Tests: high-confidence auto-resolve; low-confidence consult; noMatch abstain/error; hallucinated/absent id -> guard rejects. **Partial (FT-2.2 / FT-3.2):** membership unit coverage + Abstain egress shipped; e2e / relational remain.
...
| FT-2 identity tier + recovery | **FT-2.2 shipped (2026-07-10)**: membership propose-N + FT-5 tuple selector + single-step dry-run + existence guard; relational bridge retained; LLM joint proposer deferred to Phase C (C1/C4) |
| FT-3 Abstain vs Consult | **FT-3.1 + FT-3.2 shipped (2026-07-10)**: membership Consult + Abstain egress + actions OOC handlers; ownership on deterministic path; LLM joint proposer + complexity/frame-extract retirement **deferred to Phase C**. FT-3.3 (relational selector) still open. Wire shape: **Consult** + **Abstain** as first-class `ParseCommandResult` variants |
...
| FT-5 auto-resolve / selection gate | **Decided (2026-07-08)**; **membership implementation shipped FT-2.2 (2026-07-10)** --- cross-tuple selection + existence/presence guard on membership path; relational still bridge; floor/margin constants calibration-owned |
...
| **Gateway exit** (unblocks Phase C) | Not started (FT-3.1 + FT-3.2 done; FT-3.3 / FT-4 still open; hop retirement deferred to Phase C) |
