...
**Status:** In progress (FT-3.1 Consult wire shipped 2026-07-10). **Gateway** for Phase C of [`AGENT.manipulationFrameAndRelational.planning.md`](AGENT.manipulationFrameAndRelational.planning.md) --- do not begin Plan IR / compiler work until **Gateway exit** below is satisfied.
...
**Still pending (implementation, not decision):** LLM joint proposer (FT-3.2); relational tuple selector + fuller C1 packaging (FT-3.3). **FT-3.1 shipped (2026-07-10):** membership Consult egress + actions OOC handler. **FT-2.1 shipped (2026-07-09):** pool emission + bridge selector; identity LLM + v1 terminal embed resolve retired from production. **FT-2.2 shipped (2026-07-10):** membership tuple selector + single-step dry-run + existence guard.
...
Feeds FT-4 `SpanResolution.status` (`consult`); types shipped FT-0; **membership wire shipped FT-3.1 (2026-07-10)**.
...
- [ ] **FT-3** decided --- Abstain vs Consult wire types + owner stages documented (**FT-3.1 wire shipped 2026-07-10**; FT-3.2 / FT-3.3 still open).
...
- [ ] **FT-3.3 Selector + existence-guard rules (foundation for C1)**
  ...
  - [ ] Tests: high-confidence auto-resolve; low-confidence consult; noMatch abstain/error; hallucinated/absent id -> guard rejects. **Partial (FT-2.2):** membership unit coverage shipped; e2e / relational remain.
...
| FT-2 identity tier + recovery | **FT-2.2 shipped (2026-07-10)**: membership propose-N + FT-5 tuple selector + single-step dry-run + existence guard; relational bridge retained; LLM joint proposer still pending (FT-3.2) |
| FT-3 Abstain vs Consult | **FT-3.1 shipped (2026-07-10)**: membership Consult egress + actions OOC handler wired; FT-3.2 (joint proposer / hop retirement) and FT-3.3 (relational selector) still open. Wire shape: **new `ParseCommandResult` Consult variant**, actions emits alternate proposed commands + v1 OOC stub (perception copy follow-on), **not resumable** this iteration |
...
| **Gateway exit** (unblocks Phase C) | Not started (FT-3.1 done; FT-3.2 / FT-3.3 / FT-4 still open) |
