...
**Still pending (implementation, not decision):** wire [`identityStage.ts`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/identityStage.ts) / [`resolveRelationalGrounding.ts`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/resolveRelationalGrounding.ts) to emit pools; retire v1 terminal embedding resolve in production path; durable-doc updates in gateway checklist.
...
- [ ] **FT-2** decided --- identity LLM role in steady path documented (**decided (e) 2026-07-08**; **FT-2.1 2026-07-09**: standalone identity LLM removed, joint proposer + FT-5 selector owns semantics; durable-doc write-up + enabler build threads still pending).
...
| FT-1 candidate pool (embedding + lexical) | **FT-1.4 leading-article normalization shipped (2026-07-09)** --- peel-when-tail at classify; FT-1.3.1 admissibility gate retired; canonical snapshot `embedding-identity-pool-v1-2026-07-09-bias-sweep.json`; production `identityStage` still v1 until FT-2 |
...
- **FT-1.4 leading-article normalization (2026-07-09):** [`peelLeadingArticleWhenTail`](../../../../../lambda/ephemera/dataSource/actions/discriminateIntent/peelLeadingArticleWhenTail.ts) at classify --- strips `a` / `an` / `the` / `some` only when non-empty tail remains; article-only spans pass through. Wired in [`deterministicChecks.ts`](../../../../../lambda/ephemera/dataSource/actions/discriminateIntent/deterministicChecks.ts) + defensive pass in [`intentClassification.ts`](../../../../../lambda/ephemera/dataSource/actions/discriminateIntent/intentClassification.ts). QoL only --- not Gateway-blocking.
