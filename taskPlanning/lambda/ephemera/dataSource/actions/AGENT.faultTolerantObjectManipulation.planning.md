...
**Status:** In progress (FT-4.1 e2e + FT-4.2 durable docs shipped 2026-07-10; FT-4.3 Gateway sign-off still open; LLM joint proposer deferred to Phase C). **Gateway** for Phase C of [`AGENT.manipulationFrameAndRelational.planning.md`](AGENT.manipulationFrameAndRelational.planning.md) --- do not begin Plan IR / compiler work until **Gateway exit** below is satisfied.
...
| **FT-2** | **Identity LLM necessity** --- If we no longer require a **trusted-output single `objectId`** per span at the identity tier, do we still need a dedicated **identity LLM** hop? Rejected: (a) pool + deterministic adjudication only; (b) pool + lightweight validator LLM; (c) pool + identity LLM only on **backtrack** when pool ambiguous; (d) retain identity LLM but change output to **rank/reason** not pick-one. **Chosen: (e) merge identity into a joint `(identity, plan)` adjudicator** over the FT-1 pool, with an **unchanged deterministic legality/commit tail** --- see **FT-2 decisions so far** + **FT-2.1 decisions so far** + **Instruction compiler + validator architecture**. Bedrock-bypass for common cases is answered by **staged fast-path composition** (mechanism documented; enabling capabilities relocated to their own build threads, not decision blockers). | FT-3, FT-7, gateway | **Decided (e) (2026-07-08)**; **FT-2.1 (2026-07-09)** + **FT-2.2 (2026-07-10)** shipped (membership selector); LLM joint proposer deferred to Phase C (C1) |
| **FT-3** | **Abstention vs Consulting downstream** --- **Direction set (2026-07-08, see FT-3 decisions so far):** **complexity LLM** and **frame extract** do **not** survive as distinct LLM steps (complexity LLM -> deterministic sandbox legality + shared validator; frame extract -> **net-new** deterministic frame templating + general joint proposer); no narrow-scope "tier-2" LLM (the untemplated-simple-language case is a **classify** concern). **Abstain / Consult** owned by the **proposer / joint hop**, **defer -> Error** by the **shared validator**, **commit vs Consult** by the **FT-5 gate** --- never the deterministic compiler (BD-12). **Wire shape decided (2026-07-08, see Consult wire shape):** Consult is a **new `ParseCommandResult` variant** (not enriched `Error`); **actions** emits structured **alternate proposed commands** (X / Y), **perception** assembles copy; **not resumable this iteration** (terminates with no graph change + a terminal prompt inviting a re-entered scanning command). **FT-3.1 membership Consult wire shipped 2026-07-10**; **FT-3.2 (2026-07-10):** product **`ParseCommandAbstain`** + grey-band Abstain egress on deterministic path; **FT-3.3 (2026-07-10):** relational selector + Consult/Abstain egress; **LLM joint proposer + hop retirement deferred to Phase C (C1/C4)** --- see **FT-3.2 decisions so far**. | FT-4, gateway, client | Decided (2026-07-08); **FT-3.1 + FT-3.2 + FT-3.3 shipped** |
...
| **FT-8** | **Per-signal relevance normalization (`embed` + `lex` -> `[0,1]`)** --- **Decided (2026-07-08, see FT-8 decisions so far):** two-point **log map** for embedding cosine (`c_min` -> 0, `c_max=1` -> 1); **substring-biased edit distance** for lexical relevance (two-tier boundary discount on flanks; proportional penalty on the shorter normalized side). **FT-1.3.1 (2026-07-09):** short-span admissibility gate **retired** --- lexical always on for non-empty spans. **Constraint (from FT-1 no-floor):** both mappings are **absolute / globally parametric** --- fixed anchors and formula constants, never within-candidate-set rescale. **Still calibration-owned:** anchor values (`c_min` ~ 0.05, `L_min` ~ 5, `S_min` ~ 3), boundary vs alpha-adjacent edit costs, index-shape fork. | FT-1, FT-5, gateway | **Decided (2026-07-08)**; FT-1.3.1 gate retired 2026-07-09 |
...
- **`noMatch` becomes a judgment (numeric *and* semantic), not an empty pool.** With no floor and a non-empty catalog there is essentially always a top candidate, so `noMatch` derives from the head being *unfit*, not structural emptiness (only truly empty catalog stays structural, `NoCatalog`; reconcile with FT-4 `SpanResolution.status`). Two layers: a cheap **numeric** prefilter (top-1 absolute relevance, FT-8), and --- when the head is mediocre-but-not-zero --- a **semantic** adjudication that reads span + shortlist and concludes *"the best available (bag of beans, shawl, stunned fish) are simply not a sword,"* distinguishing a genuinely absent object from a numeric quirk (a paraphrase the vectors under-scored but which *is* present). That semantic call is the **joint proposer** Abstain/Consult (FT-2.1: standalone identity LLM removed) / FT-3 --- not a per-span identity pick-one. **Artifact implication:** the shortlist must carry candidate **identities + labels**, not just scores, so the semantic checker (and Consult copy) can reason over what the candidates *are* --- feeds the FT-4 `SpanResolution` shape.
...
- **Leading-article span normalization (FT-1.4, deferred QoL):** peel `a` / `an` / `the` / `some` at classify only when non-empty content remains after peel; otherwise leave span unchanged (`get a` -> span `a`, not rejected). Supersedes earlier junk-span / `S_min` fast-reject sketch --- not Gateway-blocking; pool + FT-5 handle short spans post FT-1.3.1.
...
- **Leading-article span normalization (FT-1.4, deferred QoL):** shared peel-when-tail helper at classify (`the broom` -> `broom`; `a` / `ax` unchanged). Normalization only --- no upstream fast-reject. Independent of FT-1.3.1 admissibility retirement.
...
**Direction: joint `(identity, plan)` adjudication --- FT-2 option (e).** Fold the identity tier and the front half of the Phase C compiler into one **fault-tolerant hybrid adjudicator** that ranks **`(identity, plan-candidate)` tuples** over the FT-1 candidate pools, backed by an **unchanged deterministic legality/commit tail**. Membership state + language direction become **evidence for identity**, not a downstream consequence of an already-committed id (today's membership observation can only veto). This **relocates** semantic identity work into the **joint proposer** and **removes** the standalone per-span identity LLM ([**FT-2.1 decisions so far**](#ft-21-decisions-so-far-2026-07-09)); the joint hop remains the semantic **backtrack** owner the exploration notes require. FT-2's literal fork is "merge scope + retire identity hop", not option (a) "pool + purely deterministic adjudication only" --- open-language paraphrase still falls to the general joint proposer when fast-path predicates fail.
...
Identity LLM today: [`buildPrompt.ts`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/buildPrompt.ts) + [`interpretIdentity.ts`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/interpretIdentity.ts) --- optimistic single pick. **Decided removed (FT-2.1, 2026-07-09):** backtrack and grey-band semantics owned by joint proposer propose-N + FT-5 selector, not identity-LLM retry.

### FT-2.1 decisions so far (2026-07-09)

Plan-only; graduate to durable docs when FT-2.1 ships. **Status: Decided 2026-07-09.** Closes the FT-2.1 sub-decision ("steady path, backtrack-only, or removed") under the already-locked FT-2 (e) fork. **Not** a reopen of option (a) (pool + purely deterministic adjudication only); semantic identity work relocates to the **joint proposer**, not a standalone per-span identity hop.

**Standalone identity LLM: removed (steady path and backtrack).** Retire [`buildObjectManipulationIdentityPrompt`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/buildPrompt.ts) + [`interpretObjectManipulationIdentityBody`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/interpretIdentity.ts) + [`invokeIdentityLlmForSpan`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/identityStage.ts) as the per-span resolution owner. Rationale (post FT-1): rank-all `SpanCandidatePool` + per-candidate `locus` + FT-5 `(identity, plan)` tuple selection strictly dominates optimistic single pick --- membership state becomes evidence *for* identity (e.g. "drop bag"), cross-span coupling is only possible at tuple granularity, and FT-1.3.1 lexical-always + absolute relevance makes numeric `noMatch` / Abstain viable without a dedicated identity prompt on most tails.

**Steady-path identity resolution (replacement pipeline).**

1. **Exact resolve** -> candidate with `sourceTags: ['exact']`, `jointRelevance: 1` (optional short-circuit to auto-resolve when closed-world).
2. **Non-exact** -> [`buildSpanCandidatePool`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/embeddingMatch/buildSpanCandidatePool.ts) emits `SpanCandidatePool` (full ranked list + gap-trim shortlist; no terminal embedding `Resolved`).
3. **Deterministic fast-path** when closed-world predicate holds (unique exact label, structural gap) -> synthesize `(identity, plan)` proposal; **zero** Bedrock.
4. **Otherwise** -> general **joint proposer** proposes N ranked `(identity, plan)` tuples over the pools (+ classify hints, locus, language).
5. **FT-5 selector** -> auto-resolve one legal tuple | **Consult** (runner-up legal tuples = menu) | **Abstain** / **Error**; post-selection **existence/presence guard** only.

**Semantic grey band (mediocre top-1 relevance).** Numeric prefilter first (`T_JOINT_ABS` / FT-5 floor); when head is mediocre-but-not-zero, **joint proposer Abstain/Consult** owns the "best available aren't X" vs paraphrase distinction --- **not** a restored identity LLM. If calibration shows paraphrase regression under numeric-only Abstain, widen joint-proposer scope; do **not** reintroduce per-span identity pick-one.

**Recovery patterns (FT-2.1 documentation target).**

| Pattern | Owner | Trigger |
| --- | --- | --- |
| **Supplement** | Orchestrator (future re-entrant loop) | Dependent grounding needs expanded pool (e.g. `withinObject` contents after host resolved) |
| **Correct** | Shared deterministic validator + dry-run sandbox | Chosen tuple illegal / undecidable (`Custom` blast radius -> LLM validator per FT-3) |
| **Backtrack** | Joint proposer re-propose-N (not identity LLM retry) | Selector declines auto-resolve; propose-N collapses old embedding-abstain -> identity-LLM round-trip |

**Calibration gate before gateway sign-off:** absent-object -> Abstain (not Consult with nonsense); paraphrase -> auto-resolve or Consult (not Abstain); duplicate-label + locus -> resolves without LLM on golden path; thin-margin ambiguous pools -> Consult from legal runner-up tuples.

**Still pending (implementation, not decision):** LLM joint proposer + complexity/frame-extract retirement (**Phase C C1/C4**, deferred from FT-3.2); FT-4 e2e + durable docs + gateway sign-off. **FT-3.3 shipped (2026-07-10):** relational tuple selector + Consult/Abstain egress. **FT-3.1 shipped (2026-07-10):** membership Consult egress + actions OOC handler. **FT-3.2 shipped (2026-07-10):** membership Abstain egress + ownership invariants on deterministic path. **FT-2.1 shipped (2026-07-09):** pool emission + bridge selector; identity LLM + v1 terminal embed resolve retired from production. **FT-2.2 shipped (2026-07-10):** membership tuple selector + single-step dry-run + existence guard.
...
Feeds FT-4 `SpanResolution.status` (`consult`); types shipped FT-0; **membership Consult wire shipped FT-3.1 (2026-07-10)**; **membership Abstain wire shipped FT-3.2 (2026-07-10)**.

### FT-3.2 decisions so far (2026-07-10)

Plan-only; graduate to durable docs when FT-3.2 ships (done same day).

**Scope choice: encode ownership on the deterministic path; defer LLM joint hop to Phase C.** Building a Bedrock joint `(identity, plan)` proposer before Plan IR, compound sandbox, and FT-7 reunify (C1/C4) would be throwaway. Phase C already owns:

- **C1** --- compiler as deterministic fast-pass *or* LLM fallback proposer
- **C4** --- retire ad-hoc stage graph (complexity LLM + frame extract) under reunified entry

**What FT-3.2 shipped:** product **`ParseCommandAbstainResult`** + actions OOC handler; membership grey-band / unfit-head selector decline -> **Abstain** (not Error, not Consult with nonsense); Consult remains FT-5-only (legal runner-up menus); validator `defer` stays Error path (complexity LLM interim unchanged).

**Accepted interim until Phase C:** numeric-only grey-band (no semantic paraphrase adjudication); complexity LLM + frame extract still live production hops.
...
- [X] **FT-2** decided --- identity LLM role in steady path documented (**decided (e) 2026-07-08**; **FT-2.1 shipped 2026-07-09**; **FT-2.2 shipped 2026-07-10**: membership tuple selector + single-step dry-run + existence guard; **FT-3.1 Consult egress shipped 2026-07-10**; **FT-3.3 relational selector shipped 2026-07-10**).
- [X] **FT-3** decided --- Abstain vs Consult wire types + owner stages documented (**FT-3.1 Consult + FT-3.2 Abstain + FT-3.3 relational selector shipped 2026-07-10**; hop retirement / LLM joint proposer **deferred to Phase C C1/C4** with written rationale --- see **FT-3.2 decisions so far**).
- [ ] **FT-4** shipped --- span-resolution types + guards in actions layer (**shape decided 2026-07-08**; **types/guards shipped FT-0**; selector wiring shipped FT-2.2/FT-3.3; **FT-4.1 e2e + FT-4.2 durable docs shipped 2026-07-10**; **FT-4.3 Gateway sign-off still open**).
- [X] **FT-5** decided --- auto-resolve / selection gate (floor + margin over runner-up) + post-selection existence/presence guard documented; `resolveComponent` retired as a standalone primitive (**decided 2026-07-08**; **membership + relational implementation shipped FT-2.2 / FT-3.3**; floor/margin constants calibration-owned).
- [X] **FT-8** decided --- per-signal embed + lex relevance normalization to absolute `[0,1]` scale documented (**decided 2026-07-08**: log map + substring edit distance + two-tier boundary discount + catalog-derived short-span admissibility; anchor constants calibration-owned).
- [X] Identity tier emits **provisional pool** (not terminal embedding **`Resolved`**) on non-exact paths, or shim documented with sunset date (**FT-2.1 shipped 2026-07-09** --- [`resolveCatalogSpanToPool`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/resolveCatalogSpanToPool.ts); bridge [`selectSingleSpanFromPool`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/selectSingleSpanFromPool.ts) retired from production by FT-2.2/FT-3.3; v1 `decideEmbeddingMatch` / identity LLM retired from production).
- [X] [`objectManipulation/AGENT.md`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/AGENT.md) updated: trust posture + recovery patterns per hop (**FT-4.2 shipped 2026-07-10** --- fault-tolerant enrich selection; recovery table; terminal parse trusted-output).
...
| **Backtrack** | Embedding abstain -> identity LLM (single retry owner) | Joint proposer re-propose-N + FT-5 selector; semantic abstain / Consult (FT-2.1 / FT-3) |
...
| `SpanGrounding.resolved` + `IdentityStageResult.success` | Post-LLM pick-one (v1) | `SpanResolutionOutcome { verdict: 'resolved' }` at FT-5 selector only |
| `IdentityStageResult.error` | noCatalog, LLM failure (v1 identity hop) | `SpanResolutionOutcome { verdict: 'error' }` or upstream error; multi-span ambiguity -> `consult` not Error (FT-4.1) |
...
- [X] **FT-1.3.1 Short-span admissibility retirement** **shipped 2026-07-09** (full gate removal)
  - [X] **Identity ranking unchanged** between legacy gated baseline and gate-off production.
  ...
  - [X] **Diverse-catalog length-1:** gate-off top joint on `a` vs multi-token catalog stays below `T_JOINT_ABS` with weak embed (~0.34).
  - [X] **Prefix shorthand OK:** `ax`/`rusty axe` and `a`/`axe` may score moderately-to-highly --- desired pool behavior; auto-resolve gated by embed + FT-5 margin.
  - [X] Removed production [`admissibleShortSpans`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/embeddingMatch/admissibleShortSpans.ts) gate; lexical always on for non-empty spans. Legacy baseline in [`testing/legacyLexicalChannelGate`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/embeddingMatch/testing/legacyLexicalChannelGate.ts). **Harness retention review deferred** to sibling plan Phase **C5** (after Plan IR steady-state).

- [X] **FT-1.4 Leading-article span normalization (deferred QoL --- not Gateway-blocking)** **shipped 2026-07-09**
  - [X] Shared helper aligned with classify prompt (`a` / `an` / `the` / `some`): peel leading article + whitespace **only when** non-empty content remains after peel; **otherwise leave span unchanged** (`the broom` -> `broom`; `a` -> `a`; `ax` -> `ax`). Normalization only --- no fast-reject, Error, or Abstain; short / article-only spans pass through to downstream pool + FT-5 (safe post FT-1.3.1).
  - [X] Wire deterministic classify [`deterministicChecks.ts`](../../../../../lambda/ephemera/dataSource/actions/discriminateIntent/deterministicChecks.ts) (replaces `the`-only `stripLeadingArticle`).
  - [X] Optional defensive pass on LLM-parsed manipulation / Acme spans in [`intentClassification.ts`](../../../../../lambda/ephemera/dataSource/actions/discriminateIntent/intentClassification.ts) for consistency with prompt stripping rules.
  - [X] Tests: peel-when-tail vs leave-alone cases; **not** pool-builder or embed-skip assertions.

### FT-2. Identity tier + recovery loop

- [X] **FT-2.1 Steady-path identity** **shipped 2026-07-09**
  - [X] Wire [`identityStage.ts`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/identityStage.ts) + [`resolveRelationalGrounding.ts`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/resolveRelationalGrounding.ts) to emit **`SpanCandidatePool`** artifacts (retire v1 terminal embedding `Resolved` + identity LLM pick-one on production path).
  - [X] **FT-2 / FT-2.1 decision:** standalone identity LLM **removed** (steady path and backtrack); semantic identity -> **joint proposer** + **FT-5 selector** over FT-1 pools + `locus` (see **FT-2.1 decisions so far**).
  - [X] Document recovery: **supplement** (catalog/embed refresh / future `withinObject` pool expand), **correct** (shared validator + dry-run sandbox), **backtrack** (joint proposer re-propose-N --- not identity LLM retry).
  - [X] **FT-2.1 bridge:** interim single-span auto-resolve via [`selectSingleSpanFromPool`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/selectSingleSpanFromPool.ts) (`T_JOINT_*`); declines map to **Error** until FT-2.2 tuple selector + FT-3 Consult wire.

- [X] **FT-2.2 Selector + validator integration** **shipped 2026-07-10**
  - [X] Wire FT-5 selector + shared validator against pool relevance (FT-1) and legality (single-step dry-run); no standalone identity validation hop. Membership production: [`proposeMembershipTuples`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/proposeMembershipTuples.ts) + [`validatePlanDryRun`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/validatePlanDryRun.ts) + [`selectIdentityPlanTuple`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/selectIdentityPlanTuple.ts) + [`existencePresenceGuard`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/existencePresenceGuard.ts) via [`selectMembershipFromPool`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/selectMembershipFromPool.ts). Relational path retains bridge. **Superseded for Consult egress by FT-3.1.**
  - [X] Tests: illegal-if-wrong tuple rejected by selector; thin-margin -> Consult (selector verdict; egress Error until FT-3.1); absent-object grey band -> Abstain/`noMatch` per FT-2.1 calibration gate.

### FT-3. Abstention vs Consulting

- [X] **FT-3.1 Parse result contract** **shipped 2026-07-10**
  - [X] Add **`ParseCommandConsultResult`** variant + guards; distinguish from **`Error`** and success variants (**decided**: new variant, not enriched `Error`). Types/guards from FT-0; production wire in this slice.
  - [X] Actions carries the structured **alternate proposed commands** (candidate re-phrasings that would scan) as **data**; perception owns the copy (v1: actions OOC stub via `consultMessageForPlayer`).
  - [X] Handler in [`index.ts`](../../../../../lambda/ephemera/dataSource/actions/index.ts): Consult -> player-visible message via OOC, **no** positions stream, **no** resumable state (terminal prompt inviting a re-entered scanning command). Membership egress: [`selectMembershipFromPool`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/selectMembershipFromPool.ts) + [`compileMembershipAtomic`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/compileMembershipAtomic.ts).

- [X] **FT-3.2 Proposer Abstain/Consult behavior** **shipped 2026-07-10** (ownership on deterministic path; LLM joint hop + hop retirement deferred to Phase C per **FT-3.2 decisions so far**)
  - [X] Deterministic propose-N / FT-5 selector: **Abstain** on grey-band / unfit head (`ParseCommandAbstainResult` + OOC handler); **Consult** only with structured, catalog-backed alternatives (FT-3.1 unchanged); policy/legality failures stay **Error**.
  - [X] Deterministic fast-path + shared validator: no Consult authoring (defer -> Error via complexity interim; commit-vs-Consult is the FT-5 gate).

- [X] **FT-3.3 Selector + existence-guard rules (foundation for C1)** **shipped 2026-07-10**
  - [X] Deterministic **selector**: N `(identity, plan)` candidates -> auto-resolved tuple | consult | error per **FT-5** policy; per-span **existence/presence guard** (referential integrity) on the chosen tuple. (`resolveComponent` retired as a standalone primitive --- these are selector-tail facets, not a runtime Plan IR step.) Membership: FT-2.2. Relational: [`proposeRelationalTuples`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/proposeRelationalTuples.ts) + [`selectRelationalFromPools`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/selectRelationalFromPools.ts) via shared [`selectPlanTuple`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/selectIdentityPlanTuple.ts); bridge retired from production.
  - [X] Tests: high-confidence auto-resolve; low-confidence consult; noMatch abstain/error; hallucinated/absent id -> guard rejects. Membership + relational unit + `compileRelational` Consult/Abstain; broader membership e2e matrix remains FT-4.1.

### FT-4. Integration + gateway verification

- [X] **FT-4.1 End-to-end parse tests** **shipped 2026-07-10**
  - [X] Membership path: paraphrase -> consult; exact -> zero Bedrock; ambiguous pool -> consult ([`parseCommand.test.ts`](../../../../../lambda/ephemera/dataSource/actions/parseCommand.test.ts) `FT-4.1 membership / relational e2e` + strengthened exact takeHold / PA-5 `embedSpan` assertions).
  - [X] Relational path: two-span consult on one ambiguous subject (**absorbed into FT-3.3** `compileRelational` / selector coverage).
  - [X] Regressions: Phase B establishRelation / dissolveRelation still apply on resolved paths (establish On/Under retained; dissolve via frame extract + compiler added in FT-4.1).

- [X] **FT-4.2 Durable docs** **shipped 2026-07-10**
  - [X] Update [`objectManipulation/AGENT.md`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/AGENT.md) trust posture + recovery table.
  - [X] Update [`embeddingMatch/AGENT.md`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/embeddingMatch/AGENT.md) v1 -> v2 steady-state.
  - [X] Update [`actions/AGENT.implementation.md`](../../../../../lambda/ephemera/dataSource/actions/AGENT.implementation.md) handoff / egress for Consult.
...
| FT-1 candidate pool (embedding + lexical) | **FT-2.1 pool emission shipped (2026-07-09)** --- production identity emits `SpanCandidatePool` via [`resolveCatalogSpanToPool`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/resolveCatalogSpanToPool.ts); bridge selector [`selectSingleSpanFromPool`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/selectSingleSpanFromPool.ts); identity LLM + v1 terminal embedding resolve retired |
| FT-8 per-signal relevance normalization | **Decided + calibrated (2026-07-09)**: log map for embed; Sellers + tanh flank combine for lex; anchor constants locked in [`thresholds.ts`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/embeddingMatch/thresholds.ts); short-span admissibility gate retired (FT-1.3.1) |
| FT-2 identity tier + recovery | **FT-2.2 shipped (2026-07-10)**: membership propose-N + FT-5 tuple selector + single-step dry-run + existence guard; **FT-3.3 (2026-07-10)** relational cutover; LLM joint proposer deferred to Phase C (C1/C4) |
| FT-3 Abstain vs Consult | **FT-3.1 + FT-3.2 + FT-3.3 shipped (2026-07-10)**: membership + relational Consult/Abstain egress + actions OOC handlers; ownership on deterministic path; LLM joint proposer + complexity/frame-extract retirement **deferred to Phase C**. Wire shape: **Consult** + **Abstain** as first-class `ParseCommandResult` variants |
...
| FT-5 auto-resolve / selection gate | **Decided (2026-07-08)**; **membership + relational implementation shipped FT-2.2 / FT-3.3 (2026-07-10)** --- cross-tuple selection + existence/presence guard on both paths; floor/margin constants calibration-owned |
...
| FT-4 integration + gateway | **FT-4.1 e2e + FT-4.2 durable docs shipped (2026-07-10)**; **FT-4.3 Gateway sign-off still open** |
| **Gateway exit** (unblocks Phase C) | In progress (FT-4.1/4.2 done; FT-4.3 open; hop retirement deferred to Phase C) |
...
- **FT-0 shipped (2026-07-09):** [`spanResolution.ts`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/spanResolution.ts) defines `SpanCandidatePool`, `ObjectSpanCandidate`, `SpanResolutionOutcome` + guards; `ParseCommandConsultResult` stub in [`baseClasses.ts`](../../../../../lambda/ephemera/dataSource/actions/baseClasses.ts). **FT-3.1 (2026-07-10)** wired membership Consult egress + actions handler. **FT-3.2 (2026-07-10)** wired membership Abstain egress + actions handler. **FT-3.3 (2026-07-10)** wired relational selector + Consult/Abstain. LLM joint proposer deferred to Phase C.
- **FT-1.3.1 admissibility gate retired (2026-07-09):** [`buildSpanCandidatePool`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/embeddingMatch/buildSpanCandidatePool.ts) scores lexical for every non-empty span. Diverse-catalog length-1 spurious joint ~0.34 < `T_JOINT_ABS`; prefix shorthand (`ax`/`rusty axe` ~0.62) is desired. Harness fixture intent corrected; legacy gate in [`testing/legacyLexicalChannelGate`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/embeddingMatch/testing/legacyLexicalChannelGate.ts). **Re-examine harness retention after Phase C** (sibling plan **C5** --- gate-off will outlive legacy by then).
...
- **FT-2.1 steady-path identity (2026-07-09):** [`identityStage.ts`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/identityStage.ts) + [`resolveRelationalGrounding.ts`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/resolveRelationalGrounding.ts) emit `SpanCandidatePool` per span via [`resolveCatalogSpanToPool`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/resolveCatalogSpanToPool.ts). Production bridge: [`selectSingleSpanFromPool`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/selectSingleSpanFromPool.ts) (`T_JOINT_*` auto-resolve; declines -> Error). Identity LLM + v1 terminal `decideEmbeddingMatch.Resolved` retired from production path. **Superseded on membership by FT-2.2.**
- **FT-2.2 selector + validator (2026-07-10):** membership path cut over to deterministic propose-N + FT-5 cross-tuple selector ([`selectMembershipFromPool`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/selectMembershipFromPool.ts)); single-step locus dry-run; existence/presence guard. Illegal-if-wrong ("drop bag") works. Thin-margin Consult and grey-band Abstain proven at selector. **FT-3.1 (2026-07-10):** membership Consult egress + actions OOC handler wired. **FT-3.2 (2026-07-10):** grey-band -> product **`Abstain`** (not Error); LLM joint proposer + compound sandbox + hop retirement remain Phase C (C1/C4).
- **FT-3.3 relational selector (2026-07-10):** relational path cut over to propose-N + shared FT-5 [`selectPlanTuple`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/selectIdentityPlanTuple.ts) via [`selectRelationalFromPools`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/selectRelationalFromPools.ts); pools-only [`resolveRelationalGrounding`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/resolveRelationalGrounding.ts); Consult/Abstain egress from [`compileRelational`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/compileRelational.ts). Bridge [`selectSingleSpanFromPool`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/selectSingleSpanFromPool.ts) retired from production. Gateway exit still blocked on FT-4.
- **FT-4.1 / FT-4.2 (2026-07-10):** membership e2e Consult matrix + dissolveRelation parse regression in [`parseCommand.test.ts`](../../../../../lambda/ephemera/dataSource/actions/parseCommand.test.ts); durable trust posture + recovery table + embeddingMatch v2 steady-state + Consult egress docs graduated. **FT-4.3 Gateway sign-off still open** --- Phase C remains blocked.
- **FT-8 relevance normalization (2026-07-08, lexical body updated 2026-07-09, combine shipped FT-1.1.5 2026-07-09, FT-1.3.1 2026-07-09):** per-signal **`[0,1]`** mapping decided --- embedding: two-point log (`c_min` -> 0, `1` -> 1); lexical: Sellers + edit hard gate * `tanhCenteredFlankScore` on raw flank lengths. **Combine:** FT-1.1.5 shipped edit gate * `sigmoid(bias + Σ w_i tanh((m_i - x_i)/s_i))`; multiplicative v1 retained as `multiplicativeFlankScoreV1` for simulator A/B. **FT-1.3.1:** short-span admissibility gate retired; lexical always on for non-empty spans; FT-5 owns auto-resolve floors. **FT-1.4:** leading-article peel-when-tail normalization at classify (shipped 2026-07-09; no upstream fast-reject) --- not Gateway-blocking.
