...
**Still pending (implementation, not decision):** Consult wire (FT-3.1); LLM joint proposer (FT-3.2); relational tuple selector + fuller C1 packaging (FT-3.3). **FT-2.1 shipped (2026-07-09):** pool emission + bridge selector; identity LLM + v1 terminal embed resolve retired from production. **FT-2.2 shipped (2026-07-10):** membership tuple selector + single-step dry-run + existence guard; Consult/Abstain internal only.
...
- [ ] **FT-2** decided --- identity LLM role in steady path documented (**decided (e) 2026-07-08**; **FT-2.1 shipped 2026-07-09**; **FT-2.2 shipped 2026-07-10**: membership tuple selector + single-step dry-run + existence guard; Consult egress still FT-3.1; relational bridge retained).
...
| FT-2 identity tier + recovery | **FT-2.2 shipped (2026-07-10)**: membership propose-N + FT-5 tuple selector + single-step dry-run + existence guard; Consult/Abstain internal (egress Error until FT-3.1); relational bridge retained; LLM joint proposer still pending (FT-3.2) |
...
- **FT-2.2 selector + validator (2026-07-10):** membership path cut over to deterministic propose-N + FT-5 cross-tuple selector ([`selectMembershipFromPool`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/selectMembershipFromPool.ts)); single-step locus dry-run; existence/presence guard. Illegal-if-wrong ("drop bag") works. Thin-margin Consult and grey-band Abstain proven at selector; enrich egress still **Error** until FT-3.1. Relational path keeps bridge. LLM joint proposer + compound sandbox + Consult player wire remain FT-3 / Phase C. FT-3.3 still owns fuller C1 packaging / relational selector cutover.
