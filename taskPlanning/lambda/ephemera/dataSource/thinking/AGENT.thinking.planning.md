...
- [ ] **Results spine (phase-zero priority)**
  ...
  - [ ] **Hypothesis pipeline migration** (detailed checklist, GitHub issue map, job vs step failure notes): [`taskPlanning/lambda/ephemera/dataSource/coyoteGame/AGENT.thinkingMigration.planning.md`](../coyoteGame/AGENT.thinkingMigration.planning.md). Summary: mint `generationId`, pre-mint per-task `workItemId`s, persist per **Decisions** (**`JOB#${generationId}`** + **`Meta::Job`**, adjacency, **`Meta::Schedule`**, **`Meta::Result`**); **verbose** MVP; harness alignment per [`coyoteHarnessInjectTypes.ts`](../../../../../lambda/ephemera/dataSource/coyoteGame/generators/pipelines/hypothesis/coyoteHarnessInjectTypes.ts).
