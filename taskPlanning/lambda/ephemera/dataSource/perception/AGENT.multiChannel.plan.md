...
**Roster refresh:** **`roomUpdateMessage`** ([`lambda/ephemera/roomUpdate/index.ts`](../../../../../lambda/ephemera/roomUpdate/index.ts)) runs **`publishRoomAffordancePerceptionMessages`** only (wire **`displayProtocol: 'RoomUpdate'`** retired). **`moveCharacter`**, **`disconnectMessage`**, etc. still send internal bus **`type: 'RoomUpdate'`** with **`roomId`** to invoke that handler.
...
- [X] **After** the client ships the above: **retire** server **`displayProtocol: 'RoomUpdate'`** (or thin to zero emits); confirm no remaining consumer dependency. Implemented: [`lambda/ephemera/roomUpdate/index.ts`](../../../../../lambda/ephemera/roomUpdate/index.ts) affordance-only; [`lambda/ephemera/publishMessage/index.ts`](../../../../../lambda/ephemera/publishMessage/index.ts) branch removed; **`PublishRoomUpdateMessage`** removed from ephemera **`PublishMessage`** union.
...
| Server **`RoomUpdate` retirement** | Done (wire emit removed; internal bus **`type: 'RoomUpdate'`** triggers affordance **`PerceptionMessage`** only) |
...
| **Server `RoomUpdate` end state** | **Shipped:** ephemera no longer emits wire **`displayProtocol: 'RoomUpdate'`**; roster refresh is affordance **`PerceptionMessage`** only. Historical **`RoomUpdate`** rows may remain in stores until **manual** Dynamo / Dexie purge. |
