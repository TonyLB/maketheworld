...
**Status:** In progress. **Next:** [Recommended order](#recommended-order) --- **Room header** aggregation (step 5). Coordinate with [**Virtual lanes**](../../../../../packages/mtw-lambda-patterns/ts/messageBus/AGENT.implementation.md#virtual-lanes-internalmessagebus) (`InternalMessageBus`) if delivery must be lane-isolated before client output.
...
- [X] **Room description** aggregation (prototype for **correlated** perception threads; **fan-in and state first**; **delivery mechanics** (who, timeline vs in-place) refine with pass-through + [virtual bus lanes](../../../../../packages/mtw-lambda-patterns/ts/messageBus/AGENT.implementation.md#virtual-lanes-internalmessagebus) as needed).
  - **Thread shape:** add a **Room description** thread variant in [`lambda/ephemera/internalCache/perceptionThreads.ts`](../../../../../lambda/ephemera/internalCache/perceptionThreads.ts) (discriminated union) holding **runtime** fields updated as events are handled; keep using **`Perception Thread Registered`** / [`PerceptionThreadRegisteredCommand`](../../../../../lambda/ephemera/dataSource/perception/localApiEvents.ts) for ingress (**`componentId`**, **`perspectiveKey`**, **`characterId`**). For **Room Examine** / Room description, **`characterId`** (requester) is **required** (not optional); tighten types / validation for this thread when implementing.
  - **`PerceptionThreads` API (decided):** use **two** operations instead of an overloaded merge-**`set`** (clearer **open registration** vs **stream patch** lifecycle). **`set`:** initial **registration** at **`componentId` + `perspectiveKey`**; if the payload does **not** include a caller-supplied registration id, assign a **synthetic** id (**`uuidv4()`**) stored on the entry so this **registration** is addressable later. **`update`:** **shallow merge** a **partial** object into the existing **thread** / **runtime** fields for **that** registration (keyed by **map key** + **registration id**, exact API TBD); **no** deep merge required given simple thread shapes (revisit only if nested objects appear). Step 3 **`set`** is **replace**-style; step 4 introduces **`update`** and synthetic id on **`set`**. See [Room description prototype (step 4)](#room-description-prototype-step-4).
  - **Matching (decided):** for **`Render Pertains`** and **`Generation Started`**, an event is **relevant** to a thread when **`componentId`** and **`perspectiveKey`** match **exactly** (same **precise** string identity as registration). **`cacheId`** remains **in-bucket** state filled from payloads when present; it is **not** an extra match predicate in step 4 (see [Aggregation cache routing identity](#aggregation-cache-routing-identity)).
  ...
  - **Disambiguation:** multiple discriminated **`kind`** values for the same **`(componentId, perspectiveKey)`** are **not** in scope for **step 4** implementation. **Step 5** adds **Room Broadcast** alongside **Room Description** on the **same** map key where needed, relying on **singleFlight** --- see [Room header prototype (step 5)](#room-header-prototype-step-5).
...
- [ ] **Room header** aggregation (broadcast **generating** + **terminal**; **fan-in** and stream handlers analogous to step 4 **room description**, with normative choices in [Room header prototype (step 5)](#room-header-prototype-step-5)).
  - **Correlation (decided):** **`RoomCharacterList`** for the room, grouped by **`perspectiveKey`** (same **canon stack + per-character assets** shaping as [`fanOutStateChangedToPassiveRenders.ts`](../../../../../lambda/ephemera/dataSource/renderOrchestration/fanOutStateChangedToPassiveRenders.ts)); register a **`Room Broadcast`** (name TBD) perception **thread** per **`(componentId, perspectiveKey)`** holding **target character ids** for delivery; **one** passive render kick **per** **`perspectiveKey`** via **`Render Requested`** / **`orchestrateRenderRequest`** with **`targets`** (not N independent kicks per character when perspectives align).
  - **Map key vs room description (decided):** keep **`componentId` + `perspectiveKey`** as the **`PerceptionThreads`** map key; **singleFlight** on generation implies concurrent **Room Broadcast** vs **Room Description** threads on the same key should still converge on the **same** authoritative render, so we **do not** add a separate key dimension solely to disambiguate that edge race.
  - **Terminal WML (decided):** **`internalCache.ComponentRender.get(characterId, roomId, { header: true })` per target** in this iteration; **document** follow-on optimization if identical **`perspectiveKey`** can safely share one resolved header payload.
  - **`PublishMessage` shape (decided):** **multi-target** sends (**`targets`** = all characters in that perspective group), matching legacy [`sendRoomGeneratingHeader`](../../../../../lambda/ephemera/perception/index.ts) (**single** `PublishMessage`, **`displayMode: 'header'`**, shared **`messageId`** where applicable for placeholder overwrite semantics).
  - **Entry path (decided):** **new** wiring that **replaces** imperative **`type: 'Perception'`** **`header: true`** enqueue (today from **`processComponentUpdated`** in [`dataSource/index.ts`](../../../../../lambda/ephemera/dataSource/index.ts) for rooms, and the **asset** branch of [`perceptionMessage`](../../../../../lambda/ephemera/perception/index.ts) that re-queues **`Perception`** per linked room): run **grouping + thread register + render request** (reuse shared helpers with passive **`State Changed`** fan-out where practical). **Lanes / terminal dedupe / stream matching** follow step 4 patterns unless this step explicitly narrows them. **`moveCharacter`** stays **[Recommended order](#recommended-order)** steps **6--7**.
  - **Handler module:** extend [`orchestrate.ts`](../../../../../lambda/ephemera/dataSource/perception/orchestrate.ts) (or a sibling) for **`roomHeader` / Room Broadcast** fan-in; extend **registration / types** in [`perceptionThreads.ts`](../../../../../lambda/ephemera/internalCache/perceptionThreads.ts) / [`localApiEvents.ts`](../../../../../lambda/ephemera/dataSource/perception/localApiEvents.ts) as needed.
...
| 4 | Room description aggregation | **`executeAction`** only; **`characterId`** required; **`kind: 'roomDescription'`** on the thread record; **`set`** + **`update`** (**shallow merge**) + synthetic **registration id**; **`uuidv4()`** **laneId** from **`findRender`** slow path for **Generating**; default lane for **main** thread; match **`componentId` + `perspectiveKey`**; **terminal** -> **log** + ignore; see [Recommended order](#recommended-order) / [Decisions](#decisions). |
| 5 | Room header aggregation | Broadcast by **`perspectiveKey`**; same **`PerceptionThreads`** key (**singleFlight**); **`ComponentRender.get(..., { header: true })` per target** (optimize later); **multi-target** **`PublishMessage`** like **`sendRoomGeneratingHeader`**; **replace** imperative **`Perception`** **`header: true`** entry paths; see [Room header prototype (step 5)](#room-header-prototype-step-5). |
...
- **DataSource behavior:** **`mtw.ephemera.perception`** `receiveEvents` (or the single subscription path) **must** recognize this envelope and call **`internalCache.PerceptionThreads.set(...)`** so registration is stored under the map key **`componentId` + `perspectiveKey`**. **No** other new perception DataSource behavior is required in step 3. **Step 4** adds **`update`** (**shallow merge**) and **synthetic registration id** on **`set`**; see [Room description prototype (step 4)](#room-description-prototype-step-4).
...
- **`PerceptionThreads`:** **`set`** (initial row per **`componentId` + `perspectiveKey`**; assign **synthetic registration id** **`uuidv4()`** when ingress omits one) and **`update`** (**shallow merge** of a partial into the stored **thread** / runtime fields, by **registration id**). Prefer **two functions** over a single overloaded **`set`**: **registration open** vs **fan-in patch** are different intents and stay easier to reason about.
...
- **Map key:** Reuse **`componentId` + `perspectiveKey`**; **singleFlight** means concurrent **Room Broadcast** vs **Room Description** on the same key need **no** extra key segment --- both should treat the **same** generation result as authoritative.
...
- **Delivery:** **Multi-target** **`PublishMessage`** aligned with legacy [`sendRoomGeneratingHeader`](../../../../../lambda/ephemera/perception/index.ts) (one send, **`targets`** array, **`displayMode: 'header'`**).
