...
- [ ] **Room header** aggregation (broadcast **generating** + **terminal**; **fan-in** and stream handlers analogous to step 4 **room description**, with normative choices in [Room header prototype (step 5)](#room-header-prototype-step-5)).
  ...
  - **Composite key vs room description (decided):** keep **`componentId` + `perspectiveKey`** as the **`PerceptionThreads`** bucketing key (**multiple** rows per bucket); **singleFlight** on generation implies **Room Broadcast** and **Room Description** rows for the same bucket still observe the **same** authoritative render from the pipeline, so we **do not** add a separate key dimension solely to disambiguate that edge case.
  ...
  - **Thread runtime (decided):** mirror **`roomDescription`** (**`status`**, **`messageId`**, optional **`cacheId`**, terminal dedupe per row) unless implementation discovers a slimmer shape; exact field list is **implementation detail**.
  - **`generationContextWml` (decided):** **not** required on **`Render Requested`** for this slice; omit it (or equivalent) on the broadcast-header kick unless a later constraint appears.
  - **Placeholder WML (decided):** align **Generating** / **Error** header placeholders with legacy [`sendRoomGeneratingHeader`](../../../../../lambda/ephemera/perception/index.ts) and step 4 dirt-simple copy norms; exact WML structure is **implementation detail**.
  - **Factoring (decided):** extract **shared** helpers (for example grouping **`RoomCharacterList`** by **`perspectiveKey`** and invoking **`orchestrateRenderRequest`**) where it reduces duplication with [`fanOutStateChangedToPassiveRenders.ts`](../../../../../lambda/ephemera/dataSource/renderOrchestration/fanOutStateChangedToPassiveRenders.ts); **module boundaries** are **implementation detail**.
...
| 5 | Room header aggregation | Broadcast by **`perspectiveKey`**; same bucket as description; **`ComponentRender.get(..., { header: true })` per target**; **multi-target** **`PublishMessage`**; **no** **`generationContextWml`**; placeholder WML + factoring + thread runtime per plan; extend **`PerceptionThreadRegisterCommand`** with a **third** **`threadKind`** for broadcast registration; see [Room header prototype (step 5)](#room-header-prototype-step-5). |
...
- **Registration / ingress shape:** **TBD** (next decision): how **`api.ephemera`** carries **broadcast** registration (**`targets`**, **`kind`**, second ingress, etc.) and how [`localApiEvents.ts`](../../../../../lambda/ephemera/dataSource/perception/localApiEvents.ts) validates room rows. Everything below assumes a row is **registered** and **kicked** somehow consistent with step 4 patterns.
...
- **Placeholder WML:** Match legacy header placeholder spirit ([`sendRoomGeneratingHeader`](../../../../../lambda/ephemera/perception/index.ts)) and step 4 placeholder norms; **exact markup** left to implementation.
