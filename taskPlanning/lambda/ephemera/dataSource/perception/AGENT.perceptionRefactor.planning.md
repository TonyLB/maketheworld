...
- [ ] **Room description** aggregation (prototype for **correlated** perception threads; **fan-in and state first**; **delivery mechanics** (who, timeline vs in-place) refine with pass-through + [virtual bus lanes](../../../../../packages/mtw-lambda-patterns/ts/messageBus/AGENT.implementation.md#virtual-lanes-internalmessagebus) as needed).
  - **Thread shape:** add a **Room description** thread variant in [`lambda/ephemera/internalCache/perceptionThreads.ts`](../../../../../lambda/ephemera/internalCache/perceptionThreads.ts) (discriminated union) holding **runtime** fields updated as events are handled; keep using **`Perception Thread Registered`** / [`PerceptionThreadRegisteredCommand`](../../../../../lambda/ephemera/dataSource/perception/localApiEvents.ts) for ingress (**`componentId`**, **`perspectiveKey`**, optional **`characterId`** for the requester today; extend if **`threadKind`** or equivalent must be on the wire).
  - **Routing / correlation:** match **`renderOrchestration`** / **`renderCache`** stream events to the bucket using **lean routing** (**`componentId`**, **`perspectiveKey`**, in-bucket **`cacheId`** when present; see [Aggregation cache routing identity](#aggregation-cache-routing-identity)). **Disambiguate** this flow from other threads with an explicit **thread kind** in stored state (e.g. **`Room Examine`**) so the same component perspective can host multiple thread types later.
  - **Runtime fields (plan):** **`status`**: **`Initial` | `Generating` | `Terminal`** (names provisional until typed); **`messageId`**: last client **`MessageId`** for the **Generating** placeholder so **terminal** **`PublishMessage`** can **overwrite** in the client.
  ...
  - **Handler module (plan):** implement **fan-in** reactions in a dedicated file under [`lambda/ephemera/dataSource/perception/`](../../../../../lambda/ephemera/dataSource/perception/) (e.g. **`orchestrate.ts`**) instead of only inline in **`index.ts`** / `receiveEvents`.
    - **`Render Pertains`** (**`mtw.ephemera.renderCache`**), when **relevant** to the registration: **dispatch** full room description **`PublishMessage`**; if a placeholder **`messageId`** exists, **overwrite**; set **`status`** to **`Terminal`**.
    - **`Generation Started`** (**`mtw.ephemera.renderOrchestration`**, the **slow-path / in-flight** signal in the [six-outbound taxonomy](../AGENT.passThrough.contract.planning.md); not a separate **`Generation Requested`** event name): when **relevant** and **`status`** is **not** **`Terminal`**, **dispatch** a **Generating** **`PublishMessage`**, persist the returned **`messageId`**, set **`status`** to **`Generating`** (duplicate **`Generation Started`** is an **acceptable intermediate** per pass-through **uncertainty 6**).
...
| 4 | Room description aggregation | Thread kind **`Room Examine`**, **`status`/`messageId`**, **`Render Pertains`** + **`Generation Started`** handling; see [Recommended order](#recommended-order) nested bullets. |
