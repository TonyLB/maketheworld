...
**Status:** Recommended order **steps 1--7** shipped (see [Recommended order](#recommended-order)). **Next:** pass-through / **Obligations** upstream, contract test activation, and **retire or archive** this plan when you fold remaining debt into durable docs per [When this task completes](#when-this-task-completes). **Done (steps 6--7):** **`moveCharacter`** --- **`threadKind: 'characterMove'`** on [`internalCache.PerceptionThreads`](../../../../../lambda/ephemera/internalCache/perceptionThreads.ts), synchronous **`register`** before transact (so Dynamo callbacks see rows; bus **`sendPerceptionThreadRegistered`** alone would queue until **`flush`**), Leave/Arrive via [`characterMoveDelivery.ts`](../../../../../lambda/ephemera/dataSource/perception/characterMoveDelivery.ts) with **`OrchestrateMessages`** **`before` / root / `after`** ids, **`kickPassiveRenderRequestedForCharacterInRoom`**, header fan-in in [`orchestrate.ts`](../../../../../lambda/ephemera/dataSource/perception/orchestrate.ts); no imperative **`Perception`** when arrival **`perspectiveKey`** exists. **Done (step 5):** room header aggregation (**`roomHeaderBroadcast`**). **Decided:** **Generating** header **`PublishMessage`** uses a **virtual lane** (slow-path **`laneId`** from **`findRender`**, **`flushOrchestrationLane`**). **Decided:** header **WML** aligns with **[`lambda/ephemera/perception`](../../../../../lambda/ephemera/perception/)**. Transport: [**Virtual lanes**](../../../../../packages/mtw-lambda-patterns/ts/messageBus/AGENT.implementation.md#virtual-lanes-internalmessagebus).
...
- [X] **Refactor `moveCharacter`** to start a **`renderOrchestration`**-driven cascade (replace or narrow direct imperative `Perception` where appropriate).
  - **`kickPassiveRenderRequestedForCharacterInRoom`** on room change; **`getCharacterRoomPerspectiveKey`** in [`kickRoomHeaderBroadcast.ts`](../../../../../lambda/ephemera/dataSource/perception/kickRoomHeaderBroadcast.ts).
  - When arrival room has a **non-empty** filtered perspective: **`internalCache.PerceptionThreads.register`** with **`threadKind: 'characterMove'`** (not bus-only --- registration must exist **before** transact success callbacks).
  - **Imperative `Perception` / `header: true`** skipped on that path; **fallback** (empty perspective, same-room moves) unchanged.
- [X] **Aggregate `moveCharacter`** outcomes in the perception DataSource (end-to-end with room description/header).
  - **`orchestrate.ts`** handles **`characterMove`** alongside **`roomHeaderBroadcast`**: **`Generation Started`**, **`Render Pertains`**, **`Orchestration Error`**, **`Generation Deferred`**; **`headerTargets ?? [characterId]`**; shared **`messageId`** for Generating overwrite.
  - Tests: [`moveCharacter/index.test.ts`](../../../../../lambda/ephemera/moveCharacter/index.test.ts), [`dataSource/perception/index.test.ts`](../../../../../lambda/ephemera/dataSource/perception/index.test.ts) (**characterMove** fan-in), [`perceptionThreads.test.ts`](../../../../../lambda/ephemera/internalCache/perceptionThreads.test.ts), [`localApiEvents.test.ts`](../../../../../lambda/ephemera/dataSource/perception/localApiEvents.test.ts).
...
| 6 | `moveCharacter` -> orchestration cascade | Done --- passive **`Render Requested`** + **`characterMove`** register when **`perspectiveKey`** present. |
| 7 | Aggregate move in perception DS | Done --- **`orchestrate.ts`** **`characterMove`** branch; tests above. |
...
**`moveCharacter` (partial migration):** when the mover has a **non-empty** arrival-room **`perspectiveKey`**, header delivery is **DataSource fan-in** (**`characterMove`**) --- not imperative **`Perception`**. Other sources in this table still enqueue **`Perception`** until follow-on.

Documenting this list avoids the false assumption that **only** passive render + **fully migrated move** exist in production.
...
| Steps 6--7 (`moveCharacter`) | Done --- [`moveCharacter/index.ts`](../../../../../lambda/ephemera/moveCharacter/index.ts), [`characterMoveDelivery.ts`](../../../../../lambda/ephemera/dataSource/perception/characterMoveDelivery.ts), [`orchestrate.ts`](../../../../../lambda/ephemera/dataSource/perception/orchestrate.ts) |
| Initiative complete; durable docs updated; task plan retired | **Deferred** --- [Obligations](#obligations-accruing-to-future-perception-working-list) still upstream; archive when folded per [When this task completes](#when-this-task-completes) |
