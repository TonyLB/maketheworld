...
- [X] **Room description** aggregation (prototype for **correlated** perception threads; **fan-in and state first**; **delivery mechanics** (who, timeline vs in-place) refine with pass-through + [virtual bus lanes](../../../../../packages/mtw-lambda-patterns/ts/messageBus/AGENT.implementation.md#virtual-lanes-internalmessagebus) as needed).
  ...
  - **Disambiguation:** multiple discriminated **`kind`** values for the same **`(componentId, perspectiveKey)`** are **not** in scope for step 4. If that collision ever arises before a design upgrade, treat as **out of scope** / explicit follow-on.
...
- [ ] **Room header** aggregation (**generating** + **terminal** results; same delivery footnote as description; reuse **orchestrate**-style handlers and thread machinery where applicable).
...
| 5 | Room header aggregation | Generating + terminal; **`orchestrate.ts`** pattern; same delivery footnote as step 4. |
