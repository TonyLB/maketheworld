...
| **Pointers** (`currentCacheId` on the `Cache::${perspectiveKey}` catalog row) - **which** `CACHE#...` id is current for a component + perspective | **`mtw.ephemera.renderCache`** --- a field on the catalog row it already owns, maintained at the same **`handleRenderOrchestrationInbound`** seam that emits **`Render Pertains`**. **`mtw.ephemera.currentCachePointers` is cancelled (CP-4)**: the separate DataSource this row used to name will not be built; see [`renderCache/AGENT.md`](../../../../lambda/ephemera/dataSource/renderCache/AGENT.md#perspective-pointers). Not all components use this pattern (e.g. some **Feature** flows may resolve on read instead). |
