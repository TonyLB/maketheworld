...
**Directional priority (`renderOrchestration`):** We intend to **remove dependency on `conversation.sendMessage`** for orchestration outcomes **as early as practical** and replace each former use with **outgoing events on the `mtw.ephemera.renderOrchestration` DataSource stream** (product decision; uncertainty 8 **transport** resolved). **Module location** for those six-outbound TypeScript types is **resolved** (uncertainty 8): **`publishedEvents.ts`** colocated with the DataSource ( **`busOnly`** ), not **`mtw-interfaces`**. **Exact envelopes** ( **`header.type`** per outbound, **`getContent`** wiring) and **identifier spellings** in code remain **implementation** details until the stream skeleton lands; see **Uncertainties** and [`renderOrchestration/AGENT.passThrough.planning.md`](renderOrchestration/AGENT.passThrough.planning.md).
...
| **`Render Generated`** | **Target contract:** emit when generation output is **ready**, with **full** content (**`cacheRecord`** / **`renderedContent`**) in the payload; **decoupled** from **`publishPutCacheRecord`** (orchestration does **not** assert write-through). **Orchestration must not** enqueue **`Put Cache Record`** for this path (see **uncertainty 1** resolved). **Today:** success path often follows **`publishPutCacheRecord`** in code --- **migrate** to stream-only handoff + **`renderCache`** write on **`Render Generated`**. | **`RenderReady`** via **`toRenderReady`** (legacy coupling; not the target durability split). | **`cacheId`**, **`cacheRecord`**, **`renderedContent`** (**full** generated content required). Optional: provenance **`generated`**. |
...
- **Per-call-site mapping:** Prose mapping is in **Limited refinement**; **implementation** and **envelope** construction ( **`header.type`**, **`getContent`**) still TBD until code lands (**uncertainty 8**). **Typed module path** for orchestration outbounds is **`publishedEvents.ts`** (**uncertainty 8** pattern resolution).
...
| **No `RenderReady` / materialize bus** in target (legacy terminals retired for orchestration outcomes) | Done |
...
| Types / interfaces landed | Partial (**renderOrchestration** outbounds: **`publishedEvents.ts`** ; uncertainty 8) |
