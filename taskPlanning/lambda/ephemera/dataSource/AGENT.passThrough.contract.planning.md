...
Hold the **canonical cross-cutting contract** for the pass-through pattern: a single observable notion that a given render cache record is **the relevant answer** for a component/perspective (and correlation), whether that record was **just written** (miss path) or **already present** (hit path). Package-local execution detail lives in [`lambda/ephemera/dataSource/renderCache/AGENT.md`](../../../../lambda/ephemera/dataSource/renderCache/AGENT.md), [`lambda/ephemera/dataSource/renderOrchestration/AGENT.md`](../../../../lambda/ephemera/dataSource/renderOrchestration/AGENT.md), and [`currentCachePointers/AGENT.cachePointersRefactor.planning.md`](currentCachePointers/AGENT.cachePointersRefactor.planning.md); those docs should reference this file for shared semantics and payload shape.

**Directional priority (`renderOrchestration`):** Passive orchestration outcomes are **outgoing events on the `mtw.ephemera.renderOrchestration` DataSource stream** (product decision; uncertainty 8 **transport** resolved). **Module location** for six-outbound TypeScript types is **resolved** (uncertainty 8): **`publishedEvents.ts`** colocated with the DataSource (**`busOnly`**), not **`mtw-interfaces`**. **Exact envelopes** ( **`header.type`** per outbound, **`getContent`** wiring) remain **implementation** details; see [`lambda/ephemera/dataSource/renderOrchestration/AGENT.md`](../../../../lambda/ephemera/dataSource/renderOrchestration/AGENT.md). The passive **`Render Requested`** path does **not** use **`conversation.sendMessage`** or legacy **`RenderReady`** / **`materializeRoomStateRender`** for pipeline outcomes (stream-only).

**Where types live:** [`packages/mtw-interfaces`](../../../../packages/mtw-interfaces) is the **client and cross-service** contract package. **Internal-only** DataSource streams (orchestration -> **`renderCache`**, etc.) need **stable TypeScript types** shared between producers and consumers, but those types **do not** have to live in **`mtw-interfaces`** unless the same payloads are (or will be) **serialized to the client** or **another service**. For **`publisherStrategy: 'busOnly'`** DataSources, the DataSource pattern requires **outgoing** payload types in **`publishedEvents.ts`** next to the DataSource (see [`mtw-lambda-patterns` DataSource implementation](../../../../packages/mtw-lambda-patterns/ts/dataSource/AGENT.implementation.md) (section **publishedEvents.ts and outgoing update payloads**)). **`renderCache`** consumes **`renderOrchestration`** outbounds by importing the same **ephemera-local** definitions (or a shared barrel under **`lambda/ephemera/`** if you split files later). Prefer **ephemera-local** or **lambda-internal** modules until a boundary requires **`mtw-interfaces`**.
...
| [`currentCachePointers/AGENT.cachePointersRefactor.planning.md`](currentCachePointers/AGENT.cachePointersRefactor.planning.md) | **`mtw.ephemera.currentCachePointers`** - cache pointer commissioning (active; all decisions made, ready to implement). **Note:** CP-4 **cancelled** the separate DataSource --- pointer maintenance moves into `renderCache`; obligations below are superseded |
...
This initiative is aimed at [completion rubric section 4](../../../../lambda/ephemera/AGENT.ephemeraPerceptionVertical.planning.completionRubric.md#4-coherent-ready-to-show): one readiness story for hits and misses, clear **durability** ownership (**`renderCache`** **`Render Pertains`** / **`Cache Updated`**, not orchestration **`Render Generated`**), and an explicit documented contract for graduation vs older paths.
...
| **`renderOrchestration`** | Unit tests for the **six outbound types** (see **Orchestration outbounds**), payloads (**hit** outbounds: **IDs only** --- uncertainty 3), **non**-ownership of the final correlated "ready for perception" signal per this doc, and **no** reliance on **`conversation.sendMessage`** once migrated (replace with stream assertions). |
| **`mtw.ephemera.renderCache` (DataSource)** | Unit tests for **`Render Pertains`**, **`Cache Updated`** pairing on generate (single write from **`Render Generated`** handler per **uncertainty 1**), **hit path**: **`Current Cache Valid`** / **`Exact Match Found`** with **IDs only** then **refetch** before **`Render Pertains`**, and **lean routing** fields (**`componentId`**, **`perspectiveKey`**, **`cacheId`**) --- **no** synthetic correlation id (product decision; was uncertainty 9). |
| **`mtw.ephemera.renderCache`** (pointer field, folded) | **`mtw.ephemera.currentCachePointers` is cancelled (CP-4)** --- there is no separate DataSource to hold pointer tests. Unit tests for **pointer updates** vs **no touch** to **`CACHE#...`** rows fold into the **`renderCache`** row above: pointer set happens at the same **`handleRenderOrchestrationInbound`** seam as **`Render Pertains`**; **`Generation Deferred`** needs **no** clear-pointer test because the pointer self-invalidates on next read (CP-2). See [`currentCachePointers/AGENT.cachePointersRefactor.planning.md`](currentCachePointers/AGENT.cachePointersRefactor.planning.md). |
| **`perception`** | Placeholder or skipped tests for fan-in, out-of-order aggregation, and delivery gating **until** the package is re-architected; still **checked in** so the consumer side of the contract is not invisible. **Obligations** and consumer norms: [`lambda/ephemera/dataSource/perception/AGENT.md`](../../../../lambda/ephemera/dataSource/perception/AGENT.md#obligations-accruing-to-future-perception-working-list). |
...
- **Sequencing (orchestration + renderCache):** Prefer landing **skipped** contract tests for **both** producer (**`streamEvent`**) and consumer (**`renderCache`** subscription) expectations **before** wiring **`streamEvent`** / handlers, so the contract stays encoded in CI while behavior catches up ([`renderOrchestration` **Stream skeleton sequencing**](../../../../lambda/ephemera/dataSource/renderOrchestration/AGENT.md#stream-skeleton-sequencing)).
...
| **Enqueue `Put Cache Record` for six-outbound generation completion** | **`mtw.ephemera.renderCache`** only (on **`Render Generated`**). **`renderOrchestration`** must **not** enqueue **`Put Cache Record`** by any path (uncertainty **1**; [`lambda/ephemera/dataSource/renderOrchestration/AGENT.md`](../../../../lambda/ephemera/dataSource/renderOrchestration/AGENT.md#pass-through-durability-rule)). |
...
| **Who needs player-visible updates** (targets, timelines, message grouping, audience) | **`mtw.ephemera.perception`** (future): holds **delivery correlation** at **registration**; **does not** rely on producers repeating **`characterId`**, **`targets`**, **`messageGroupId`**, **`conversationId`** on streams (see **Routing identity on producer streams** below). [`lambda/ephemera/dataSource/renderCache/AGENT.md`](../../../../lambda/ephemera/dataSource/renderCache/AGENT.md#correlation-vs-routing). |
| **Pointers** (`currentCacheId` on the `Cache::${perspectiveKey}` catalog row) - **which** `CACHE#...` id is current for a component + perspective | **`mtw.ephemera.renderCache`** --- a field on the catalog row it already owns, maintained at the same **`handleRenderOrchestrationInbound`** seam that emits **`Render Pertains`**. **`mtw.ephemera.currentCachePointers` is cancelled (CP-4)**: the separate DataSource this row used to name will not be built; see [`currentCachePointers/AGENT.cachePointersRefactor.planning.md`](currentCachePointers/AGENT.cachePointersRefactor.planning.md). Not all components use this pattern (e.g. some **Feature** flows may resolve on read instead). |
...
| **`Generation Started`** | Orchestration commits to **slow path** generation (e.g. after `allowGeneration` and preconditions). Consumer handling **deferred** (perception / placeholders, etc.). **At least once** on the wire is acceptable: duplicate **`Generation Started`** for the same logical job is a **known edge case** (e.g. **`singleFlight`** leader timeout / self-promote) until orchestration adds **idempotency** or extends the pattern; **clients** and **Perception** should tolerate **repeated intermediates** (see **uncertainty 6**). | TBD beyond contract; not required for **`Render Pertains`** mapping in the first pass. |
| **`Render Generated`** | **Generation** finished in orchestration terms: **full** generated **content** in the payload; **no** promise that **`CACHE#...`** is **durably written** (uncertainty 5 resolved). | **`renderCache`** **persists** (if not already), then **`Render Pertains`** + (maybe) **`Cache Updated`** --- those **do** assert durability; see **`renderCache` reactions**. |
...
| **`Generation Deferred`** | **No** generation run now when policy says defer (e.g. `allowGeneration === false` and no cheap hit, cost cap, or equivalent **invalidate** / hand-off semantics). Distinct from **error** where appropriate. | **No subscriber action required.** The pointer needs no explicit clear: **`findRender`**'s `markStatesEqual` check fails on the first read after state changes, which already clears it (CP-2). **`renderCache`** subscription is a **no-op** for this event (decided, not TBD). |
...
**Legacy `messageBus` terminals (product):** The **target** architecture **does not** emit **`RenderReady`**, **`RenderInvalidate`**, or **`RenderError`** via **`materializeRoomStateRender`** for orchestration outcomes. Those terminals are **superseded** by the **`mtw.ephemera.renderOrchestration`** **six outbounds** and by **`renderCache`** **`Render Pertains`** / **`Cache Updated`** (and stream-level error/defer handling). **Implementation** may still emit them **until** cutover; there is **no** plan to keep them as a **parallel strangler** alongside the new pipeline on **`main`** once the refactor lands.
...
**Intent:** **`renderOrchestration`** and **`mtw.ephemera.renderCache`** streams should **not** carry **`characterId`**, **`targets`**, **`messageGroupId`**, **`conversationId`**, or similar as a **subscriber contract** for reconnecting lifecycle events to **who** gets player-visible output. **`mtw.ephemera.perception`** registers interest with whatever **delivery** fields it needs (audience, grouping, etc.) and **matches** incoming events by **`componentId` + `perspectiveKey`** (and event type / domain payload). Producers stay **registry-free**; **race and uniqueness** when multiple waiters share the same key are **Perception** policy (see [`lambda/ephemera/dataSource/perception/AGENT.md`](../../../../lambda/ephemera/dataSource/perception/AGENT.md#normative-decisions-and-obligations)).
...
**`Render Pertains` / `Cache Updated`:** Carry at least the same **lean routing identity** so **`renderCache`**'s own pointer set and Perception can index consistently. They **also** carry **content** and, together with their emission contract, **guarantee** that the **cache row is durably recorded** (contrast **`Render Generated`** from orchestration, which does **not** assert Dynamo durability --- uncertainty 5). **Product decision (no synthetic id):** **`Render Pertains`** does **not** require **`conversationId`** or any other **synthetic** correlation field. **`componentId` + `perspectiveKey` + `cacheId`** (and durable cache facts in the payload) are enough for the catalog-row pointer write and for Perception indexing. Revisit only if implementation uncovers a concrete gap.

### Limited refinement: per-outbound body fields (narrow agreement)

**Scope:** This subsection records **only** the **body** fields for each orchestration outbound **after** **routing identity** (**`componentId`** + **`perspective`** / **`perspectiveKey`** --- see **Routing identity and Perception** above). **Delivery** correlation (**`characterId`**, **`targets`**, etc.) is **not** part of this table. **Transport** for these outbounds is **resolved:** **`mtw.ephemera.renderOrchestration`** **DataSource stream** (not **`messageBus`** as the contract's primary carrier). **Envelopes** are implemented in code (**`publishedEvents.ts`**, **`sendRenderOrchestrationPublish`**). **Passive path (shipped):** orchestration outcomes use the DataSource stream only; see [`lambda/ephemera/dataSource/renderOrchestration/AGENT.md`](../../../../lambda/ephemera/dataSource/renderOrchestration/AGENT.md). Fine-grained envelope polish and per-event **`renderCache`** handling for some orchestration types may still evolve (uncertainty 8 remainder).
...
**Hit-path payload (product):** **`Current Cache Valid`** and **`Exact Match Found`** carry **IDs only** (at minimum **`cacheId`** plus shared **routing** fields from **Limited refinement**). They **do not** embed a full **`EphemeraCacheDynamoItem`** or **`renderedContent`**. **`renderCache`** **refetches** the row (typically via **`internalCache`** **`RenderCache.get`** / query dedupe for performance) before emitting **`Render Pertains`** with full cache facts. Uncertainty 3 resolved.

**Generation vs durability (product):** **`Render Generated`** on **`mtw.ephemera.renderOrchestration`** means **only** that **content has been generated**; the payload must include the **entire** generated content. It **does not** promise that Dynamo (or any **`renderCache`** write) has completed. **`Render Pertains`** and **`Cache Updated`** on **`mtw.ephemera.renderCache`** **also** carry content and are the signals that **assert durable persistence** for subscribers that need a canonical cache fact (uncertainty 5 resolved).
...
| --- | --- | --- | --- |
| **`Current Cache Valid`** | [`findRender`](../../../../lambda/ephemera/dataSource/renderOrchestration/findRender.ts): **`pointerHint`** set, row validates (`markState`, matcher). | Legacy **`RenderReady`** forwarded **full** rows via **`toRenderReady`**; **target** outbound is **IDs only** (uncertainty 3). | **`cacheId`** (pointer / row id to resolve). **No** **`cacheRecord`** / **`renderedContent`** on the orchestration stream; **`renderCache`** refetches before **`Render Pertains`**. |
| **`Exact Match Found`** | **`findRender`**: pointer branch skipped or invalid after **`clearPerspectivePointer`**, then **`getExactMatch`** returns a row. | Same legacy note as **`Current Cache Valid`**. | Same as **`Current Cache Valid`**: **`cacheId`** + routing; **IDs only** on orchestration; **`renderCache`** refetches. Discriminated only by **outbound type**. |
| **`Generation Started`** | **`generateRoomPreview`** publishes **`Generation Started`** on **`mtw.ephemera.renderOrchestration`** after context parse, before LLM ([`generateRoomPreview.ts`](../../../../lambda/ephemera/dataSource/renderOrchestration/generateRoomPreview.ts)). | Legacy conversation **`sendMessage('generating')`** is **not** used on the passive path. | **No extra payload** beyond shared correlation. Optional: **`phase: 'generating'`**. **Duplicate emissions** possible under **`singleFlight`** recovery; see [`lambda/ephemera/dataSource/renderOrchestration/AGENT.md`](../../../../lambda/ephemera/dataSource/renderOrchestration/AGENT.md#single-flight-generation). |
| **`Render Generated`** | **Shipped:** emit when generation output is **ready**, with **full** content in the payload; orchestration does **not** enqueue **`Put Cache Record`** (**uncertainty 1**); **`renderCache`** performs the durable write on this inbound (see [`lambda/ephemera/dataSource/renderCache/AGENT.md`](../../../../lambda/ephemera/dataSource/renderCache/AGENT.md)). | Passive path does **not** emit **`RenderReady`** via conversation; stream-only. | **`cacheId`**, **`cacheRecord`**, **`renderedContent`** (**full** generated content required). Optional: provenance **`generated`**. |
...
| **`Generation Deferred`** | **`findRender`**: no pointer hit, no exact match, **`allowGeneration === false`** -> **`invalidate`** with **`reason`** [`RENDER_INVALIDATE_REASON_NO_CACHE_NO_GENERATION`](../../../../lambda/ephemera/dataSource/renderOrchestration/baseClasses.ts) (`NO_CACHE_MATCH_AND_GENERATION_NOT_RUN`). | **`RenderInvalidate`** via **`toRenderInvalidate`** (optional **`reason`**). | **`reason`** (string, from invalidate terminal). Optional: **`policy: 'costCap'`** or similar **TBD**. Distinct from **`Orchestration Error`**: not a hard failure; **no** pointer-clear action is needed on this event (the pointer already self-invalidates on the next read that fails `markStatesEqual`, CP-2; see **`renderCache` reactions**). |

**Legacy collapse note:** Today's **`RenderReady`** bus message does **not** distinguish pointer vs exact match; the split is **`Current Cache Valid`** vs **`Exact Match Found`** with **the same** **ID-level** hit shape on the **target** stream, differentiated by **outbound type** only --- no redundant **`hitKind`** on the payload. (Legacy **`RenderReady`** often carried **full** cache rows; **target** orchestration outbounds do **not**.)

### `renderCache` reactions (draft)

- On **`Current Cache Valid`** or **`Exact Match Found`:** **Refetch** by **`cacheId`** (e.g. **`internalCache`** **`RenderCache.get`**) then emit **`Render Pertains` only** (no new Dynamo write), with **full** cache payload after refetch and **lean routing identity** (**`componentId`**, perspective / **`perspectiveKey`**) plus **`cacheId`**; **not** a requirement to echo request-scoped **`conversationId`** for Perception (see **Routing identity on producer streams**). Orchestration hit outbounds carry **IDs only** (uncertainty 3). Envelope detail: [`lambda/ephemera/dataSource/renderCache/AGENT.md`](../../../../lambda/ephemera/dataSource/renderCache/AGENT.md).
- On **`Render Generated`** (from orchestration): **`renderCache`** **durably** records the content (if not already), then emits **`Render Pertains`** and possibly **`Cache Updated`** --- those outbounds **carry** the content **and** the **durable** guarantee (orchestration **`Render Generated`** alone does **not**; uncertainty 5). **Duplicate `Cache Updated` from orchestration also enqueueing `Put Cache Record`** is ruled out by the pass-through rule in **uncertainty 1** (resolved) and [`lambda/ephemera/dataSource/renderOrchestration/AGENT.md`](../../../../lambda/ephemera/dataSource/renderOrchestration/AGENT.md#pass-through-durability-rule).
- On **`Generation Started`**, **`Orchestration Error`:** **`renderCache`** subscription is a **no-op** (no cache readiness signal on either).
- On **`Generation Deferred`:** **`renderCache`** subscription is a **no-op** --- **not** TBD. There is no separate clear-pointer obligation: the pointer already fails validation and clears itself on the next read once `markState` no longer matches (CP-2). **`mtw.ephemera.currentCachePointers` is cancelled (CP-4)**; nothing subscribes to this event to touch pointers.
- **Pointer set (folded into `renderCache`, not a separate DataSource):** at the same seam that emits **`Render Pertains`** (hit refetch or post-generate write), **`renderCache`** sets **`currentCacheId`** on the catalog row from the **cache id** and **lean** routing keys already in scope (**`componentId`**, **`perspectiveKey`**) --- **no** synthetic id required (product decision, unchanged).
...
These are **not** small details; **open** items block a normative contract until addressed. **Resolved** items stay in the list for traceability with package docs ([`lambda/ephemera/dataSource/renderOrchestration/AGENT.md`](../../../../lambda/ephemera/dataSource/renderOrchestration/AGENT.md), [`lambda/ephemera/dataSource/renderCache/AGENT.md`](../../../../lambda/ephemera/dataSource/renderCache/AGENT.md)).

1. **`Cache Updated` / generate path write ownership - resolved (product).** For **passive / six-outbound** generation completion, **`renderOrchestration`** must **not** enqueue **`Put Cache Record`** by any path (**`api.ephemera`**, **`sendPutCacheRecord`**, **`publishPutCacheRecord`**, **`defaultPublishPutCacheRecord`**, or other helpers). **Durable** persistence and subscriber outbounds (**`Render Pertains`**, **`Cache Updated`**) come **only** from **`renderCache`** when it handles **`Render Generated`** (using the same internal write primitive other **`Put Cache Record`** handlers use). **Other producers** (e.g. asset-blueprint) may still enqueue **`Put Cache Record`** for their own writes; that is **not** removed --- the rule is **orchestration** does **not** participate in the put for this pipeline. **Implementation:** after one write, **`renderCache`** emits **`Render Pertains`** then **`Cache Updated`** (see [`lambda/ephemera/dataSource/renderCache/AGENT.md`](../../../../lambda/ephemera/dataSource/renderCache/AGENT.md)). Documented in [`lambda/ephemera/dataSource/renderOrchestration/AGENT.md`](../../../../lambda/ephemera/dataSource/renderOrchestration/AGENT.md#pass-through-durability-rule).

2. **`renderCache` wiring - resolved (product).** **`mtw.ephemera.renderCache`** **subscribes** to **`mtw.ephemera.renderOrchestration`** stream events. **`renderOrchestration`** does **not** **invoke** **`renderCache`** directly, and does **not** use **`api.ephemera`** to invoke it **indirectly**. Documented in [`lambda/ephemera/dataSource/renderCache/AGENT.md`](../../../../lambda/ephemera/dataSource/renderCache/AGENT.md#ingress-and-wiring).

3. **Hit-path outbound payload authority - resolved (product).** **`Current Cache Valid`** and **`Exact Match Found`** carry **IDs only** (**`cacheId`** + routing). **`renderCache`** **refetches** before **`Render Pertains`** (e.g. **`internalCache`** **`RenderCache.get`**). See **Hit-path payload** under **Limited refinement**.
...
6. **Idempotency and duplicate collapse** for subscribers if multiple signals can fire for one logical outcome. **Split:** Producers aim to avoid duplicate **work** (generation, etc.) --- **`singleFlight`** around the generation step in [`lambda/ephemera/dataSource/renderOrchestration/AGENT.md`](../../../../lambda/ephemera/dataSource/renderOrchestration/AGENT.md#single-flight-generation) **reduces concurrent duplicate LLM** for the same key but **does not** alone guarantee **exactly-once** **`Generation Started`** (library **self-promote** / timeout paths). **Perception** owns **subscriber** dedupe so **terminal / final** outputs are not delivered twice for the same logical completion while **intermediate** repeats (including **multiple** **`Generation Started`**) remain acceptable --- see [`lambda/ephemera/dataSource/perception/AGENT.md`](../../../../lambda/ephemera/dataSource/perception/AGENT.md#obligations-accruing-to-future-perception-working-list) (**uncertainty 6** row). **Unsettled** until that behavior is specified in types/tests.
...
8. **Stream event taxonomy (`renderOrchestration`) - partially resolved.** **Documented in this doc (prose):** the **six outbound types** (**Orchestration outbounds** table), **per-outbound body** fields (**Limited refinement**), **legacy terminal** lineage, and **routing identity** for producer streams (**Routing identity on producer streams** --- **`componentId`** + **`perspective`** / **`perspectiveKey`**; **not** request-scoped correlation for Perception). **Resolved (product):** **transport** --- **`mtw.ephemera.renderOrchestration`** **DataSource stream** for the six outbounds (**not** **`messageBus`** as the contract's primary carrier); **`renderCache`** **subscribes** (no direct / **`api.ephemera`** invoke from orchestration --- uncertainty 2). **Resolved (pattern):** **module location** for **outgoing** TypeScript types --- **`publishedEvents.ts`** in [`lambda/ephemera/dataSource/renderOrchestration/`](../../../../lambda/ephemera/dataSource/renderOrchestration/) ( **`mtw.ephemera.renderOrchestration`** is **`busOnly`**; **`mtw-interfaces`** is **not** required for this handoff). See [`mtw-lambda-patterns` DataSource implementation](../../../../packages/mtw-lambda-patterns/ts/dataSource/AGENT.implementation.md) (section **publishedEvents.ts and outgoing update payloads**). **Landed in code:** stream emissions, **`publishedEvents.ts`**, **`renderCache`** subscription and **Hit** + **Generate** handlers (see [`lambda/ephemera/dataSource/renderOrchestration/AGENT.md`](../../../../lambda/ephemera/dataSource/renderOrchestration/AGENT.md) and [`lambda/ephemera/dataSource/renderCache/AGENT.md`](../../../../lambda/ephemera/dataSource/renderCache/AGENT.md)). **Still unsettled:** fine-grained envelope polish, per-event **`renderCache`** handling for **`Generation Started`** / **error** / **defer** (often no-op).

9. **`Render Pertains` wire extras - resolved (product).** **Perception** does **not** depend on **`conversationId`** (or similar) **on producer streams**; it reconstructs from **registration + `(componentId, perspectiveKey)`** (see **Routing identity on producer streams**). **`Render Pertains`** **does not** use a **synthetic id** on the wire; **component x perspective** (+ **`cacheId`**) is sufficient. **`mtw.ephemera.currentCachePointers` is cancelled (CP-4)** --- the pointer field is set in-process by **`renderCache`** from data already on the payload, so the justification for lean routing now rests on **Perception's** needs alone, not a second (never-built) subscriber. Documented in [`lambda/ephemera/dataSource/renderCache/AGENT.md`](../../../../lambda/ephemera/dataSource/renderCache/AGENT.md#correlation-vs-routing).
...
- **Per-call-site mapping:** Prose mapping is in **Limited refinement**; **implementation** is **landed** for stream payloads (**`publishedEvents.ts`**, **`renderCache`** handlers). Passive orchestration does not use conversation **`sendMessage`** for outcomes (**uncertainty 8** passive path).
...
1. **`renderOrchestration`:** **Shipped** per [`lambda/ephemera/dataSource/renderOrchestration/AGENT.md`](../../../../lambda/ephemera/dataSource/renderOrchestration/AGENT.md): six outbounds on **`mtw.ephemera.renderOrchestration`**, **`publishedEvents.ts`**, **singleFlight**, passive fan-out **S**, stream-only passive outcomes, thin integration test [`passThroughOrchestrationToCache.integration.test.ts`](../../../../lambda/ephemera/dataSource/passThroughOrchestrationToCache.integration.test.ts).

2. **`renderCache`:** **Shipped** per [`lambda/ephemera/dataSource/renderCache/AGENT.md`](../../../../lambda/ephemera/dataSource/renderCache/AGENT.md): subscription to orchestration, Hit + Generate paths (**`Render Generated`** -> durable write -> **`Render Pertains`** / **`Cache Updated`**), same integration test as (1).
...
- [ ] Package `AGENT.md` files for orchestration and renderCache DataSources updated to stop duplicating contract text; they link here for semantics
...
| **Exit `conversation.sendMessage`** priority + uncertainty 8 (six-type taxonomy + per-outbound body + legacy mapping in prose; **transport:** DataSource stream; **types:** **`publishedEvents.ts`**; envelopes / code TBD) | Done |
...
| **`Render Generated`** vs durability (**orchestration** = generation only; **`renderCache`** = durable; uncertainty 5 resolved) | Done |
| **Generate path:** orchestration does **not** enqueue **`Put Cache Record`**; **`renderCache`** owns write on **`Render Generated`** (uncertainty 1 resolved) | Done |
| **No `RenderReady` / materialize bus** for orchestration outcomes (contract target) | **Shipped** for passive orchestration per [`lambda/ephemera/dataSource/renderOrchestration/AGENT.md`](../../../../lambda/ephemera/dataSource/renderOrchestration/AGENT.md); other conversation-backed callers may still use [`materializeRoomStateRender`](../../../../lambda/ephemera/conversations/conversationTypes/roomStateRender/materialize.ts) outside the passive handler |
| **Hit-path** orchestration outbounds **IDs only**; **`renderCache`** refetch (uncertainty 3 resolved) | Done |
