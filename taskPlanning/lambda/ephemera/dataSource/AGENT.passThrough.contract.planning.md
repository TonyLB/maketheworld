...
**`Render Pertains` / `Cache Updated`:** Carry at least the same **lean routing identity** so **`currentCachePointers`** and Perception can index consistently. They **also** carry **content** and, together with their emission contract, **guarantee** that the **cache row is durably recorded** (contrast **`Render Generated`** from orchestration, which does **not** assert Dynamo durability --- uncertainty 5). **Product decision (no synthetic id):** **`Render Pertains`** does **not** require **`conversationId`** or any other **synthetic** correlation field. **`componentId` + `perspectiveKey` + `cacheId`** (and durable cache facts in the payload) are enough for **`currentCachePointers`** meta pointers and for Perception indexing. Revisit only if implementation uncovers a concrete gap.
