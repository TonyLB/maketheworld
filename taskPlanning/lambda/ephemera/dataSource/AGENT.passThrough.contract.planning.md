...
**Directional priority (`renderOrchestration`):** We intend to **remove dependency on `conversation.sendMessage`** for orchestration outcomes **as early as practical** and replace each former use with **outgoing events on the `mtw.ephemera.renderOrchestration` DataSource stream** (product decision; uncertainty 8 **transport** resolved). **Module location** for those six-outbound TypeScript types is **resolved** (uncertainty 8): **`publishedEvents.ts`** colocated with the DataSource ( **`busOnly`** ), not **`mtw-interfaces`**. **Exact envelopes** ( **`header.type`** per outbound, **`getContent`** wiring) and **identifier spellings** in code remain **implementation** details where not yet pinned; the **`mtw.ephemera.renderOrchestration`** stream skeleton and **`publishedEvents.ts`** types are **landed** (see **Uncertainties** and [`renderOrchestration/AGENT.passThrough.planning.md`](renderOrchestration/AGENT.passThrough.planning.md)). **Still open in code:** conversation **`sendMessage`** / legacy **`RenderReady`** until **Conversation removal** in that plan.
...
| **Enqueue `Put Cache Record` for six-outbound generation completion** | **`mtw.ephemera.renderCache`** only (on **`Render Generated`**). **`renderOrchestration`** must **not** enqueue **`Put Cache Record`** by any path (uncertainty **1**; [`renderOrchestration/AGENT.passThrough.planning.md`](renderOrchestration/AGENT.passThrough.planning.md) **No `Put Cache Record` from orchestration**). |
...
**Scope:** This subsection records **only** the **body** fields for each orchestration outbound **after** **routing identity** (**`componentId`** + **`perspective`** / **`perspectiveKey`** --- see **Routing identity and Perception** above). **Delivery** correlation (**`characterId`**, **`targets`**, etc.) is **not** part of this table. **Transport** for these outbounds is **resolved:** **`mtw.ephemera.renderOrchestration`** **DataSource stream** (not **`messageBus`** as the contract's primary carrier). **Envelopes** are implemented in code (**`publishedEvents.ts`**, **`sendRenderOrchestrationPublish`**). **Replacing** legacy **`conversation.sendMessage`** / **`RenderReady`** for orchestration outcomes in **code** remains **open** (**Conversation removal** in [`renderOrchestration/AGENT.passThrough.planning.md`](renderOrchestration/AGENT.passThrough.planning.md); uncertainty 8 remainder).
...
- **Per-call-site mapping:** Prose mapping is in **Limited refinement**; **implementation** is **landed** for stream payloads (**`publishedEvents.ts`**, **`renderCache`** handlers). **Remaining:** retire conversation **`sendMessage`** for orchestration outcomes (**uncertainty 8** conversation half).
...
1. **`renderOrchestration`:** [`renderOrchestration/AGENT.passThrough.planning.md`](renderOrchestration/AGENT.passThrough.planning.md) is an **active** task plan. **Shipped:** six outbounds on **`mtw.ephemera.renderOrchestration`**, types in **`publishedEvents.ts`**, **no** orchestration **`Put Cache Record`** on generation success. **Open:** passive fan-out **S**, **singleFlight**, **Conversation removal**, thin **integration** test.

2. **`renderCache`:** [`renderCache/AGENT.passThrough.planning.md`](renderCache/AGENT.passThrough.planning.md) is **active**. **Shipped:** subscription to orchestration, Hit path + Generate path (**`Render Generated`** -> durable write -> **`Render Pertains`** / **`Cache Updated`**). **Open:** thin **integration** test with orchestration (**OI-7** / **Cache-OI-6**).
...
| **No `RenderReady` / materialize bus** for orchestration outcomes (contract target) | Product direction agreed; **not** shipped --- legacy **`RenderReady`** still emitted via conversation until **Conversation removal** ([`renderOrchestration/AGENT.passThrough.planning.md`](renderOrchestration/AGENT.passThrough.planning.md)) |
...
| **Uncertainty 6:** duplicate **`Generation Started`** / intermediates acknowledged; **`singleFlight`** intent in [`renderOrchestration/AGENT.passThrough.planning.md`](renderOrchestration/AGENT.passThrough.planning.md) (terminal dedupe still **Perception**) | Done |
...
| Types / interfaces landed | Partial (**renderOrchestration** **`publishedEvents.ts`** + **`renderCache`** consumer handlers; uncertainty 8 remainder: envelope polish, **Conversation removal**) |
| Implementation tracked in child plans | Partial ([`renderOrchestration`](renderOrchestration/AGENT.passThrough.planning.md), [`renderCache`](renderCache/AGENT.passThrough.planning.md) --- stream + generate path done; **S** fan-out, **singleFlight**, **integration**, **Conversation removal** open) |
| **Exploration sequencing** (orchestration wave, then cache wave, then design for rest) | Orchestration stream + **`renderCache`** Hit/Generate paths **shipped**; remaining slices in child plans |

**Recommended order:** Intentionally omitted until this document is promoted from draft; see **When this leaves draft status** and **Intended implementation sequencing (exploration)**. Actionable checkboxes live in [`renderOrchestration/AGENT.passThrough.planning.md`](renderOrchestration/AGENT.passThrough.planning.md) and [`renderCache/AGENT.passThrough.planning.md`](renderCache/AGENT.passThrough.planning.md).
