...
- **Not [EA-3](AGENT.edgeAbstractions.planning.md#open-decisions-design--plan-only).** The subset question --- *which members support a given coarse claim* --- is adjacent and is **not** asked here. This case asks what a claim ranges over given a partition; EA-3 asks whether the partition is fine enough. **Do not file a result here against EA-3.**
- **Not [EA-5](AGENT.edgeAbstractions.planning.md#open-decisions-design--plan-only).** The tie is a new authored fact, not a propagation of the cut.
...
1. **Against [EA-4](AGENT.edgeAbstractions.planning.md#open-decisions-design--plan-only), and it is the only row this case is booked for.** EA-4's live answer is *naming is enough; traversability is not required of a whole-scale edge reading.* **It falls** if the fiction, pressed here, wants a **two-terminal** coarse claim over the spatial peers --- some *X stands in relation R to Y* that a player would act on or an author would write down, and which no member edge already states. **It survives** if the coarse reading is the predication above and there is simply no second terminal for an edge to have --- **while `OBJECT#PlaceSetting` independently occupies the node-coarse cell**, which is what shows the edge axis is not parasitic on the node one.
2. **It does not grade [EA-3](AGENT.edgeAbstractions.planning.md#open-decisions-design--plan-only).** EA-3 asks which members a coarse claim ranges over. If this case answers as predicted there is **no coarse edge here to range over anything**, so EA-3 gains nothing either way, and a finding written from this case may not claim it does.
3. **It does not grade [EA-10](AGENT.edgeAbstractions.planning.md#open-decisions-design--plan-only).** A carrier question presupposes something to carry.
4. **It establishes nothing about whether the node abstraction is *required*.** [EA-6](AGENT.edgeAbstractions.planning.md#open-decisions-design--plan-only) holds that the two axes are each evidence for the other and neither forces anything; a pass here is equally consistent with an author minting `OBJECT#PlaceSetting`, declining to, or doing it much later.
...
**`PlaceSetting On Table` is a *fine* edge whose subject happens to be a *coarse node*.** That is the cell **(node-coarse, edge-fine)**, which [EA-7](AGENT.edgeAbstractions.planning.md#open-decisions-design--plan-only)'s second pass already records as needing no new mechanism, port-qualified terminals being legal on both sides of a Relational edge today. It summarises nothing; it involves none of the `LeftOf`/`RightOf`/`InFrontOf` peers this case's Arrangement is over; **it corresponds to nothing in that Arrangement at all.** **The rule the mistake violated, stated so it is reusable: an edge's coarseness is not inherited from its terminals'.**
