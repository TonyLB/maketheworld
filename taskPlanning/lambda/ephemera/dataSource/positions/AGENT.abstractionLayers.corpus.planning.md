...
Four gaps surfaced while implementing [LP4](AGENT.ludicGraphPorts.planning.md#recommended-order) and re-deriving candidate (c) independently in conversation. **The re-derivation arrived at (c) unprompted, which is corroboration of the case rather than new content** --- Findings 1 and 5 already say it, and say it better. What follows is only what this case does *not* already contain.

**Finding 6: the independence criterion is unstated, and naming it creates a dependency this case predates.** Candidate (c) says construct the whole and transfer it, but never says what makes the whole *ready* to transfer. The sharp form: **reduce the room-side attachment to exactly one `In` edge**, at which point the transfer is moving a single thing and every other relation is either interior to the whole or a port crossing. That criterion is testable where "appropriately independent" is not. **It also makes this case depend on [LP4c](AGENT.ludicGraphPorts.planning.md#recommended-order)** --- the containment kinds --- which did not exist when C14 was written (2026-08-07; premise 9's `In`/`PartOf` decision was 2026-08-09, and nothing implemented it until the ports plan). **Candidate (c) cannot be built before `In` exists**, and neither this case nor Phase 5(b)'s gating line says so; Phase 5(b) names only AB-3's root concept. **Both gating lines corrected 2026-08-19, and this Finding's own criterion is what narrows the gate.** Because it needs `In` to be **representable** and says nothing about how a containment edge behaves under transfer, the gate is satisfied by [LP4c-i](AGENT.ludicGraphPorts.planning.md#recommended-order) --- the representation half of the LP4c split --- **not by all of LP4c**, whose behaviour half **stopped being blocked at all: CD2 (2026-08-22) put `On` into the hosting-kind throw beside `In`/`PartOf`, LD-11 dissolved rather than being answered, and the ports plan closed LP4c on 2026-08-23.** What survives of LD-11 is the `Against` half, now [AB-56](AGENT.abstractionLayers.planning.md#open-decisions-design--plan-only). AB-3's root gate is separately discharged: LP4a landed 2026-08-19. **[Finding 7](#c14-the-tray-its-cups-and-how-many-times-it-gets-picked-up)'s `Against` contradiction is unaffected by any of this** and remains the live reason candidate (c) inherits LD-11 --- it is a disagreement about the classifier, not about whether `In` can be said. **One further consequence, recorded 2026-08-19 as [AB-53](AGENT.abstractionLayers.planning.md#settled-register):** this Finding's criterion --- *reduce the room-side attachment to exactly one `In` edge* --- reads naturally under root-to-part containment, but if that restriction lifts, *"exactly one `In` edge"* needs restating in terms of which node the edge attaches **to**, since a nested arrangement can hold several containment edges without any of them being the room's.
