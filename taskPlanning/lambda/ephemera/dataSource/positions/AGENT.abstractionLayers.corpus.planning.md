...
The evidence base later phases argue from. **Three admission rules.** The first two were restated 2026-08-14 to describe what this corpus had actually been doing since [C13](#c13-taking-and-returning-the-fork), rather than only what it set out to do; **the third, added 2026-08-27, promotes an observation this file already carried into a binding rule rather than adding new doctrine.**
...
3. **A case states which granularity level(s) of the fiction it stages, and where the same fiction could reasonably be staged at more than one, it prefers staging more than one and records how the levels interplay** --- rather than settling for whichever level happened to be in view when it was written. **This is not a new axis; it is the depth blind spot already recorded below in this section promoted from a finding to a rule** --- two days of two-level-only cases stood unchallenged until [C11](#c11-the-rube-goldberg-machine) staged four levels at once, and nothing before this rule required a case to try. **The falsifier is built in, not left implicit:** if a real case's fiction genuinely admits only one reasonable level, saying so is a satisfying answer, not a failure to meet the rule --- an unwritable second level is a finding, the same way [EA-3](AGENT.edgeAbstractions.planning.md#ea-3) already treats an unwritable falsifier. **Amended 2026-08-27, one day later, and the amendment is one word: *level* was written as a scalar.** This family has more than one axis to be coarse or fine on --- [EA-7](AGENT.edgeAbstractions.planning.md#ea-7)'s second pass establishes that **node granularity and edge granularity are separable axes**, and the presence corpus carries bucket-granularity as a third --- so a level here is a **coordinate, not a number**. **As amended: a case states which axes its fiction has levels on and where it sits on each, and a re-grading names the *cell* rather than the case** --- *C19 at node-coarse, edge-coarse* --- which gives [rule 5's](AGENT.edgeAbstractions.corpus.planning.md#how-to-read-and-write-cases-here) *re-grade, never copy* an address it can actually use. **What it prevents is the reason it exists, and it is not tidiness:** without a coordinate the corpus fragments into per-axis cases --- *this fiction, but only for nodes* against *this fiction, but only for edges* --- and the same world gets written twice under two IDs. **The concrete failure it was written from:** on 2026-08-27 [EA-4](AGENT.edgeAbstractions.planning.md#ea-4) went looking for a case and **rejected the place setting on the ground that `OBJECT#PlaceSetting` already exists**, reasoning that an available node answer spoils discrimination. **It does not** --- it answers a different question about the same fiction --- and the unamended rule had no vocabulary to say so. **The two axes are non-exclusive by design** ([EA-6](AGENT.edgeAbstractions.planning.md#ea-6): an arrangement and a node abstraction are each evidence for the other and *neither direction forces anything*), so **a case selected for one axis being unavailable cannot exist**, and hunting for one yields fiction bent to a method rather than fiction. **Non-obligation is not exclusivity**, and that is the sentence the search misread. **The falsifier still travels:** a fiction with only one axis genuinely in play satisfies this by saying so.
...
| [C5](#c5-the-chain-mounted-at-both-ends) --- chain mounted at both ends | **Mutation** | Positioning | `removeObject` asserts and throws; re-hosting and reabsorption are separate *events* (was *migration-down*; renamed 2026-09-11, naming only) |
...
| [C20](#c20-the-locked-room-and-nothing-obvious) --- the locked room, and *nothing obvious* | **Authoring** | Reasoning, Mutation | P5's first falsification case, **re-aimed twice before landing**. Narrowings turn out not to be extensional: *no **obvious** entrances* denotes a **manner of minting**, not a set of things --- which vindicates P5 clause 1 and falsifies an assumption P5 clause 2 never stated |
...
**Origin:** not a park --- raised in conversation 2026-08-05 as a sharper instrument than rope state (iv) for the same test. **This is the first [H1](AGENT.abstractionLayers.proposals.rejected.planning.md#h1-single-host-objects-hosting-at-the-containing-node) falsification case run**, and it falsified a sub-claim; see below.
...
**What it establishes.** **Co-location and load-bearing are independent axes**, so ~~migration-down~~ **re-hosting** and reabsorption are different events. All three nodes (chain, chain-end-1, chain-end-2) must persist in the cell's graph for the relations to have endpoints, ~~*while* the whole is correctly hosted in the cell rather than the Area. Dragging the chest out and back cycles the chain up to the Area and down again~~ **--- struck 2026-09-11, naming only; the finding is untouched.** *Hosted in the cell rather than the Area* and *cycles up to the Area and down again* are [H1 clauses 4 and 5](AGENT.abstractionLayers.proposals.rejected.planning.md#h1-single-host-objects-hosting-at-the-containing-node), withdrawn 2026-08-07: no host is computed for a multi-located thing and nothing migrates up or down. **Restated:** dragging the chest out and back changes which hosts the chain holds presence ports into --- it gains one and loses one --- **without ever dissolving a part**, which is exactly the independence the case was run to show. The co-location axis is a read off the egress list under [C9](#c9-coiling-the-rope-back-in) (*"all ports share an egress host"*), not a height comparison.

**The guard is already shipped, and it throws.** Reabsorbing a part means removing its node, and [`removeObject`](../../../../../lambda/ephemera/dataSource/positions/ludicGraph/AGENT.md#known-limitation-deferred) **asserts and throws** `RelationalEdgeStillReferencedError` when a Relational edge still references it (BD-33/35, with deliberately no silent-strip variant). So dissolving chain-end-2 while it is attached to the chest is not a subtle data-loss bug --- it fails at the kernel. The load-bearing test for the relational half is therefore a **graph query** (`nodeHasRelationalEdge`, already on the class), not an LLM judgment, and the unsafe version is already impossible. Third instance of the same convergence AB-28 records: the boundary-edge discipline anticipated the case.
...
**The constraint it puts on AB-25.** ~~The two wholes host at **different heights** --- the rope spans room and garden so it hosts at the Area; the timer's members are both in the room so it hosts in the room. Walking up from the shared span therefore yields parents at inconsistent levels, so **hosting height does not correlate with composition depth** and no implementation may treat the parent index as "the next level up."~~ **Struck 2026-09-11, at source, because two live rows were drinking from it.** *Hosts at the Area* is [H1 clause 4](AGENT.abstractionLayers.proposals.rejected.planning.md#h1-single-host-objects-hosting-at-the-containing-node) --- a single host computed for a multi-located thing --- and H1 was [withdrawn 2026-08-07](AGENT.abstractionLayers.proposals.rejected.planning.md#rollback-mechanics-executed-2026-08-07). Under the locked frame the rope has **two presence ports and no host height at all** ([PQ-3](AGENT.abstractionLayers.proposals.planning.md#open-questions-ab-34-sub-questions-ids-stable-never-reused), 2026-08-06; [AB-8](AGENT.abstractionLayers.planning.md#open-decisions-design--plan-only)'s (a)-with-ports re-framing, 2026-08-09), so there are no heights to be inconsistent between. **The prohibition survives and stops being a constraint.** *Do not treat the parent index as "the next level up"* is still correct, but it is now trivially entailed rather than earned by this case: there is no hosting stratification for an index to mistake for one. **Recorded rather than deleted because the case's *reason* for the prohibition is what this sweep found stale, and a reader who only sees the surviving one-liner would not know it had ever been load-bearing.** AB-25's core question --- whether Room/Area containment is a tree --- is untouched either way.
...
**Verdict, after two reversals recorded below: C7 is a *passed discriminating test of [H1 clause 3](AGENT.abstractionLayers.proposals.rejected.planning.md#h1-is-a-conjunction-and-its-clauses-are-tested-separately)* --- spanning things are modelled as parts.** **Re-homed 2026-09-11, and the pass survives with its force reduced.** H1 clause 3 has a live successor, [H3 clause 3](AGENT.abstractionLayers.proposals.planning.md#h3-ports-as-the-boundary-mechanism-decomposition-as-a-modellers-choice), which **demotes** it: C11 Finding 2 made *spanning things are parts* a modeller's **option** rather than a requirement. C7 still passed, and what it passed is that the one-node alternative **breaks** --- a finding about framing (a), not a law compelling decomposition. **Read the verdict as: parts must be *available*, not mandatory.** The strongest result a discriminating test can give: not that H1 survived, but that the alternative **broke**. It says nothing about H1 clauses 1, 4 or 5, which were used in working the case and never stressed --- nothing here asks whether the LCA is computable or the cascade plannable. The axis it actually tests is **one node versus parts** --- and conceding parts lands in AB-8's framing (c), which H1 embodies. An (a)-with-segments variant would be framing (c) with multi-host still permitted, at which point single-host is a secondary question rather than a settled one.

> **The (a) row's grading is dated, annotated 2026-08-26 rather than rewritten** (this case is a dated record of what was true when written --- same convention as [C10](#c10-the-moonbase-computer-system)'s annotation below). **The judgement was correct on the evidence it had. What is out of date is the structure it graded, not the grading and not the tests.**
>
> **What survives untouched, and it is the durable half.** The three demands the case makes of *any* representation: a bystander grabs the thread **trailing past**, not the whole of it; three attachments --- tied at the entrance, tied to a pillar, held by Theseus --- sit at different points **along** it; and the route is **not recoverable from exits**, because a labyrinth has multiple paths. **These were written 2026-08-05 with no notion of ports**, they are properties of the object rather than of any schema, and they are what a later candidate has to answer.
>
> **What is out of date is framing (a) itself.** It is *"add the room to the thread's container set"*, and *"a container set is unordered"* is the sentence carrying the whole refusal. **Multi-room presence is no longer a container set:** it is a presence port per room --- *present in room R iff some presence port has `fromHostId === R`*, the field on [`EphemeraLudicGraphPort`](../../../../../packages/mtw-interfaces/ts/ephemeraMeta.ts), as [C10's annotation](#c10-the-moonbase-computer-system) states it against shipped code --- and a port is separately addressable in a way a set entry never was.
>
> **So (a)-as-tested is not a candidate anyone would now build, and this refusal must not be inherited by a later one-object representation merely because it also has one node.** That inheritance is the `overbroad-inheritance` pattern in [the performance tally](../../../../AGENT.designVariant.performance.md), whose recorded hits are **mostly read-side** --- a later reader carrying a correct verdict onto something that resembles what it was passed on. Labelling the row is the only fix that reaches that; asking readers to be careful is not.
>
> **What this annotation deliberately does NOT say: that ports answer the three tests.** Whether a **zero-part-node** representation --- one `Thread`, presence ports, and presence-port-to-presence-port `ConnectedTo` edges carrying the route --- passes them is **an open grading, not settled here**. The bystander test is the sharp one: it needs an edge to terminate **at** a presence port, which is [PR-15](AGENT.presence.planning.md#open-decisions-design--plan-only)'s open question. **Stating the premise is superseded and stopping there is the point** --- a grading written after its own conclusion cannot falsify it. **That grading was then run, separately and after this annotation, as [the zero-part-node question](#the-zero-part-node-question-run-2026-08-26) below; it passes all three, and it found the bystander is *not* the test that needs PR-15.**
>
> **Reach.** The Verdict above rests on (a) breaking; *"framing (a) needs the same derivation"* below re-uses *"a flat container set"*; and [the nesting question](#the-nesting-question-run-2026-08-07) opens by taking the one-node framing as *"eliminated (above)"*. **All three inherit this premise. None is withdrawn**, because no replacement has been graded yet. Note also that the *(a)-with-segments* variant named in the Verdict is a **different** third option from the zero-part-node one above --- it keeps part-nodes and drops single-host; do not read one as the other.
...
**And the summary costs something the comparison does not price.** Authored, it goes stale when the thread changes; derived, the derivation has moved to write time rather than gone away. That is the **mutation** axis charging for a reasoning gain --- exactly the cross-axis cost the frame exists to surface --- and it is now **AB-37**. One encouraging wrinkle: by [the strength test](AGENT.abstractionLayers.proposals.planning.md#the-port-record-split-complementary-with-the-interior-authoritative-settled-2026-08-06), a whole-level summary saves reading twenty constituents, making it **N:1 and therefore strong** --- the same class as the membership node, and unlike the ingress list that got moved to cache.

**Competitor check.** The flat form can answer *is there a snag* with a reverse index over constituents --- **AB-36**. But that is a special-purpose index recovering what nesting supplies structurally: a second mechanism for a subclass, which is [position 3](AGENT.abstractionLayers.proposals.planning.md#three-positions-on-reference-and-what-h1-clause-1b-actually-costs). **Rescuable only by an epicycle**, stated rather than asserted.

#### The zero-part-node question (run 2026-08-26)

**The third run on this object, and the first to grade a level the case never considered.** Run 1 (2026-08-05) refuted framing (a); run 2 (2026-08-07) ranked flat against nested. **Both took *one node versus parts* as the axis and both conceded parts.** This run asks whether parts were necessary at all. Admitted under [rule 2](#phase-0-corpus) --- it grades an unbuilt proposal and names the clauses: [PR-15](AGENT.presence.planning.md#open-decisions-design--plan-only)'s terminal permission, [PR-9](AGENT.presence.planning.md#settled-register)'s totality, and **AB-26 as this case itself corrected it**. It is downstream of the annotation above, which is what made the run possible by dating (a)'s verdict rather than leaving it to be inherited.

**The candidate.** One `Thread` node; **zero part-nodes**; a presence port per room it lies in; `ConnectedTo` edges between adjacent presence ports carrying the route. **The negative commitment is the whole claim** --- presence ports *plus* part-nodes is a different proposal, and in particular is not the *(a)-with-segments* variant named in the Verdict, which keeps part-nodes and drops single-host.

**The port's meaning has to be primitive, and PR-15 is split on this.** The row's denotation clause reads *the `Present`-kind walk from that presence port*, which is derived. With one node that walk returns `{Thread}` from **every** port --- PR-9 puts the root in every bucket --- so it distinguishes nothing and totality goes vacuous, as PR-15 separately notes. **The port must therefore *be* the referent rather than point at a set of nodes**, and the phrase is *the part present **via this port***, not *in this place*: a thread crossing one room twice has two ports on the same room, legal because the presence rule quantifies over *some* port.

**Setup finding, and it redistributes two of the three tests: holding is not an edge.** [`AGENT.concepts.md`](../../../../../lambda/ephemera/dataSource/positions/AGENT.concepts.md) has held inventory as membership on the character host, with `takeHold` an `executeObjectMove(ROOM# -> CHARACTER#)`; and `fromHostId` is an `EphemeraMembershipHostId`, which **includes `EphemeraCharacterId`**. So *held by Theseus* is a presence port egressing to a character --- same shape as a room port, different host --- and needs no PR-15. **Only the inanimate attachments need an edge**, because *tied to a pillar* is a relation and not presence.

**Test 3 (the route) --- passes, and it is the test that actually forces PR-15.** The port-to-port chain records the room sequence; reading `fromHostId` along the walk retrieves it. **Why it must be edges rather than an ordering field on the port is supplied by this case's own AB-26 correction** --- *not assumed to be a sequence... a net's is not.* An integer presumes a sequence; a chain does not. **Edges terminating at ports is PR-15**, needed here with zero attachments present.

**Test 2 (the attachments) --- passes, with *where along* derived rather than stored.** Entrance and pillar terminate at `port_1` and `port_k`; Theseus is the chain's last link. The ordering falls out of the walk Test 3 already required: **one mechanism, two demands.** Resolution bottoms out at one point per room-visit, so two attachments in one room cannot be ordered --- **but H1 mints one span-part per crossing and bottoms out identically. Not a discriminator.**

**Test 1 (the bystander) --- passes on the demand as written, and it is the informative one.** `Bystander -> Thread#port_k` is faithful and is not the whole thread. **Framing (a)'s refusal does not inherit** because the grab targets a **port**, not the node: same one node, different thing pointed at, and a port is addressable where a set entry was not.

**The one place H1 wins: the bit-ness is recoverable but not local.** Read one port and you cannot tell a middle from a whole --- you must read the port list and count twenty. H1 inverts this: `ThreadSpan#k` is a part by its own identity and the whole-ness is remote. **[The nesting question](#the-nesting-question-run-2026-08-07)'s own standard --- *the information cannot be omitted, only missed* --- applies here and runs against the candidate.** Recorded as a real cost, not a defeat.

**What is *not* a cost, corrected in conversation 2026-08-26 (user): the grab is a mint plus a splice rather than an `executeObjectMove`, and that is correct rather than expensive.** *Grab the thread* introduces a distinction the fiction did not previously contain --- the bit of thread here that is held, against the bit that is not --- and **a new distinction has to be recorded somewhere; there is no representation in which it is free.** The general pattern is **coarse until the fiction complicates it, then mint the detail on demand**, which is this case's existing *Improvisation, on-demand, per crossing* annotation arriving from the mutation side, and it should be expected to recur. **The comparison is a wash in any event:** under plain H1 the grab was the ordinary shipped take, but [C13](#c13-taking-and-returning-the-fork) already records that this stops being true under H3, whose version needs a step *neither shipped code nor H3's clause table describes*.

**A gain outside the three tests: AB-4 dissolves.** This case called held-and-present *the gap underneath the whole case*. The candidate's `Thread` node is in no host at all --- presence is entirely via ports --- so a port at `CHARACTER#Theseus` and a port at `ROOM#7` coexist with nothing to reconcile. **Not a workaround; the same mechanism doing both.**

**The one shortfall, and it is owed to another plan.** The chain records *room k is adjacent to room k+1*, where H1's connection recorded **which exit was traversed** --- and a labyrinth may join two rooms by two doors. Not one of the three demands, and repairable: **edge-to-edge association, with the `Exit` edge *licensing* the `ConnectedTo` span**, is [`AGENT.edgeAbstractions.planning.md`](AGENT.edgeAbstractions.planning.md)'s subject, opened naming *the exit that licenses a room-to-room span* as one of the four faces it exists to hold. **That design owes this case a grading of its own when it refines**, per the user's direction on the same date; it is expected to score better here, and expectation is not a result.

**One inbound consequence, recorded rather than claimed.** [EA-10](AGENT.edgeAbstractions.planning.md#ea-10) names its own weakness as *port-to-port edges are only ever summary edges*, failing on an authored Arrangement where such an edge could be primary. **This candidate's route chain is exactly that counterexample and is not authored** --- the `ConnectedTo` edges *are* the route rather than a summary of any finer edge. So the weakness has an instance from the corpus side, and EA-10's candidate `kind`-marker fix should be sized against it.

**Verdict: the candidate passes all three demands as C7 stated them. It is not a walkover** --- the locality cost is real and is priced by this case's own reasoning axis --- **and it does not retire H1**, which was tested here on three demands rather than on the mutation and reasoning axes run 2 used. **What it does settle is that C7 no longer refuses a one-node representation**, and the refusal it does carry belongs to a container set rather than to node count.

> **Annotation, 2026-08-26 (same day, after this run).** **[PR-15 was decided in favour and graduated](AGENT.presence.planning.md#settled-register)**, so the two references above to its *open question* are dated: the terminal permission this candidate needs now exists. **Nothing in the grading changes and nothing is withdrawn** --- the run was written against an open row on purpose, and its value was that it **moved** the necessity argument off the bystander and onto the route chain rather than confirming the row as written. **The one thing worth carrying forward:** the decision's stated reason was **not** the disjunct this section argues (*terminal or unrepresentable*). It was that **a presence port is a referent to an implicit abstraction** --- there is no node for *the part of the thread in this room*, and the port names that part directly. **That reading generalizes a finding this section left as a special case:** the port **is** the referent at every level, not only at zero part-nodes.

**Rows grounded (outside this plan).** PR-15 (its necessity relocates --- see the row), PR-9 (totality goes vacuous at this level), PR-4, AB-4, AB-26, EA-10.
...
**Origin:** seed example 2, run 2026-08-06 against [the non-positional-whole candidate](AGENT.abstractionLayers.proposals.rejected.planning.md#candidate-drop-h1-clause-1b-and-the-whole-chain-with-it) --- which it broke. The case had been in this plan since day one and the candidate was never checked against it.
...
**The operation.** Standing in Room A, haul the rope in. `RopeSpan` moves B -> A: **change port 2's egress to `ROOM#A`** --- and that is the entire write, since [the ingress list lives in `positionCache`](AGENT.abstractionLayers.proposals.planning.md#the-port-record-split-complementary-with-the-interior-authoritative-settled-2026-08-06), not in the rooms' graphs. `RopeEnd2` moves C -> B the same way, and because that was `ROOM#C`'s last port, **the `OBJECT#ROPE` node leaves `ROOM#C` entirely.**
...
**What it does to [H1](AGENT.abstractionLayers.proposals.rejected.planning.md#h1-single-host-objects-hosting-at-the-containing-node) H1 clause 5: leaves it nothing to do.** Under H1 coiling migrates the *whole* down from the Area into a room, with the cascade and plan-time pre-locking H1 clause 4 demands. Under multi-host + ports **there is no whole-migration** --- the whole is present wherever its ports say, and coiling shrinks the port set. H1 clause 5's queued concurrent-migration and thrash cases target machinery that would not exist. **Third time a queued test set has dissolved rather than run.**

**Two rows it clears outright.**

- **AB-15's naming seam** --- flagged as the one step of [part-mediated reference](AGENT.abstractionLayers.proposals.rejected.planning.md#part-mediated-reference-how-h1-answers-its-own-replacement-trigger) that does not fall out of existing machinery. Here the room's node **is** `OBJECT#ROPE`: the rope is present *by name* in every room it passes through, with the ports carrying the where-along. No catalog widening, no aliasing, no two-phase match.
...
> **Superseded twice over, annotated 2026-08-21 rather than rewritten** (this case is a dated record of what was true when written; see the corpus convention that cases are written against **today's** model). **(1) The vocabulary is out of date.** *Egresses there* predates the port list: the rule now reads *present in room R iff some **presence** port has `fromHostId === R`* --- `fromHostId` being the field on [`EphemeraLudicGraphPort`](../../../../../packages/mtw-interfaces/ts/ephemeraMeta.ts), where `hostId` is the graph's own. **(2) The framing is out of date too, and this paragraph argues against a position the case itself abandons** in the reframe below --- *does the whole surface at the level of its parts, or only by traversal from them.* **What is *not* out of date is the counter-example, and the refinement does not touch it:** `MoonbaseComputer` still has a presence port into the lab, because `Terminal1` is there and that is how the graph records it. **Rope and computer still have identical presence-port topology and still require different room-scale answers**, so *"no structural predicate separates them, so it must be declared"* survives verbatim. **What changed is where the refusal lands.** Under [PR-1](AGENT.presence.planning.md#settled-register)'s split, C10 refuses **apprehensibility**, not **membership**: the computer system genuinely *is* present in the lab in the membership sense, and what is false is the inference from *present in the lab* to *answers "what is here" in the lab*. **So read *"that is the wrong answer"* as a claim about the second, never the first.** **This case is therefore not evidence that membership must be declared**, and it was briefly read that way on 2026-08-21 while [PH0d](AGENT.presence.planning.md#recommended-order) was being closed.
...
Under ports there is exactly **one** system graph either way, and the rope is not "hosted in a room" in any strong sense --- it is *listed* where it is present, which [the re-pricing section](AGENT.abstractionLayers.proposals.planning.md#consequence-ports-re-price-the-three-positions-table) already noted is closer to a presence index than to real multi-hosting. So (a)-versus-(b) was largely dissolved before this case arrived. **What actually varies is narrower: does the whole surface at the level of its parts, or only by traversal from them.**

**One mechanism with a declared field, not two modes.** Same shape as [PQ-2](AGENT.abstractionLayers.proposals.planning.md#open-questions-ab-34-sub-questions-ids-stable-never-reused)'s "a port's exterior end is a graph, plus optionally a local endpoint," and the same shape as the argument that rescued [position 2](AGENT.abstractionLayers.proposals.planning.md#three-positions-on-reference-and-what-h1-clause-1b-actually-costs). Working name for the field: **apprehension scale** --- at which level is this thing the answer to *what is here*.

**Consequence for the pending clause-1b decision, and it is the largest fold.** This case suggests **the (a)/(b) fork was mis-framed as global**. The live question has been *which one wins*; C10 argues *that was never one question*. Worth having in hand **before** 1b is decided, because it changes what is being decided rather than adding to one side of it. Note this is not a fifth pressure on 1b in the sense [the motivation section](AGENT.abstractionLayers.proposals.planning.md#the-motivation-stated-plainly-2026-08-06) tallies --- it does not push toward withdrawal; it questions the shape of the choice.
...
~~**The defence, and it is not yet strong enough to close the question.** [Part-mediated reference](AGENT.abstractionLayers.proposals.rejected.planning.md#part-mediated-reference-how-h1-answers-its-own-replacement-trigger) **already decouples reference from presence**: "the computer system" binds by traversal from the terminal without the system being present, exactly as with the rope. So the perspective-relative thing is *reference*, handled elsewhere, and what is stored is the structural question of what a room contains.~~ **Withdrawn 2026-09-12 --- and not merely for its carrier. H3 inverts its premise.** The defence needed the whole to be *absent* and reached by traversal. Under [H3 clause 4](AGENT.abstractionLayers.proposals.planning.md#h3-ports-as-the-boundary-mechanism-decomposition-as-a-modellers-choice) the room's node **is** the whole (*"the room's node **is** `OBJECT#ROPE`"*), so grounding names it directly: **reference and presence are now the same mechanism, and there is no *elsewhere* to hand perspective to.** Worse for the defence, clause 4 hardcodes the **engineer's** reading --- a room referencing a whole's port names the whole. **The newcomer's terminal is not unrepresentable** (it is a node in the system's interior graph) **but it is unselectable**, which is a different and smaller problem: what is missing is a rule that lets a room-level reference bind to it, not new structure. **This is the objection that would unseat the case if it holds**, so it is written here rather than dismissed in passing --- **and it now stands undefended.** See [AB-23](AGENT.abstractionLayers.planning.md#open-decisions-design--plan-only), which owns it as the *observer* key, the one of its three with no carrier.
...
#### Finding 1: H1 clause 1a is falsified, on a single object
...
| --- | --- | --- |
| **H1 clause 1a** --- parts have exactly one host | **Single-host.** A span crossing A and B must be decomposed further | `StringSpan` is a part of `String` |
| **H1 clause 1b withdrawn** --- wholes may be multi-host | **Multi-host is permitted** | `StringSpan` is a whole of `SpanA` and `SpanB` |
...
**The claim, now withdrawn:** that [`complexityPreGates.ts:29`](../../../../../lambda/ephemera/dataSource/actions/enrich/objectManipulation/complexityPreGates.ts)'s `multiPresent` terminal error survives multi-host wholes untouched, because [part-mediated reference](AGENT.abstractionLayers.proposals.rejected.planning.md#part-mediated-reference-how-h1-answers-its-own-replacement-trigger) binds a player's utterance to the locally-present **part**, which is single-hosted.

**That rested entirely on H1 clause 1a.** With 1a gone, a player in `ROOM#A` can say *"pull the string"* and ground it on an undecomposed, genuinely multi-present object; `containers.length === 2`; the command is refused. **P1's five-code-sites argument is therefore back in force**, not answered.
...
**Neither locked clause fails.** A whole has its own graph with a root node: true at every level of the chain, which is the recursion working. Boundary crossings are interior-owned bindings: `ROOM#A -> RGM -> String -> StringSpan` is [PQ-5](AGENT.abstractionLayers.proposals.planning.md#open-questions-ab-34-sub-questions-ids-stable-never-reused)'s chaining, and **this is the first case that *requires* it rather than merely permitting it.** The [re-open criterion](AGENT.abstractionLayers.planning.md#what-would-re-open-a-locked-clause) --- a case a clause cannot represent --- is **not met**.

**One clarification the frame does need, and it is a foreclosed misreading rather than a lifted clause.** The lock's wording, and the vocabulary table graduated to [`AGENT.concepts.md`](../../../../../lambda/ephemera/dataSource/positions/AGENT.concepts.md#wholes-parts-and-ports), define *whole* and *part* in a way a reader can take as **kinds**. They are **roles an object occupies relative to a level, simultaneously and at every level.** Same category as [what a port number is not](AGENT.abstractionLayers.proposals.planning.md#what-a-port-number-is-not-added-2026-08-06): the construct was right and its negative statement was missing.
...
**This is a requirements argument, not a corpus finding**, and it should be labelled as such: it is the same class as AB-32's same-mechanism requirement, which produced [the three positions](AGENT.abstractionLayers.proposals.planning.md#three-positions-on-reference-and-what-h1-clause-1b-actually-costs). That class has been decisive here twice, which is why it carries weight the structural evidence alone did not.
...
Fourth time this plan has found a discipline anticipating a case outside its brief (AB-28, AB-29, C5, now this). **Worth noting the pattern is becoming evidence rather than luck**, and worth resisting: the [carry-closure-meets-composition-edge test](AGENT.abstractionLayers.proposals.planning.md#what-counts-as-a-falsification-case-learned-the-hard-way-2026-08-05) is still queued precisely to find somewhere it *does not* hold.
...
**It also sharpens the two-operations split.** [Coarsening was found to name failure-recovery and deliberate summarization](AGENT.abstractionLayers.proposals.planning.md#a-port-is-a-scale-boundary-not-a-relay-2026-08-06), with opposite requirements about silence. **On causal edges, failure-recovery is not safe either** --- resolving an unreachable address by falling back to the whole is exactly the unsound inference above. The safe default on a causal graph is **"unknown", not "the whole's answer"**, which is a third behaviour neither operation currently has.
...
Position has the opposite pressure: AB-6 was forced toward **stored** because `computeStepSequenceFootprint` runs once and every part and host must be known before the walk begins. **Re-pointed 2026-09-11:** the argument arrived via ~~[part-mediated reference](AGENT.abstractionLayers.proposals.rejected.planning.md#part-mediated-reference-how-h1-answers-its-own-replacement-trigger)~~ but never depended on it --- it is a fact about shipped code, and [H3](AGENT.abstractionLayers.proposals.planning.md#h3-ports-as-the-boundary-mechanism-decomposition-as-a-modellers-choice)'s stored interior graph (clause 1) and port sets (clause 6) carry it now.
...
A virtual abstraction touched repeatedly (beat 4) is, structurally, exactly what [P4](AGENT.abstractionLayers.proposals.planning.md#proposal-p4-settling-dataflow-and-narrative-heat)'s narrative heat is for: a signal that an ephemeral thing is getting attention and might be worth making durable. **Recorded as the natural home for the promotion decision, not as a solved mechanism** --- heat is iteration 3, not iteration 1, and P4-iteration-1's [named rollback trigger](AGENT.abstractionLayers.proposals.planning.md#what-p4-being-load-bearing-here-means-2026-08-07) could still fire before heat is ever built. Until then, (c) alone --- construct, use, discard, every time, with no promotion --- is a complete and correct answer on its own; promotion is a strict improvement layered on top, not a dependency this case needs to work.
...
**Verdict, stated plainly because it is the case's real payoff.** Object-abstractions --- mint, use, retire --- are not a feature carry closure needs to interoperate with; they are an **internal framework that sometimes gets externalized (persisted, durable) when and as helpful**, and carry closure is one caller of it that happened to get built before the framework existed. This is better-supported than Finding 1 alone states: carry closure is actually **two** sub-mechanisms, not one, and both already have H3-native equivalents found independently. The BFS-walk-and-bundle half collapses into ephemeral-whole construction (Finding 1, this case). The boundary-edge classification half (`boundaryEdgeOutcomes`, verdicts `dissolve` / `carry` / `defer` --- **checked against [`interactionUnderTransfer.ts`](../../../../../lambda/ephemera/dataSource/positions/ludicGraph/expandValidate/interactionUnderTransfer.ts) 2026-08-07: three outcomes, not four; `error` was this plan's own recurring conflation of this classifier with `removeObject`'s separate assert-and-throw guard, corrected here rather than carried forward again**) was already shown, across this same session and independently of this case, to be closely-related vocabulary to what ports' own boundary-edge discipline uses --- the fifth time AB-28 has now caught that discipline anticipating a case outside its brief (P4's topology-change rule, C5, C9, C12, and this). **Carry closure is therefore not a parallel path that resembles the abstraction framework --- it is the abstraction framework, called before the framework had a name, from a caller that never got migrated.**

#### Findings 6--9, recorded 2026-08-17 from the ludicGraph ports implementation

Four gaps surfaced while implementing LP4 and re-deriving candidate (c) independently in conversation. **The re-derivation arrived at (c) unprompted, which is corroboration of the case rather than new content** --- Findings 1 and 5 already say it, and say it better. What follows is only what this case does *not* already contain.

**Finding 6: the independence criterion is unstated, and naming it creates a dependency this case predates.** Candidate (c) says construct the whole and transfer it, but never says what makes the whole *ready* to transfer. The sharp form: **reduce the room-side attachment to exactly one `In` edge**, at which point the transfer is moving a single thing and every other relation is either interior to the whole or a port crossing. That criterion is testable where "appropriately independent" is not. **It also makes this case depend on LP4c** --- the containment kinds --- which did not exist when C14 was written (2026-08-07; premise 9's `In`/`PartOf` decision was 2026-08-09, and nothing implemented it until the ports plan). **Candidate (c) cannot be built before `In` exists**, and neither this case nor Phase 5(b)'s gating line says so; Phase 5(b) names only AB-3's root concept. **Both gating lines corrected 2026-08-19, and this Finding's own criterion is what narrows the gate.** Because it needs `In` to be **representable** and says nothing about how a containment edge behaves under transfer, the gate is satisfied by LP4c-i --- the representation half of the LP4c split --- **not by all of LP4c**, whose behaviour half **stopped being blocked at all: CD2 (2026-08-22) put `On` into the hosting-kind throw beside `In`/`PartOf`, LD-11 dissolved rather than being answered, and the ports plan closed LP4c on 2026-08-23.** What survives of LD-11 is the `Against` half, now [AB-56](AGENT.abstractionLayers.planning.md#open-decisions-design--plan-only). AB-3's root gate is separately discharged: LP4a landed 2026-08-19. **[Finding 7](#c14-the-tray-its-cups-and-how-many-times-it-gets-picked-up)'s `Against` contradiction is unaffected by any of this** and remains the live reason candidate (c) inherits LD-11 --- it is a disagreement about the classifier, not about whether `In` can be said. **One further consequence, recorded 2026-08-19 as [AB-53](AGENT.abstractionLayers.planning.md#settled-register):** this Finding's criterion --- *reduce the room-side attachment to exactly one `In` edge* --- reads naturally under root-to-part containment, but if that restriction lifts, *"exactly one `In` edge"* needs restating in terms of which node the edge attaches **to**, since a nested arrangement can hold several containment edges without any of them being the room's.

**Finding 7: Finding 5 and [the tranche-B back-sweep](AGENT.abstractionLayers.proposals.planning.md#the-parts-and-ports-back-sweep-2026-08-10) contradict each other on `Against`, three days apart, and the reconciliation is load-bearing.** Finding 5 above: *"`Against` and `On`-subject dissolving is very likely still correct regardless of ports"* --- only `Under`-subject and `Custom` are port-preservation candidates. The back-sweep (2026-08-10): *"a rope `Against` a tree is **untied** by recognizing the snare"*, cited as the shipped classifier deciding **wrongly**. Same edge kind, opposite verdicts. **The distinction that reconciles them is not recorded in either place: C14's beat is a *physical displacement* (the tray really moves, so `Against` really should break), and the back-sweep's is *recognition* (nothing moved --- a level was interposed around things that stayed put).** If that holds, then `classifyInteractionUnderTransfer` needs to know **why** the boundary moved, which is a **new input to the classifier, not a new case in it** --- a materially bigger change than adding two rows to its switch. **Carried into [AB-56](AGENT.abstractionLayers.planning.md#open-decisions-design--plan-only)** (formerly LD-11, which was removed 2026-08-23 with the ports plan), which is the decision that will otherwise settle that switch with only the containment half in view.

**Finding 8: the no-read-no-write claim names a construct that does not exist, and the real one has a boundary condition.** The in-memory-only framing is right in spirit --- an ephemeral whole is constructed, used and discarded within one operation --- but it has been discussed as `multiKeyOptimisticUpdate`, which **matches nothing in this repo**. The shipped construct is `MultiKeyUpdate`, and its reducers run under Immer `produce()`: anything retained past a reducer's return must be plain-copied **per element**, or later reads throw on a revoked proxy (the rule [`fromFieldPayload`](../../../../../lambda/ephemera/dataSource/positions/ludicGraph/index.ts) already follows deliberately). An ephemeral whole built inside the reducer and discarded there is *probably* fine --- discarding is the whole point --- **but "probably fine" is the wrong standing for the claim that this costs neither a read nor a write.** Verify against that boundary rather than assuming it.

**Finding 9: the parked footprint item is written for the durable case and gets *sharper*, not merely inherited, under (c).** [Parked](AGENT.abstractionLayers.planning.md#parked-extension-points-this-initiative-would-unpark) via the discussion row: *the footprint locking a host that does not exist at plan time* --- mint's `toHostId` is knowable, since the planner mints the id, but is not yet in any graph. **For a durable mint that is a timing problem, and it resolves the moment the whole is written.** For an ephemeral whole it never resolves: the graph is never written to Dynamo at all, ever. So the question stops being *when does this host appear* and becomes **should `computeStepSequenceFootprint` be locking that host at all** --- a different question with a possibly different answer, and one the parked row's phrasing does not reach.
...
**Consequence for [PS-3](AGENT.abstractionLayers.proposals.planning.md#open-questions-p4-sub-questions-ids-stable-never-reused), recorded before it is answered rather than after.** That row is writing one settle-state vocabulary for four consumers that all want "wait." This is a **fifth consumer that must not share it** --- a shared flag would make a read into a coarse kitchen block on a settle nobody scheduled, and would import exactly the convergence implication this finding rules out.
...
**The state.** Three `OBJECT#PlaceSetting` wholes, in three different taverns, each with `Plate`, `Cup`, `Fork`, `Knife`, `Spoon` as parts. **Under [H3](AGENT.abstractionLayers.proposals.planning.md#h3-ports-as-the-boundary-mechanism-decomposition-as-a-modellers-choice) they are structurally identical** --- a root node, five `part` edges, sibling links, a port to the room. They differ only in how each came to exist:
...
#### Finding 3: P5 clause 2 has been assuming narrowings are extensional --- (B) falsifies it

**[P5 clause 2](AGENT.abstractionLayers.proposals.planning.md#the-scheme-as-a-clause-table-numbered-2026-08-09) says a negative is a recorded narrowing.** Every instance the plan has worked --- *no deep fryer*, *no napkins in stock*, a place setting closed to further members --- **denotes a set of things.** **(B) does not.** *Obvious* is a **predicate that must be evaluated**, and *is this entrance obvious?* is not decidable by lookup at any vocabulary size.

**So there are now two judgment points, where PL-1 established one.** [PL-1](AGENT.abstractionLayers.proposals.planning.md#open-questions-p5-sub-questions-ids-stable-never-reused): **which** ancestor facts are relevant is a judgment. **New here:** **whether a candidate falls inside a narrowing** is also a judgment. **This makes [AB-43](AGENT.abstractionLayers.planning.md#open-decisions-design--plan-only) materially worse** --- *describe a closed region* may have **no descriptive answer at all** for some regions, only a predicate a reasoning step evaluates. **Found by numbering the clauses**, exactly as that exercise was meant to produce: an assumption nobody had stated could not be falsified until it was written down.

#### Finding 4: narrowings may constrain *manner* rather than *content* --- and that is what saves P5 clause 1

**(B) does not say the entrance does not exist.** It says it **may not be established cheaply**. So the three-value model is untouched --- the entrance is **not established** --- and [P5 clause 1](AGENT.abstractionLayers.proposals.planning.md#the-scheme-as-a-clause-table-numbered-2026-08-09)'s unbounded-by-default survives intact. **What the narrowing shapes is *how* a thing may become established, not *whether*.**
...
**So the construct is *not established* plus a promise**, and it costs one sentence. **Sharpened 2026-08-09: the promise is specifically a *minimum-investment floor*** --- *this will not be revealed before a certain amount of player effort* --- **not a deferral.** A mystery whose answer is available on request is worthless, which is why the floor is the whole construct and why *nothing obvious* works where *no* does not. **That also makes a mystery a [manner narrowing](AGENT.abstractionLayers.proposals.planning.md#the-scheme-as-a-clause-table-numbered-2026-08-09) applied to something not yet established** --- Finding 4's split reaching the case that motivated it, rather than a second construct. **That is the whole reason the technique works in practice**, and it is a strong argument that the model should support it directly rather than requiring an author to pre-place a hidden exit --- which would be (A) wearing better clothes, since the solution would again be the one the author envisioned.

**Verdict.** **P5 clause 1 survives and is vindicated** --- the locked room never needed a closed world. **P5 clause 2 is falsified in an assumption it never stated**: narrowings are not always extensional, and a manner narrowing denotes a way of minting rather than a set of things. **P5 clause 9 gains its hardest content** and **AB-43 gets materially worse**, since some regions are predicates rather than descriptions. **AB-46 does real work** as the effort behind a manner narrowing, refined by reincorporation as the manner required. And the case records a **bound** (no authorial control over the path) with its disambiguation from AB-47 (which controls the palette). **Three targets over three re-aimings, and only the third was P5's rather than this plan's** --- worth noting before the next falsification case is queued.
...
**Consequently it should feed [PL-1](AGENT.abstractionLayers.proposals.planning.md#open-questions-p5-sub-questions-ids-stable-never-reused)'s tavern illustration** --- authoring that tavern as *poor* versus *war-stripped* tests this seam at the scale the illustration already occupies, at no extra cost. **Redirected 2026-08-09 from C20**, which was re-aimed at the closed-world case and no longer has a tavern in it.
...
**Rows grounded.** **AB-47** (opened by this case), P5 clauses 2, 8 and 9 and **PL-1** (a second, orthogonal aspect), AB-46 (the first construct in the design that is **not** a matter of degree, which bounds narrative weight's reach), AB-19 (description/mechanism, and probe 4's *decline rather than improvise*), and [PL-1](AGENT.abstractionLayers.proposals.planning.md#open-questions-p5-sub-questions-ids-stable-never-reused)'s tavern illustration, which this case widens at no cost.

### C22. The ship's wheel and the lighthouse

**Origin:** proposed 2026-08-10 from conversation, and **it had already done argumentative work before it was written** --- a ship's cabin (a `Room` that moves) is what defeated a proposed *Features may only be subjects toward Room / Area / Feature targets* rule, on the grounds that mobility is a per-instance fact and not a kind fact. This plan records that it has *"twice mistaken a remembered claim for a verified one"*; writing this case makes that three caught rather than three made. **Phase 0 discipline observed: this case states the situation and what breaks. The candidate answer that prompted it lives at [P3's port-chain subsection](AGENT.abstractionLayers.proposals.planning.md#candidate-perceptual-links-as-port-chains-through-the-containment-hierarchy-2026-08-10) and deliberately not here.**
...
2. **Relations are intra-host.** [`applyStepSequenceCore`](../../../../../lambda/ephemera/dataSource/positions/manipulation/kernel/applyStepSequenceCore.ts) derives a relational step's shared host live and **throws** on mismatch. The bridge and the lighthouse are not in one host and cannot be.
