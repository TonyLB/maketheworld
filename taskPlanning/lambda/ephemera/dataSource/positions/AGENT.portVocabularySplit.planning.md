...
**Phase 3 decided PR-15, 2026-08-26, and it settled IN FAVOUR of the partition** --- a presence port may be a terminal for foreign-kind edges; presence-branch kind-agreement is a category error. [The row graduated](AGENT.presence.planning.md#settled-register) and its live out-of-scope question split to [PR-16](AGENT.presence.planning.md#open-decisions-design--plan-only). **This is the outcome Phase 4's payoff depends on** --- the [risk register](#what-would-make-this-plan-wrong) named *PR-15 settles against the partition* as the way Phase 4 evaporates, and it did not.

**Phase 3's code half shipped the same day.** `classifyLudicGraphPortMismatch` now branches on `kind === 'Present'` and reports no mismatch for a presence port, so the self-heal can no longer rewrite one's `kind` from an edge that merely lands on it. **The branch is a test in code, not an invariant**, and `AGENT.contract.md` had already claimed the opposite --- it said the heal path was crossing-port-only *"by construction"*, which was true of the intent and false of the code. That clause is corrected; the correction is the finding worth carrying out of this phase.

**Phase 4 shipped 2026-08-26.** `EphemeraLudicGraphPort` split into `EphemeraCrossingPort | EphemeraPresencePort` in `packages/mtw-interfaces/ts/ephemeraMeta.ts`, discriminated on `kind`; `exteriorRelationLabel` now exists only on `EphemeraCrossingPort`, so CD6's category error is a compile-time-unrepresentable shape on the presence branch, not just a documented rule. `isEphemeraLudicGraphPort` was narrowed to match: a `Present` port carrying any `exteriorRelationLabel` (string or not) now fails validation, closing a gap the old, looser guard left open. **No collateral breakage turned up** --- contrary to this phase's own prediction (see the superseded verification note below), `npm run build` at the repo root, `tsc --noEmit` on `lambda/ephemera`, and all four areas' full test suites (`mtw-interfaces`, `mtw-gateways`, `lambda/ephemera` including integration tests, `lambda/diagnostics`) came back clean with no source changes needed. Two reasons: the repo-root `npm run build` doesn't cover `lambda/*` at all (only the `packages/*` project references), and the two call sites that build a full port object from a flat `{ kind: HostRelationalEdgeKind, exteriorRelationLabel? }` shape (`classifyLudicGraphPortMismatch.ts`'s early `kind === 'Present'` return, `healLudicGraphPortMismatch.ts`'s `repairedPort` construction) type-check against the new union without narrowing --- TypeScript accepts a non-literal-union-typed `kind` property against a discriminated-union target more leniently here than a minimal reproduction of the same shape predicted (confirmed by direct comparison; not fully explained, but empirically reproducible in this project's actual settings). **This does not mean those two sites are already narrowed for real safety** --- `healLudicGraphPortMismatch.ts:102`'s `repairedPort` literal is exactly the site to double-check by hand in Phase 5, since the type system will not catch a `Present`-kind correction that also carries a label.

**Next step: Phase 5 --- call sites.** No known compile-breakage to work through (see above); Phase 5's job is auditing the 25 `exteriorRelationLabel` sites for correctness under the new discriminant, not fixing type errors.
...
- [X] **Phase 3 --- decide PR-15.** Its own plan's process, not this one's; this plan only records that it is the gate for Phase 4. **Decided 2026-08-26, in favour, and its code and doc consequences shipped the same day.**
  ...
  - [X] **One durable-doc edit the verdict leaves owed**, distinct from the code fix below: [the origin section's failure-recovery row](AGENT.abstractionLayers.proposals.planning.md#a-port-is-a-scale-boundary-not-a-relay-2026-08-06) reads *"where 4 is not a legal port, **or no interior edge leaves it**"* --- lumping a **broken address** with a **legitimately atomic** fact. Those are quiet-because-error and quiet-because-nothing-finer, and PR-15's branch is what separates them. **Done 2026-08-26: split into two rows**, with `kind === 'Present'` named as the atomic case's discriminator. **The section itself already asked for a third row**, so this cost one edit, not one discovery. **Recorded with it: why one word covered both for three weeks** --- the two cases agree on every column that table has (no finer fact, no announcement) and differ only in what a reader may conclude from the silence, which no column was measuring.
  - [X] **Fix the live self-heal hazard in `classifyLudicGraphPortMismatch.ts` and `healLudicGraphPortMismatch.ts`.** [PR-15's row itself names this](AGENT.presence.planning.md#open-decisions-design--plan-only) as **already shipped and live**, not future work like CD6: `edgesReferringToPort` matches `from` **or** `to` against the port address and cannot tell *terminates at* from *crosses*, so a port-to-port `ConnectedTo` between two `Present` ports reads as a mismatch and the self-heal rewrites both presence ports' `kind` to `Custom`, **destroying the presence binding at both ends** --- the diagnostics sweep runs this against real stored port records today, independent of whether any producer yet authors such edges. **Distinct from CD6**, which is scoped to future producer-side write validation once a real producer exists; this is a fix to an already-live read/repair path. Shape follows the verdict this phase just wrote: for `kind === 'Present'`, kind-agreement against a referring edge is the category error, not a check to soften. **Must land before Phase 4/5 or any future producer can author a presence-port terminal edge without it being silently corrected away.** **Done 2026-08-26.** One branch in the **shared** classifier --- `kind === 'Present'` returns no mismatch --- so the sweep and the heal's recheck cannot drift apart on what a presence port means; `healLudicGraphPortMismatch` needed no logic change and inherits it. **Two things the fix turned up.** (1) `AGENT.contract.md` already asserted this heal path was crossing-port-only *"by construction"* --- **Phase 1 wrote the intent and no code held it**, which is the same doc-ahead-of-code gap as finding 1's stale glossary row, in the opposite direction; the clause now names the explicit test and says why incidence cannot supply it. (2) **The test suites were carrying the bug as their default**: `kind: 'Present'` was the port factory's default in `classifyLudicGraphPortMismatch.test.ts` and the stale-value stand-in in eight heal and sweep cases, so *"a presence port gets rewritten to `On`"* was the shape most of this area's coverage asserted. Those are now crossing kinds --- rewritten with the verdict, not made to pass --- and the presence cases are their own block. **Not repaired: records the old path already damaged.** Rebuilding a `kind` overwritten off a presence port needs the interior edges, which this comparison never reads; the LPM reset makes such rows unlikely to exist.
- [X] **Phase 4 --- types. The only genuinely gated slice.**
  - [X] Split `EphemeraLudicGraphPort` into `EphemeraCrossingPort | EphemeraPresencePort` discriminated on `kind`, **keeping the old name as the union alias**. Named per **D-1** below (`Ephemera`-prefixed, matching all 17 other structural types in the file).
  - [X] **Move `exteriorRelationLabel` onto `EphemeraCrossingPort` only.** **This is the gated line and the payoff** --- CD6's category error is now unrepresentable on `EphemeraPresencePort`, a type error at the site that would commit it.
  - [X] Narrow `isEphemeraLudicGraphPort`; **kept the `kind === 'Custom'` requiredness guard** --- it is a crossing-branch rule and was not lost. Added the new-in-this-phase rejection: `kind === 'Present'` with any `exteriorRelationLabel` present (previously only its *type*, not its presence, was checked).
- [ ] **Phase 5 --- call sites.**
  - [ ] Work the **25 `exteriorRelationLabel` sites**; the type change drives them. **Phase 4 found no compile breakage among them** (see Phase 4's shipped note above) --- `npm run build` doesn't reach `lambda/*`, and the two flat-shape construction sites type-check against the new union without narrowing. Treat this as an audit of correctness under the new discriminant, not a compile-error punch list. **Start with `healLudicGraphPortMismatch.ts:102`'s `repairedPort` literal** --- it builds an `EphemeraLudicGraphPort` from a flat `{ kind, exteriorRelationLabel? }` correction and the type system will not catch a `Present`-kind correction that also carries a label.
  - [ ] `classifyLudicGraphPortMismatch` and `healLudicGraphPortMismatch` carry behaviour, not just field access. **Their known PR-15 fix shipped in Phase 3, not here** --- if the type split surfaces a *further* behaviour question in the diff, it goes to PR-16 or CD6, not into the diff. **The presence branch is now the shape the type split has to agree with**: moving `exteriorRelationLabel` onto `CrossingPort` makes that branch's premise a type fact, and `LudicGraphPortExteriorValues` is the seam to watch, since it stays flat while the port type stops being.
...
No open rows remain --- both below shipped in Phase 4.
...
**Shipped in Phase 4, removed per this table's own header instruction:**

- **D-1** (do the two type names take a prefix?): **Yes, `Ephemera`-prefixed** --- `EphemeraCrossingPort` / `EphemeraPresencePort`. Checked all 20 top-level exported types in `ephemeraMeta.ts`: all 17 structural/entity types carry the `Ephemera` prefix without exception; the 3 that don't (`HostRelationalEdgeKind`, `RelationalEdgeKindAndLabel`, `RelationalKindAndLabel`) are explicitly documented as domain-agnostic utilities meant for reuse outside the Ephemera lane, not the norm to follow here.
- **D-3** (does the split imply a data migration?): **No, confirmed.** Grepped test fixtures/factories repo-wide for `kind: 'Present'` combined with `exteriorRelationLabel`; the only hit already asserted the wrong-type case `false`. No fixture, factory, or producer anywhere constructs a `Present` port with a valid string label, so tightening the guard to reject that shape entirely was safe.
