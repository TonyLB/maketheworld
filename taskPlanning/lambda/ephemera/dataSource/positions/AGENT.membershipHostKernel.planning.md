...
**Status: MK0--MK4 landed 2026-08-16 (grounding, decision, and Object-, Feature-, and Area-hosted storage). Blocked on nothing upstream; blocks nothing downstream yet.** MD-1 is decided: option (c), one plain shared shape for Character/Object/Feature/Area, Room layered on top as the documented exception. MK2 shipped `Meta::Object` storage, MK3 shipped `Meta::Feature` storage, and MK4 shipped `Meta::Area` storage, each pairing the `hostDataCategory`/`graphFromMeta` dispatch with play-position cache-gateway widening in one slice (see MK2's finding below for why cache-gateway support could not be deferred). MK4 also folded in `assertForwardHostId`'s Area widening, closing MK5's functional work early. Next step is [MK5](#recommended-order): confirm/close out the cache-gateway widening, then [MK6](#recommended-order): retire the `KNOWN GAP` comments.
...
| **MK5** (confirm cache-gateway widening) | Functionally done as a side effect of MK4 | Confirmation/cleanup pass only |
