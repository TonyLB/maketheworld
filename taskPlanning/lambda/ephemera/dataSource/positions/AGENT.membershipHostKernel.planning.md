...
**Status: MK0--MK3 landed 2026-08-16 (grounding, decision, and Object- and Feature-hosted storage). Blocked on nothing upstream; blocks nothing downstream yet.** MD-1 is decided: option (c), one plain shared shape for Character/Object/Feature/Area, Room layered on top as the documented exception. MK2 shipped `Meta::Object` storage and MK3 shipped `Meta::Feature` storage, each pairing the `hostDataCategory`/`graphFromMeta` dispatch with play-position cache-gateway widening in one slice (see MK2's finding below for why cache-gateway support could not be deferred). Next step is [MK4](#recommended-order): Area-hosted storage.
...
| **MK4--MK5** (Area storage + cache) | Not started | Build Area storage against the shared plain shape, repeating MK2/MK3's storage+cache-gateway pairing (MK4); MK5 confirms/cleans up once all three per-type slices are done. Area has no existing `Meta::Area` record either (per MK3's finding, don't assume it does without checking) |
