...
**Status:** Slice 0 done 2026-09-10; Slice 1 (1a, 1b, 1c) done 2026-09-10. Two slices: presence sub-graph utilities, then a pairwise merge reducer that combines a parent graph with a child sub-graph into an `EphemeraLudicCacheData`. **Next step: Slice 2, the merge reducer in `mergeReducer.ts`.**
...
- [X] **Slice 1. Presence sub-graph utilities**, in `positions/ludicGraph/presenceSubGraph.ts`.
  - [X] **1a. `nodesFromPresencePort(graph, portId)`.** Keyed on `portId`, never on host id (LR-3). Returns the bucket's node set: **the root unconditionally** ([PR-9](AGENT.presence.planning.md#settled-register): the root is in every bucket), plus the nodes the membership rule reaches. **Decide and state the membership rule in the doc comment** (LR-2) --- it is being invented here. **Done 2026-09-10**, in [`presenceSubGraph.ts`](../../../../../lambda/ephemera/dataSource/positions/ludicGraph/presenceSubGraph.ts) (+ [test](../../../../../lambda/ephemera/dataSource/positions/ludicGraph/presenceSubGraph.test.ts)). Zero-or-one presence port returns every node (degenerate arm, corrected from LR-2's literal "exactly one" wording to also cover the zero-port case); more than one is a direct `Present`-edge lookup from the port-qualified terminal, not a transitive walk, since PR-4 fixes `Present` edges as PORT -> NODE only.
  - [X] **1b. `subGraphFromNodes(graph, nodes)`.** All the nodes, all the edges **between** them, and edges to ports per LR-1's rule. **Do not silently drop straddlers** --- [C7](AGENT.abstractionLayers.corpus.planning.md#c7-ariadnes-thread) makes drop unavailable before the question is argued. Two foundations, decided alongside LR-1 and built first because the rest of 1b calls them:
    - [X] **1b-i. Edge identity key.** A pure function, local to `presenceSubGraph.ts`, computing a deterministic string from exactly the fields `edgesMatch` ([`baseClasses.ts`](../../../../../lambda/ephemera/dataSource/positions/ludicGraph/baseClasses.ts)) treats as an edge's identity. Deliberately duplicates that field list rather than importing from `baseClasses.ts`, to keep the Prototype's rollback set at exactly its two tagged files (LR-1's dependency tag).
    - [X] **1b-ii. Stub-port minting.** For an edge with one endpoint outside `nodes`: if the edge already lands on a real port, use it; otherwise mint a stub port keyed on 1b-i's identity key. Minting is transient --- nothing is written to `edge.edgeId` or back to the stored graph.
  - [X] **1c. Unit tests, including the cases that carry information:** a two-bucket hand-authored child; an edge cut at a real port; **PR-C1's intra-graph straddle with no port at the cut**, now resolved by minting (LR-1); **two distinct edges straddling to the same external node**, to pin non-collision; and **the same graph and bucket derived twice**, to pin that a minted port's id is identical both times. The first two confirm; the third decides; the last two are 1b-i/1b-ii's own correctness, not LR-1's.
  - **Done 2026-09-10**, in [`presenceSubGraph.ts`](../../../../../lambda/ephemera/dataSource/positions/ludicGraph/presenceSubGraph.ts) (+ [test](../../../../../lambda/ephemera/dataSource/positions/ludicGraph/presenceSubGraph.test.ts)). `subGraphFromNodes` returns an `EphemeraLudicGraph` (via `fromFieldPayload`, same `hostId`/`rootId`) rather than a bespoke shape --- the induced sub-graph is a cut of the same graph, not a different one, so the shipped class's own accessors serve Slice 2 directly. **Amended 2026-09-10 (LR-6):** the returned graph's `ports` are the stub ports minted for this bucket **plus every one of `graph`'s own presence ports, regardless of bucket** (not just the ones the current bucket touches); `Present`-kind edges are excluded from the edge output entirely rather than classified as interior/straddle content --- deactivated in code with a comment that it may return in a different form once Slice 2's merge design is built.
- [ ] **Slice 2. First-draft merge reducer**, in `positions/ludicCache/mergeReducer.ts`. Parent graph plus one child sub-graph, present at that parent, in; one `EphemeraLudicCacheData` out. **Pairwise, never batch** (LR-7).
  - [ ] **2a. Collapse of crossing relation ports** --- an edge ending at a port in one graph and continuing from that port in the other becomes one edge carrying a **`chains` entry** (LR-8; `EphemeraLudicCacheEdge.crossings: EphemeraLudicCacheCrossing[]` becomes `chains: EphemeraLudicCacheCrossing[][]`, one array element per independently-consolidated route to the same edge identity).
  - [ ] **2b. Merge of two presence sub-graphs for the same child host.** The root arrives twice in a naive node-list union; dedupe by `universalKey` (any node whose buckets overlap, not only the root --- LR-6) rather than tracking `homeShards` on the node. **The shard-membership fact `homeShards` existed to preserve is already carried by the graph's own presence ports**, preserved by Slice 1b regardless of bucket --- no merge-time reconciliation needed. `EphemeraLudicCacheNode.homeShards` is expected to be dropped from the type (or computed on read from `ports`) as part of this slice, per LR-6. **Key the merge on binding, not on host** --- keying on host id collapses the two buckets and re-introduces the one-to-many hop [PR-12](AGENT.presence.planning.md#open-decisions-design--plan-only) Obligation B forbids.
...
| LR-6 | **What tracks "which shard(s) is a node home to" across a bucket merge --- `homeShards` accumulation, or something else?** | Slice 1b (amended), 2b | **Decided 2026-09-10, revised from an earlier "accumulate `homeShards`" framing.** Dedup applies to **any** node whose buckets overlap ([PR-9](AGENT.presence.planning.md#settled-register) permits overlap generally, not only at the root), not only the root Slice 2b's own bullet names. But the fact `homeShards` existed to preserve doesn't need a node-level field at all for the same-graph case: `EphemeraPresencePort.fromHostId` is already that fact, denormalized, independent of bucket. **`subGraphFromNodes` (Slice 1b) now carries through *every* presence port of `graph`, regardless of which bucket is being extracted** --- so "which shards is this whole home to" is recoverable from any one bucket's sub-graph directly, with no merge-time reconciliation. Consequence: `Present`-kind edges are excluded from `subGraphFromNodes`'s edge output entirely, not routed through the interior/straddle classification --- they are bucket-membership metadata already consumed by `nodesFromPresencePort`, not spatial content, and (mechanically) a `Present` edge's `from` always resolves to the root, which is in every bucket, so it would otherwise straddle-mint a redundant synthetic port for a crossing a real port already documents. **Deactivated rather than resolved-away:** whether an interior node's own `Present` edge needs representing again, e.g. as merge-time provenance once Slice 2b's actual merge is built, is left open and commented in place at the code site --- it may return in a different form. **Explicitly not addressed by this:** a node being a member of two genuinely *different* wholes' graphs ([LC8](AGENT.abstractionLayers.ludicCache.corpus.planning.md#lc8-the-spring-in-the-box-and-in-the-contraption)'s cross-host case) --- that isn't a bucket question on one graph, and stays out of scope for this Prototype. **Expected follow-on, not yet done:** `EphemeraLudicCacheNode.homeShards` ([`ludicCache/types.ts`](../../../../../lambda/ephemera/dataSource/positions/ludicCache/types.ts)) is no longer needed for the case this row covers and is expected to be dropped (or computed on read from `ports`, never stored) when Slice 2 touches that type --- a real type change to already-shipped CC0b contract, raised here rather than taken silently. |
...
| LR-8 | **A collapsed edge's identity can be produced by more than one consolidatable route --- how is more than one route kept without merging or tie-breaking them (PR-8's own preserved constraint)?** | Slice 2a | **Decided 2026-09-10, not yet implemented.** `EphemeraLudicCacheEdge.crossings: EphemeraLudicCacheCrossing[]` (one chain) becomes `chains: EphemeraLudicCacheCrossing[][]` (an array of chains). Edge identity (`edgesMatch`'s field list) doesn't see the route a consolidation took, so two independently-discovered routes to the same `(from, to, kind)` triple cannot be kept as two edge records --- they must land on one. `chains` lets that one record keep every route's breadcrumbs side by side rather than one overwriting another, and as long as each `chains[i]` stays an untouched, independent single-path list, this does not reopen [PR-8](AGENT.presence.planning.md#open-decisions-design--plan-only)'s preserved "never merged, never tie-broken" constraint --- the routes sit next to each other, they are not combined. This is a separate mechanism from LR-6: LR-6 is about a *node* (or a whole's binding) recurring across a bucket merge; LR-8 is about an *edge identity* recurring across independently-consolidatable routes. Also a type change to the shipped CC0b contract, same as LR-6's follow-on --- both land together whenever Slice 2 touches `ludicCache/types.ts`. |
