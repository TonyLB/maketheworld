...
- [X] **Slice 1. Presence sub-graph utilities**, in `positions/ludicGraph/presenceSubGraph.ts`.
  ...
  - **Done 2026-09-10**, in [`presenceSubGraph.ts`](../../../../../lambda/ephemera/dataSource/positions/ludicGraph/presenceSubGraph.ts) (+ [test](../../../../../lambda/ephemera/dataSource/positions/ludicGraph/presenceSubGraph.test.ts)). `subGraphFromNodes` returns an `EphemeraLudicGraph` (via `fromFieldPayload`, same `hostId`/`rootId`) rather than a bespoke shape --- the induced sub-graph is a cut of the same graph, not a different one, so the shipped class's own accessors serve Slice 2 directly. The returned graph's `ports` are exactly the stub ports minted for this bucket, not `graph`'s own port entries.
