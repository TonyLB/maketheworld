...
- [ ] **Slice 1. Presence sub-graph utilities**, in `positions/ludicGraph/presenceSubGraph.ts`.
  ...
  - [ ] **1b. `subGraphFromNodes(graph, nodes)`.** All the nodes, all the edges **between** them, and edges to ports per LR-1's rule. **Do not silently drop straddlers** --- [C7](AGENT.abstractionLayers.corpus.planning.md#c7-ariadnes-thread) makes drop unavailable before the question is argued.
  - [ ] **1c. Unit tests, including the three that carry information:** a two-bucket hand-authored child; an edge cut at a port; and **PR-C1's intra-graph straddle with no port at the cut**. The first two confirm; the third decides.
...
| --- | --- | --- | --- |
| LR-1 | **What becomes of an edge with one endpoint outside the bucket** --- keep with a dangling terminal, drop, or emit a record of the cut. | Slice 1b | **Open, with a candidate:** *the cut is legal where it lands on a port, and only there.* **Drop is unavailable** (C7). **The undecided case is PR-C1's**: a straddle inside one graph where the cut is not at a port. This row is the substance of PR-8; answering it here is the point of the Prototype. |
