...
**Status:** In progress (domain types merged into [`dataSource/renderCache/baseClasses.ts`](../../../../lambda/ephemera/dataSource/renderCache/baseClasses.ts); next: move `markStateUtils`, merge `AGENT.md`, remove `lambda/ephemera/renderCache/` TS).

This task plan supports **colocating** Ephemera render-cache **domain types** and **mark-state helpers** under [`lambda/ephemera/dataSource/renderCache/`](../../../../lambda/ephemera/dataSource/renderCache/), then **deprecating** the standalone [`lambda/ephemera/renderCache/`](../../../../lambda/ephemera/renderCache/) package as a place for TypeScript modules (`markStateUtils` remains there temporarily; it imports types from `../dataSource/renderCache/baseClasses`). Steady-state documentation should live next to the DataSource after merge; **delete this file** when the initiative is complete (see [`taskPlanning/AGENT.md`](../../../AGENT.md) durability ladder).
...
- **No import ping-pong:** nothing under `dataSource/renderCache/` imports **up** to `lambda/ephemera/renderCache/` for types (done). `markStateUtils` still lives under `renderCache/` and imports types from `dataSource/renderCache/baseClasses` until the helpers move.
...
- **`markStateUtils`** temporarily imports from [`../dataSource/renderCache/baseClasses`](../../../../lambda/ephemera/renderCache/markStateUtils.ts); move helpers into `dataSource/renderCache/` in a follow-up step.
...
- **Grep hygiene (adjust patterns after renames):**
  ...
  - After `markStateUtils` moves: no `renderCache/markStateUtils` imports; tighten greps accordingly.
