...
# compiler (kind: 'narrate' + direction: 'leave' | 'arrive', not a narrate-leave/narrate-arrive
# kind split --- see Phase 2's closing note). Every hit outside the compiler's own directory
# constructing a `{ kind: 'narrate', ... }` or `{ kind: 'capture', ... }` literal is a call site
# hand-rolling its own plan.
grep -rn "kind: 'narrate'\|kind: 'capture'" --include="*.ts" lambda/ephemera | grep -v node_modules | grep -v "\.test\.ts" | grep -v "manipulation/kernel/compile/" | grep -v "manipulation/kernel/kernelStep.ts"
