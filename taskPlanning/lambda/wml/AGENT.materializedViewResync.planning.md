...
| --- | --- | --- | --- |
| **OD-1** | Does `assetId` in the new event need normalization (`AssetKey()`) inside the handler, or is it always published in canonical `ASSET#`-prefixed form by whoever triggers it? | Phase 3 | Open --- re-scoped off Phase 2 during that phase's execution: `template.yaml`'s `CloudWatchEvent` `Pattern` matches only `source`/`detail-type` (envelope metadata), never inspects `Detail.assetId`, so routing is content-agnostic and cannot resolve this. `handleCacheConsistencyFinding` (`lambda/assets/dataSource/index.ts:136-152`) normalizes defensively; recommend the same here for consistency, but confirm `AssetWorkspace.fromUUID` accepts either form before assuming it matters. |
...
| **OD-4** | Does the handler need to guard against re-triggering on an asset whose `.ndjson` is already fresh (i.e. is `loadWML()` + `pushJSON()` safe/idempotent to run redundantly)? | Phase 3 | Open --- likely yes (parsing + rewriting identical content should be a no-op diff downstream in `cacheAsset`), but confirm empirically in Phase 3's own tests rather than assuming. |
...
- [ ] **Phase 3. Implement the handler.** `lambda/wml/dataSource/mtw-wml.ts`: add `processWMLMaterializedViewFinding`, dispatched from the `isDiagnosticsEnvelope` branch alongside `processS3StructureFinding`. Implements the 5-step body from [Design](#design-settled-in-conversation-2026-08-11-not-re-litigated-here) --- `AssetWorkspace.fromUUID` -> `loadWML()` -> `pushJSON()` -> `streamEvent({ header: { type: 'Content Update' } })`. Resolve **OD-4** (idempotency) with an explicit test case. Add/adjust the envelope type guard in `lambda/wml/dataSource/subscribedEvents.ts` if the existing catch-all (`dataSourceKey: 'mtw.diagnostics'`, no `type` narrowing --- `subscribedEvents.ts:32,39-40`) doesn't already cover a new `type` value cleanly.
  - [ ] `cd lambda/wml && npm test`
- [ ] **Phase 4. Verify the downstream chain end to end.** Confirm the published `Content Update` event actually reaches `lambda/assets`' `handleContentUpdate` -> `cacheAsset` in a real (or realistically mocked) environment, not just unit-level assertions that `streamEvent` was called with the right arguments --- this is the whole point of the feature, and the chain crosses a Lambda boundary (`packages/mtw-lambda-patterns/ts/dataSource/index.ts`'s `streamEvent` -> real `PutEventsCommand` -> `lambda/assets/app.ts`'s EventBridge branch -> `handleContentUpdate`). Prefer an integration test if one of comparable shape already exists for `applyEdit`'s own `Content Update` path; otherwise a manual end-to-end run against dev (per the [manual trigger](#design-settled-in-conversation-2026-08-11-not-re-litigated-here) command) is acceptable for this scale of feature.
  - [ ] `cd lambda/assets && npm run test -- --watchAll=false` (confirm no regressions on the consumer side; this phase shouldn't need source changes there, only verification)
