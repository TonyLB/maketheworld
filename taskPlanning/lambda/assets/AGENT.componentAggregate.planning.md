...
**Status:** Phase 1 in progress (aggregate **compute-only** assembly + **`InternalCache`**-slice factory landed under **`mtw-gateways`**; see [`taskPlanning/packages/mtw-gateways/AGENT.internalCacheOpinionated.planning.md`](../../packages/mtw-gateways/AGENT.internalCacheOpinionated.planning.md)). **In scope:** sequence work so **query / assembly logic** and (next) a **`DeferredCache`**-backed **handler factory** from the same **`aggregate/`** tree black-box cache orchestration for lambdas, then add **`mtw.assets.components.aggregate`** as a **non-replayable** derived [`AssetsDataSource`](../../../lambda/assets/dataSource/abstract.ts) under [`lambda/assets/dataSource/components/aggregate/`](../../../lambda/assets/dataSource/components/aggregate/) for **streaming** and **invalidation-oriented** signals. **Next:** **`mtw-gateways`** **handler factory** slice, then **Golden / comparison tests** (Phase 1 [**Recommended order**](#recommended-order)); vertical contract is already shipped---touch base with verticals readers only when the **anchor-only participant closure** or batch shape needs alignment.
...
**Architecture note (InternalCache opinionated migration):** [`taskPlanning/packages/mtw-gateways/AGENT.internalCacheOpinionated.planning.md`](../../packages/mtw-gateways/AGENT.internalCacheOpinionated.planning.md) is canonical for sequencing. **`mtw-gateways`** will own the aggregate **`DeferredCache`** **handler factory**; the assets lambda **constructs** that handler and **registers** it on **`InternalCache`** (plus **`clear`/`flush`**). Any remaining prose in this file that says **`DeferredCache`** or **`InternalCache`** integration **must** live only in lambdas is **historical** unless it explicitly refers to lambda **singleton** types.
...
- **Shared, read-only** helpers and (per [**Recommended order**](#recommended-order)) a **`DeferredCache`** **handler factory** in **`mtw-gateways`** under [`ts/assets/components/aggregate/`](../../../packages/mtw-gateways/ts/assets/components/aggregate/) that compose:
...
| **Gateway package layout** | **Decided (Phase 1):** New aggregate read surface and its **realm of authority** (assembly, graph-derived participation order, merge orchestration, aggregate-specific types and factories, and the planned **`DeferredCache`** **handler factory**) live under [`packages/mtw-gateways/ts/assets/components/aggregate/`](../../../packages/mtw-gateways/ts/assets/components/aggregate/). **`Meta::Import`** reads remain in [`ts/assets/components/verticals`](../../../packages/mtw-gateways/ts/assets/components/verticals); aggregate code **imports** that module (deep import per [`mtw-gateways/AGENT.md`](../../../packages/mtw-gateways/AGENT.md)) and does **not** fork vertical SK/query logic. |
| **Assembly materialization (v1)** | **Decided (Phase 1):** Merged output is derived **on demand** from vertical reads + batched bodies + merge (no aggregate-owned Dynamo blobs). A package **`DeferredCache`** handler may **memoize** repeated merge reads **within** a lambda invocation; that does **not** change authoritative storage. **Working hypothesis:** no merged primary truth in Dynamo under this initiative. **Phase 2** / **consumers** may still introduce their own **caches** or **bus payloads**. |
...
- [X] **Assembly core:** Implement **`assembleMergedComponent`** in the same **`aggregate/`** tree: parallel loads via **`ComponentAggregateInternalCacheSlice`** / **`AggregateGatewayDeps`** (**`ImportVerticalMetaImportProjectionLoader`** / **`Meta::Import`** envelope + **`ImportVerticalAuthoritativeComponentDataLoader`** / authoritative **`byAssets`**), then pure merge along participation order (`assemble.ts`). **`createComponentAggregateGateway`** (primary) and **`createAggregateGateway(deps)`** (analyzer-shaped); no **`assetDB`** factory inside **`mtw-gateways`** for this compute gateway.
- [ ] **`mtw-gateways` aggregate cache handler factory:** Ship **`DeferredCache`**-orchestrated **cache-shaped** handler (or factory returning one) from [`packages/mtw-gateways/ts/assets/components/aggregate/`](../../../packages/mtw-gateways/ts/assets/components/aggregate/) per [`taskPlanning/packages/mtw-gateways/AGENT.internalCacheOpinionated.planning.md`](../../packages/mtw-gateways/AGENT.internalCacheOpinionated.planning.md) **Recommended order**; package tests for keys / batching / delegation to **`gateway.assembleMergedComponent`** (or equivalent).
...
| Aggregate `DeferredCache` handler factory in `mtw-gateways` | 1 | Not started |
