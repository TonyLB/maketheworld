...
**Status:** Phase 1 in progress: aggregate **compute-only** assembly, **`InternalCache`**-slice composition, and the shipped **`DeferredCache`**-backed **handler factory** (**`ComponentAggregateMergedCache`**, **`createComponentAggregateCacheHandler`**, **`aggregatePerspectiveCacheKey`**) live under **`mtw-gateways`** [`ts/assets/components/aggregate/`](../../../packages/mtw-gateways/ts/assets/components/aggregate/); see [`taskPlanning/packages/mtw-gateways/AGENT.internalCacheOpinionated.planning.md`](../../packages/mtw-gateways/AGENT.internalCacheOpinionated.planning.md). **In scope:** **Golden / comparison tests**, assets **`InternalCache`** registration of the package handler, **`fetchImportDefaults`** migration, then **`mtw.assets.components.aggregate`** as a **non-replayable** derived [`AssetsDataSource`](../../../lambda/assets/dataSource/abstract.ts) under [`lambda/assets/dataSource/components/aggregate/`](../../../lambda/assets/dataSource/components/aggregate/) for **streaming** and **invalidation-oriented** signals. **Next:** **Golden / comparison tests** (Phase 1 [**Recommended order**](#recommended-order)), then assets **`InternalCache`** registration; vertical contract is already shipped---touch base with verticals readers only when the **anchor-only participant closure** or batch shape needs alignment.
