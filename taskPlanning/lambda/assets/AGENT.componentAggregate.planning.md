...
**Status:** Phase 1 in progress: aggregate **compute-only** assembly, **`InternalCache`**-slice composition, assets **`InternalCache`** **`ComponentAggregate`** registration ([`lambda/assets/internalCache/index.ts`](../../../lambda/assets/internalCache/index.ts)), and the shipped **`DeferredCache`**-backed **handler factory** (**`ComponentAggregateMergedCache`**, **`createComponentAggregateCacheHandler`**, **`aggregatePerspectiveCacheKey`**) under **`mtw-gateways`** [`ts/assets/components/aggregate/`](../../../packages/mtw-gateways/ts/assets/components/aggregate/). Steady-state gateway + **`InternalCache`** norms: [`packages/mtw-gateways/AGENT.md`](../../../packages/mtw-gateways/AGENT.md). **In scope:** **`fetchImportDefaults`** migration, then **`mtw.assets.components.aggregate`** as a **non-replayable** derived [`AssetsDataSource`](../../../lambda/assets/dataSource/abstract.ts) under `lambda/assets/dataSource/components/aggregate/` for **streaming** and **invalidation-oriented** signals. Golden / comparison tests vs **`merge*AcrossStack`** live in [`lambda/assets/componentAggregate.mergeParity.test.ts`](../../../lambda/assets/componentAggregate.mergeParity.test.ts). **Next:** **`fetchImportDefaults`** migration (Phase 1 [**Recommended order**](#recommended-order)); vertical contract is already shipped---touch base with verticals readers only when the **anchor-only participant closure** or batch shape needs alignment.
...
**Architecture note (InternalCache opinionated model):** Canonical playbook and sequencing live in [`packages/mtw-gateways/AGENT.md`](../../../packages/mtw-gateways/AGENT.md) (**Purpose**, [**Wrapping gateways in InternalCache (playbook)**](../../../packages/mtw-gateways/AGENT.md#wrapping-gateways-in-internalcache-playbook)). **`mtw-gateways`** owns the aggregate **`DeferredCache`** **handler factory** (cache-shaped reads); lambdas **construct + register** only on **`InternalCache`** (plus **`clear`/`flush`**). Any remaining prose in this file that says **`DeferredCache`** or **`InternalCache`** integration **must** live only in lambdas is **historical** unless it explicitly refers to lambda **singleton** types.
...
3. Read **gateway rules** and mirror existing assets gateway **layout** (shared read surfaces---do not invent a parallel package architecture):
   - [`packages/mtw-gateways/AGENT.md`](../../../packages/mtw-gateways/AGENT.md): read at least [**How to add a gateway**](../../../packages/mtw-gateways/AGENT.md#how-to-add-a-gateway), [**Projection-read vs compute-only**](../../../packages/mtw-gateways/AGENT.md#projection-read-vs-compute-only-gateways), and [**Wrapping gateways in InternalCache (playbook)**](../../../packages/mtw-gateways/AGENT.md#wrapping-gateways-in-internalcache-playbook). Opinionated direction: **`mtw-gateways`** owns **`DeferredCache`** orchestration for aggregate **cache-shaped** handlers; lambdas **construct + register** on **`InternalCache`**.
   - Code to mirror: [`packages/mtw-gateways/ts/assets/components/componentData/`](../../../packages/mtw-gateways/ts/assets/components/componentData/) (row normalization; projection-read gateways inject `assetDB` here) and [`packages/mtw-gateways/ts/assets/components/verticals/`](../../../packages/mtw-gateways/ts/assets/components/verticals/) (sibling module; **`Meta::Import`** reads and **`ImportVerticalConsistencyAnalyzer`** loader contracts). **Aggregate** is compute-only for **merge assembly** (no **`assetDB`** in-package); **primary** integration is **`createComponentAggregateCacheHandler`** / **`ComponentAggregateMergedCache`** / **`aggregatePerspectiveCacheKey`** (see [**Aggregate read surfaces (primary vs secondary)**](../../../packages/mtw-gateways/AGENT.md#aggregate-read-surfaces-primary-vs-secondary)); **`createComponentAggregateGateway`** / **`createAggregateGateway`** over **`ComponentAggregateInternalCacheSlice`** are **secondary** uncached surfaces---see [**InternalCache composition (Phase 1)**](#internalcache-composition-phase-1).
   - Aggregate home: [`packages/mtw-gateways/ts/assets/components/aggregate/`](../../../packages/mtw-gateways/ts/assets/components/aggregate/) ([**Gateway package layout**](#phase-1-gateway))
...
5. Read a **derived** assets DataSource that already merges across a stack for product-specific events (pattern reference only; contracts will differ):
   - [`lambda/assets/componentExamples/index.ts`](../../../lambda/assets/componentExamples/index.ts) and `exampleEnrichment` (`getOrderedAssetStack`, `merge*AcrossStack`)
...
7. **Command authority:** For gateway tests, follow [`packages/mtw-gateways/AGENT.md`](../../../packages/mtw-gateways/AGENT.md) (`npm test` from package root). For assets lambda tests, prefer `lambda/assets/AGENT.development.md` if it exists; otherwise root [`AGENT.md`](../../../AGENT.md) and existing lambda test conventions.
...
- **Shared, read-only** helpers **and** the shipped **`DeferredCache`** **cache-shaped handler** (**`ComponentAggregateMergedCache`**, **`createComponentAggregateCacheHandler`**, **`aggregatePerspectiveCacheKey`**) in **`mtw-gateways`** under [`ts/assets/components/aggregate/`](../../../packages/mtw-gateways/ts/assets/components/aggregate/) that compose:
  ...
  - **Authoritative bodies:** `BatchGetItem`-style reads of per-asset component projections already owned elsewhere (same Dynamo rows [`componentData`](../../../packages/mtw-gateways/ts/assets/components/componentData/) and related shapes touch).
...
- Add **`mtw.assets.components.aggregate`**: [`AssetsDataSource`](../../../lambda/assets/dataSource/abstract.ts), **`replayable: false`** lean, living under `lambda/assets/dataSource/components/aggregate/`.
...
**Historical context (opinionated InternalCache migration):** Completed; durable norms are in [`packages/mtw-gateways/AGENT.md`](../../../packages/mtw-gateways/AGENT.md). Older text below the **Rule** heading that forbade **`DeferredCache`** in **`mtw-gateways`** is **obsolete**; the blessed direction is **package-owned** cache orchestration for the aggregate **handler**, with lambdas doing **thin** construction and **`InternalCache`** registration. The first concrete **`mtw-gateways`** delivery in that migration was the aggregate **`DeferredCache`** handler factory.

**Rule:** **`mtw-gateways`** owns **merge assembly** (pure **`assemble.ts`** / **`createComponentAggregateGateway`** **`gateway`** methods) **and** the shipped **`DeferredCache`**-backed handler (**`ComponentAggregateMergedCache`**, **`createComponentAggregateCacheHandler`**, **`aggregatePerspectiveCacheKey`**; see [**Aggregate read surfaces (primary vs secondary)**](../../../packages/mtw-gateways/AGENT.md#aggregate-read-surfaces-primary-vs-secondary)) built from **`ComponentAggregateInternalCacheSlice`**: **`ComponentData`** must satisfy **`ComponentDataParticipationLoader`** (**`getAcrossAssets`** / pair reads via **`authoritativeFromParticipationOrder`**); **`ComponentVerticals`** unchanged. **Lambda registration:** pass **`createComponentDataCacheHandler`** on the aggregate slice (**`createAuthoritativeComponentDataCacheHandler`** is removed). See [**Component data reads: ephemera vs assets**](../../../packages/mtw-gateways/AGENT.md#component-data-reads-ephemera-vs-assets). **`ImportVerticalConsistencyAnalyzer`** remains a **separate** surface with **`exhaustivePartitionLoader`** **`deps`** only (not **`internalCache.ComponentData`**).

**Lambda composition:** Instantiate the gateway **handler** once per invocation (or per the package factory's documented lifecycle), pass a slice backed by **`internalCache.ComponentData`** / **`internalCache.ComponentVerticals`**, assign the handler on the lambda **`InternalCache`** singleton (e.g. **`ComponentAggregate`** on assets), and include it in **`InternalCache.clear()`** / **`flush()`** as for other handlers. **Do not** duplicate aggregate **`DeferredCache`** wiring in the lambda when the package factory supplies it.

**Cache keys and invalidation:** Choose a **cache key** that matches how callers ask for merges (e.g. **`U` + serialized `orderedAssetIds`**); **invalidate** when vertical or component inputs for that merge change (start coarse, e.g. invalidate on **`U`** partition changes, then tighten if needed). Encode defaults in **`mtw-gateways`** tests; document assets-specific choices in [`lambda/assets/internalCache/AGENT.md`](../../../lambda/assets/internalCache/AGENT.md).

**Ephemera** (or other lambdas) repeat the same **pattern** with their own sibling caches satisfying the **same loader contracts**---**do not** import another lambda's **`InternalCache`** singleton into **`mtw-gateways`**.
...
- **Hot keys (design rule):** Repeated merges may repeat vertical **`Query`** + batch reads. **`mtw-gateways`** aggregate **handler** owns **per-invocation** **`DeferredCache`** state for merged results; avoid **cross-invocation** singletons in the package. Per-lambda **`InternalCache`** still owns **registration** and **clear** orchestration (see [`packages/mtw-gateways/AGENT.md`](../../../packages/mtw-gateways/AGENT.md)). Operational tuning if one **U** becomes extremely hot is mostly a **consumer** topic but informs Phase 1 API shape (batch-friendly, no accidental N+1 in helpers).
...
- [X] **Aggregate types:** Sketch **`AggregatePerspective`** (names TBD), **`OrderedAssetStack`**, and **`MergedComponentResult`** under [`packages/mtw-gateways/ts/assets/components/aggregate/`](../../../packages/mtw-gateways/ts/assets/components/aggregate/) (pure types + factories). Follow **Getting Started** step 3: per-gateway **`index.ts`** public surface, **deep imports**; **`InternalCache`** / lambda singleton **classes** stay out of **`mtw-gateways`**, but **`DeferredCache`** from **`mtw-lambda-patterns`** is **allowed** inside the aggregate **handler factory** (see next rows).
- [X] **Assembly core:** Implement **`assembleMergedComponent`** in the same **`aggregate/`** tree: parallel loads via **`ComponentAggregateInternalCacheSlice`** / **`AggregateGatewayDeps`** (**`ImportVerticalMetaImportProjectionLoader`** / **`Meta::Import`** envelope + **`ImportVerticalAuthoritativeComponentDataLoader`** / authoritative **`byAssets`**), then pure merge along participation order (`assemble.ts`). **`createComponentAggregateGateway`** (secondary) and **`createAggregateGateway(deps)`** (analyzer-shaped secondary); no **`assetDB`** factory inside **`mtw-gateways`** for this compute gateway.
- [X] **`mtw-gateways` aggregate cache handler factory:** Shipped **`DeferredCache`**-orchestrated **`ComponentAggregateMergedCache`** / **`createComponentAggregateCacheHandler`** in [`factory.ts`](../../../packages/mtw-gateways/ts/assets/components/aggregate/factory.ts); keys in [`keys.ts`](../../../packages/mtw-gateways/ts/assets/components/aggregate/keys.ts); package tests in [`cacheHandler.test.ts`](../../../packages/mtw-gateways/ts/assets/components/aggregate/cacheHandler.test.ts) (keys, batching, parity with **`gateway.assembleMergedComponent`**); durable docs [**Aggregate read surfaces (primary vs secondary)**](../../../packages/mtw-gateways/AGENT.md#aggregate-read-surfaces-primary-vs-secondary). Aligns with [**Wrapping gateways in InternalCache (playbook)**](../../../packages/mtw-gateways/AGENT.md#wrapping-gateways-in-internalcache-playbook) and related **`mtw-gateways`** norms.
- [X] **Golden / comparison tests:** Fixtures proving parity with legacy merge path for representative stacks; record commands in **Verification**.
- [X] **`InternalCache` registration (assets):** Instantiate the package **handler factory** output once per invocation; add **`ComponentAggregate`** (or chosen name) field on [`lambda/assets/internalCache/index.ts`](../../../lambda/assets/internalCache/index.ts); wire **`clear`/`flush`** with sibling handlers; document in [`lambda/assets/internalCache/AGENT.md`](../../../lambda/assets/internalCache/AGENT.md) per [**InternalCache composition (Phase 1)**](#internalcache-composition-phase-1).
...
- [ ] **Scaffold DataSource:** Add `lambda/assets/dataSource/components/aggregate/` with **`dataSourceKey: 'mtw.assets.components.aggregate'`**, **`replayable: false`**, **`subscribedEvents.ts`** stubs, side-effect import from [`lambda/assets/app.ts`](../../../lambda/assets/app.ts).
...
| Aggregate `DeferredCache` handler factory in `mtw-gateways` | 1 | Done |
| Golden / comparison tests vs legacy merge | 1 | Done ([`lambda/assets/componentAggregate.mergeParity.test.ts`](../../../lambda/assets/componentAggregate.mergeParity.test.ts)) |
| Assets `InternalCache` `ComponentAggregate` registration + docs | 1 | Done |
...
- [X] `lambda/assets` aggregate merge parity (golden / comparison vs **`merge*AcrossStack`**): `cd lambda/assets && npm test -- --testPathPattern=componentAggregate.mergeParity`.
- [X] `lambda/assets` **`InternalCache`** **`ComponentAggregate`** wiring: `cd lambda/assets && npm test -- --testPathPattern=internalCache/componentAggregate`.
