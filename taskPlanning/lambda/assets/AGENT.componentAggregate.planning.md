...
**Architecture note (InternalCache opinionated migration):** [`taskPlanning/packages/mtw-gateways/AGENT.internalCacheOpinionated.planning.md`](../../packages/mtw-gateways/AGENT.internalCacheOpinionated.planning.md) is canonical for sequencing. **`mtw-gateways`** owns the aggregate **`DeferredCache`** **handler factory** (cache-shaped reads); lambdas **construct + register** only on **`InternalCache`** (plus **`clear`/`flush`**). Any remaining prose in this file that says **`DeferredCache`** or **`InternalCache`** integration **must** live only in lambdas is **historical** unless it explicitly refers to lambda **singleton** types.
...
**Supersedes (opinionated migration):** [`taskPlanning/packages/mtw-gateways/AGENT.internalCacheOpinionated.planning.md`](../../packages/mtw-gateways/AGENT.internalCacheOpinionated.planning.md). Older text below the **Rule** heading that forbade **`DeferredCache`** in **`mtw-gateways`** is **obsolete**; the blessed direction is **package-owned** cache orchestration for the aggregate **handler**, with lambdas doing **thin** construction and **`InternalCache`** registration. The first concrete **`mtw-gateways`** delivery in this migration is the aggregate **`DeferredCache`** handler factory.

**Rule:** **`mtw-gateways`** owns **merge assembly** (pure **`assemble.ts`** / **`createComponentAggregateGateway`** **`gateway`** methods) **and** the shipped **`DeferredCache`**-backed handler (**`ComponentAggregateMergedCache`**, **`createComponentAggregateCacheHandler`**, **`aggregatePerspectiveCacheKey`**; see [**Aggregate read surfaces (primary vs secondary)**](../../../packages/mtw-gateways/AGENT.md#aggregate-read-surfaces-primary-vs-secondary)) built from **`ComponentAggregateInternalCacheSlice`** (same **`get`** contracts as **`ImportVerticalAuthoritativeComponentDataLoader`** / **`ImportVerticalMetaImportProjectionLoader`**, named **`ComponentData`** / **`ComponentVerticals`** like assets **`internalCache`**). **`ImportVerticalConsistencyAnalyzer`** remains a **separate** pure analyzer surface with analyzer-shaped **`deps`** field names; aggregate composition prefers the **slice** naming.
...
- [X] **`mtw-gateways` aggregate cache handler factory:** Shipped **`DeferredCache`**-orchestrated **`ComponentAggregateMergedCache`** / **`createComponentAggregateCacheHandler`** in [`factory.ts`](../../../packages/mtw-gateways/ts/assets/components/aggregate/factory.ts); keys in [`keys.ts`](../../../packages/mtw-gateways/ts/assets/components/aggregate/keys.ts); package tests in [`cacheHandler.test.ts`](../../../packages/mtw-gateways/ts/assets/components/aggregate/cacheHandler.test.ts) (keys, batching, parity with **`gateway.assembleMergedComponent`**); durable docs [**Aggregate read surfaces (primary vs secondary)**](../../../packages/mtw-gateways/AGENT.md#aggregate-read-surfaces-primary-vs-secondary). Aligns with [`taskPlanning/packages/mtw-gateways/AGENT.internalCacheOpinionated.planning.md`](../../packages/mtw-gateways/AGENT.internalCacheOpinionated.planning.md) **Recommended order**.
