...
| 11 | Refactor diagnostics to receive problem reports with DataSource pattern | Complete | Producer cutover now emits `Session Disconnect` through DataSource `streamEvent`; diagnostics now receives `Session Disconnect Problem` + `New Player` via subscribed intake and routes direct commands through synthetic `api.diagnostics` envelopes with shared contracts, malformed payload drop handling, per-batch dedupe on `dedupeKey`, and app-boundary `ReturnValue`/`Error` extraction parity with connections |
