...
| **A row that corrects another row records the correction on itself**, and the corrected row goes on being read as written. Deliberately *not* made a discipline: locks are rare and can afford ceremony, row-to-row correction is continuous. **The 2026-08-21 presence sweep is a third data point and it moves the cost side, not the rate side:** PR-C1 recorded its correction on itself and on the one row it was aimed at, and the two sites phrased in the old vocabulary went on being read as written --- one of them for a day, and *it was the half that got transcribed into a durable doc*. **The end-of-day sweep is the cheap fix this row was waiting for**, since it catches the same failure without asking a correcting row to chase its readers. | Abstraction layers --- measured 3 in 27, confirmed in a second tranche; presence plan, 2026-08-21 | Rate confirmed once; **a fix is now identified but is the sweep's, not this row's** |
...
| **Therefore: sweep a batch of same-day decisions at the end of that day**, while the knowledge is present. **First executed 2026-08-21** on the presence plan's four-row batch (PR-1, PR-2, PR-9, PR-10). **It paid, and what it caught was narrower than expected:** not four rows disagreeing, but **one amendment propagating unevenly** --- a correction made that morning reached the row it was written against and the corpus case it came from, and missed a row graduated hours later that reasoned in the superseded vocabulary. **The cheap version of this sweep is therefore not row-against-row but *amendment-against-batch*: list what was corrected that day, then check every row settled after the correction.** **A blind spot, found hours after the first run and worth as much as the rule: the sweep grades *propagation*, never *premises*.** It looks for divergence --- a correction reaching some rows and missing others --- so **a batch that is uniformly wrong produces no signal at all.** On 2026-08-21 four cells written from one misread premise passed pipe, cell-count, anchor and cross-row checks **because they agreed with each other perfectly**, and the two readings even produced the same headline answers, diverging only in their *reasons* --- which is exactly what verdict cells carry and exactly what a consistency sweep does not read. **What caught it was the design's author objecting to a paraphrase**, which is the only cheap oracle for a premise. **Do not let a green sweep be read as a graded batch.** | Derived from the above; executed once, 2026-08-21, presence plan | **Ran once: 1 finding, 1 omission, 4 candidate collisions checked and cleared.** Keep. Needs a second run before the amendment-first refinement is more than one day's evidence |
| **Restate a user-supplied mechanism *mechanically* before building on it** --- what value, at what time, computed by whom --- rather than semantically. **A semantic paraphrase can be internally coherent while describing a mechanism that does not exist**, and once it is compressed into a phrase it stops being re-derived and starts being reused. 2026-08-21: `FALLTHROUGH` was read as *consult composition for the default* rather than as *the else-branch absorbing whatever the other landing instructions did not claim*. The wrong reading produced a verdict, a cleared threat clause on a **second** row, a spun-off new row, and four mutually consistent cells --- none of which could catch it, because none re-derived anything. **The mechanical restatement, once finally written, exposed the gap in a sentence** --- and had already surfaced a genuine finding (a write that changes composition without moving anything) *while still running on the false premise*, which is evidence the practice pays independently of the error it catches. **The discipline is narrow: when a mechanism arrives in rough form, echo it back in mechanism terms the first time, so it can be rejected for the price of one sentence.** **Second instance, 2026-09-11, and it extends the rule from *a mechanism someone hands you* to *a question you pose yourself*.** The abstraction-layers back-sweep asked for a year --- *does locked-frame clause 2 forbid outward boundary crossings as well as inward ones?* --- and the question **had no referent**: it presumed a relation is *traversed* across a boundary, where `buildCrossingLegs` **composes** one out of legs local to each graph, so no reference ever crosses in either direction. **Nothing internal to the plan could catch it**, because a malformed question generates perfectly coherent work: the cluster reached **seven members**, acquired a *not offered as meeting the re-open bar* disclaimer, was cited as sequencing PQ-14 and as sitting under a Prototype with code, and drew two candidate answers --- one of which was proposed and withdrawn the same day. **What caught it was working one concrete case through the constructor** (a bridge on a boat signalling a lighthouse), which is the mechanical restatement applied to a question instead of an answer. **So: before counting how many rows depend on a question, trace one case through the code that would implement it.** | Presence plan, 2026-08-21; abstraction-layers back-sweep, 2026-09-11 | **Two instances in two initiatives** --- the independence bar this row was waiting on is met, and the second extends its scope to self-posed questions. **Counterfactual priced both times:** one sentence on turn one against four cells rewritten; one worked case against a month, seven members and two candidate answers |
| **Verify against the claim, not against the wording you remember writing --- and check the columns you did not edit.** 2026-08-21, within one hour of the above: a grep for the exact phrase from a just-rewritten paragraph returned zero, and was reported as *the framing is gone*. **The discredited phrase was alive in the same row's Status cell** --- the cell a table scan actually reads, and the one the edit never touched. **The failure is that the search pattern was drawn from memory of the text rather than from the idea**, so it could only find what was already fixed. **Cheap form: grep the *concept* with a deliberately loose pattern** (both word orders, a small character window between the terms) **across every file rather than the one edited**, and read a row's short cells last, not first. | Presence plan, 2026-08-21 | One instance; **pairs with the row above --- the same error at two layers**, which is why neither is filed as carelessness |
| **A Prototype needs a booking slot for follow-on implementation, and it is a parallel *channel* in Recommended order --- not a phase, and not the proposal's own graduation checklist.** Phases sequence the *design*; a build elected mid-design accrues steps while the phases are still open, and those steps get added, gated and overturned individually. A graduation checklist is the wrong home for the same reason: **it closes when the proposal earns its tier**, so work booked there either blocks graduation falsely or is orphaned by it. **The construct handles the hard case too** --- a step whose owning producer does not exist yet is booked with a **travel rule** (*this step belongs to whichever step first writes X*) rather than a position. | Abstraction layers, Channels C (2026-08-13), D (2026-08-21) and E (2026-08-30); attention history, Channel B. Instances of the travel rule: CD6 (relocated from a **closed** sibling plan), CE2/CE3 | **Not yet two independent instances, and the reason is precise:** the two plans are siblings that **coordinated** --- Channel B was re-scoped on 2026-08-13 in the same conversation that opened Channel C --- so *converge without coordination* is unmet. **What confirms it is an initiative that reaches for a channel with no contact with these two.** **What deflates it** is a plan that books follow-on in its phase list and does not suffer for it. Recorded now because the absence was **noticed as a question** (2026-08-30, *"I'm not sure we've defined a slot"*) before it stranded anything, and a near-miss is cheaper evidence than the loss |
| **Present implementation state is evidence about the present, never authority over the design --- and the failure recurs in three distinct disguises, all traced to one habit: reading the current implementation as the design.** *(i) A comment's rationale read as a decision:* a gate's guarding comment (*"someone decided against it"*) was granted the corpus's authority with no plan row, contract clause or discussion entry checked first --- a comment is written by the same process that writes the code and inherits none of that authority. *(ii) A missing consumer read as a verdict on consumers:* *"the gap is invisible because presence ports are write-only"* was circular --- ports were write-only substantially because extension was repeatedly declined, each time citing the absence of consumers as the reason; refuse to extend, observe no consumers, cite no-consumers as grounds not to extend. *(iii) A step's commit semantics read as a representation's expressiveness:* one port's replace-all commit behaviour was read as a limit on what N ports could express, when N presence ports are exactly as expressive as an N-element reverse-index set --- the argument assumed away its own subject. **All three were made confidently in one day's conversation, and all three were withdrawn or refuted on inspection**, by the same test applied three times: check the claim against the corpus, the code, or the shipped step, rather than against how confidently it reads. | Presence-refactor plan, 2026-09-04 (RA-2, RA-6, RA-7; `AGENT.presenceRefactor.planning.md`, deleted 2026-09-05 on graduation) | One instance across three disguises within a single initiative; needs a second initiative before promotion |
| **A phase that schedules *grading* work dissolves when its rows turn out to be settled by construction or by scope.** Fiction, cases and corpora grade claims about the world; **they cannot grade a constructor discipline or a scoping decision**, because what would change those is the maturity of other work rather than any case. 2026-08-21: the presence plan's PH0 lost three of four sub-steps this way (two retired as unwritable, one closed on *no falsifier can be written*), and PH1 then lost **all five** of its rows --- every one answered from conversation ahead of the corpus, each ungradeable for its own reason. **PH0's own closure note already states the weaker version** (*a brief should say what it would falsify before it is scheduled*); **the stronger one is that the test belongs at phase-booking time, not case-booking time** --- ask whether a phase's rows are the kind of thing a corpus can grade **at all**, before the phase exists. | Presence plan, 2026-08-21 (PH0, then PH1) | **Two phases in one initiative**, which is a rate rather than an anecdote; needs a second initiative before promotion |
| **A sweep's diagnoses age well and its prescriptions age badly --- so record what is wrong with a row and resist naming the patch.** A consistency sweep is good at *this row is confused*, because that is a reading of text against text. Its proposed repair is a design act performed in passing, under the vocabulary of the moment, and it competes with the real fix that arrives later --- which repeatedly turns out to be **dissolving the question one level up** rather than answering it. **Twice in one sweep:** the `positionCache` objection was answered by P6 changing what a cache *delivers* (a handle, not a subgraph), not by the expansion-stops-at-a-port rule the sweep asked for; and AB-48's containment outcomes were answered by AB-54 putting hosting kinds in the host's own shard, so the classifier **throws** rather than taking either the *unconditional carry* the row pre-filled or the per-kind `dissolve`/`defer` the sweep prescribed. **The finding still earned its keep both times, and that is the useful half:** refusing a wrong answer is what holds the slot open for the right one. **The cost is that a prescription left standing reads as scheduled work** --- a repair to a union that has since lost the arm, or to a traversal that has since been deleted. **A control arrived 2026-09-11, and it is better evidence than the second initiative this row was waiting for.** The [whole/part dissolution sweep](lambda/ephemera/dataSource/positions/AGENT.abstractionLayers.proposals.planning.md#disposition-2026-09-11-this-sweep-is-spent-and-it-is-spent-the-good-way) was written **one day before** the back-sweep, by the same author, into the same file, over the same subject matter --- and **every one of its findings landed**, three of them within a day, one row citing the sweep by name as it retired its own decider. **The variable that differs is the output form:** the dissolution sweep wrote *diagnoses* (*this horn is dead*), which a row can act on with what it already has; the back-sweep wrote *prescriptions* (*the patch is X*), which a row cannot act on until the mechanism exists, by which time the mechanism has chosen differently. **A diagnosis ages into history; a prescription ages into a wrong instruction.** **A second, cheaper signal worth checking rather than believing:** the sweep whose findings landed is cited **outward** (12 links, 11 from other documents); the one whose findings did not is cited **inward only** (44 links, all internal). Direction of causation unestablished --- a sweep may be cited outward *because* it was actionable --- but inbound-link provenance is free to measure at disposition time. | Abstraction layers back-sweep, dispositioned 2026-09-11 (AB-35/P6; Finding 5/AB-54); **negative control**: whole/part dissolution sweep, same file, 2026-08-09 | **Two instances plus a matched control.** Still one initiative, so the independence bar is formally unmet --- but the control tests the mechanism directly, which is what the bar was a proxy for. **Promote on the next sweep in any initiative, or on one outward-citation measurement that confirms** |
| **A findings document needs a disposition pass the way a row needs a back-link, and without one it *generates* false work rather than merely failing to inform.** The [row-corrects-row finding](#candidate-method-findings) above is about a correction failing to travel. **This is the same gap one construct over and it is worse**, because a row at least carries its own Status cell where a reader will meet it: a *finding* has no status, is written in the present tense, and is reached by grep at its own anchor rather than through any perimeter that could mark it. **The measured cost, 2026-09-11:** a back-sweep finding said PQ-7 *"should be answered or retired explicitly"*; PQ-7 had been answered and retired **the same day the finding was written**, and a month later a reader reached PQ-7 through the finding, inherited a closed question as open, and worked on it. **The cheap fix is not a back-link per finding** --- findings are numerous and their subjects move --- **but a dated disposition block at the head of any findings section that outlives its sweep**, saying of each finding whether it still binds a live mechanism. **The trigger to watch for: a sweep whose findings are cited by anchor rather than read in sequence.** Forty-four links pointed into this one and **thirty went straight to an individual finding**, bypassing any header where a disposition could have been read. | Abstraction layers back-sweep, 2026-08-10 / cost measured 2026-09-11 | One measured instance, but the counterfactual is priced --- one turn of work on a question closed a month earlier |
| **A correction that fixes the vocabulary but not the mechanism is worse than no correction, because its dated note reads as a completed fix and suppresses the next look.** Sibling to [the row-corrects-row finding](#candidate-method-findings) above, and sharper: there the correction failed to *reach* other sites, here it reached the right row and stopped half way. **AB-25 is the specimen.** On 2026-08-09 it struck the word ***above*** as "H1-era height language --- a whole hosts its parts **in itself**," and three clauses later in the *same row* went on computing *"the rope spans room and garden so it hosts at the Area."* The vocabulary was H1's; so was the mechanism; only the vocabulary was corrected, and the dated note made the row look swept. **The 2026-09-11 sweep found four escapees of one withdrawn rule** (H1 clauses 4--5, withdrawn 2026-08-07) across two live rows and two corpus cases, **and its replacement had been written twice already** --- PQ-3 on 2026-08-06 (*"a coiled rope has one presence port; spread across three rooms it has three"*) and AB-8's re-framing on 2026-08-09. **Two rows had corrected this very rule before and still carried it.** **The cheap detector, and it is what the sweep actually used:** a withdrawn rule has a *fingerprint* --- the phrases that only make sense if it holds. Here, anything computing a **single host for a multi-located thing**: *hosts at the Area*, *hosting height*, *migrates down*, *needs an LCA of*. **Grep the fingerprint, not the rule's name**, because the rows that cite the name by hand are the ones already handled. | Abstraction layers, 2026-09-11 --- 4 sites, 2 in rows that had already corrected the same rule | One initiative, but the instance count is inside it and the detector is reusable. **Promote on one fingerprint grep run at a withdrawal in any other initiative**, which is also the test: a withdrawal that greps clean is the negative control this needs |
