...
## Note on WML Producer Rework (Deferred)

- The WML lambda remains a legacy producer that predates the DataSource pattern and currently publishes raw EventBridge detail payloads.
- Subscriptions pipeline has been consolidated to use EventBridge external types strictly; consumers are fully typed and validated.
- Reworking the WML lambda to publish via serializer-backed EventBridge contracts is deferred to a separate project and does not block the remaining subscriptions work (e.g., Initialize Subscription routing and handling).
