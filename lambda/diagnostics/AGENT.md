...
**Downstream handling:** Ephemera **`mtw.ephemera.positions`** consumes `mtw.diagnostics` / `Ludic Graph Stale Structure Finding` via [`healLudicGraphStructure`](../ephemera/dataSource/positions/ludicGraph/healLudicGraphStructure.ts) (idempotent repair, scoped to defaulting `rootId`/the root's own node only --- see that file's own doc comment for what is deliberately left unhealed, e.g. `ports`/LD-17). The consumer always commits (`dryRun: false`); dry-run mode exists for manual/operator invocation, not for the finding-triggered path.
