...
const convertAssetQuery = (queryItems) => {
    const Characters = queryItems
        ...
        .map(({ AssetId, Name, scopedId, fileName, fileURL, Pronouns, OneCoolThing }) => ({ CharacterId: splitType(AssetId)[1], Name, scopedId, fileName, fileURL, Pronouns, OneCoolThing }))
...
export const fetchLibrary = async (RequestId: string) => {
    const Items = await assetDB.query({
        ...
        ProjectionFields: ['AssetId', 'DataCategory', 'Connected', 'RoomId', 'Name', 'fileURL', 'Pronouns', 'OneCoolThing']
    })
