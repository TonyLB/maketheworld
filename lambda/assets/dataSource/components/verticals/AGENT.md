...
**Today:** each subscribed **`Component Updated`**, **`Component Republished`**, or **`Component Removed`** event triggers **[`projectImportVerticalHop`](./projectImportVerticalHop.ts)**, which calls **[`syncImportVerticalPartition`](./syncImportVerticalPartition.ts)** for **`component.universalKey`**. **`syncImportVerticalPartition`** builds **`ImportVerticalConsistencyAnalyzer`** deps at the call site (shared partition **`Query`** promise for authoritative rows + **`Meta::Import::...`** slice), runs **`check`**, then applies **`putItem`** / **`deleteItem`** from analyzer findings (**`categoriesToAdd`** / **`metaRowsToDelete`**). Authoritative writes always come from **`assetDB.query`**, not from the event payload alone. Diagnostics **`componentVerticalMisalignmentSweep`** uses the same analyzer with **`assetDB`**-backed loaders for parity.
