...
**Today:** each subscribed **`Component Updated`**, **`Component Republished`**, or **`Component Removed`** event triggers **[`projectImportVerticalHop`](./projectImportVerticalHop.ts)**, which calls **[`syncImportVerticalPartition`](./syncImportVerticalPartition.ts)** for **`component.universalKey`**. **`syncImportVerticalPartition`** **`Query`**s the universal-key partition, derives hops from NDJSON component rows (**`deriveRawImportVerticalHopsFromComponents`**), applies **`salvageImportVerticalHops`** from **`@tonylb/mtw-gateways`** (cycle omission), then deletes **`Meta::Import::...`** rows not in the salvaged set and **`putItem`**s missing rows. Authoritative writes always come from **`assetDB.query`**, not from the event payload alone. The same derive/salvage/diff logic is also packaged as **`ImportVerticalConsistencyAnalyzer`** in **`@tonylb/mtw-gateways`** for injected reads; adopting it inside **`syncImportVerticalPartition`** is a planned follow-up so heal and diagnostics share one orchestration path.
