...
**Today:** each subscribed **`Component Updated`**, **`Component Republished`**, or **`Component Removed`** event triggers **[`projectImportVerticalHop`](./projectImportVerticalHop.ts)**, which calls **[`syncImportVerticalPartition`](./syncImportVerticalPartition.ts)** for **`component.universalKey`**. **`syncImportVerticalPartition`** wires **`ImportVerticalConsistencyAnalyzer`** with **[`exhaustivePartitionLoader`](./exhaustivePartitionLoader.ts)** (`exhaustiveScanCache` subpath) and **`internalCache.ComponentVerticals`** (**`queryImportVerticalMeta`** runs inside **`ComponentVerticals`**), runs **`check`**, then applies **`putItem`** / **`deleteItem`** from analyzer findings (**`categoriesToAdd`** / **`metaRowsToDelete`**). Authoritative partition reads are **not** from the event payload alone. Diagnostics **`componentVerticalMisalignmentSweep`** uses the same analyzer **`deps`** shape with its own **`exhaustivePartitionLoader`** module; it still **`Query`**s **`DataCategoryIndex`** to list universal keys under the asset (not covered by per-component cache APIs). **[`cacheAsset`](../../caching/cacheAsset.ts)** calls **`invalidateExhaustivePartitionCache`** when component rows change.
