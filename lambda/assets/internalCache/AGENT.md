...
| --- | --- | --- |
| **`ComponentData`** | [`index.ts`](./index.ts) (field initializer) | **`createAuthoritativeComponentDataCacheHandler(assetDB)`** from [`authoritativeComponentDataCache.ts`](../../../packages/mtw-gateways/ts/assets/components/assetMeta/authoritativeComponentDataCache.ts) (partition shim until pair-addressed swap). Used by **`componentExamples`** and **`ComponentAggregate`** slice today. **Vertical sync** uses [`exhaustivePartitionLoader`](../dataSource/components/verticals/exhaustivePartitionLoader.ts), not this field. |
