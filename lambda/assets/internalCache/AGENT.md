...
| **`ComponentAggregate`** (planned) | TBD (`componentAggregate.ts` or similar) | Types + **`createAggregateGateway`** live in **`mtw-gateways`** [`ts/assets/components/aggregate`](../../../packages/mtw-gateways/ts/assets/components/aggregate/index.ts). **`DeferredCache`** for merged assembly: **`promiseFactory`** delegates to **`ComponentVerticals`** + **`ComponentData`** (then pure assembly). Injects those siblings as **`deps`** closures into **`createAggregateGateway`**---see [**InternalCache composition (Phase 1)**](../../../taskPlanning/lambda/assets/AGENT.componentAggregate.planning.md#internalcache-composition-phase-1) in aggregate planning and [**Wrapping gateways in InternalCache**](../../../packages/mtw-gateways/AGENT.md#wrapping-gateways-in-internalcache-playbook). |
