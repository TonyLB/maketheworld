...
| --- | --- | --- |
| **`ComponentData`** | [`index.ts`](./index.ts) (field initializer) | **`createComponentDataCacheHandler(assetDB)`** from [`componentData`](../../../packages/mtw-gateways/ts/assets/components/componentData/index.ts) (pair-addressed **`getItems`** / **`getAcrossAssets`**). **`ComponentAggregate`** slice uses participation batch only. **`componentExamples`** mirroring loads via [`loadAuthoritativeForMirroring`](../componentExamples/loadAuthoritativeForMirroring.ts) (pair + derived participation order) --- partition reads on those paths are **retired**. Diagnostics reseed is **retired**; mirror emitter removal is tracked in [`renderCache` on-demand planning](../../../taskPlanning/lambda/ephemera/dataSource/renderCache/AGENT.onDemandAuthoredExamples.planning.md). **Vertical sync** uses [`exhaustivePartitionLoader`](../dataSource/components/verticals/exhaustivePartitionLoader.ts), not this field. |
