...
| --- | --- | --- |
| **`ComponentData`** | [`index.ts`](./index.ts) (field initializer) | **`createComponentDataCacheHandler(assetDB)`** from [`componentData`](../../../packages/mtw-gateways/ts/assets/components/componentData/index.ts) (pair-addressed **`getItems`** / **`getAcrossAssets`**). **`ComponentAggregate`** slice uses participation batch only. **`componentExamples`** mirroring and diagnostics reseed load via [`loadAuthoritativeForMirroring`](../componentExamples/loadAuthoritativeForMirroring.ts) (pair + derived participation order) --- partition reads on those paths are **retired**. Full mirror/reseed **pipeline** removal is tracked in [`renderCache` on-demand planning](../../../taskPlanning/lambda/ephemera/dataSource/renderCache/AGENT.onDemandAuthoredExamples.planning.md). **Vertical sync** uses [`exhaustivePartitionLoader`](../dataSource/components/verticals/exhaustivePartitionLoader.ts), not this field. |
...
| **`ComponentExamples`** (optional) | Not registered on assets today | Ephemera and diagnostics register **`createComponentExamplesCacheHandler({ ComponentAggregate })`** for on-demand authored-example reads. Add here when an assets call site needs **`internalCache.ComponentExamples.get`**. |
...
**Same invocation after writes:** Paths that **`invalidate`** **`ComponentData`** (pair keys: **`invalidate(universalKey, assetId)`**), **`invalidateExhaustivePartitionCache`** (vertical maintenance), or **`ComponentVerticals`** (for example [`cacheAsset`](../dataSource/caching/cacheAsset.ts), [`syncImportVerticalPartition`](../dataSource/components/verticals/syncImportVerticalPartition.ts)) do **not** automatically invalidate **`ComponentAggregate`** merge entries; a cached merged result can stay stale until **`clear`**, **`ComponentAggregate.invalidate`** with **`aggregatePerspectiveCacheKey(...)`**, or a coarse **`ComponentAggregate.clear()`**. Tighten incrementally when call sites need same-invocation parity after sibling mutations.
...
**`ComponentVerticals`** (and any future reader of that partition) would **depend on** that shared fetch and **project** in memory (filter / parse) instead of each owning a separate top-level partition read where overlap exists.
