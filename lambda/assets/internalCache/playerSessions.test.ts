...
describe('CachePlayerSessionsData', () => {
    ...
    it('queries separately for a different player', async () => {
        connectionDBMock.query
            .mockResolvedValueOnce([{ ConnectionId: 'PLAYER#Testy', DataCategory: 'SESSION#123' }] as any)
            .mockResolvedValueOnce([{ ConnectionId: 'PLAYER#Other', DataCategory: 'SESSION#456' }] as any)
        const cache = new CachePlayerSessionsData()
        expect(await cache.get('Testy')).toEqual(['123'])
    ...
    it('re-queries after clear', async () => {
        connectionDBMock.query
            .mockResolvedValueOnce([{ ConnectionId: 'PLAYER#Testy', DataCategory: 'SESSION#123' }] as any)
            .mockResolvedValueOnce([{ ConnectionId: 'PLAYER#Testy', DataCategory: 'SESSION#789' }] as any)
        const cache = new CachePlayerSessionsData()
        expect(await cache.get('Testy')).toEqual(['123'])
        cache.clear()
