...
| **C.5** | **Cache pointer (`currentCacheId`)** - **folded into Phase C, not a separate DataSource.** CP-4 cancelled the planned **`mtw.ephemera.currentCachePointers`** DataSource; pointer maintenance is a field on the `renderCache`-owned catalog row, set at the same `handleRenderOrchestrationInbound` seam that emits **`Render Pertains`**. No **`Generation Deferred`** subscriber is needed --- the pointer self-invalidates on the next read after a state change (CP-2). | Phase C; **`Render Pertains`** payloads **component x perspective** + **`cacheId`** (uncertainty 9 resolved). Ordering concern for current scope is resolved in contract uncertainty 11 via Perception fan-in + message-group choreography. | [`dataSource/renderCache/AGENT.md`](dataSource/renderCache/AGENT.md#perspective-pointers) |
