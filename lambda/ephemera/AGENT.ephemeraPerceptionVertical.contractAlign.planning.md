...
**Contract alignment** = moving from expedient correlation (e.g. conversation `sendMessage` terminals, `RenderReady` materialization) toward **explicit events** and **clear ownership** (`renderOrchestration` policy terminals vs `mtw.ephemera.renderCache` correlated and abstract outbounds vs **perception** fan-in), as sketched in the contract draft. **`renderOrchestration`** is explicitly intent on **removing** dependency on **`conversation.sendMessage`** **as soon as** replacements exist, using the **`mtw.ephemera.renderOrchestration`** **DataSource stream** for the six outbounds (details: [`renderOrchestration/AGENT.passThrough.planning.md`](../../taskPlanning/lambda/ephemera/dataSource/renderOrchestration/AGENT.passThrough.planning.md), contract uncertainty 8).
...
| **renderCache slice** | [`taskPlanning/lambda/ephemera/dataSource/renderCache/AGENT.passThrough.planning.md`](../../taskPlanning/lambda/ephemera/dataSource/renderCache/AGENT.passThrough.planning.md) | DataSource-local tasks |
| **renderOrchestration slice** | [`taskPlanning/lambda/ephemera/dataSource/renderOrchestration/AGENT.passThrough.planning.md`](../../taskPlanning/lambda/ephemera/dataSource/renderOrchestration/AGENT.passThrough.planning.md) | Orchestration-local tasks |
...
**Wave 1:** Implement [`renderOrchestration/AGENT.passThrough.planning.md`](../../taskPlanning/lambda/ephemera/dataSource/renderOrchestration/AGENT.passThrough.planning.md) (orchestration slice; graduated task plan; contract may remain draft in places). **Wave 2:** Implement [`renderCache/AGENT.passThrough.planning.md`](../../taskPlanning/lambda/ephemera/dataSource/renderCache/AGENT.passThrough.planning.md) (renderCache slice; graduated task plan; contract may remain draft in places). **After those:** expect a **design phase** for Perception, **`currentCachePointers`**, **`messageBus`** ordering, and other plans **not** closed by producer + cache. Full prose: [pass-through contract - Intended implementation sequencing](../../taskPlanning/lambda/ephemera/dataSource/AGENT.passThrough.contract.planning.md#intended-implementation-sequencing-exploration). Parent epic: [AGENT.ephemeraPerceptionVertical.planning.md](./AGENT.ephemeraPerceptionVertical.planning.md#intended-sequencing-pass-through-implementation-waves-exploration).
...
| **B** | **renderOrchestration** - emit the **six outbound types** (`Current Cache Valid`, `Exact Match Found`, `Generation Started`, `Render Generated`, `Orchestration Error`, `Generation Deferred` per contract); **remove `conversation.sendMessage`** in favor of **streamed outbounds**; migrate off owning the **final** correlated "ready" story as agreed | Phase A stable enough to implement without weekly renames | [`renderOrchestration/AGENT.passThrough.planning.md`](../../taskPlanning/lambda/ephemera/dataSource/renderOrchestration/AGENT.passThrough.planning.md) |
| **C** | **renderCache** - correlated (`Render Pertains`) and abstract (`Cache Updated`) behavior; reconcile duplicate-`Cache Updated` risk on generate path | Phases A-B; contract uncertainties on write vs notify | [`renderCache/AGENT.passThrough.planning.md`](../../taskPlanning/lambda/ephemera/dataSource/renderCache/AGENT.passThrough.planning.md) |
