...
- **`RoomUpdate`:** **Target end state:** migrate **character roster refresh** from **`displayProtocol: 'RoomUpdate'`** to **`PerceptionMessage`** with **`metaData.roomChannel: 'affordances'`**, then **retire or thin** **`RoomUpdate`** on the server. **Phase B (task plan):** add affordance **`PerceptionMessage`** alongside existing **`RoomUpdate`**; **do not** retire **`RoomUpdate`** until **after Phase C** when the client consumes affordance rows in the sticky header. **Shipped (post Phase C):** the ephemera lambda no longer emits wire **`displayProtocol: 'RoomUpdate'`**; internal bus **`type: 'RoomUpdate'`** enqueues **`Affordances Requested`** via **`mtw.ephemera.affordanceOrchestration`** (reason: **`roster`**). **Interim (M4):** orchestration is log-only; affordance **`PublishMessage`** resumes on **`Affordances Pertain`**. The **`RoomUpdate`** shape remains in **`@tonylb/mtw-interfaces`** for historical message rows.
- **`mtw.ephemera.objects` `Objects Changed`:** **`mtw.ephemera.affordanceOrchestration`** subscribes and fans out to **`orchestrateAffordanceRequest`** (reason: **`objects`**). **Interim:** no affordance **`PublishMessage`** until **`Affordances Pertain`**. **Target terminal shape (when wired):** **`displayProtocol: 'PerceptionMessage'`**, explicit **`metaData.roomChannel: 'affordances'`**, **one `PublishMessage` per character** with **`targets: [characterId]`**, **`wmlContent`** from **`ComponentStackMerge.get(characterId, roomId)`**, and a **new** **`messageId`** per row (**`MESSAGE#${uuid}`**).
- **Unit tests:** Assert orchestration ingress for roster/objects triggers; affordance **`PublishMessage`** shape tests move to **`Affordances Pertain`** slice. See [`affordanceOrchestration/AGENT.md`](affordanceOrchestration/AGENT.md) and [`perception/AGENT.md`](perception/AGENT.md).
