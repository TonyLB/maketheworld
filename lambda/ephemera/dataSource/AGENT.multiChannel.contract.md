...
- **`RoomUpdate`:** **Target end state:** migrate **character roster refresh** from **`displayProtocol: 'RoomUpdate'`** to **`PerceptionMessage`** with **`metaData.roomChannel: 'affordances'`**, then **retire or thin** **`RoomUpdate`** on the server. **Phase B (task plan):** add affordance **`PerceptionMessage`** alongside existing **`RoomUpdate`**; **do not** retire **`RoomUpdate`** until **after Phase C** when the client consumes affordance rows in the sticky header. **Shipped (post Phase C):** the ephemera lambda no longer emits wire **`displayProtocol: 'RoomUpdate'`**; internal bus **`type: 'RoomUpdate'`** still triggers **`publishRoomAffordancePerceptionMessages`** only. The **`RoomUpdate`** shape remains in **`@tonylb/mtw-interfaces`** for historical message rows.
- **`mtw.ephemera.objects` `Objects Changed`:** **`mtw.ephemera.perception`** subscribes (extend **`subscribedEvents`** / **`receiveEvents`**) and emits affordance **`PublishMessage`** rows (**`displayProtocol: 'PerceptionMessage'`**, explicit **`metaData.roomChannel: 'affordances'`**). **Every character in the target room** must receive the update using **one `PublishMessage` per character**: **`targets: [characterId]`** (length one), **`wmlContent`** from **`ComponentStackMerge.get(characterId, roomId)`** (Option B), and a **new** **`messageId`** per row (**`MESSAGE#${uuid}`**, same as [`publishMessage`](../publishMessage/index.ts)). Do **not** send one shared row for the whole room when bodies differ by viewer. (The **`publishMessage`** handler still resolves targets to connections the same way; the bus should carry **separate** **`PublishMessage`** events per occupant.)
- **Unit tests:** Assert **`roomChannel`** discrimination, isolated **`messageId`** spaces, and **`Objects Changed`** → affordance **`PublishMessage`** shape; see [`perception/AGENT.md`](perception/AGENT.md) and ephemera **`dataSource/perception/`** tests.
