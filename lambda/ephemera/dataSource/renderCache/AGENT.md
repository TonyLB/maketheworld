...
**This file is the canonical home** for both DataSource behavior and **render-cache domain** documentation: Dynamo schema, `internalCache.RenderCache`, persistence primitives ([`putCacheRecord.ts`](putCacheRecord.ts), [`deleteCacheRecord.ts`](deleteCacheRecord.ts), [`queryCacheRecordsForComponent.ts`](queryCacheRecordsForComponent.ts), [`queryAllRenderCacheDataCategoriesForComponent.ts`](queryAllRenderCacheDataCategoriesForComponent.ts)), exact-match lookup, and orchestration interaction. **Domain** cache record types and outbound payload guards live in [`baseClasses.ts`](baseClasses.ts). **Mark-state** normalization and equality (`normalizeMarkState`, `markStatesEqual`) live in [`utils/markState.ts`](utils/markState.ts).
...
- **Commands:** Validate `api.ephemera` envelopes, persist cache rows or delete them, update **`internalCache.RenderCache`**, publish **`Cache Updated`**, **`Cache Deleted`**, or **`Cache Error`** as appropriate (`index.ts`, `putCacheRecord.ts`, `deleteCacheRecord.ts`). Outbounds use **`outboundBusDelivery: 'publish'`** on the DataSource; boundary **`flushAndSettle`** at lambda exit quiesces concurrent subscribers (no producer-side scoped flush). **`Delete Cache Records`** also cleans up Situation adjacency: before deleting the named rows, [`cleanupSituationAdjacencyForDeletedRecords.ts`](cleanupSituationAdjacencyForDeletedRecords.ts) re-queries the component's `CACHE#`/`Cache::` rows, restricts to the `dataCategories` actually being deleted, and for every `(situationId, perspectiveKey)` pair found among them, deletes the corresponding **`Link::`** adjacency row (generalizing the per-row cleanup [`hydrateAuthoredCatalogDiff.ts`](hydrateAuthoredCatalogDiff.ts) already does for its own single-perspective case). First production caller: [`../objects/persistImprovisationObject.ts`](../objects/persistImprovisationObject.ts)'s object-delete render-cache teardown.
- **Pass-through:** On orchestration stream events, apply contract rules in [`handleRenderOrchestrationInbound.ts`](handleRenderOrchestrationInbound.ts):
  - **`Current Cache Valid`** / **`Exact Match Found`**: orchestration sends **IDs only** + routing; this DataSource **refetches** via **`internalCache.RenderCache.get`**, sets the perspective's **`currentCacheId`** pointer (best-effort; see **Perspective pointers** below), then emits **`Render Pertains`** (no `CACHE#` row write).
  - **`Render Generated`**: orchestration signals generation-complete with **full** content and **no** durability promise; this DataSource performs the **single** `putCacheRecord`, sets the **`currentCacheId`** pointer for the new row (best-effort), then emits **`Render Pertains`** then **`Cache Updated`** (same pairing as the direct **`Put Cache Record`** command path).
...
**Improvisation merge participation (`OBJECT#` / `CHARACTER#` hosts):** the `mergeParticipationOrder` handed to `ComponentExamples.get` is **not** the raw `perspective.assetStack` for Objects or Characters --- `ensureAuthoredCatalog` runs it through **`appendImprovisationToPerspective`** first, so `ASSET#IMPROVISATION` is the last participation layer. An Object's or Character's authored `SITUATION#DEFAULT` facet lives on its `(OBJECT#|CHARACTER#, ASSET#IMPROVISATION)` pair row, so without that layer the merge cannot see the facet, the desired set comes back empty, and **no `CACHE#` row is ever written** --- the catalog row still marks itself hydrated, so the failure is silent and looks like "hydrate ran, found nothing." Every other Object-merging call site (`objects/objectShortName.ts`, `actions/heldInventoryCatalogForCharacter.ts`, `internalCache/affordanceRoomDeliverable.ts`, the two perception presentation-label resolvers) performs the same append; Character's guest-facet write (see [`AGENT.guestCharacterDescription.planning.md`](../../../../taskPlanning/lambda/ephemera/dataSource/actions/AGENT.guestCharacterDescription.planning.md)) relies on it too. **This is merge participation only:** `perspectiveKey` and the catalog row's stored `assetStack` stay the raw perspective, because the improvisation layer is appended per-merge rather than being part of the perspective's identity --- which is why a correctly-hydrated Object/Character catalog row shows an `assetStack` *without* `ASSET#IMPROVISATION` in it. Other hosts pass an empty scope and are unaffected.
...
**Perspective pointers:** [`perspectivePointer.ts`](perspectivePointer.ts) reads (`resolvePerspectivePointer`), writes (`setPerspectivePointer`), and clears (`clearPerspectivePointer`) `currentCacheId` on catalog rows only --- the legacy `Meta::Room.currentCacheByPerspective` fallback was removed once the M2 migration finished (no dual-read window, no data ever lived there). **Write call site:** [`handleRenderOrchestrationInbound.ts`](handleRenderOrchestrationInbound.ts), both the hit path (`Current Cache Valid` / `Exact Match Found`) and the generate path (`Render Generated`) --- best-effort (logged, non-blocking) since the pointer is a fast-path optimization, not the source of truth. **Read call site:** [`requestIntake.ts`](../renderOrchestration/requestIntake.ts) populates `pointerHint`; **clear call site:** [`findRender.ts`](../renderOrchestration/findRender.ts) on pointer-validation failure (state changed since the pointer was written). Pointer entries also feed the **P** (pointer-only) set in [`fanOutStateChangedToPassiveRenders.ts`](../renderOrchestration/fanOutStateChangedToPassiveRenders.ts) via `collectPerspectivePointerEntries`.
...
- Package: [`index.test.ts`](index.test.ts), [`putCacheRecord.test.ts`](putCacheRecord.test.ts), [`deleteCacheRecord.test.ts`](deleteCacheRecord.test.ts), [`queryCacheRecordsForComponent.test.ts`](queryCacheRecordsForComponent.test.ts), [`queryAllRenderCacheDataCategoriesForComponent.test.ts`](queryAllRenderCacheDataCategoriesForComponent.test.ts), [`cleanupSituationAdjacencyForDeletedRecords.test.ts`](cleanupSituationAdjacencyForDeletedRecords.test.ts), [`catalogRow.test.ts`](catalogRow.test.ts), [`catalogGuards.test.ts`](catalogGuards.test.ts), [`situationAdjacency.test.ts`](situationAdjacency.test.ts), [`handleExampleInvalidated.test.ts`](handleExampleInvalidated.test.ts), [`handleRenderCacheFinding.test.ts`](handleRenderCacheFinding.test.ts), [`ensureAuthoredCatalog.test.ts`](ensureAuthoredCatalog.test.ts), [`hydrateAuthoredCatalogDiff.test.ts`](hydrateAuthoredCatalogDiff.test.ts), [`authoredCatalogHydrateExactMatch.test.ts`](authoredCatalogHydrateExactMatch.test.ts), [`perspectivePointer.test.ts`](perspectivePointer.test.ts).
