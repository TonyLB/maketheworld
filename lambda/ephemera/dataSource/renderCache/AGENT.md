...
**Improvisation merge participation (`OBJECT#` hosts):** the `mergeParticipationOrder` handed to `ComponentExamples.get` is **not** the raw `perspective.assetStack` for Objects --- `ensureAuthoredCatalog` runs it through **`appendImprovisationToPerspective`** first, so `ASSET#IMPROVISATION` is the last participation layer. An Object's authored `SITUATION#DEFAULT` facet lives on its `(OBJECT#, ASSET#IMPROVISATION)` pair row, so without that layer the merge cannot see the facet, the desired set comes back empty, and **no `CACHE#` row is ever written** --- the catalog row still marks itself hydrated, so the failure is silent and looks like "hydrate ran, found nothing." Every other Object-merging call site (`objects/objectShortName.ts`, `actions/heldInventoryCatalogForCharacter.ts`, `internalCache/affordanceRoomDeliverable.ts`, the two perception presentation-label resolvers) performs the same append. **This is merge participation only:** `perspectiveKey` and the catalog row's stored `assetStack` stay the raw perspective, because the improvisation layer is appended per-merge rather than being part of the perspective's identity --- which is why a correctly-hydrated Object catalog row shows an `assetStack` *without* `ASSET#IMPROVISATION` in it. Non-Object hosts pass an empty object scope and are unaffected.
