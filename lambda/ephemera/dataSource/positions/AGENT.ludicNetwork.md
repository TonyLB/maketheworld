...
## 6. Lifting a shape into WML

Everything above is proven **inside ephemera** first: a shape gets prototyped, exercised, and settled here before anything asks whether the authoring/wire layer should carry it too. `ludicGraph` is the running example of that progression --- ephemera's `_nodes`/`_playOnlyEdges`/`ports` model stabilized, and WML's authoring shape was then brought to the same type contract, field for field. There will be others: presence is shipped as structure here and not yet lifted: WML has no surface tag that authors a presence node, so nothing outside ephemera can mint one. That is a **wire-lift**, not a design gap --- the shape exists and is provably correct; only the crossing is missing.

**Readiness is judged per layer, not per initiative.** A layer is liftable once it is **shipped as structure**, whether or not every behavior it enables is built yet. `ludicGraph`'s node/edge/port shape has not moved since ports became structured values, which is what made it liftable regardless of how much of presence's *behavior* (enumerated covers, descent) was still unbuilt at the time. A layer that is still being argued --- open design rows, a structure the corpus itself calls **Prototype** --- is not liftable, no matter how badly a consumer wants it; lifting an unsettled shape just moves the argument into a second layer instead of resolving it. Judge each layer against this test on its own terms, not against whether some parent initiative has "finished."

**Wire-lift and author-lift are two different costs, and they can move independently.** WML is authoring format and wire format at once. Carrying a shape across the wire is cheap: components are mode-blind (they parse the tags they understand regardless of mode), and the server-side projection does the real work. Making that same shape *authorable* is expensive: it drags in asset-stack merge, identity across cache runs, retraction --- the concerns that make presence's `PRESENCE#<uuid>` unsafe to mint at merge time (a stable identity derived from what is being bound, never a minted uuid, is the shape any future authoring surface for it will need; a merge that mints twice yields two bindings where the correct answer is one). [`assetWirePolicy.ts`](../../../../packages/mtw-wml/ts/standardize/assetWirePolicy.ts) is the seam that makes this a per-structure decision rather than one gate for the whole layer: a field can cross the wire and still be lint-blocked from authoring, structure by structure, so the expensive half of a lift can be deferred without blocking the cheap half.

**The lift rule: take the type contract, defer the behavior.** A lift takes the proven layer's type contract in full, immediately; it takes the behavior that contract enables only on demand. An arm of the type with no writer yet is still typed, guarded by its typeguard, and exercised by a round-trip unit test in isolation --- it is never omitted from the shape to save near-term work. The test for whether something may be deferred is not "does anything need it yet," it is **does deferring it write an assumption into a signature or a stored shape**. Skipping an unreachable arm costs nothing later; encoding "there is only ever one of these" into a call boundary because nothing needs more than one yet is the mistake this rule exists to prevent --- unwinding an assumption baked into a signature is materially more expensive than filling in a typed, untested arm once a real caller shows up.

This codebase's worked precedent for the rule is presence itself, proven here before anything lifted it: `fullCoverage` ([`presenceSubGraph.ts`](ludicGraph/presenceSubGraph.ts)) is written at every arity, not derived from arity, even though every binding minted today is arity 1 --- the `presencePortCount <= 1` short-circuit that would have matched current reality was deliberately not taken. `nodesFromPresenceBinding` takes a binding parameter explicitly, with `nodesFromPresenceBindings` unioning over a set, rather than assuming "the" binding. [`collapseCrossingPorts`](ludicCache/mergeReducer.ts) takes `presenceUuid` as an explicit parameter --- the caller states which of a graph's own bindings it is folding, rather than the function guessing there is only one. [`EphemeraLudicCacheSupportHop.presenceBucketIds`](ludicCache/types.ts) is a list, justified as binding-grained rather than host-grained. And the `Enumerated` cover arm is typed and unit-exercised even though no writer computes one yet; only `Full` is minted. None of that shape would need to change if a second binding arrived tomorrow --- which is the whole point of taking the contract before the behavior forces the question.
