...
| Name | Layer | Role |
| --- | --- | --- |
| **`EphemeraLudicRelationalEdgeData`** | `mtw-interfaces` JSON | Stored/wire envelope (`tag: 'Relational'`, ...) on `ludicGraph.edges`. `from`/`to` are `EphemeraLudicTerminalPrimitive` (any legal host kind, not only Object) |
| **`HostRelationalEdge`** | `baseClasses.ts` | Parsed in-memory view (`from`, `to`, `kind`, optional `relationLabel`) --- class API, legality, kernel simulation. **Duplicated** (not consolidated) in `manipulation/types.ts`, widened in lockstep --- a pre-existing split, not new to the terminal widening |
...
| File | Role |
| --- | --- |
| `index.ts` | **`EphemeraLudicGraph` class** (immutable instance methods) + module-level factories (`fromRoomMeta`, `fromCharacterMeta`, `seedFromActiveCharacters`) + node builders, including `nodeFromId` (dispatches an arbitrary `EphemeraLudicTerminalPrimitive` to its correctly-tagged node) |
| `baseClasses.ts` | **`HostRelationalEdge`** parsed in-memory view; relational parse/match/serialize helpers |
| `healLudicGraphStructure.ts` | Self-heal for concepts clause 3's root-in-nodes requirement: idempotent, dry-run-capable repair for `ludicGraph` structural staleness, scoped to `rootId`/root-node and `ports` (defaults a missing/malformed `ports` to `[]` --- the interim posture, short of the reverse index that would answer who *should* refer here) defaulting only. Never called from a read boundary --- only from the `Ludic Graph Stale Structure Finding` consumer (`positions/index.ts`) or an explicit manual invocation |
| `healLudicGraphPortMismatch.ts` | Self-heal for a port's exterior denormalization going stale: idempotent, dry-run-capable repair for a port whose denormalized exterior values (`kind`, `exteriorRelationLabel`) disagree with the edge held by the host the port **names**. **The division of labour with `healLudicGraphStructure.ts` is the record count, not the field list:** shape staleness is judged from one row, a mismatch needs two, so this one reads the referrer's row as well and that one never may. Repairs a disagreement with a named referrer only --- an absent referrer edge is reverse-index work --- who *should* refer here. Never called from a read boundary; only from the `Ludic Graph Port Mismatch Finding` consumer or an explicit manual invocation |
| `wireProjection.ts` | The stored-to-wire projection (`toWireLudicGraph`, `toWireLudicGraphFull`, `fromWireLudicGraph`). See [Wire projection](#wire-projection) below |
...
class EphemeraLudicGraph {
  ...
  readonly rootId: EphemeraLudicTerminalId // designated root node; recorded, never derived
  ...
  get ports(): EphemeraLudicGraphPort[] // egress list; required, possibly empty. Produced by the crossing-port producer (crossing
                                        // establish mints one per crossing); NOT inert --- chain discovery reads it
                                        // entries carry `kind` (PR-11) --- a presence binding iff 'Present', so the
...
- Getters: `characterIds`, `objectIds`, `nodeIds` (`Set`, kind-indifferent)
...
**Feature (and Area) landed as node tags, type-level only and deliberately inert --- no factory or production path constructs one yet, matching the terminal-union widenings' own "nothing downstream wired up" precedent.** `EphemeraLudicGraphNode`'s `tag` union is now the full terminal-kind set (`Character | Object | Room | Feature | Area`), closing a referential-integrity gap: terminal kinds (`EphemeraLudicTerminalPrimitive`) already admitted Feature/Area, so an edge or a future port `owner` could already name one with no node in `nodes` to back it.

**Feature remains deliberately more static than Object, and this is a participation-subset limitation, not a not-a-node-at-all one (confirmed through conversation 2026-07-24, re-scoped 2026-08-19 for containment):** a Feature node is never **membership-moved**, and is never the **subject** of the four *non-containment* relation kinds (`On`/`Under`/`Against`/`Custom`) --- deliberately static, more like the *walls* of a room than its contents (look-at-able, and possibly a relation *target*, but not `transferMembership`-able). **That "never a subject" reading does not extend to containment.** `FEATURE#Niche -PartOf-> FEATURE#Wall` (decided 2026-08-15) puts a Feature in the `from`/subject position legitimately --- **direction corrected 2026-08-20: containment runs member -> root**, so the *niche* is the subject asserting it is part of the wall, not the wall asserting it is part of the niche. The case and the exception it forces are unchanged; only which Feature sits at `from` moved --- the original rule was argued in 2026-07-24's vocabulary, before containment kinds existed (`In`/`PartOf` decided 2026-08-09), and reads as a contradiction only if inherited unchanged into the later one. So: no non-containment subject, no membership move, but containment subject is fine. Do not assume a Feature node otherwise inherits Object-node capabilities; it is a narrower participant by design. (This is also why `EphemeraThingId`'s inclusion of `Feature` at the catalog/Identify layer does **not** imply a graph-level `thingIds` --- the two universes differ regardless, see the Node model paragraph above.)

## Wire projection

**`wireProjection.ts`** is the stored-to-wire half of the WML `ludicGraph` alignment (`StandardLudicGraphData` in `packages/mtw-wml` is the same type contract as `EphemeraLudicGraphFieldPayload`; see [`AGENT.ludicNetwork.md`](../AGENT.ludicNetwork.md#6-lifting-a-shape-into-wml) for the lift this follows). Three functions:

- **`toWireLudicGraph(graph, presenceUuid)`** --- the binding-cut projection. Cuts `graph` to one presence binding's bucket via the real arity-general primitives (`subGraphFromNodes`, `nodesFromPresenceBinding` --- never a hand-rolled node walk), then reshapes the cut's stored fields into the wire type. **Binding-grained even though every real binding is `Full` today** --- the lift rule's central case: nothing about the shape or signature would need to change if a second binding arrived. No production caller yet; a future capability projecting "what's nested inside this Object/Feature/Character" from its `ludicGraph` is the intended consumer (see [Forwarded findings](../../affordanceOrchestration/AGENT.md) and [`perception/AGENT.md`](../../perception/AGENT.md)'s Object section for what that would extend, and its known invalidation gap). **`Full` is legal on this projection even though `ludicCache` forbids a folded presence node from carrying it** (section 5's "every node of the host" has no referent in a merge) --- that rule is scoped to the cache's own fold and must not be imported into this projection or its validators; a wire message carries exactly one binding's cover, never a fold of several.
- **`toWireLudicGraphFull(graph)`** --- the un-cut projection, for a host that is never itself a hosted thing with presence multiplicity (a Room: rooms are never members of another play graph, section 1). Ships the host's whole stored graph as-is. Production consumer: [`affordanceRoomDeliverable.ts`](../../../internalCache/affordanceRoomDeliverable.ts) (Room's wire `ludicGraph`).
- **`fromWireLudicGraph(hostId, data)`** --- the inverse, primarily for the totality round-trip test (`wireProjection.test.ts`): wire shape back to a stored field payload. Throws on a `Navigation`-kind or Replace/Remove-wrapped edge entry --- a stored payload never carries either (`Exit` lives only in Area's own authored `edges`), so a caller feeding one through here has fed the wrong data in, not data this function is entitled to discard.

**LG-11, decided while building this file:** stored relational edges can land on a port-qualified terminal (a crossing port, or a presence binding's exterior address) or directly on a bare presence-node id (PR-15/PN-6 clause (c)). Both were unrepresentable in WML's `StandardLudicRelationalEdgeData.from`/`to` before this --- widened to a three-arm union (`StandardLudicTerminalData` in `packages/mtw-wml/ts/standardize/keys/edges/dataTypes/ludicEdge.ts`, mirroring `EphemeraLudicTerminalId`) so the projection has something to project onto.

## Verification hazard: guards whose contract is to return `false`

**One verification lesson from the work that built ports into `ludicGraph` belongs with this code.** Two of its hazards failed by **discarding valid data and returning `false`** rather than by throwing or failing to compile --- a stale `HOST_RELATIONAL_EDGE_KINDS` set (a `Set` literal has no exhaustiveness requirement) and `typeof === 'string'` terminal pre-checks, both inside `isEphemeraLudicRelationalEdgeData`, **whose whole contract is to return `false`.** A passing suite is not evidence against this class. When widening a kind union or a terminal shape, add the case that *should* be accepted and watch it fail first.
