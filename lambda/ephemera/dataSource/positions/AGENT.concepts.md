...
| **Presence port** | A port that carries presence, as opposed to a purely relational crossing (a cord threading into a flashlight locates nothing) --- see **Crossing port**, [above](#wholes-parts-and-ports). **The discriminator exists in code** --- `EphemeraLudicGraphPort.kind: HostRelationalEdgeKind` (shipped in `e6bef5096`) --- so *count the presence ports* is a field read (`kind === 'Present'`). **What is not yet settled is what follows from that read** --- terminal-only, kind-agreement as a category error, and related consequences are [PR-15](../../../../taskPlanning/lambda/ephemera/dataSource/positions/AGENT.presence.planning.md#open-decisions-design--plan-only)'s, not this row's |
