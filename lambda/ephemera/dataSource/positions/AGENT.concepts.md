...
- **Cross-host apply:** [`manipulation/membership/executeMembershipTransfer.ts`](manipulation/membership/executeMembershipTransfer.ts) (`honorDefer: true` mode) --- one atomic remove-from-host + add-to-host for **either** direction, taking a **host pair** rather than a verb or an acting character. Since MS-8 (2026-09-07) it classifies the boundary directly (`boundaryEdgeOutcomes`/`classifyInteractionUnderTransfer`) and commits through the kernel, with no Synthesize-executor detour --- **no** new `update*LudicGraphs` fork. `takeHold` is `(ROOM# -> CHARACTER#)`, `drop` is the reverse, and `give` would be `(CHARACTER# -> CHARACTER#)` with no new machinery. See [Intent vs. world-effect](#intent-vs-world-effect).
