...
**Container scale (D16 shipped v1; Object storage MK2):** **`Meta::Character.ludicGraph`** hosts held **`OBJECT#`** inventory nodes; reverse via **`POSITION#CHARACTER#...`** adjacency. **`Meta::Object.ludicGraph`** storage ships as of MK2 (see [Object-hosted graph](#object-hosted-graph-mk2-storage-only) above) --- **an Object-hosted transfer has a live route since PV1-2/CD2h (corrected 2026-09-03; this line previously said storage only, which predates PV1-2), for `On`**; `In`/`PartOf` remain unwired. Area hosts remain deferred until needed.
