 ...
 *
 * The leave/arrive slot ids live in `manipulation/kernel/compile/moveBundleSlotIds.ts`, host-typed
 * (object take/drop puts a character on one side of the move). They belong to the compiler: it is
 * the only thing that mints them, and both the declarer (`plan.slots`) and the reporter (each
 * narrate step's `slotId`) read them off the same compiled plan.
 */
