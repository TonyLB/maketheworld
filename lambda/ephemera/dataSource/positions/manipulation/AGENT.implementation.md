...
[`dryRunStepSequence`](kernel/dryRunStepSequence.ts) (Phase 3c) exposes `applyStepSequenceCore`'s legality check as a standalone `plan*`-tier stage: `computeStepSequenceFootprint` the input steps, fetch each footprint host's graph (default `internalCache.Positions.getLudicGraph`, injectable), run `applyStepSequenceCore`, return its `MutationKernelApplyOutcome` unchanged --- no new logic, just a seam onto the two functions above that a caller can reach outside the locked transaction. **No live caller yet**: `executeMembershipTransfer`'s `honorDefer` block and the enrich tier's `sandboxMembershipDryRun` both still hand-roll their own pre-commit check; wiring this stage in to replace `honorDefer`'s is Phase 3d.
...
Carry-closure-transfer routes (object take-hold / drop)
  Ingress args (orchestrateObjectMove / executeMembershipTransfer, honorDefer: true)
    -> single-hop boundaryEdgeOutcomes classify against the departure host's own graph, fresh at
       execute time (second, later snapshot than Plan-stage) --- no Synthesize executor (MS-8, 2026-09-07)
    -> classified dissolves -> PositionKernelMoveOp -> compilePositionKernelOp
...
| Transfer planning lives in `executeMembershipTransfer` (inline diff, or a single-hop boundary classify in `honorDefer` mode), never in the kernel | [`membership/executeMembershipTransfer.ts`](membership/executeMembershipTransfer.ts) |
...
| Path | Role |
| --- | --- |
| [`membership/orchestrateObjectMove.ts`](membership/orchestrateObjectMove.ts) | Narration owner for both object-move directions: derives actor + room from the host pair, resolves labels, calls `executeMembershipTransfer` with `honorDefer: true`, declares the bundle and presents on `ok: true` |
| [`membership/executeMembershipTransfer.ts`](membership/executeMembershipTransfer.ts) | One function, one entry point, two internal dissolve mechanisms selected by `honorDefer` (unified from the former `executeObjectMove`/`executeMembershipTransfer` split, MS-8, 2026-09-07). `honorDefer: true` (take/drop/give only): a single from-host resolved via a caller-supplied `getMembershipContainers` override (no priorContainers diff), single-hop `boundaryEdgeOutcomes` classify against only that host's own graph, refusing the whole move on a `'defer'` outcome --- no Synthesize executor, no `GroundingContext`, no referent round trip; compiles once via `buildObjectMoveOp`/`compilePositionKernelOp`, commits the plan's mutation steps. Default (every other caller): single entity (object or character), diffed against its own fetched `priorContainers`, no defer concept --- object entities get an explicit chain-aware relational sweep ([`findRelationalChainsForRemoval.ts`](relational/findRelationalChainsForRemoval.ts), following crossing ports across hosts, unconditionally dissolving everything it finds), character entities never do |
