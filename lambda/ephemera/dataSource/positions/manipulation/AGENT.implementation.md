...
Carry-closure-transfer routes (object take-hold / drop)
  Ingress args (orchestrateObjectMove / planObjectMoveTransfer, 3d 2026-09-08)
    ...
    -> dryRunStepSequence (plan*-tier, fresh snapshot, second/later than Plan-stage) --- no Synthesize
       executor (MS-8, 2026-09-07); repairable/mechanical -> repairMechanicalDissolve -> rebuild+recompile;
...
| [`membership/types.ts`](membership/types.ts) | Bare, host-general `MembershipDiff` (shared by `buildObjectMovedFact` and `buildCharacterMovedFact`; the character route's own `MembershipDiff<EphemeraRoomId>` instantiation is documented in `../AGENT.implementation.md`'s `membership/` code map) --- 3h/3h-ii, MS-5 |
