...
import { applyTransferSetAsserted } from './applyTransferSetAsserted'
...
describe('applyTransferSetAsserted', () => {
    it('legal: moves a complete set with no boundary edges and mutates both graphs', () => {
        ...
        const outcome = applyTransferSetAsserted(sourceGraph, destGraph, new Set([trayId]))
    ...
    it("legal: BD-13's worked example, with the tray-table dissolve edge already explicitly removed", () => {
        ...
        // loop --- the real precondition this function assumes, unlike applyTransferSet.
        ...
        const outcome = applyTransferSetAsserted(sourceGraph, destGraph, new Set([trayId, glassId]))
    ...
    it('reorder regression: an internal edge between two transferred objects does not spuriously throw', () => {
        ...
        // stripping internal edges before the per-object removeObjectAsserted loop, removing tray
        // first would throw on its still-live edge to glass (not yet removed).
        const sourceGraph = testPositionGraph(roomId, {
            nodes: [
                { tag: 'Object', universalKey: trayId },
                { tag: 'Object', universalKey: glassId },
        ...
        expect(() => applyTransferSetAsserted(sourceGraph, destGraph, new Set([trayId, glassId]))).not.toThrow()
    ...
    it('never leaks a raw RelationalEdgeStillReferencedError past an unresolved boundary dissolve edge', () => {
        ...
        expect(() => applyTransferSetAsserted(sourceGraph, destGraph, new Set([trayId]))).not.toThrow(
