...
Actions **must** publish **`Object Establish Relation`** and **`Object Dissolve Relation`** on grounded **`EstablishRelation`** parse from **`Parse Requested`** ([`../actions/index.ts`](../actions/index.ts); payload + guards in [`../actions/publishedEvents.ts`](../actions/publishedEvents.ts)). Positions **must** subscribe in [`subscribedEvents.ts`](subscribedEvents.ts) and delegate to coordinators under [`manipulation/relational/`](manipulation/relational/). Normative handler rules: [Ingress --- `Object Establish Relation`](#object-establish-relation-positions-owned), [Ingress --- `Object Dissolve Relation`](#object-dissolve-relation-positions-owned). Coordinators **must** trust actions-resolved **`subjectId`**, **`targetId`**, **`relationKind`**, optional **`relationLabel`**, and **`hostId`** (Room or Character) --- no catalog re-resolve in positions v1. **`Object Establish Relation`** (PV1-3b-2) additionally carries **`steps: MutationKernelStep[]`**, the Expansion-derived, already-ordered mutation-kernel chain (port steps before the legs that reference them); this is what the coordinator actually commits --- **`hostId`** on that payload is narration/perception use only (a genuine crossing has no single host). A genuine crossing (peer relation across a shard boundary, **C2**) now reaches ingress and commits in one `commitStepSequence` transact via [`executeEstablishEdgeChain`](manipulation/relational/executeObjectEstablishRelation.ts); **`Object Dissolve Relation`** has not been widened the same way yet (PV1-3b-15) and still declines a crossing at the compiler.
...
- **Ingress:** typed dissolve via actions **`Parse Requested`** only (no **`Action Assessed`** branch in v1). Stream contract: **`Object Dissolve Relation`**, same payload shape establish had before PV1-3b-2 --- **not yet widened with `steps`** (PV1-3b-15). Actions **`Parse Requested`** publishes when enrich yields grounded **`EstablishRelation`** with **`operationKind: dissolveRelation`**.
