...
**`takeHold`** and **`drop`** reach the same `executeMembershipTransfer`, through its own `honorDefer: true` mode (unified from the retired `executeObjectMove`, MS-8, 2026-09-07) --- a single-hop, defer-aware boundary check against only the departure host's own graph, refusing the whole move on a `'defer'` outcome, in place of the default mode's chain-aware unconditional sweep. Neither mode runs the Synthesize executor any more: both call `boundaryEdgeOutcomes`/`classifyInteractionUnderTransfer` or `findRelationalChainsTouching` directly --- see [`Object Take Hold`](#object-take-hold-positions-owned) and [`Object Drop`](#object-drop-positions-owned). **`honorDefer` is a temporary flag, not a permanent two-tier design** --- it exists only because administrative repositions have no legality question to ask yet, not because the two mechanisms are conceptually separate; if that ever changes (every route needs to honor defer, or this stays the only one that ever will), the flag should be removed rather than renamed again.

Module paths: [`manipulation/membership/executeMembershipTransfer.ts`](manipulation/membership/executeMembershipTransfer.ts) (transfer pipeline, both modes), [`manipulation/kernel/`](manipulation/kernel/) (kernel --- `commitStepSequence.ts`, `applyStepSequenceCore.ts`, `kernelStep.ts`). Kernel invariants: [`manipulation/AGENT.implementation.md` --- Kernel invariants](manipulation/AGENT.implementation.md#kernel-invariants).
...
**One execution path covers both directions.** [`executeMembershipTransfer`](manipulation/membership/executeMembershipTransfer.ts)'s `honorDefer: true` mode takes a concrete **host pair** (a single from-host, resolved via a caller-supplied `getMembershipContainers` override, and a concrete `target`) --- and takes **no acting character and no verb**. By execute time both hosts are already concrete, so there is nothing left for Grounding to resolve: it calls `boundaryEdgeOutcomes`/`classifyInteractionUnderTransfer` directly against the departure host's own graph (refusing the whole move, `ok: false`, on a `'defer'` outcome), rather than re-running the general Synthesize executor (MS-8, 2026-09-07 --- the executor detour this mode used to run, solely to re-derive a one-object carry closure, is retired). It compiles the resulting move into a plan and commits the plan's mutation steps via [`commitStepSequence`](manipulation/kernel/commitStepSequence.ts) --- one atomic `MultiKeyUpdate` over the departure and arrival hosts.
