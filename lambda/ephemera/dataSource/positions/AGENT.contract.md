...
- **One present-tier function, gated on `to`:** navigate/home/connect/disconnect/ghost-purge all present through [`presentCharacterMove`](navigate/presentCharacterMove.ts) (3f, MS-6, 2026-09-08 --- merged from the former, guard-clause-split `orchestrateCharacterNavigate`/`orchestrateCharacterDisconnect`). It declares the bundle and presents leave/arrive narration for every caller; only when `to !== null` does it also resolve the arrival header slot (`registerIngressSlot`) or fall back to an imperative `Perception` publish. Disconnect and ghost-purge pass `to: null`, which is `planCharacterMoveTransfer`'s own guarantee that no header slot was ever compiled for them --- so that branch is simply never reached, not separately guarded against. Named `present*`, not `orchestrate*`, under the Phase 3 tier rule ([`AGENT.mutationStack.planning.md`](../../../../taskPlanning/lambda/ephemera/AGENT.mutationStack.planning.md)): it calls no other tier, so it does the core work of exactly one.
