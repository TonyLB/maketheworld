...
- **Must not** handle removal-on-deauthoring: a child that later drops out of `ludicGraph.nodes` keeps its node membership, port, and edge. Provenance and removal are out of scope for the CoyoteGame prototype, deferred deliberately rather than overlooked.
