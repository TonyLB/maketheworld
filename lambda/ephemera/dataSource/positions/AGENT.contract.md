...
**--- amendment, 2026-09-16, per [`AGENT.presenceNodes.planning.md`](../../../../taskPlanning/lambda/ephemera/dataSource/positions/AGENT.presenceNodes.planning.md#recommended-order)'s Slice 0 ([PN-9](../../../../taskPlanning/lambda/ephemera/dataSource/positions/AGENT.presenceNodes.planning.md#open-decisions-implementation--plan-only)(b)): the "where an exterior reference exists it governs; where none exists the stored value stands" conditional above can no longer be false, once that migration lands.** A presence binding is leaving the port population entirely (`EphemeraPresencePort` retires; see that plan's Slice 3), so every port this rule is asked about becomes a **crossing** port, and a crossing port already has exactly one interior edge and one exterior referrer --- the "none exists" branch has no remaining case to cover. **`exteriorRelationLabel` STAYS OPTIONAL regardless** --- its optionality is `kind === 'Custom'` carrying a label versus other kinds carrying none, a different axis from whether a port *has* an exterior endpoint, and this amendment does not touch it. This note describes the state after that plan's Slice 3 lands; the conditional as stated above is still correct of the code as shipped today.
