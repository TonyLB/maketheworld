...
- **Open question, narrowed: a `describe` step cannot request a *header* slot.** Slots express format; producers do not. `ExecutorDescribeStep` has no format field and every kernel-produced describe is unconditionally full, so there is no way for the presentation kernel to ask for a header.

  **The adjacent half is now answered and needs no design:** a move's header slot is **caller-supplied** on the op (`PositionKernelMoveOp.headerSlot`), not derived by `compilePositionKernelOp`. Navigate resolves it from an async perspective-key lookup and passes it; object routes pass `null`. So "suppress the header when an endpoint is a character host" is true by construction, and the compiler deliberately has **no** host-kind branch for it --- adding one would put the decision in the one place that cannot know whether a header applies. Recorded at the compiler's own doc comment as well.

  What remains open is only the *describe* case: a kernel-produced description that wants header format rather than full. No production route needs it yet.
...
| [`positions/AGENT.contract.md` --- Narration and presentation](../positions/AGENT.contract.md#narration-and-presentation) | The successor initiative shipped: narration is compiled into positionally-bound kernel steps and declares its own bundle slots |
| [`taskPlanning/lambda/ephemera/AGENT.relationalNarration.planning.md`](../../../../taskPlanning/lambda/ephemera/AGENT.relationalNarration.planning.md) | Follow-on: the same treatment for reposition-within-a-host, incl. an open slot-ordering obligation |
