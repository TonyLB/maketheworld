...
const PHASE_PLAN_HOP_INTRO = [
    ...
    'The **chosen plan summary** and **rubric issues** below were produced by an earlier selection step. Treat them as the committed maneuver --- do not substitute a different plan or revert to comparing alternatives.',
    '',
    '## Output order (strict)',
    '1. **First**, output **one** Markdown **` ```json ` ** fenced block whose JSON has **exactly** a top-level **`phases`** key (non-empty array). Each phase object includes **`stableKeysUsed`**, **`virtualEntities`** (each with **`label`**, **`derivedFrom`** string array, **`phaseKind`** gathered | synthesized | deployed), and **`achievement`**. Optional **`prepVsBeat`**: prep | creation.',
    '   - Reference staged objects only by **`stableKey`** from the snapshot / combined clustering. For virtual entities, **`derivedFrom`** may cite snapshot keys, seam room labels / topology tokens consistent with prompts, or the reserved grounding token **`setting`** (cartoon stock affordances) where no staged row applies --- see interfaces package validators.',
    '2. **Second**, write **`## Scene analysis`** (Markdown): chain-of-reasoning and spatial analysis for the player for **this** plan only.',
    '3. **Third**, after "## Scene analysis", output a **final** fenced block with language **`text`** whose **only** content is exactly one plain-text line beginning with "Hypothesis:".',
    '4. Do not put any other text after the closing **` ```text ` ** fence.',
    '',
    '## Scene analysis and Hypothesis output',
    '- Your "## Scene analysis" section should commit to the single reading above and build spatial and causal logic. Do not survey multiple plans.',
    '- Ground "## Scene analysis" on **combined clustering**, **## Outliers**, topology, and the plan-selection grounding block.',
    '- Open **` ```text ` ** only after "## Scene analysis". The fenced interior must contain **only** the Hypothesis line.',
    '- No extra commentary outside the leading **` ```json ` ** phase plan, "## Scene analysis", and the fenced Hypothesis line.',
] as const
