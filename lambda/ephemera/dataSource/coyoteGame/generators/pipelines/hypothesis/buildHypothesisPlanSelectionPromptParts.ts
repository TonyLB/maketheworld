...
const PLAN_SELECTION_READING_RULES = [
    ...
    '- Ground evidence in **stableKey**, cluster membership, and **## Outliers** --- outliers are eligible props; do not fold them into named clusters they were not assigned to.',
] as const
...
const PLAN_SELECTION_INTRO = [
    ...
    '- Build a **criterion-first rubric matrix**: **one row per plan sketch**, **one column per dimension**: **coverage**, **completeness**, **coherence**.',
    '- Each matrix cell: short, evidence-grounded prose referencing **stableKey** and cluster membership where relevant --- comparison, not abstract letter grades.',
    '- Treat **coverage**, **completeness**, and **coherence** as **equally important** when judging rows. Do not emphasize one dimension over another in prose or tie-break language.',
    '- Do **not** grade or bias sketches on Road Runner safety, villain effectiveness, or outcome comedy --- those belong to later execution prompts, not this rubric.',
    '- After the matrix: **selection second** --- pick exactly **one** winning sketch with no ties unless you apply an explicit tie-break stated in one line (prefer avoiding ties). You may rank ordinally (1 = best) or name the winner matching the sketch labels.',
    '- End your reply with **only** a Markdown **` ```json ` ** fenced block (language tag **json**) containing **exactly** these keys (no extras): **`',
    ...
    '`** (array of strings: concrete issues / gaps from the rubric for that chosen plan --- staged keys still vague, synthesis needs, etc.).',
