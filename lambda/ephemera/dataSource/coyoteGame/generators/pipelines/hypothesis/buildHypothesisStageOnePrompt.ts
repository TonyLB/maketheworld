...
const STAGE_ONE_JSON_CONTRACT_LINES = [
    ...
    '      - **`intendedRole`** (object): when that staged row lists **`affinities`**, echo exactly one persisted role by value as `{ "role": "<stored-role>" }` (no aptness, no decomposition).',
    ...
    '- **`intendedRole.role`** must match one stored affinity role for that exact staged object: flat modification tags, **`prep`** / **`creation`**, or structural roles. Do not include legacy tuple keys such as **`target`** or **`mode`**.',
] as const
