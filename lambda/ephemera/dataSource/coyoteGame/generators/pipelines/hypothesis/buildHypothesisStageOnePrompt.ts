...
/** Few-shot: **intendedRole** echoes one stored flat/structural/generative **role** only (no tuple fields, no aptness). */
  ...
  "clusters": [
    {
      ...
      "members": [
        ...
        { "stableKey": "rocket-skates", "intendedRole": { "role": "delivery" } }
      ]
    }
  ],
  "outliers": [
    { "stableKey": "glue", "intendedRole": { "role": "connect-props" } }
  ],
...
const STAGE_ONE_JSON_CONTRACT_LINES = [
    ...
    '- Root object keys (**emit in this order — `clusters`, then optional `outliers`,',
    '  then optional `notes` last**):',
    '  - **`clusters`** (required non-empty array): upper bound **one cluster per',
    '    staged object**. Each element has:',
    '    - **`clusterName`** (string): short human-readable label for a',
    '      **functional/thematic** group.',
    '    - **`members`** (required non-empty array): objects that belong together',
    '      in that cluster. Each member object has:',
    ...
    '    appear in **`clusters`** only. When present, list every staged object that',
    '    does not belong to a cluster — each entry has **`stableKey`** (string) and',
    '    optional **`intendedRole`** (same echo rules as cluster members; omit',
    '    **`intendedRole`** when affinities are missing or failed). **`clusters`** and',
    '    **`outliers`** must partition staged **`stableKey`**s (no overlap, no omissions).',
    '  - **`notes`** (optional string, **last property in the root object**): at most',
    '    one short paragraph for spatial / cross-room context only',
    '    (no forward-looking plan narrative). Emit **`notes` after `clusters`**',
    '    (and after **`outliers`** when used) so cross-room framing reflects the',
    '    clustering you already committed to.',
    '- **Functional/thematic clustering only.** Group props that work together toward',
    '  one maneuver. Do **not** encode temporal ordering or beat sequencing here.',
    '- **Omit** **`intendedRole`** when `affinities` are missing or marked failed for',
    '  that row (do not invent roles).',
    '- **Coverage:** Each staged **`stableKey`** appears **exactly once** across',
    '  **`clusters`** ∪ **`outliers`** (when **`outliers`** is omitted, all keys',
    '  appear only in **`clusters`**).',
    '- **Strict keys:** root object may contain only **`clusters`**, optional',
    '  **`outliers`**, and optional **`notes`**. Cluster objects may contain only',
    '  **`clusterName`** and **`members`**. Member objects may contain only',
    '  **`stableKey`** and optional **`intendedRole`**.',
