...
const PLAN_SELECTION_INTERNAL_PHASES_SINGLE_CANDIDATE = [
    ...
    '',
    '### Phase 1 - cleanup: materialized candidate set (internal mini-schema)',
    '- **Primary output:** `materializedCandidates` --- an array with **exactly one** fully materialized candidate object for the **sole** input `candidates[]` row, with the **same top-level shape** as that row: `candidateId`, `executionSummary`, `tropeAssignments`, `outliers`.',
    `- **Goal:** For that option, **close resolvable gaps** so later scoring judges the plan at its **best achievable** form: promote affordance-backed staged props into the correct **trope** \`members\` rows (and add synthetic \`members\` whose \`stableKey\` uses the materialized prefix \`${MATERIALIZED_AFFORDANCE_STABLE_KEY_PREFIX}\` when rules require), per the trope candidates JSON section.`,
    '- **Soft verbosity caps.** Keep the materialized candidate **concise**: at most **~12** non-empty bullet or short line entries total across that candidate\'s internal representation (sum of trope rows, member lines, outlier notes you choose to expand); at most **one sentence** per `tropeAssignments[*].executionDetail` rewrite unless the trope candidates JSON already implies longer beat text; avoid repeating the full input JSON verbatim.',
    '',
    '- **Sub-step A - Finishing Move guarantee (required).** Before Phase 2, the materialized candidate must have an **adequately filled Finishing Move** trope slot (or defensible equivalent via materialization). This is **spot and address**, not a parallel `planIssues` taxonomy.',
    '  - Internal control-flow only (never wire, never handoff): you may note `MISSING_FINISHING_MOVE` **while** cleanup still owes a finishing mechanism; **always resolve** it before Phase 2 so it **never** appears in the trailing fence.',
    '  - **Resolution priority** (first match wins; stop when Finishing Move is adequate): (1) A staged prop already fills Finishing Move --- done. (2) **`affordancesProvided`** on any staged **member** or **outlier** clearly indicates a finishing mechanism --- materialize from that evidence (strongest: player-ordered props). (3) Candidate context implies **Coyote-as-payload** --- materialize `affordance:coyote` (or suffixed variant). (4) A **Contraption** member\'s **`environmentAffordances`** lists **Finishing Move** in **roles** --- materialize from those env refs. (5) **Fallback:** `affordance:coyote`.',
    '  - **Disambiguation (2-4):** Read player intent from the **whole** staged prop set plus candidate text. **`affordancesProvided` on staged props outranks** passive **`environmentAffordances`** on the contraption.',
    '',
...
const PLAN_SELECTION_INTRO_SINGLE_CANDIDATE = [
    ...
    '  Optional materialized finishing rows in the handoff still must trace to those same input fields.',
