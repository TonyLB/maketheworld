...
Three orchestration steps complete a thinking segment and call **`emitThinkingResultForSegmentIfActive`** in [`coyoteHypothesisPipeline.ts`](coyoteHypothesisPipeline.ts) (which delegates to **`emitHypothesisThinkingResult`**): **`seamCombineRender`** (`candidates`), **`parsePlanSelectionHandoff`** (`planSelect`), **`parseNarrativeBeatRecord`** (`narrativeBeats`). Each step assembles plain **`nextState`**, emits using verbose built from **`nextState`** fields only, then **`return { state: nextState }`**. No success emit on **`{ state, abort: true }`** paths.

**Writes on segment success** (same module): **`emitHypothesisThinkingResult`** **`publish`es** **`Thinking Result`** on the internal bus (`header.dataSourceKey` **`mtw.ephemera.coyoteGame`**; **`mtw.ephemera.thinking.results`** persists **`Meta::Result`**) then **`Put Thinking Schedule`** with **`scheduleStatus: 'completed'`**. Verbose payloads align with harness inject shapes (`roomObjectsByRoom`, `combined`, `planSelectOutput`, Bedrock invoke summaries) plus per-hop **`bedrockPrompt`** (`invariantPrefix`, `dynamicSuffix`, `fullText`). Async bus drain can **`marshall`** safely because emits never reference Immer drafts. **Boundary drain** (`flushAndSettle` at lambda exit) quiesces persistence; no producer-side **`settle()`** in the pipeline.

**Writes on run failure** (centralized): [`runCoyoteHypothesisPipeline`](coyoteHypothesisPipeline.ts) calls **`finalizeHypothesisThinkingOnRunFailure`** when **`runPipeline`** fails and **`state.thinking`** is set. Product abort uses the step's **`{ state, abort: true }`** return; unexpected failures use **`throw`**. Finalize **`publish`es** **`ok: false`** **`Thinking Result`** for the failing hop's segment (when mapped and harness-active) and **`Put Thinking Job Error`**. **`loadRoomObjects`** and other unmapped steps mark the job failed without a step result row.
...
**Scene Dressing:** full rubric in [`../../../AGENT.tropes.md`](../../../AGENT.tropes.md) (Scene Dressing section). Stage-one **`decisionFocus`:** [`candidates/serializeStagedObjectsForCandidatePrompt.ts`](candidates/serializeStagedObjectsForCandidatePrompt.ts). Candidate clustering guidance: [`candidates/buildCandidatePrompt.ts`](candidates/buildCandidatePrompt.ts).
