...
**Writes on segment success** (same module): **`emitHypothesisThinkingResult`** **`publish`es** **`Thinking Result`** on the internal bus (`header.dataSourceKey` **`mtw.ephemera.coyoteGame`**; **`mtw.ephemera.thinking.results`** persists **`Meta::Result`**) then **`Put Thinking Schedule`** with **`scheduleStatus: 'completed'`**. Emit after **`seamCombineRender`** (`candidates`), **`parsePlanSelectionHandoff`** (`planSelect`), and **`parseNarrativeBeatRecord`** (`narrativeBeats`). Each emit runs only after the step assembles plain committed **`nextState`**; verbose builders read fields from **`nextState`** (not Immer drafts), so async bus drain can **`marshall`** safely. Verbose payloads align with harness inject shapes (`roomObjectsByRoom`, `combined`, `planSelectOutput`, Bedrock invoke summaries) plus per-hop **`bedrockPrompt`** (`invariantPrefix`, `dynamicSuffix`, `fullText`). **Boundary drain** (`flushAndSettle` at lambda exit) quiesces persistence; no producer-side **`settle()`** in the pipeline.
