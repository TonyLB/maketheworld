...
**Writes on segment success** (same module): **`emitHypothesisThinkingResult`** posts **`Thinking Result`** on the internal bus (`header.dataSourceKey` **`mtw.ephemera.coyoteGame`**; **`mtw.ephemera.thinking.results`** persists **`Meta::Result`**) then **`Put Thinking Schedule`** with **`scheduleStatus: 'completed'`** on lane **`thinkingResults:${generationId}`**, then **`flush`**. Emit after **`seamCombineRender`** (`candidates`), **`parsePlanSelectionHandoff`** (`planSelect`), and **`parseNarrativeBeatRecord`** (`narrativeBeats`). Verbose payloads align with harness inject shapes (`roomObjectsByRoom`, `combined`, `planSelectOutput`, Bedrock invoke summaries) plus per-hop **`bedrockPrompt`** (`invariantPrefix`, `dynamicSuffix`, `fullText`).

**Writes on run failure** (centralized, not in per-step **`abort()`**): [`runCoyoteHypothesisPipeline`](coyoteHypothesisPipeline.ts) calls **`finalizeHypothesisThinkingOnRunFailure`** when **`runPipeline`** fails and **`state.thinking`** is set. That posts **`ok: false`** **`Thinking Result`** for the failing hop's segment (when mapped and harness-active) and **`Put Thinking Job Error`** on the same **`thinkingResults:${generationId}`** lane, then **`flush`**. **`loadRoomObjects`** and other unmapped steps mark the job failed without a step result row.
