...
**Phase B (server, shipped):** Correlated render **`PublishMessage`** rows set **`metaData.roomChannel: 'render'`**. **Affordance ingress (M4):** internal bus **`type: 'RoomUpdate'`** and **`mtw.ephemera.objects` `Objects Changed`** enqueue **`mtw.ephemera.affordanceOrchestration`** (no direct **`publishRoomAffordancePerceptionMessages`**). **Interim:** orchestration is log-only; affordance **`PublishMessage`** resumes on **`Affordances Pertain`** when cache wiring lands. Wire **`displayProtocol: 'RoomUpdate'`** is **retired**. Render vs affordance WML **de-dupe** on the server is **deferred** per contract. See [`AGENT.multiChannel.contract.md`](../AGENT.multiChannel.contract.md) **Phase B server migration (agreed)** and **De-duplication (norm vs Phase B)**.
...
| [`index.ts`](index.ts) **`receiveEvents`** | | **`Character Perception Requested`** → [`characterPerception.ts`](characterPerception.ts). **`Perception Thread Registered`** → **`PerceptionThreads.register`**. Render-cache / orchestration payloads → [`orchestrateRoomDescriptionStreams`](orchestrate.ts). |
...
| [`publishRoomAffordancePerceptionMessages.ts`](publishRoomAffordancePerceptionMessages.ts) | **`'affordances'`** | **Future terminal path** on **`Affordances Pertain`**: **`ComponentStackMerge.get(characterId, roomId)`** once per occupant; **`targets: [characterId]`**; new **`MESSAGE#${uuid}`** per row. **No production callers** after ingress migration. |
