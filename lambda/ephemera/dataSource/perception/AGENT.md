...
| **Move, arrival-room header** | [`moveCharacter`](../../moveCharacter/index.ts) when the arrival room has a **non-empty** **`perspectiveKey`** | **`threadKind`:** **`characterMove`** (targeting-only). Generating/terminal header **`PerceptionMessage`** to mover via [`orchestrate.ts`](orchestrate.ts). |
