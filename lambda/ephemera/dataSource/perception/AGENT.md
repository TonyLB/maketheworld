...
| **Feature / Knowledge look (UI + link)** | [`executeAction`](../../parse/executeAction.ts) or [`app.ts`](../../app.ts) -> **`sendActionAssessed`** **`LookComponent`** -> **`Look Command Requested`** -> [`handleLookCommandRequestedForRenderOrchestration.ts`](../renderOrchestration/handleLookCommandRequestedForRenderOrchestration.ts) | **`threadKind`:** **`featureDescription`** / **`knowledgeDescription`**. Knowledge **`directResponse`** targets session on terminal delivery. |
