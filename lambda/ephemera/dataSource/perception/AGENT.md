...
**Phase B (server, shipped):** Correlated render **`PublishMessage`** rows set **`metaData.roomChannel: 'render'`**. **Affordance ingress (M4, shipped):** internal bus **`type: 'RoomUpdate'`** and **`mtw.ephemera.objects` `Objects Changed`** enqueue **`mtw.ephemera.affordanceOrchestration`** -> **`affordanceCache`** -> **`Affordances Pertain`** -> [`handleAffordancesPertain`](handleAffordancesPertain.ts) (no direct **`publishRoomAffordancePerceptionMessages`** on ingress). Wire **`displayProtocol: 'RoomUpdate'`** is **retired**. Render vs affordance WML **de-dupe** on the server is **deferred** per contract. See [`AGENT.multiChannel.contract.md`](../AGENT.multiChannel.contract.md) **Phase B server migration (agreed)** and **De-duplication (norm vs Phase B)**.
...
| [`orchestrate.ts`](orchestrate.ts) | **`'render'`** | **`handleRenderPertains`**: terminal **`wmlContent`** from [`roomRenderWmlFromCacheRecord`](roomRenderWmlFromCacheRecord.ts) --- one shared string per event for **`roomDescription`**, **`roomHeaderBroadcast`**, and **`characterMove`** headers (cache-backed prose; no **`ComponentRender`** on this terminal path). **`handleGenerationStarted`** / **`handleOrchestrationErrorOrDeferred`**: header placeholders via **`roomHeaderGeneratingPlaceholderWml`** / **`roomHeaderErrorPlaceholderWml`**; full-room placeholders via **`placeholderRoomFullWml`** (Room **`render`** payload aligned with cache-row mapping, not **`StandardRoom.examples`** / synthetic **`Example`**), not **`ComponentRender`**. |
| [`handleAffordancesPertain.ts`](handleAffordancesPertain.ts) | **`'affordances'`** | Terminal on **`Affordances Pertain`**: perspective-filtered occupants via **`getCharacterRoomPerspectiveKey`**; **`ComponentStackMerge.get(characterId, roomId)`** per target; **`targets: [characterId]`**; new **`MESSAGE#${uuid}`** per row. Uncoupled from render threads (**D38**). |
...
**Uncoupled signals:** **`mtw.ephemera.state` `State Changed`** drives **render** / passive-render fan-out only --- it does **not** emit affordance-channel **`PerceptionMessage`** rows. **`RoomUpdate`** and **`mtw.ephemera.objects` `Objects Changed`** enqueue **`mtw.ephemera.affordanceOrchestration`**; terminal affordance **`PublishMessage`** follows **`Affordances Pertain`** ([`handleAffordancesPertain.ts`](handleAffordancesPertain.ts)).
