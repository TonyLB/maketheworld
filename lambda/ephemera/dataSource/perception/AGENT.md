...
**Bus delivery:** All perception **`PublishMessage`** producers use **`messageBus.publish`**; lambda boundary **`flushAndSettle`** quiesces the invocation. **Delivery is unconditionally immediate wire** --- `publishMessage`'s coalescer and the `deliveryMode` field it read (`'immediate' | 'deferred'`) were removed in their entirety (Phase 5 of [`AGENT.messageOrchestrationConsolidation.planning.md`](../../../../taskPlanning/lambda/ephemera/AGENT.messageOrchestrationConsolidation.planning.md), MO-8): every remaining `'deferred'` producer had no genuine reason to hold delivery, so there is no longer a delivery-timing choice for a producer to make. **`characterMove` bundle fill rule:** either the placeholder (**`Generation Started`**) or the terminal (**`Render Pertains`** / error / deferred) can complete a bundled slot (when `mtw.ephemera.messageOrchestration`'s slot-match index returns a declared slot matching `(componentId, perspectiveKey, 'characterMove')` --- MO-9, see [`dataSource/messageOrchestration/AGENT.md`](../messageOrchestration/AGENT.md)'s "Slot match index" section; `PerceptionThreads` no longer carries any `bundleId`/`slotId` correlation fields itself), whichever is processed first for the thread (**`thread.status === 'Initial'`**). If the terminal arrives second, it delivers **standalone** (a direct publish, never re-entering the bundle) reusing the placeholder's already-assigned **`messageId`**; the client's per-**`MessageId`** aggregation turns the pair into one live-updating row. If the placeholder arrives second (cache hit, or a race), it is **silently discarded** --- both terminal handlers remove the **`PerceptionThreads`** row immediately after delivering, so a late **`Generation Started`** finds no entry to publish against. **Ordering within a bundle is not each producer's job:** `mtw.ephemera.messageOrchestration`'s fan-in cluster (`MessageOrchestrationFanInCluster.handler`, [`dataSource/messageOrchestration/messageOrchestrationFanIn.ts`](../messageOrchestration/messageOrchestrationFanIn.ts)) assigns each flushed message's `createdTime` itself --- sequential in declared slot order, 1ms apart --- **overwriting** whatever `createdTime` a slot's message arrived with. This is why `leave`/`header`/`arrive` sort correctly regardless of any individual producer's own timing. Render-correlated threads outside a bundle (**`roomDescription`**, **`roomHeaderBroadcast`**, **`sessionOrientationRender`**, **`featureDescription`**, **`knowledgeDescription`**, and **`characterMove`** when no bundle was declared) use explicit **`createdTime`**: **`T0`** on Generating (stored on **`PerceptionThreads`** row), terminal **`max(T0 + 1, now)`** with same **`messageId`**; a bundled `characterMove` terminal delivered standalone (second frame, after its placeholder already flushed through the bundle) uses this same non-bundled path, since it is a separate later wire push outside the bundle's own flush. Ingress helpers **`sendCharacterPerceptionRequested`** / **`sendPerceptionThreadRegistered`** publish **`StreamingEvent`** on the bus. COMP-KICK (**`kickRoomHeaderBroadcastForRoom`**) publishes thread registration + render kicks.
