...
**Object manipulation presentation fan-in (Phase 5 shipped --- v1 `takeHold` only):** Pick-up **transcript** (intent + fact -> single **`WorldMessage`**) runs on a **separate** **`FanInClusterStore`** + **`ObjectManipulationPresentationFanInCluster`** --- parallel to membership fan-in, not mixed into **`MembershipPresentationFanInCluster`**. **Cluster spec:** [`objectManipulationPresentationFanIn.ts`](objectManipulationPresentationFanIn.ts). **Ingress adapters:** [`objectManipulationPresentationLegAdapters.ts`](objectManipulationPresentationLegAdapters.ts) (`toObjectManipulationPresentationLeg`); **`Object Take Hold`** (actions intent) + **`Object Moved`** (positions fact, cross-host **`takeHold`**). **Correlation:** actor **`characterId`** + grounded **`objectId`** + fact **`beatAnchorTime`**; endpoint checks: intent **`roomId`** in fact **`froms`**, fact **`to`** = intent **`characterId`**. **Publish:** [`publishObjectManipulationPresentation.ts`](publishObjectManipulationPresentation.ts) --- deterministic template **`${Player} picks up ${Object}`** (no copy LLM); **`createdTime`** = fact **`beatAnchorTime`** exactly (Model A, no leave/arrive epsilon); targets source **`roomId`** + actor **`characterId`**; **`deliveryMode: 'deferred'`**. **Labels at emit:** [`resolveTakeHoldPresentationLabels.ts`](resolveTakeHoldPresentationLabels.ts) via **`CharacterMeta`** + merged **`ComponentAggregate`** (does not require object to remain in room graph post-apply). **Unknowns (D12):** withhold unstated spatial detail --- template only, fallbacks **`Someone`** / **`something`**. **Deferral:** fact-only settle derives actor from character **`to`** host when intent absent. Shares deferral tag **`fanIn-mtw.ephemera.perception`** with membership store ([`index.ts`](index.ts)).
...
| [`publishObjectManipulationPresentation.ts`](publishObjectManipulationPresentation.ts) | --- | **`WorldMessage`** pick-up transcript (**`displayProtocol: 'WorldMessage'`** only; no **`roomChannel`**). Single line per successful **`takeHold`**; **`createdTime`** = fact **`beatAnchorTime`**. |
