...
**Phase B (server, shipped):** Roster refresh emits affordance **`PerceptionMessage`** from [`publishRoomAffordancePerceptionMessages.ts`](publishRoomAffordancePerceptionMessages.ts) when [`roomUpdateMessage`](../../roomUpdate/index.ts) runs (internal bus **`type: 'RoomUpdate'`** only; wire **`displayProtocol: 'RoomUpdate'`** retired). **`mtw.ephemera.objects` `Objects Changed`** is subscribed on this DataSource and invokes the same helper. Correlated render **`PublishMessage`** rows set **`metaData.roomChannel: 'render'`**; affordance rows use **`roomChannel: 'affordances'`** and **new** **`MESSAGE#${uuid}`** ids. Render vs affordance WML **de-dupe** on the server is **deferred** per contract. See [`AGENT.multiChannel.contract.md`](../AGENT.multiChannel.contract.md) **Phase B server migration (agreed)** and **De-duplication (norm vs Phase B)**.
...
| [`index.ts`](index.ts) **`receiveEvents`** | | **`Character Perception Requested`** → [`characterPerception.ts`](characterPerception.ts). **`Perception Thread Registered`** → **`PerceptionThreads.register`**. Render-cache / orchestration payloads → [`orchestrateRoomDescriptionStreams`](orchestrate.ts). **`Objects Changed`** → [`publishRoomAffordancePerceptionMessages.ts`](publishRoomAffordancePerceptionMessages.ts) (not orchestration). |
...
| [`publishRoomAffordancePerceptionMessages.ts`](publishRoomAffordancePerceptionMessages.ts) | **`'affordances'`** | **`ComponentStackMerge.get(characterId, roomId)`** once per occupant; **`targets: [characterId]`**; new **`MESSAGE#${uuid}`** per row. |
| [`roomUpdate/index.ts`](../../roomUpdate/index.ts) | (affordance path only) | Internal bus **`type: 'RoomUpdate'`** invokes **`publishRoomAffordancePerceptionMessages`**; wire **`displayProtocol: 'RoomUpdate'`** is **retired**. |

**Uncoupled signals:** **`mtw.ephemera.state` `State Changed`** drives **render** / passive-render fan-out only --- it does **not** emit affordance-channel **`PerceptionMessage`** rows. **`mtw.ephemera.objects` `Objects Changed`** and **`RoomUpdate`**-triggered roster refresh emit **only** affordance **`PerceptionMessage`** (per character), not a paired render row.
