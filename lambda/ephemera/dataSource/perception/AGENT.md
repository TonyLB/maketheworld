...
The **main** cascade (**`executeAction`**, **`renderOrchestration`**, **`renderCache`**, perception **`receiveEvents`** for non-Generating work) stays on the **default** lane unless a slice opts in. **Render orchestration** slow-path generation **no longer** allocates a generation lane (P3 **`publish`** migration); **`Generation Started`** publishes concurrent with LLM work and boundary **`flushAndSettle`** quiesces the invocation. **Affordance orchestration** and other unmigrated slices may still use lane-scoped flush until their P3 migration. **Owned detail:** [`AGENT.implementation.md`](../../../../packages/mtw-lambda-patterns/ts/messageBus/AGENT.implementation.md#virtual-lanes-internalmessagebus) (**Virtual lanes**), [`lambda/ephemera/messageBus/AGENT.md`](../../messageBus/AGENT.md).
