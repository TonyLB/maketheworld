...
**Structural room WML (affordances):** Merged room **`StandardForm`** without **`RenderCache`**, **`Examples`**, or **`StandardRoomData.render`** (exits, characters present, merged **shortName**, **`Meta::Room.objects`**, etc.) comes from **`internalCache.ComponentStackMerge`** ([`componentStackMerge.ts`](../../internalCache/componentStackMerge.ts)). Steady-state documentation: [`internalCache/AGENT.md`](../../internalCache/AGENT.md) (**Component stack merge**). **WML composition (affordance channel):** **`ComponentStackMerge.get`** per viewer, **`ephemeraWire`**; future cache-key migration note in **`internalCache`** docs --- see [Server publish sites (multi-channel)](#server-publish-sites-multi-channel) above.
