...
**Membership presentation fan-in (Phase 1, in progress):** Move **emission policy** (intent + fact -> leave/arrive shape + copy) migrates to generic **`FanInClusterStore`** + **`MembershipPresentationFanInCluster`** on this DataSource --- replacing pre-baked world lines on **`characterMove`** PerceptionThreads rows (Phase 2 retires the legacy path). **Cluster spec shipped:** [`membershipPresentationFanIn.ts`](membershipPresentationFanIn.ts) (`MembershipPresentationFanInCluster`, `buildMembershipEmissionPlan`, synthetic-leg tests). **Not yet wired:** `subscribedEvents` leg adapters, module-scoped **`FanInClusterStore`** on [`index.ts`](index.ts), or **`PublishMessage`** emission. **Fact** leg: **`Character Moved`** on **`mtw.ephemera.positions`**; **intent** legs: **`mtw.ephemera.actions`** + **`mtw.connections.characters`**. **`MembershipEndpoint`:** `null` = out of play (provisional per fan-in **F1-7**; must match positions **`Character Moved`** payload at slice 1b). Header render / **`Render Pertains`** correlation stays on existing PerceptionThreads + [`orchestrate.ts`](orchestrate.ts) --- **not** the membership fan-in cluster. Distinct from **`PerceptionFanInOrchestrationPayload`** (render **`Generation Started`** lifecycle). Task plan: [`AGENT.fanInPattern.planning.md`](../../../../taskPlanning/packages/mtw-lambda-patterns/ts/dataSource/AGENT.fanInPattern.planning.md).
