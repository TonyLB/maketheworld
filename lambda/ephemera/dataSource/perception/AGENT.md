...
**Structural room WML (affordances):** Merged room **`StandardForm`** without **`RenderCache`**, **`Examples`**, or **`StandardRoomData.render`** (exits, characters present, merged **shortName**, **`Meta::Room.objects`**, etc.) comes from **`internalCache.AffordanceRoomDeliverable`** ([`affordanceRoomDeliverable.ts`](../../internalCache/affordanceRoomDeliverable.ts)). Steady-state documentation: [`internalCache/AGENT.md`](../../internalCache/AGENT.md) (**Affordance room deliverable**). **WML composition (affordance channel):** **`AffordanceRoomDeliverable.get`** per viewer, **`ephemeraWire`**; future cache-key migration note in **`internalCache`** docs --- see [Server publish sites (multi-channel)](#server-publish-sites-multi-channel) above.
...
| [`handleAffordancesPertain.ts`](handleAffordancesPertain.ts) | **`'affordances'`** | Terminal on **`Affordances Pertain`**: **`sessionOrientationAffordances`** thread lookup first (**`publishAffordancePerceptionForTargets`**, **`registration.targets`** e.g. **`SESSION#...`**, merge via **`registration.characterId`**); **fallback** when no affordance thread: perspective-filtered occupants via **`getCharacterRoomPerspectiveKey`**; **`AffordanceRoomDeliverable.get(characterId, roomId)`** per target; **`targets: [characterId]`**; new **`MESSAGE#${uuid}`** per row. Uncoupled from render threads (**D38**). |
