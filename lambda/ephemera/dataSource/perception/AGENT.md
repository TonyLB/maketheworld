...
**Render Pertains (terminal, render channel):** In [`orchestrate.ts`](orchestrate.ts) **`handleRenderPertains`** for room threads only, terminal **`PublishMessage`** rows with **`metaData.roomChannel: 'render'`** use [`roomRenderWmlFromCacheRecord.ts`](roomRenderWmlFromCacheRecord.ts): **`wmlContent`** is built from **`payload.cacheRecord.renderedContent`** (prose **`<Render>`** mirror), not **`internalCache.ComponentRender.get`**. Imperative [`perceptionMessage`](../../perception/index.ts) and any path without a concrete cache row still use **`ComponentRender`**.
...
| [`perception/index.ts`](../../perception/index.ts) **`perceptionMessage`** | **`'render'`** | **`PerceptionRoomMessage`**: **`ComponentRender.get`** per **`characterId`**, one **`PublishMessage`** per target. **`sendRoomGeneratingHeader`**: one multi-target **`PublishMessage`**. **`PerceptionAssetMessage`**: **`kickRoomHeaderBroadcastForRoom`** (DataSource kick), not direct room WML in this handler. |
...
**Open follow-ups (server, multi-channel):** Full **render-channel** de-duplication vs affordances (beyond **Render Pertains** terminal and placeholders) remains a **contract norm**, not a Phase B server requirement --- imperative paths still use **`ComponentRender`** until a later milestone. **`ComponentStackMerge`** cache keys may move from **`(characterId, roomId)`** toward **`(componentId, perspectiveKey)`**; see [`internalCache/AGENT.md`](../../internalCache/AGENT.md) (**Component stack merge**). **Coupled** render+affordance **PerceptionThread** pairing stays **deferred** unless product requests it (contract **Coupled delivery**).
