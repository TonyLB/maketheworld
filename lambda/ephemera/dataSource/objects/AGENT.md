...
This package owns **runtime object lists** for rooms: stored on ephemera **`Meta::Room`** as **`objects?: EphemeraMetaRoomObject[]`** (optional; missing treated as empty when patching). Each row is **`uuid`** + **`shortName`**, with optional **`stableKey`** (Coyote machine-correlation slug; distinct from human-facing **`shortName`**; uniqueness for Acme-created keys is Coyote-wide --- see [`../coyoteGame/AGENT.md`](../coyoteGame/AGENT.md) and **`mtw.ephemera.actions`**), optional **plan-role `affinities`**, and optional **`affinitiesFailed`** when Acme enrich did not validate affinities for that line (see [`packages/mtw-interfaces/ts/ephemeraMeta.ts`](../../../../packages/mtw-interfaces/ts/ephemeraMeta.ts), affinity shapes in [`packages/mtw-interfaces/ts/coyotePlanAffinities.ts`](../../../../packages/mtw-interfaces/ts/coyotePlanAffinities.ts)). It uses a dedicated **`dataSourceKey`** (**`mtw.ephemera.objects`**) in **symmetry** with **`mtw.ephemera.state`**: a **semantic domain** for object membership, **not** named as a sub-feature of a room aggregate, even though v1 stores the field on **`Meta::Room`**. Long term, object lists may attach to **other** ephemera kinds; v1 does **not** encode "objects are only a room concern" in the **`dataSourceKey`**.
...
**Not** asset / blueprint authoring: **`mtw-wml`** **`standardizeMode: 'ephemeraWire'`** allows **`<Object uuid=(id)><ShortName>label</ShortName></Object>`** under **`Room`**. **`uuid`** is canonical **`OBJECT#...`** in memory and **`StandardRoom.objects`** / **`toJSON`**; **`Objects Change`** **`add`** uses full **`EphemeraMetaRoomObject`** rows (WML authoring supplies **`uuid`** + **`shortName`**; server flows such as Acme enrich may add **`affinities`** / **`affinitiesFailed`** and **`stableKey`**); **`remove`** uses **`OBJECT#...` ids**. Normative detail: [`packages/mtw-wml/ts/standardize/AGENT.md`](../../../../packages/mtw-wml/ts/standardize/AGENT.md).
...
| **Field shape** | **`objects`:** **`EphemeraMetaRoomObject[]`** --- required **`uuid`**, **`shortName`**; optional **`stableKey`** ([**`EphemeraMetaRoomObject`**](../../../../packages/mtw-interfaces/ts/ephemeraMeta.ts)); optional **`affinities`** (plan-role possibilities + **`aptness`**), optional **`affinitiesFailed`** when enrich validation failed for that row (distinct from legacy omitted affinity fields). |
...
| [`../coyoteGame/AGENT.md`](../coyoteGame/AGENT.md) | Coyote hypothesis / plan-outcome prompts consume **`Meta::Room.objects`** via staged-object snapshot (**plan-role** text in **`## Current staged objects by room`**) |
| [`../actions/`](../actions/) ([**`AGENT.md`**](../actions/AGENT.md), `parseCommand.ts`, `publishedEvents.ts`, `mergeAcmeOrderEnrich.ts`, [`index.ts`](../actions/index.ts)) | Normative **`stableKey`** contract (**`actions/AGENT.md`**); two-step Acme parse (**intent** + **enrich**); Coyote-wide occupancy (**`collectCoyoteOccupiedStableKeys`**) + deterministic finalize (**`finalizeStableKeysDeterministic`**) before **`Acme Order`**; **`AcmeOrderPublishedPayload.orders`**, confidence combine rule |
