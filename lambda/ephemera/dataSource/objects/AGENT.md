...
**Handler + outbound:** [`handleApiObjectsChange.ts`](handleApiObjectsChange.ts) --- non-room **`componentId`** no-op; on successful persist, **`streamEvent`** with **`Objects Changed`** (typed payload in [`events.ts`](events.ts): **`priorObjects`** / **`newObjects`** mirror the stored shape).
