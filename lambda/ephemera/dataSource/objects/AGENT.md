...
**Not** asset / blueprint authoring: **`mtw-wml`** **`standardizeMode: 'ephemeraWire'`** allows **`<Object uuid=(id)><ShortName>label</ShortName></Object>`** under **`Room`**. **`uuid`** is canonical **`OBJECT#...`** in memory and **`StandardRoom.objects`** / **`toJSON`**; **`Objects Change`** **`add`** uses full **`EphemeraMetaRoomObject`** rows (WML authoring supplies **`uuid`** + **`shortName`**; optional affinity fields are added by server flows such as Acme enrich); **`remove`** uses **`OBJECT#...` ids**. Normative detail: [`packages/mtw-wml/ts/standardize/AGENT.md`](../../../../packages/mtw-wml/ts/standardize/AGENT.md).
...
| **Field shape** | **`objects`:** **`EphemeraMetaRoomObject[]`** --- required **`uuid`**, **`shortName`**; optional **`affinities`** (plan-role possibilities + **`aptness`**), optional **`affinitiesFailed`** when enrich validation failed for that row (distinct from legacy omitted affinity fields). |
...
| [`../actions/`](../actions/) (`parseCommand.ts`, `publishedEvents.ts`, `mergeAcmeOrderEnrich.ts`) | Two-step Acme parse (**intent** + **enrich**), **`AcmeOrderPublishedPayload.orders`**, confidence combine rule |
