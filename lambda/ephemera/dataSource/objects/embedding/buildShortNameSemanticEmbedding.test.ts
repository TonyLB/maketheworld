...
import {
    ...
    buildShortNameSemanticEmbedding,
} from './buildShortNameSemanticEmbedding'
...
describe('buildShortNameSemanticEmbedding', () => {
    it('returns quantized SemanticEmbedding with Dynamo record shape on success', async () => {
        ...
        const result = await buildShortNameSemanticEmbedding('brass lantern', { invokeEmbed })
    ...
    it('normalizes shortName before embed and hash', async () => {
        const invokeEmbed = jest.fn().mockResolvedValue({
            success: true,
    ...
    it('fails before Bedrock when shortName is empty after normalization', async () => {
        const invokeEmbed = jest.fn()

        const result = await buildShortNameSemanticEmbedding('   ', { invokeEmbed })

        expect(invokeEmbed).not.toHaveBeenCalled()
        expect(result).toEqual({
            success: false,
            errorMessage: 'shortName must be non-empty after normalization',
        })
    })
    ...
    it('propagates transport failure', async () => {
        ...
        const result = await buildShortNameSemanticEmbedding('anvil', { invokeEmbed })
    ...
    it('passes custom timeoutMs to transport', async () => {
        ...
        await buildShortNameSemanticEmbedding('anvil', { invokeEmbed, timeoutMs: 3000 })
