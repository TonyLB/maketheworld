...
1. **Classify order type (Chain-of-reason markdown):** Walk each **distinct product / line item** you extracted (see below). Use **one section or bullet block per item**. For **every** item, reason **in the order below** (same classify step). That order matters: corrections can move nonsense into a valid gloss; cartoon physics can move **real-world impossible** into **in-genre** before you pick the primary bucket.

   **First — Correctable user error:** Check whether the surface text is plausibly a typo, speech-to-text glitch, or wrong-word slip against Coyote / Acme vocabulary (**potable**→**portable**, etc.). If so, state the **intended gloss** and use **only that gloss** for the rest of the reasoning on this line. If no correction applies, say so briefly. **Do not** label **Not a thing** when a reasonable correction yields a clear deliverable.

   **Second — Cartoon physics modifier:** Given the **effective wording** (after any correction), set **Cartoon physics: yes** when the deliverable **defies real-world physics or manufacturing** but is **normal Coyote vs. Road Runner stock** (flying carpets, portable holes, delayed-fall kits, etc.). Otherwise **Cartoon physics: no**. Use this **after** corrections so an impossible-looking string is re-judged after fixing the words. This is a **modifier** stacked on the primary category — not a substitute for **Phenomenon** / **Diffuse** / **Self-contained**.

   **Third — Primary category (exactly one):** Pick **one** bucket for the line. **Do not** reject for insufficient silliness or gag quality — genre vibe is **not** eligibility here. **Do not** shoehorn into **Not tangible** for vibe reasons.
   ...
   - **Too large:** Only when the ask names **scale that breaks the cartoon stage as one deliverable** — for example **the Galilean moons**, **the Moon**, **North America**, **the jet stream** as a boxed SKU. **Do not** use real-world freight intuition: tower cranes, diesel locomotives, moon rockets, grand pianos, and similar **Chuck Jones oversized props** are **Self-contained** (or **Diffuse** / **Phenomenon** when they fit), **not** **Too large**. **Cartoon physics** can stack here (cosmic gag, still rejected for scale).
   - **Phenomenon:** Concrete, but an ongoing process or event (laser beam, earthquake, lightning storm). Stay **brief**: state that label, then list **two** plausible Acme gadgets or deliveries that could **produce** that phenomenon.
   - **Diffuse:** Tangible, but not one self-contained unit (hydrogen gas, flock of crows, **cloud of mosquitos**, waterfall). Stay **brief**: state that label, then list **two** plausible **packages or generators**.
   - **Self-contained:** A single ship-ready article (crate, coil, costume, bottle, beehive). **Ordinary objecthood is not required** when **Cartoon physics: yes** — a flying carpet is still **Self-contained** (one SKU), not **Not a thing**.
