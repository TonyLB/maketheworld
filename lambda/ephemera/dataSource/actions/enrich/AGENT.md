...
- [`objectManipulation/embeddingMatch/`](./objectManipulation/embeddingMatch/) - pure embedding identity fast path (EM-2): `decideEmbeddingMatch`, `rankCatalogByCosineSimilarity`, `simulateEmbeddingIdentity`, placeholder `thresholds.ts`; unit-tested with mocked vectors against calibration corpus; not wired into identity stage until EM-5. Span Bedrock embed lives in [`../../objects/embedding/embedObjectSpan.ts`](../../objects/embedding/embedObjectSpan.ts) (EM-3).
