...
| [`testing/compareAdmissibilityArms`](testing/compareAdmissibilityArms.ts) | FT-1.3.1 retirement harness (legacy vs narrowed production policy) |
...
| [`admissibleShortSpans`](admissibleShortSpans.ts) | Catalog-derived short-span gate (length 2..S_min-1); length-1 active under narrowed FT-1.3.1 policy |
...
| `C_MIN`, `L_MIN`, flank combine, `S_MIN` | per-signal `[0,1]` | `embedRelevance`, `lexicalRelevance`, narrowed admissibility gate | **No** --- relevance normalization |
...
**Short-span admissibility (FT-1.3.1, 2026-07-09):** **narrowed retirement shipped.** Default `lexicalChannelPolicy: 'narrowed'` on [`buildSpanCandidatePool`](buildSpanCandidatePool.ts) --- length-1 lexical **active** (spurious `a`/multi-token catalog joint ~0.34 < `T_JOINT_ABS` with weak embed); inadmissible length-2 gate **retained** (`ax`/`rusty axe` lex ~0.84 --- full gate-off rejected). Policies: `legacy` (pre-FT-1.3.1), `narrowed` (production), `alwaysActive` (harness only). Harness: [`compareAdmissibilityArms`](testing/compareAdmissibilityArms.ts) with revised criteria + realistic weak embed vectors. Identity corpus ranking unchanged legacy vs narrowed.
