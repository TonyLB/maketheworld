...
**Status: shipped (FT-2.1 production pool path + FT-1.2 pool builder + FT-1.3 calibration).** **Production path (FT-2.1, 2026-07-09):** [`resolveCatalogSpanToPool`](../resolveCatalogSpanToPool.ts) -> [`buildSpanCandidatePool`](buildSpanCandidatePool.ts) -> bridge [`selectSingleSpanFromPool`](../selectSingleSpanFromPool.ts) (`T_JOINT_*`). **Calibration regression only:** v1 cosine + conjunctive gates via [`decideEmbeddingMatch`](decideEmbeddingMatch.ts) / [`simulateEmbeddingIdentity`](simulateEmbeddingIdentity.ts). Identity LLM retired from production.
...
| [`thresholds.ts`](thresholds.ts) | Locked constants: v1 shim (`T_ABS`...), FT-8 normalization, FT-1.2 pool merge, FT-5 `T_JOINT_*` (wired via bridge) |
