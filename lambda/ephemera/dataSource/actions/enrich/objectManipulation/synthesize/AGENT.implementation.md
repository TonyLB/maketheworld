...
| [`expandSameHost.ts`](expandSameHost.ts) | BD-16's `sameHost` command-expansion: resolves where a relational `Change` goes --- confirms an already-shared host, expresses a shard boundary as crossing legs plus a port, or declines. (It once *repaired* a mismatch by inserting a `transferMembership`; PV1-3b-9 retired that, 2026-09-01.) Calls `findShardBoundary.ts`/`buildCrossingLegs.ts` unconditionally for both `operationKind`s today --- silently correct for `dissolveRelation` only in the portless case; `findRelationalChain.ts` (below) is not yet wired in to fix that. |
| [`findRelationalChain.ts`](findRelationalChain.ts) | PV1-3b-12's dissolve-side discovery primitive, `findShardBoundary.ts`'s mirror image: walks existing relational edges/ports outward from a subject (not containment ancestry) to find the chain a prior `establishRelation` already built, so a `dissolveRelation` can remove it. `found` / `notFound` (decline) / `ambiguous` (decline, more than one qualifying chain). Standalone and unit-tested; not yet called by `expandSameHost.ts` (PV1-3b-14). |
| [`buildCrossingLegs.ts`](buildCrossingLegs.ts) | `findShardBoundary.ts`'s `'crossed'` verdict into establish-side legs/port (`buildCrossingLegs`, PV1-3), and, since PV1-3b-13, `findRelationalChain.ts`'s `'found'` result into dissolve-side removal steps (`buildCrossingDissolveLegs`) --- one `dissolveRelation` per discovered edge, one `removeCrossingPort` per discovered port, in the chain's own discovery order (no data dependency between the two step kinds, unlike establish's port-before-leg requirement, and no hop cap --- it only maps already-stored ids). Standalone and unit-tested; not yet called by `expandSameHost.ts` (PV1-3b-14) or a commit path (PV1-3b-3/16). |
