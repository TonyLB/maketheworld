...
Before classify or enrich, **`parseCommand`** parallel-fetches the actor's **room object catalog** and **held inventory catalog** so later hops ground spans against in-play objects without re-fetching. This is not a Bedrock hop; it packages authoritative catalog slices for identity and (on the relational path) frame-extract context.
...
- **Embedding tier (EM-2 pure policy, EM-3 span embed, wiring in EM-5):** cosine-similarity fast path between exact match and identity LLM when conjunctive gates pass on pre-attached **`EMBEDDING#IMPROMPTU`** vectors --- [`decideEmbeddingMatch`](embeddingMatch/decideEmbeddingMatch.ts), [`rankCatalogByCosineSimilarity`](embeddingMatch/rankCatalogByCosineSimilarity.ts), [`simulateEmbeddingIdentity`](embeddingMatch/simulateEmbeddingIdentity.ts); span embed via [`../../objects/embedding/embedObjectSpan.ts`](../../objects/embedding/embedObjectSpan.ts) (shared Titan path with storage write path); unit-tested with mocked vectors against [`calibration/objectMatch/corpus.ts`](../../../../calibration/objectMatch/corpus.ts). See [`embeddingMatch/`](embeddingMatch/) and [`AGENT.objectEmbeddingMatch.planning.md`](../../../../../../taskPlanning/lambda/ephemera/dataSource/actions/AGENT.objectEmbeddingMatch.planning.md).
