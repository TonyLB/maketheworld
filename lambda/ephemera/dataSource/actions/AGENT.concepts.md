...
**Code home:** Expansion's two known instances live in `enrich/objectManipulation/synthesize/`, alongside each other --- not yet a dedicated subdirectory (two items, one not yet built, don't justify a third nesting level; revisit if a third instance arrives, especially one using a genuinely different technique). Grounding's code lives in the same directory (`groundReferent.ts`/`groundChange.ts`, built 2026-07-13) --- unwired by any real command, same as Expansion's `computeCarryClosure` instance was before Slices 4a/4b converged Validation onto it.
