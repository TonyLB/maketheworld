...
- **Grounding:** resolves each `Referent` in an ungrounded plan into its actual `EphemeraObjectId` / `EphemeraMembershipHostId`, combining Identify's per-span resolution with structural interpretation of composed referents --- `objectSpan` looks up Identify's resolved candidate for that span directly; `actingCharacter` resolves to the session's known character id; `currentHost(X)` requires first grounding `X`, then looking up *its* current host in the live KR graph. This is the join step that literally turns Plan's `UngroundedPlanStep[]` output into a grounded candidate (`ParsePlanStep`-shaped, though possibly still incomplete) --- the act the "ungrounded vs. grounded" vocabulary is named after. **Not yet built as its own code** --- no shipped or sandboxed module does this compositional resolution today, though its necessity is already implicit in `ParsePlanStep`'s own doc comment ("grounding is the selector verdict + existence/presence guard in the compiler tail, not an executable step").
- **Expansion:** given a grounded-but-possibly-**incomplete** plan plus KR context, produces the *additional steps* needed to satisfy a precondition or validity requirement the plan didn't already include --- growing it into something Validation can then check. Instances: `computeCarryClosure` (`interactionUnderTransfer.ts`, shipped --- BD-13) and the not-yet-built `sameHost`-violation repair table (BD-16, **Target**: inserting a `transferMembership` step when an `establishRelation`/`dissolveRelation` intent's subject and target don't already share a host).
...
**Code home:** Expansion's two known instances live in `enrich/objectManipulation/synthesize/`, alongside each other --- not yet a dedicated subdirectory (two items, one not yet built, don't justify a third nesting level; revisit if a third instance arrives, especially one using a genuinely different technique). Grounding has **no code home yet at all** --- entirely unbuilt.
