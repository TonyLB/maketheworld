...
The discriminate-intent prompt, deterministic checks (including Coyote slash-command matchers), JSON interpreter, and intent-only types/guards live under [`discriminateIntent/`](./discriminateIntent/). **`ObjectManipulationIntent`** (classify) -> **`enrich/objectManipulation/`** (D17 enrich + resolve; v1 **`takeHold`** only): **`ObjectManipulationIntent` steady-state** in [`AGENT.implementation.md`](./AGENT.implementation.md).
...
- Parse-based pick-up (**`Parse Requested`** -> classify **`ObjectManipulationIntent`** -> enrich -> resolve -> **`Object Take Hold`**) is **stream-only** from actions; graph apply is owned by **`mtw.ephemera.positions`** ([`manipulation/membership/executeObjectTakeHold`](../positions/manipulation/membership/executeObjectTakeHold.ts)).
