...
- Parse-based navigation (**`Parse Requested`** -> **`Character Navigate`**) and UI exit clicks (**`Action Assessed`** **`Navigation`** from [`routeTrustedUiAction`](../routeTrustedUiAction.ts)) are **stream-only** from actions; execution is owned by **`mtw.ephemera.positions`** ([`index.ts`](../positions/index.ts) -> [`navigate/orchestrateCharacterMove`](../positions/navigate/orchestrateCharacterMove.ts)).
- Parse-based home (**bare `home`**, **`HomeIntent`**, **`Parse Requested`**) and trusted home (**`Action Assessed`** **`Home`** from [`routeTrustedUiAction`](../routeTrustedUiAction.ts), `source: 'uiHome'`) **`streamEvent`** **`Character Home`**; positions executes via the same **`orchestrateCharacterMove`** path as navigate.
...
- Asset visibility repair (**`repairCharacterLegalPlacement`**) is available under [`positions/membership/`](../positions/manipulation/membership/repairCharacterLegalPlacement.ts) for future canon/zone ingress; **`CheckLocation` bus adapter retired**.
...
- **[`AcmeOrderPublishedOrder`](publishedEvents.ts):** **`stableKey: string`** required on each bus order line after wiring; optional **`tropeAffinities`** / **`tropeAffinitiesFailed`** validated by **`areCoyoteObjectTropeFieldsValid`** ([`coyotePlanAffinities.ts`](../../../../packages/mtw-interfaces/ts/coyotePlanAffinities.ts)) --- same rules as persisted **`EphemeraMetaRoomObject`** rows. Also optional **`defaultSituation`** / **`defaultSituationFailed`** (Phase 5, shipped): short **`SITUATION#DEFAULT`** flavor-text prose (`displayName`/`summary`/`description`) generated by the **same** **`invokeBedrockAcmeOrderEnrich`** call that produces **`tropeAffinities`** --- no second Bedrock call. Threaded **`AcmeOrderEnrichModelLine`** (enrich) -> **`ParseCommandAcmeOrderLine`** (`enrichLineToParseLine` in [`enrich/acmeOrder/interpretAndFinalize.ts`](./enrich/acmeOrder/interpretAndFinalize.ts)) -> **`AcmeOrderPublishedOrder`** (`buildPublishedAcmeOrdersWithStableKeys` in [`index.ts`](./index.ts)) -> consumed by **`objects/handleApiObjectsChange.ts`**'s **`acmeOrderToSpawnArgs`**, which maps present prose into a **`situations`** facet on the improvisation spawn row (see [`../objects/AGENT.md`](../objects/AGENT.md) **Spawn-time `DEFAULT` situation prose**).

  **Few-shot parity is load-bearing for `defaultSituation`.** Every **`valid`: true** example line in [`enrich/acmeOrder/buildPrompt.ts`](./enrich/acmeOrder/buildPrompt.ts) --- the JSON-shape block, `ACME_ENRICH_FEW_SHOT_CORE`, and `ACME_ENRICH_FEW_SHOT_ICONIC` --- **must** carry a `defaultSituation`, and `buildPrompt.test.ts` asserts the counts match. This is not tidiness: the field originally shipped documented in prose but absent from all ten example lines, and the model copied the examples' shape instead. The failure was *phrase-specific* and therefore easy to misread as intermittent --- `order rocket skates` matched the iconic `"name": "Rocket Skates"` line verbatim and came back with no prose every time, while differently-phrased orders got prose reliably. When adding a few-shot line, add its prose too.
