...
3. **Enrich (native Parse-skeleton pipeline, iteration 3)** --- **Parse** ([`parse/runParseStage.ts`](enrich/objectManipulation/parse/runParseStage.ts)) tokenizes the command into a `ParseSkeleton` -> **`matchRelationalTemplate`** ([`plan/matchRelationalTemplate.ts`](enrich/objectManipulation/plan/matchRelationalTemplate.ts)) deterministically derives **`operationKind: establishRelation` | `dissolveRelation`** and `relationKind` from closed `V NP prep NP` templates (reusing **`normalizeRelationSpan`** for the preposition->enum half) -> **`runIdentityStageOverSkeleton`** ([`identifySkeletonSpans.ts`](enrich/objectManipulation/identifySkeletonSpans.ts), room + held catalog pools, `stableRefKey`-keyed) -> Grounding ([`synthesize/groundChange.ts`](enrich/objectManipulation/synthesize/groundChange.ts)) -> Validation ([`synthesize/filterLegalRelationalCandidates.ts`](enrich/objectManipulation/synthesize/filterLegalRelationalCandidates.ts) + [`detectRelationalCycle.ts`](enrich/objectManipulation/synthesize/detectRelationalCycle.ts)) -> **`compileRelationalFromSkeleton`** ([`compileRelationalFromSkeleton.ts`](enrich/objectManipulation/compileRelationalFromSkeleton.ts)). No fallback to the retired frame-extract flow: a template miss abstains or errors.
...
5. **Egress** --- **`Object Establish Relation`** or **`Object Dissolve Relation`** from grounded parse on **`Parse Requested`** only. Payload + guards in [`publishedEvents.ts`](publishedEvents.ts); wire in [`index.ts`](index.ts). Both payloads carry **`steps: MutationKernelStep[]`** (**`Object Establish Relation`**: PV1-3b-2; **`Object Dissolve Relation`**: PV1-3b-15), the Expansion-derived mutation-kernel chain (PV1-3b-1, shared across both `operationKind`s) --- what the establish coordinator actually commits (`executeEstablishEdgeChain`); the dissolve coordinator does not yet read its own `steps` field (PV1-3b-16).

6. **Reference files** --- enrich: [`enrich/objectManipulation/`](enrich/objectManipulation/), [`parse/`](enrich/objectManipulation/parse/), [`plan/matchRelationalTemplate.ts`](enrich/objectManipulation/plan/matchRelationalTemplate.ts), [`identifySkeletonSpans.ts`](enrich/objectManipulation/identifySkeletonSpans.ts), [`compileRelationalFromSkeleton.ts`](enrich/objectManipulation/compileRelationalFromSkeleton.ts), [`synthesize/`](enrich/objectManipulation/synthesize/), [`evaluateRelationalLegality.ts`](enrich/objectManipulation/evaluateRelationalLegality.ts). Tests: **`dataSource/actions/enrich/objectManipulation/`**, **`parseCommand.test.ts`**, **`index.test.ts`**, **`dataSource/perception/objectManipulationPresentationFanIn.test.ts`**.

7. **Downstream** --- establish: positions [`executeEstablishEdgeChain`](../positions/manipulation/relational/executeObjectEstablishRelation.ts) -> the general kernel entrypoint [`commitStepSequence`](../positions/manipulation/kernel/commitStepSequence.ts) directly (PV1-3b-2; no `applyObjectRelationalChange` layer). Dissolve: positions [`executeObjectDissolveRelation`](../positions/manipulation/relational/executeObjectDissolveRelation.ts) -> [`applyObjectRelationalChange`](../positions/manipulation/relational/applyObjectRelationalChange.ts) -> `commitStepSequence` (Migrate slice, 2026-07-23; supersedes the retired `applyHostRelationalPatch.ts`/`applyObjectRelationalChangeWithTransfer.ts`) --- unchanged, not yet widened the way establish was. Perception fan-in intent + **`Object Relation Changed`** fact -> **`WorldMessage`** ([`objectManipulationPresentationFanIn.ts`](../perception/objectManipulationPresentationFanIn.ts)).
...
**Classify contract, superseded 2026-07-20 (iteration 7, Sub-iteration 1):** classify no longer decides **`ObjectMembershipIntent`** vs. **`ObjectRelateIntent`** --- it emits a generic **`Command`** for any in-world action (see [`AGENT.classifyPlanGeneralization.planning.md`](../../../../../taskPlanning/lambda/ephemera/dataSource/actions/AGENT.classifyPlanGeneralization.planning.md)). The membership-vs-relational split (still real, still needed by enrich) now happens **after** classify, from the Parse skeleton, via **`classifySkeletonFamily`**:

| Family | Topology | How it's decided | Enrich path |
| --- | --- | --- | --- |
| **`ObjectMembershipIntent`** | Membership host transfer | Deterministic pre-classify fast path (**`deterministicChecks.ts`**, `verbClass` `acquire`\|`release`) **or**, for classify-routed `Command`, [`classifySkeletonFamily.ts`](enrich/objectManipulation/plan/classifySkeletonFamily.ts)'s membership arm (leading skeleton text token `take`/`get` -> acquire, `drop` -> release) | Parse -> **`compileMembershipAtomic`** (via [`objectSpansFromSkeleton.ts`](enrich/objectManipulation/parse/objectSpansFromSkeleton.ts)) |
| **`ObjectRelateIntent`** (family, not a classify-emitted type anymore) | In-host edge on host graph | [`classifySkeletonFamily.ts`](enrich/objectManipulation/plan/classifySkeletonFamily.ts)'s relational arm --- tries [`matchRelationalTemplate`](enrich/objectManipulation/plan/matchRelationalTemplate.ts) against the skeleton first (relational wins ties, mirroring classify's old tie-break) | Parse -> **`matchRelationalTemplate`** -> Identify/Grounding/Validation -> **`compileRelationalFromSkeleton`** |
...
**Bedrock budget (after classify):** membership path adds **+1** Parse hop (LLM route; the deterministic take/drop/get fast path skips it), **0** enrich Bedrock when exact identity + atomic pre-gates succeed; **+1 Titan embed** per distinct span on exact miss; **1** complexity LLM when pre-gates defer (identity LLM retired FT-2.1). Relational route adds **+1** Parse hop (BD-21; replaced the retired frame-extract hop); **0--2** Titan embeds (per distinct span on exact miss). **`matchRelationalTemplate`**, **`normalizeRelationSpan`**, and **`classifySkeletonFamily`** are deterministic --- no Bedrock. Containment **`in`** / **`inside`** / **`into`** -> **`nestingRelational`** Error (not **`establishRelation`**). Eligible exact-name, single-span, single-host, edge-free **`takeHold`** or **`drop`** on the deterministic fast path may need **zero** post-classify Bedrock calls.
...
3. **Classify prompt:** narrowed (iteration 7, Sub-iteration 1) to realness/shape only --- no family-specific sections. See [`discriminateIntent/buildIntentClassificationPrompt.ts`](discriminateIntent/buildIntentClassificationPrompt.ts) and [`AGENT.classifyPlanGeneralization.planning.md`](../../../../../taskPlanning/lambda/ephemera/dataSource/actions/AGENT.classifyPlanGeneralization.planning.md). **Deterministic classify skip (unchanged):** minimal-verb **`take <object>`**, **`drop <object>`**, **`get <object>`** in [`deterministicChecks.ts`](discriminateIntent/deterministicChecks.ts) synthesize **`ObjectMembershipIntent`** (`confidence: 1`); **`get`** is label-gated, so an unrecognized product now reaches classify -> **`Command`** -> `classifySkeletonFamily`'s membership arm (no catalog match downstream) rather than the retired **`AcmeOrder`** tie-break. **Leading-article normalization (FT-1.4):** [`peelLeadingArticleWhenTail`](discriminateIntent/peelLeadingArticleWhenTail.ts) strips `a` / `an` / `the` / `some` from deterministic spans **only when** non-empty content remains after peel (article-only spans like `a` pass through unchanged); applied in [`deterministicChecks.ts`](discriminateIntent/deterministicChecks.ts).
4. **Model JSON (classify):** `{ "type": "Command", "confidence": <number> }` or `{ "type": "WorldQuestion", "confidence": <number> }` (plus the three realness/shape outcomes) --- no family fields (`verbClass`, spans, etc.) at classify anymore. Interpreter: [`intentClassification.ts`](discriminateIntent/intentClassification.ts). Family decision (`verbClass` for membership) comes from `classifySkeletonFamily` post-Parse, not from classify's JSON.

**Classify vs enrich ownership:**

| Field | Lane | Meaning |
| --- | --- | --- |
| **`verbClass`** | Deterministic pre-classify fast path (**`deterministicChecks.ts`**), or [`classifySkeletonFamily.ts`](enrich/objectManipulation/plan/classifySkeletonFamily.ts)'s membership arm for classify-routed **`Command`** (iteration 7, Sub-iteration 1 --- classify's LLM no longer decides this) | Membership **language** direction (`acquire` \| `release`) |
| **`operationKind`** (membership) | Enrich / FT-2.2 selector | Membership **ground truth** (`takeHold` \| `drop`) from propose-N + locus dry-run; complexity LLM only on exit-edge defer |
| **`operationKind`** (relational) | **Plan** stage (BD-12) | Owned by Plan, concluded in either of two realizations: **deterministic** closed-verb template ([`plan/matchRelationalTemplate.ts`](enrich/objectManipulation/plan/matchRelationalTemplate.ts) --- `put`/`place`/`lean` -> establish, `take`/`remove` -> dissolve; positive-match-required, abstain on miss), shipped; or Plan's **LLM fallback** (BD-19 plan-only/joint, iteration 2) for template misses, not yet built. Compiler grounds/validates only, never infers. Seam rule + provenance (superseded frame extract): [`../../llm/AGENT.contract.md`](../../llm/AGENT.contract.md) (BD-12 canonical incident). *Forward: `operationKind` may later become implicit in the matched `ParsePlanStep` variant rather than a distinct scalar --- the seam (Plan owns the determination) is unchanged.* |

5. **Enrich:** [`enrich/objectManipulation/`](enrich/objectManipulation/) --- **`parseCommand`** passes **`enrichRoute: 'membership' | 'relational'`** and **`hostRoomId`** (from **`roomExitContext.fromRoomId`**) from classify intent type ([`index.ts`](enrich/objectManipulation/index.ts)); preposition regex in [`relationalRoute.ts`](enrich/objectManipulation/relationalRoute.ts) is **not** the primary router (retained for unit tests / documentation). Relational path (native Parse-skeleton pipeline, iteration 3): **`matchRelationalTemplate`** (deterministic Plan match, `operationKind`/`relationKind`) -> **`identifySkeletonSpans`** -> **`groundChange`** (Grounding) -> **`filterLegalRelationalCandidates`** (Validation) -> **`compileRelationalFromSkeleton`**. Membership path: **`compileMembershipAtomic`** with **`MembershipManipulationFrame`** ([`membershipFrame.ts`](enrich/objectManipulation/membershipFrame.ts)). (The old role-tagged **`ManipulationFrame`** type in [`manipulationFrame.ts`](enrich/objectManipulation/manipulationFrame.ts) is no longer on the live relational route --- retained only for the unwired Phase C sandbox compiler `plan/compileUngroundedPlan.ts`.)
...
| 2 | Sole host `ludicGraph` has exit edges touching target | **LLM fall-through** |
...
10. **Egress:** **`streamEvent`** **`Object Take Hold`** or **`Object Drop`** (`characterId`, `objectId`, `roomId`, optional `confidence`) from **`Parse Requested`** only (no **`Action Assessed`** branch in v1). Membership **`roomId`** from **`roomExitContext.fromRoomId`**; defensive OOC when character has no current room. Relational: **`Object Establish Relation`** or **`Object Dissolve Relation`** (`characterId`, `subjectId`, `targetId`, `hostId`, `relationKind`, optional `relationLabel`, optional `confidence`, `steps: MutationKernelStep[]` --- establish: PV1-3b-2, dissolve: PV1-3b-15) from grounded **`EstablishRelation`** parse on **`Parse Requested`** only. Stream contracts in [`publishedEvents.ts`](publishedEvents.ts). Positions subscribers: [`orchestrateObjectMove`](../positions/manipulation/membership/orchestrateObjectMove.ts) (**both** **`Object Take Hold`** and **`Object Drop`** --- one entry point taking a host pair, not a verb), [`executeEstablishEdgeChain`](../positions/manipulation/relational/executeObjectEstablishRelation.ts) (establish, direct to `commitStepSequence`), [`executeObjectDissolveRelation`](../positions/manipulation/relational/executeObjectDissolveRelation.ts) (dissolve, via `applyObjectRelationalChange`). Perception fan-in applies to the **relational** pair only: actions intent + positions **`Object Relation Changed`** fact -> **`WorldMessage`** ([`objectManipulationPresentationFanIn.ts`](../perception/objectManipulationPresentationFanIn.ts)). Take/drop narration is compiled by the positions kernel and never reaches perception. Parse must reject **`multiPresent`** and relational complexity before egress so bounded apply never receives ambiguous multi-host targets. **`Consult`** and **`Abstain`** are documented terminal egress variants (OOC only; pre-commit).
