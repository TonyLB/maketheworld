...
4. **Enrich:** [`enrich/objectManipulation/`](enrich/objectManipulation/) --- Bedrock JSON (`disposition: atomic | complex`); v1 implements atomic **`takeHold`** through resolve only. **Phase 2 (bridge):** cardinality gate short-circuits **`multiObject`** before Bedrock; post-resolve membership pre-gates block **`multiPresent`** and zero-host objects via **`internalCache.Positions.getMembershipContainers`**. Full identity/complexity stage split: planning doc Phase 3.
