...
4. **Enrich:** [`enrich/objectManipulation/`](enrich/objectManipulation/) --- **split stages (O2):** cardinality gate -> identity (deterministic catalog + optional identity LLM per **O1**) -> unary collapse -> membership observation -> complexity pre-gates (**O4**) -> optional complexity LLM on edge-touch defer. Eligible single-span, in-room, single-host, edge-free **`takeHold`** uses **zero** post-classify Bedrock. v1 atomic **`takeHold`** only; held-only grounding -> **`unimplementedVerb`** Error until **`drop`** slice.
5. **Resolve:** deterministic **`shortName`** match in identity stage ([`resolveObjectSpan.ts`](enrich/objectManipulation/resolveObjectSpan.ts) + [`catalogMerge.ts`](enrich/objectManipulation/catalogMerge.ts)); identity LLM may return **`objectId`** on NoMatch / AmbiguousMatch.
6. **Terminal parse outcomes:** **`ObjectManipulation`** (`operationKind: takeHold`, grounded **`objectId`**) or **`Error`** (complex stub including **`multiObject`** / **`multiPresent`**, unimplemented atomic **`operationKind`**, resolve/enrich failure, no membership host). Complex disposition: no stream, no positions.
