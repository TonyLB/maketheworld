...
7. **Downstream** --- positions [`executeObjectEstablishRelation`](../positions/manipulation/relational/executeObjectEstablishRelation.ts) / [`executeObjectDissolveRelation`](../positions/manipulation/relational/executeObjectDissolveRelation.ts) -> [`applyObjectRelationalChange`](../positions/manipulation/relational/applyObjectRelationalChange.ts) -> the general kernel entrypoint [`commitStepSequence`](../positions/manipulation/kernel/commitStepSequence.ts) (Migrate slice, 2026-07-23; supersedes the retired `applyHostRelationalPatch.ts`/`applyObjectRelationalChangeWithTransfer.ts`); perception fan-in intent + **`Object Relation Changed`** fact -> **`WorldMessage`** ([`objectManipulationPresentationFanIn.ts`](../perception/objectManipulationPresentationFanIn.ts)).
...
10. **Egress:** **`streamEvent`** **`Object Take Hold`** or **`Object Drop`** (`characterId`, `objectId`, `roomId`, optional `confidence`) from **`Parse Requested`** only (no **`Action Assessed`** branch in v1). Membership **`roomId`** from **`roomExitContext.fromRoomId`**; defensive OOC when character has no current room. Relational: **`Object Establish Relation`** or **`Object Dissolve Relation`** (`characterId`, `subjectId`, `targetId`, `roomId` from **`hostRoomId`**, `relationKind`, optional `relationLabel`, optional `confidence`) from grounded **`EstablishRelation`** parse on **`Parse Requested`** only. Stream contracts in [`publishedEvents.ts`](publishedEvents.ts). Positions subscribers: [`orchestrateObjectMove`](../positions/manipulation/membership/orchestrateObjectMove.ts) (**both** **`Object Take Hold`** and **`Object Drop`** --- one entry point taking a host pair, not a verb), [`executeObjectEstablishRelation`](../positions/manipulation/relational/executeObjectEstablishRelation.ts), [`executeObjectDissolveRelation`](../positions/manipulation/relational/executeObjectDissolveRelation.ts). Perception fan-in applies to the **relational** pair only: actions intent + positions **`Object Relation Changed`** fact -> **`WorldMessage`** ([`objectManipulationPresentationFanIn.ts`](../perception/objectManipulationPresentationFanIn.ts)). Take/drop narration is compiled by the positions kernel and never reaches perception. Parse must reject **`multiPresent`** and relational complexity before egress so bounded apply never receives ambiguous multi-host targets. **`Consult`** and **`Abstain`** are documented terminal egress variants (OOC only; pre-commit).
