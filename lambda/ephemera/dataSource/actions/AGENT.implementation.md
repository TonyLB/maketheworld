...
5. **Egress** --- **`Object Establish Relation`** or **`Object Dissolve Relation`** from grounded parse on **`Parse Requested`** only. Payload + guards in [`publishedEvents.ts`](publishedEvents.ts); wire in [`index.ts`](index.ts). **`Object Establish Relation`** additionally carries **`steps: MutationKernelStep[]`** (PV1-3b-2), the Expansion-derived mutation-kernel chain (PV1-3b-1) --- what positions actually commits; **`Object Dissolve Relation`** has not been widened the same way yet (PV1-3b-15).
