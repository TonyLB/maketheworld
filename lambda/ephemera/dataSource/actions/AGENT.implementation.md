...
| --- | --- | --- | --- |
| **`ObjectMembershipIntent`** | Membership host transfer | **`verbClass`** (`acquire` \| `release`); spans come from Parse or the deterministic fast path, not classify (iteration 3) | Parse -> **`compileMembershipAtomic`** (via [`objectSpansFromSkeleton.ts`](enrich/objectManipulation/parse/objectSpansFromSkeleton.ts)) |
| **`ObjectRelateIntent`** | In-host edge on host graph | intent only (no **`verbClass`**, no spans) | Parse -> **`matchRelationalTemplate`** -> Identify/Grounding/Validation -> **`compileRelationalFromSkeleton`** |
...
Parse Requested
  ...
  -> classify (LLM): ObjectMembershipIntent OR ObjectRelateIntent (movementObjectLabels = room + held; classify no longer emits objectSpans)
  -> parseCommand branches by intent type; both routes run Parse (runParseStage) for tokenization unless deterministic fast path already produced spans
  -> enrichObjectManipulation(enrichRoute):
...
4. **Model JSON (classify):**
   - Membership: `{ "type": "ObjectMembershipIntent", "verbClass": "acquire" | "release", "confidence": <number> }` -> **`verbClass`** (spans no longer requested from classify; Parse tokenizes. The deterministic fast path still self-builds its own `rawObjectSpans` in `deterministicChecks.ts`)
   - Relate: `{ "type": "ObjectRelateIntent", "confidence": <number> }` (no spans; legacy **`ObjectManipulationIntent`** rejected)
