...
- **`lambda/ephemera/dataSource/thinking` (and related ephemera modules):** **`EphemeraDataSource`** code and other ephemera-owned paths perform **writes** (puts, conditional finalize, enqueue/claim) and orchestration. Thinking **result** writes live in [`results/persistThinkingResult.ts`](results/persistThinkingResult.ts). **Schedule** writes live in [`scheduling/persistThinkingSchedule.ts`](scheduling/persistThinkingSchedule.ts) ( **`Meta::Schedule`** uses overwrite **`putItem`** so `scheduled` / `claimed` / `cancelled` transitions replace the row; unlike **`Meta::Result`**, there is no **`nonCollidingPutItem`** gate). **`mtw-gateways`** does not own mutation paths (see package **Non-goals**).
...
- **Write path (schedule):** [`scheduling/persistThinkingSchedule.ts`](scheduling/persistThinkingSchedule.ts) and **`mtw.ephemera.thinking.scheduling`** in [`scheduling/index.ts`](scheduling/index.ts). Ingress is **`api.ephemera`** **`Put Thinking Schedule`** via **`sendPutThinkingSchedule`** in [`../apiEphemera.ts`](../apiEphemera.ts) (not CoyoteGame **`Thinking Schedule`** bus events). After each write, **`internalCache.ThinkingSchedules.invalidate(workItemId)`** runs. Hypothesis / pipeline code should prefer **`sendPutThinkingSchedule`** when enqueueing so work stays on the same internal command path as other **`api.ephemera`** durability; **`persistThinkingSchedule`** may be called directly only where **`messageBus.flush()`** ordering is already guaranteed without an extra send.
...
**Read gateway:** shipped in **`@tonylb/mtw-gateways/ts/ephemera/thinking`** (see package **`AGENT.md`** ownership row): results and **schedule** read helpers plus ephemera **`internalCache.ThinkingResults`** and **`internalCache.ThinkingSchedules`**. **Thinking result persistence** and **`mtw.ephemera.thinking.results`** (CoyoteGame **`Thinking Result`**) ship under [`results/`](results/). **Schedule persistence** and **`mtw.ephemera.thinking.scheduling`** (**`api.ephemera`** **`Put Thinking Schedule`**) ship under [`scheduling/`](scheduling/). **`Meta::Job`**, client **API**, and **EventBridge** replay for schedule remain in the task plan. This `AGENT.md` is the durable anchor for keys, lifecycle, and links; avoid duplicating full architecture here.
