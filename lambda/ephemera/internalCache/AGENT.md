...
**Shipped (step 3 foundation):** **`internalCache.PerceptionThreads`** ([`perceptionThreads.ts`](perceptionThreads.ts)) --- fan-in aggregation store for **`mtw.ephemera.perception`**; **multiple** independent entries per **`componentId` + `perspectiveKey`**; **`register(cmd)`** takes a **`threadKind`**-discriminated **`PerceptionThreadRegisterCommand`** (see [`localApiEvents.ts`](../dataSource/perception/localApiEvents.ts)); `clear()` wired from `InternalCache.clear()`, **no** `flush()`. Why perception owns this state: [`../dataSource/perception/AGENT.md`](../dataSource/perception/AGENT.md). Task-plan detail: [`AGENT.perceptionRefactor.planning.md`](../../../taskPlanning/lambda/ephemera/dataSource/perception/AGENT.perceptionRefactor.planning.md).
