...
4. **`AffordanceRoomDeliverable.get`** reads that slice + **`ComponentAggregate`** **`shortName`** + ephemera-only roster / **`objects`** -> ephemeraWire **`StandardForm`** for terminal publish.

**`AffordanceRoomDeliverable`** is a per-invocation compose memo: builds affordance-channel room header **`StandardForm`** (**`shortName`** via **`ComponentAggregate`**, **`exits`** via **`AffordanceCache`**, roster / **`objects`** via ephemera meta). Called only from perception on **`Affordances Pertain`** terminal publish --- not bus ingress. Routing (**`targets`**, **`SESSION#`**) remains perception / thread responsibility. It does **not** call **`ensureAffordanceTopology`** or **`ComponentTopology.get`**. The affordance path does **not** use **`mergeRoomExitsToJSON`** (shared WML merge helpers live in [`roomWireMergeHelpers.ts`](roomWireMergeHelpers.ts) for **`ComponentRender`** / **`GenerationContext`** only).
...
- **Affordance room deliverable** (rooms only): `{CharacterId}::{EphemeraRoomId}` --- no **`header`** segment; structural merge output does not depend on it (see **`generateAffordanceRoomDeliverableCacheKey`** in [`affordanceRoomDeliverable.ts`](affordanceRoomDeliverable.ts))
...
- **`AffordanceRoomDeliverable`** ([`affordanceRoomDeliverable.ts`](affordanceRoomDeliverable.ts)): **`DeferredCache`** of **room**-only merged **`StandardForm`** for affordance-channel structural WML. **`get(CharacterId, EphemeraRoomId)`** composes: **`ComponentAggregate.get`** -> **`.merged`** for **`shortName`** only; **`AffordanceCache.getAffordanceRow`** for projected **`exits`** (**D30** --- no **`mergeRoomExitsToJSON`**); ephemera-only **`RoomCharacterList`** roster and **`Meta::Room.objects`** from **`ComponentEphemeraMeta.get`**. Uses **`standardizeMode: 'ephemeraWire'`**. **Does not** call **`ensureAffordanceTopology`** or **`ComponentTopology.get`** (callers must hydrate first). **`invalidate(EphemeraRoomId)`** drops every cache key for that room. Cache key: **`generateAffordanceRoomDeliverableCacheKey`** (two-part **`characterId::roomId`**; future: **`perspectiveKey`** alignment). Shared render-path WML merge helpers (**`mergeRoomExitsToJSON`**, **`mergeRoomShortNameLiteral`**, **`roomCharacterListToStandardCharacterData`**) live in [`roomWireMergeHelpers.ts`](roomWireMergeHelpers.ts). **Navigation exit resolution (**D34**): [`getRoomExitTargetsForCharacter`](../dataSource/actions/roomExitTargetsForCharacter.ts) reads **`AffordanceCache`**, not **`ComponentRender`**.
