...
**Blueprint stack (hydrate consumers):** use **`internalCache.ComponentData.getAcrossAssets`** at the caller-supplied participation stack (pair-addressed reads). Steady-state hydrate calls **`internalCache.ComponentExamples.get(...)`** from [`ensureAuthoredCatalog`](../dataSource/renderCache/ensureAuthoredCatalog.ts) (orchestration resolve only in v1) --- not **`assembleComponentExamplesAtPerspective`**. Planned affordance-topology hydrate (**`ensureAffordanceTopology`** in **`affordanceCache`**) will call **`internalCache.ComponentTopology.get(...)`** on stale catalog reads only --- **not** on **`TopologyInvalidated`** receive (mirror **`handleExampleInvalidated`**). Ephemera does **not** register tier-1 **`ComponentVerticals`**; the aggregate slice uses an empty-hops stub because merge order is caller-supplied and vertical rows are unused in v1 assembly. See [`packages/mtw-gateways/AGENT.md`](../../../packages/mtw-gateways/AGENT.md) and [`renderCache/AGENT.md`](../dataSource/renderCache/AGENT.md) (**On-demand authored examples**).
...
- **`ComponentStackMerge`** ([`componentStackMerge.ts`](componentStackMerge.ts)): **`DeferredCache`** of **room**-only merged **`StandardForm`** for affordance-channel structural WML. **`get(CharacterId, EphemeraRoomId)`** composes: **`ComponentAggregate.get`** -> **`.merged`** for **`shortName`** only; **`AffordanceCache.getAffordanceRow`** for projected **`exits`** (**D30** --- no **`mergeRoomExitsToJSON`**); ephemera-only **`RoomCharacterList`** roster and **`Meta::Room.objects`** from **`ComponentEphemeraMeta.get`**. Uses **`standardizeMode: 'ephemeraWire'`**. **Does not** call **`ensureAffordanceTopology`** or **`ComponentTopology.get`** (callers must hydrate first). **`invalidate(EphemeraRoomId)`** drops every cache key for that room. Cache key: **`generateComponentStackMergeCacheKey`** (two-part **`characterId::roomId`**; future: **`perspectiveKey`** alignment). Exported helpers **`mergeRoomExitsToJSON`** / **`mergeRoomShortNameLiteral`** remain for **`ComponentRender`** / **`GenerationContext`**. **Navigation exit resolution (**D34**): [`getRoomExitTargetsForCharacter`](../dataSource/actions/roomExitTargetsForCharacter.ts) reads **`AffordanceCache`**, not **`ComponentRender`**.
