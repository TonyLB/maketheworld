...
4. **`AffordanceRoomDeliverable.get(roomId, perspectiveKey)`** loads the hydrated affordance row, uses **`row.assetStack`** for **`ComponentAggregate`** **`shortName`**, **`row.topology.exits`** for exits, plus **`getRoomCharacterList`** and ephemera-only **`objects`** -> ephemeraWire **`StandardForm`** for terminal publish.
