...
1. **Assets** emit skinny **`TopologyInvalidated`** on Area **`ludicGraph`** / **`referencedBy`** changes ([`lambda/assets/componentTopology/AGENT.md`](../../../lambda/assets/componentTopology/AGENT.md)).
...
**Production note:** Room-local blueprint exits were cleared; navigable exits come from merged Area **`ludicGraph.edges`** via **`projectRoomExits`**. Production Coyote topology uses the overlay-asset pattern in [`AGENT.CoyoteGame.implementation.md`](../../../AGENT.CoyoteGame.implementation.md) (**Overlay asset topology**; play-mode verified 2026-06-09). Edge authoring rules: [`packages/mtw-wml/ts/standardize/keys/edges/AGENT.edges.md`](../../../packages/mtw-wml/ts/standardize/keys/edges/AGENT.edges.md).

### Membership presentation and roster

Play **membership** on the affordances channel is **not** stored as display fields on `Meta::Room.ludicGraph`. The steady-state path (parallel to [exit presentation](#area-topology-and-affordance-exits) above):

1. **Play manipulation truth** --- **`internalCache.Positions.getLudicGraph`** returns host-bound **`EphemeraLudicGraph`** (gateway **`PlayLudicGraph`** adapted at the cache wrapper); **`getMembershipContainers`** loads adjacency ([`dataSource/positions/AGENT.concepts.md`](../dataSource/positions/AGENT.concepts.md#graph-roles-shared-shape-different-authority)). **`getMembershipContainers`** accepts **`CHARACTER#`** or **`OBJECT#`** PKs.
2. **Roster hydration** --- **`getRoomCharacterList`** ([`hydrateRoomRoster.ts`](hydrateRoomRoster.ts)) reads topology via **`internalCache.Positions.getLudicGraph`** -> **`graph.characterIds`**, then **`hydrateRoomRosterFromCharacterIds`** joins **`CharacterMeta`** (`Name` -> `DisplayName`, `Color`, `fileURL`) + **`CharacterSessions`** (`SessionIds`).
...
- **`Positions`** ([`ludicGraphCache.ts`](./ludicGraphCache.ts)): ephemera-only **`EphemeraLudicGraphCacheData`** wrapper over **`createPositionsCacheHandler`**. **`getLudicGraph(componentId)`** -> **`EphemeraLudicGraph.fromFieldPayload`**. **`set(graph: EphemeraLudicGraph)`** delegates **`graph.toStored()`** with memo key **`graph.hostId`**. (Corrected 2026-09-03: this pair was `fromPlayEnvelope`/`toPlayEnvelope`, routing live state through WML's authored shape, which has no `ports` --- so every crossing port was silently emptied on both read and write. Never put an authoring-shaped type back on this path.) **`getMembershipContainers`** / **`setMembershipContainers`** / **`invalidate*`** passthrough. Diagnostics lambda keeps raw gateway handler. **Deliberately not named `EphemeraLudicCacheData`** --- that name is now the real type (CC0b, `AGENT.abstractionLayers.planning.md`), a cross-host denormalized read structure defined at [`dataSource/positions/ludicCache/types.ts`](../dataSource/positions/ludicCache/types.ts); this class is a narrower per-request memo over one graph, and collapsing the two names would misrepresent this rename as having shipped AB-35's construct.
- **`AffordanceRoomDeliverable`** ([`affordanceRoomDeliverable.ts`](affordanceRoomDeliverable.ts)): **`DeferredCache`** of perspective-scoped merged **`StandardForm`** for affordance-channel structural WML. **`get(EphemeraRoomId, perspectiveKey)`** composes: **`AffordanceCache.getAffordanceRow`** first (required; supplies **`assetStack`** + projected **`exits`**); object ids from **`Positions.getLudicGraph`** -> **`graph.objectIds`**; **`appendImprovisationToPerspective`** on the stack when objects present; **`ComponentAggregate.get`** for room and per-object **`shortName`**; hydrated roster via **`getRoomCharacterList`**. Uses **`standardizeMode: 'ephemeraWire'`**. **Does not** read **`Meta::Room.objects`**. **Does not** call **`ensureAffordanceTopology`** or **`ComponentTopology.get`** (callers must hydrate first). **`invalidate(EphemeraRoomId)`** drops every cache key for that room. Cache key: **`generateAffordanceRoomDeliverableCacheKey`** (**`roomId::perspectiveKey`**).
...
- **`getRoomCharacterList`** ([`hydrateRoomRoster.ts`](hydrateRoomRoster.ts)): Derives **`RoomCharacterListItem[]`** on each call from **`Positions.getLudicGraph`** + **`hydrateRoomRosterFromCharacterIds`** (no per-room memo). **`hydrateRoomRosterFromCharacterIds`** composes display fields from **`CharacterMeta`** + **`CharacterSessions`**.
