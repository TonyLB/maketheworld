...
- **`PerceptionThreads`**: In-memory fan-in aggregation for **`mtw.ephemera.perception`**; [`perceptionThreads.ts`](perceptionThreads.ts), [`perceptionThreads.test.ts`](perceptionThreads.test.ts); stub, **`roomDescription`**, and **`roomHeaderBroadcast`** thread variants; **`register`** / **`list`** / **`update`** / **`remove(registrationId)`**; **`clear()`** only (no **`flush()`**). See [`AGENT.perceptionRefactor.planning.md`](../../../taskPlanning/lambda/ephemera/dataSource/perception/AGENT.perceptionRefactor.planning.md).
