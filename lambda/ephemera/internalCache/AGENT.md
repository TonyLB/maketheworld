...
**Shipped today:** **`ComponentData`** ([`componentData.AGENT.md`](./componentData.AGENT.md)); **`ThinkingResults`**, **`ThinkingSchedules`**, **`ThinkingJobs`** ([`dataSource/thinking/AGENT.md`](../dataSource/thinking/AGENT.md) --- scheduling rollup **must** use **`internalCache.ThinkingJobs.get`**, not ad-hoc **`ephemeraDB`** reads).

**Blueprint stack (on-demand authored examples wiring):** use **`internalCache.ComponentData.getAcrossAssets`** at the caller-supplied canon stack (pair-addressed reads; same handler as assets/diagnostics). Register **`ComponentAggregate`** with slice **`{ ComponentData: internalCache.ComponentData, ... }`**, then **`ComponentExamples`** via **`createComponentExamplesCacheHandler({ ComponentAggregate })`**. Hydrate calls **`internalCache.ComponentExamples.get`**. Do **not** wire **`ensureAuthoredCatalog`** to **`assembleComponentExamplesAtPerspective`** when the handler exists. See [`packages/mtw-gateways/AGENT.md`](../../../packages/mtw-gateways/AGENT.md) (**Component data read surfaces**, **Component examples read surfaces**) and [`renderCache/AGENT.onDemandAuthoredExamples.planning.md`](../../../taskPlanning/lambda/ephemera/dataSource/renderCache/AGENT.onDemandAuthoredExamples.planning.md) (**A1**).
