...
`guestCoyoteSituations(guestName)` builds the facet as a function, not a constant, and supplies all three of `displayName` / `summary` / `description` for richer prose --- not because `<Render>` requires it:

- **Historical note:** `<Render>`'s content model used to require the full triplet with a non-empty `DisplayName` (`schema/converters/components.ts`'s `finalize`, and again in Character's and Room's standardize consumers). `toProseTripletChildren` emits only the fields present on the payload, so a description-only facet round-tripped into a one-child `<Render>` the client could not reparse --- this shipped as a production show-stopper once character looks first reached the render path (`Render tag must contain exactly three children`). `<Render>` was since relaxed to tag-matched, independently-optional children (absent-over-empty); see [`standardize/keys/facets/AGENT.facets.md`](../../../packages/mtw-wml/ts/standardize/keys/facets/AGENT.facets.md) **Situation room facet prose** for the current contract. `guestCoyoteSituations` keeping the full triplet is now a content choice, not a parser requirement.
