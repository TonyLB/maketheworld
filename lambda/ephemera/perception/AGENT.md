...
- **Perception Flow** (when the mover has a **non-empty** arrival-room **`perspectiveKey`**):
  1. [`orchestrateCharacterMove`](../dataSource/positions/navigate/orchestrateCharacterMove.ts) persists via positions, then [`presentCharacterMove`](../dataSource/positions/navigate/presentCharacterMove.ts) registers a targeting-only **`characterMove`** perception thread and kicks passive **`Render Requested`** for the new room.
