...
- **`dataSource/positions/navigate/`**: Character navigate/home/connect/disconnect orchestration and post-persist presentation (`orchestrateCharacterMove`, `presentCharacterMove`); membership fan-in owns leave/arrive world copy
- **`routeTrustedUiAction`**: Trusted UI action ingress (look, move, home, speech) -> **`Action Assessed`**

## Related Documentation

- **[LLM utilities](llm/AGENT.md)**: Bedrock transport, parsers, pipeline runner; **design seams + output trust** ([`llm/AGENT.concepts.md`](llm/AGENT.concepts.md), [`llm/AGENT.contract.md`](llm/AGENT.contract.md))
