...
**Forbidden:** a **downstream** stage inferring an open-ended semantic operator or intent direction from natural-language phrase lists to fill a field its **owning stage** should conclude. The owning stage emits it --- **deterministically** when a closed-world input suffices (a frozen-template positive match), or via a **semantic-reasoning (LLM) realization** otherwise. What stays forbidden is the *downstream* gap-fill (regex / phrase-buckets in the compiler), not deterministic conclusion at the owner.

**Canonical incident:** relational `operationKind` phrase-bucket inference **in the compiler** (BD-12). **Owner: the Plan stage.** Plan concludes the establish-vs-dissolve determination in one of two realizations of the same owner --- **deterministically** via `matchRelationalTemplate` (`enrich/objectManipulation/plan/matchRelationalTemplate.ts`; closed verb set `put`/`place`/`lean` -> `establishRelation`, `take`/`remove` -> `dissolveRelation`; positive-match-required, **abstain on miss**, no default operator), or, for template misses, via BD-19's plan-only / joint **LLM fallback** (iteration 2, not yet built). The compiler only grounds and applies legality --- it never infers the operator. This superseded the retired frame-extract hop, which was the LLM owner before iteration 3; the deterministic realization is the exact relational twin of classify's `get`/`take`/`drop` membership fast path. *Forward note: as Plan matures toward emitting a compiled `ParsePlanStep` directly, `operationKind` may stop being a distinct computed scalar --- becoming implicit in the matched step variant (`establishRelation` step vs `dissolveRelation` step) --- but the seam is unchanged: Plan owns the determination.* See [`AGENT.concepts.md`](AGENT.concepts.md) (**Field ownership**, **Fast-path positive vs negative closure**).
