...
- **Initial event types**:
  ...
  - **`Affordances Requested`** (M4 scaffold): `{ roomId, perspective, reason: 'roster' | 'objects' | 'topology' }`. Send via **`sendAffordancesRequested`** in [`dataSource/affordanceOrchestration/subscribedEvents.ts`](dataSource/affordanceOrchestration/subscribedEvents.ts). Consumed by **`mtw.ephemera.affordanceOrchestration`** ([`dataSource/affordanceOrchestration/AGENT.md`](dataSource/affordanceOrchestration/AGENT.md)). Production adapters from **`RoomUpdate`** / perception **`Objects Changed`** are not wired yet.

#### **mtw.ephemera.affordanceOrchestration (affordance-channel orchestration, M4 scaffold)**

- **Implementation**: [`lambda/ephemera/dataSource/affordanceOrchestration/index.ts`](dataSource/affordanceOrchestration/index.ts); ingress in [`subscribedEvents.ts`](dataSource/affordanceOrchestration/subscribedEvents.ts); outbound types in [`publishedEvents.ts`](dataSource/affordanceOrchestration/publishedEvents.ts).
- **Inbound**: Subscribes to **`api.ephemera`** envelopes whose header type is **`Affordances Requested`**.
- **Behavior (scaffold)**: **`orchestrateAffordanceRequest`** logs ingress; no **`streamEvent`** emissions yet. **`ensureAffordanceTopology`** and **`affordanceCache`** handoff follow in later M4 slices.
