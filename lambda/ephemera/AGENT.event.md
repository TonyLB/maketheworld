...
- **Character Presence (mtw.connections.characters → mtw.ephemera.positions)**: `Character Connected` and `Character Disconnected` envelopes are consumed by the `mtw.ephemera.positions` DataSource (see [`dataSource/positions/`](dataSource/positions/)). `Character Connected` publishes `CheckLocation` (forceMove) so the existing `moveCharacter` flow drives the `Meta::Room.activeCharacters` add + arrival `WorldMessage` + `CharacterInPlay` `EphemeraUpdate`. `Character Disconnected` runs a conditional `Meta::Room.activeCharacters` projection; when the projection actually changes (idempotency gate), the handler refreshes `RoomCharacterList`, invalidates `ComponentEphemeraMeta` and `AffordanceRoomDeliverable`, and publishes the departure `WorldMessage` + `RoomUpdate`. Bus delivery: **`publish`** + boundary **`flushAndSettle`**; producer-side delivery is at-least-once; consumer idempotency is the projection gate.
