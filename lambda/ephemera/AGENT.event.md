...
- **Initial event types**:
  ...
  - **`Affordances Requested`** (M4): `{ roomId, perspective, reason: 'roster' | 'objects' | 'topology' }`. External kicks publish via **`sendAffordancesRequested`** in [`dataSource/affordanceOrchestration/subscribedEvents.ts`](dataSource/affordanceOrchestration/subscribedEvents.ts). Session orientation and in-DS fan-out call **`orchestrateAffordanceRequest`** directly. Consumed by **`mtw.ephemera.affordanceOrchestration`** ([`dataSource/affordanceOrchestration/AGENT.md`](dataSource/affordanceOrchestration/AGENT.md)). **`RoomUpdate`** enqueues via [`sendAffordanceRefreshRequestedForRoom`](dataSource/affordanceOrchestration/sendAffordanceRefreshRequestedForRoom.ts) (reason: **`roster`**). **`Objects Changed`** is handled inside the orchestration DataSource via [`fanOutAffordanceRefreshForRoom`](dataSource/affordanceOrchestration/fanOutAffordanceRefreshForRoom.ts) (reason: **`objects`**). Terminal affordance **`PublishMessage`** follows **`Affordances Pertain`** in **`mtw.ephemera.perception`** ([`handleAffordancesPertain`](dataSource/perception/handleAffordancesPertain.ts)).

#### **mtw.ephemera.affordanceOrchestration (affordance-channel orchestration, M4)**

- **Implementation**: [`lambda/ephemera/dataSource/affordanceOrchestration/index.ts`](dataSource/affordanceOrchestration/index.ts); ingress in [`subscribedEvents.ts`](dataSource/affordanceOrchestration/subscribedEvents.ts); outbound types in [`publishedEvents.ts`](dataSource/affordanceOrchestration/publishedEvents.ts); fan-out in [`fanOutAffordanceRefreshForRoom.ts`](dataSource/affordanceOrchestration/fanOutAffordanceRefreshForRoom.ts) and [`sendAffordanceRefreshRequestedForRoom.ts`](dataSource/affordanceOrchestration/sendAffordanceRefreshRequestedForRoom.ts).
- **Inbound**: Subscribes to **`api.ephemera`** envelopes whose header type is **`Affordances Requested`**, and to **`mtw.ephemera.objects` `Objects Changed`** (direct fan-out to **`orchestrateAffordanceRequest`**).
- **Behavior**: **`orchestrateAffordanceRequest`** runs intake, **`ensureAffordanceTopology`** when needed, emits **`Slice Ready`** / **`Orchestration Error`**; **`affordanceCache`** emits **`Affordances Pertain`**; **`mtw.ephemera.perception`** terminal **`PublishMessage`** via [`handleAffordancesPertain`](dataSource/perception/handleAffordancesPertain.ts).
- **Outbound payloads** (contract; v1-active types: **`Slice Ready`**, **`Orchestration Error`**; enrichment types deferred): lean **`roomId`**, **`perspective`**, **`perspectiveKey`** on each stream event. See [`dataSource/affordanceOrchestration/AGENT.md`](dataSource/affordanceOrchestration/AGENT.md).
...
- **Character Presence (mtw.connections.characters -> mtw.ephemera.positions)**: `Character Connected` and `Character Disconnected` envelopes are consumed by the `mtw.ephemera.positions` DataSource (see [`dataSource/positions/`](dataSource/positions/)). `Character Connected` resolves an eviction-ladder target room and applies membership via [`orchestrateCharacterRoomMembership`](dataSource/positions/manipulation/membership/orchestrateCharacterRoomMembership.ts), then post-persist orchestration (header render; fan-in owns arrive world copy). `Character Disconnected` applies `targetRoomId: null` through the same membership API; when `changed`, the coordinator streams `Character Moved`, refreshes caches, and publishes `RoomUpdate` / `EphemeraUpdate`; leave world copy is fan-in emission. Bus delivery: **`publish`** + boundary **`flushAndSettle`**; producer-side delivery is at-least-once; consumer idempotency is the `changed` gate.
- **Session orientation (mtw.connections → render + affordance orchestration)**: `Character Registered` envelopes are deserialized in [`app.ts`](app.ts) via `ConnectionsEventSerializer` and subscribed by **`mtw.ephemera.renderOrchestration`** and **`mtw.ephemera.affordanceOrchestration`** (guards in [`dataSource/connectionsCharacterRegistered/subscribedEvents.ts`](dataSource/connectionsCharacterRegistered/subscribedEvents.ts)). RoomHeader delivery (`targets: CHARACTER#...`) runs through [`handleCharacterRegisteredOrientation`](dataSource/connectionsCharacterRegistered/handleCharacterRegisteredOrientation.ts) and terminal perception fan-in; wired separately from the world/presence path above. See [`AGENT.md`](AGENT.md#session-orientation-character-registered).
