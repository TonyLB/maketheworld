...
- **Render cache invalidation (`mtw.assets.componentExamples` → `mtw.ephemera.renderCache`)**: Steady-state blueprint changes emit **`ExampleInvalidated`** (when Assets emitter ships); **`mtw.ephemera.renderCache`** bumps catalog rows. **`Ephemera RenderCache Finding`** from **`mtw.diagnostics`** is handled on the same DataSource (lazy catalog bump). Legacy **`ExampleUpdated`** / **`ExampleRemoved`** mirror via **`mtw.ephemera.examples`** is retired.
