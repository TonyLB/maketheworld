...
- **Behavior (interim)**: **`orchestrateAffordanceRequest`** logs ingress; no **`streamEvent`** emissions yet; no terminal affordance **`PublishMessage`** until **`Affordances Pertain`**. **`ensureAffordanceTopology`** and **`affordanceCache`** handoff follow in later M4 slices.
