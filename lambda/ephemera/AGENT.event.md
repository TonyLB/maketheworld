...
- **Egress (bus only today)**: On first active -> **`completed`** job transition, **`streamEvent`** with header **`Job Completed`** (`THINKING_JOB_COMPLETED_HEADER_TYPE`) and payload **`ThinkingJobCompletedEvent`** (schedule snapshot only; not an **`api.ephemera`** command). Per-hop **`Thinking Schedule`** wire events are **not** emitted yet (contracts ready; deferred --- see [`dataSource/thinking/AGENT.md`](dataSource/thinking/AGENT.md) **EventBridge and subscriptions**).
- **Publishing**: **`publisherStrategy: 'busOnly'`**, **`replayable: false`** until EventBridge MVP lands. **MVP scope:** replay/EventBridge/**subscriptions** for **`Job Completed` only**; **`Thinking Schedule`** stream is a follow-on.
