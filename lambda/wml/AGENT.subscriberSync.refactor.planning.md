...
- **WML snapshotContentGenerator (planned/partial)**: The base DataSource already uses `snapshotContentGenerator` as the single plug point for snapshot creation. No separate `snapshotSidecarUrlGenerator` is needed; that wrongly conflates delegation with sidecars. WML should pass `snapshotContentGenerator` that calls `getSidecarSnapshotDescriptor` (or equivalent) and returns a **domain-shaped** payload (e.g. `{ wml: { sidecarUrl } }`). The client serializer resolves sidecars when it sees that shape. See `AGENT.implementation.md` (Serialization resolution architecture) and `documentation/dataSources/AGENT.delegation.planning.md` (delegation = different implementation of snapshotContentGenerator).
...
| Backend (WML snapshotContentGenerator) | lambda/wml/dataSource/mtw-wml.ts | Planned; WML should pass `snapshotContentGenerator` (not unwired `snapshotSidecarUrlGenerator`) that returns domain-shaped payloads (e.g. `{ wml: { sidecarUrl } }`). Base DataSource already uses snapshotContentGenerator; no framework changes needed. See AGENT.delegation.planning.md. |
...
2. **Backend – WML snapshotContentGenerator**: Planned/partial. WML should pass `snapshotContentGenerator` that calls `getSidecarSnapshotDescriptor` and returns domain-shaped payloads (e.g. `{ wml: { sidecarUrl } }`). No base DataSource changes needed; `snapshotContentGenerator` is the existing plug point. See AGENT.delegation.planning.md.
