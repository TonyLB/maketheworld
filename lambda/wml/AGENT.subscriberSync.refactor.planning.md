...
**Status**: PLANNING (backend generic sidecar: implemented; front-end sidecar handling: implemented; WML dataSource slice: implemented; personalAssets refactor: complete)  
...
- Replaced `fetchAction` with `subscribeAction`: no fetch for WML body; dispatches `subscribeToWmlDataSource([id])` (wmlDataSource owns subscribe); registers LifeLine subscription for clearPendingEditsByRequestIds and Merge Conflict toast. Properties (image metadata) deprecated and stubbed until image uuid-as-filename refactor (getFetchURL removed).
...
### Client Work Item 2.5 (clearAction unsubscribe cleanup)

- Removed redundant personalAssets socket unsubscribe; wmlDataSource owns mtw.wml unsubscribe via its UNSUBSCRIBE state. clearAction now only unsubscribes the LifeLine listener and dispatches `unsubscribeFromWmlDataSource([id])`.

### Client Work Item 2.2 and 2.3 (remove base, derive from dataSource)

- Removed `base` from personalAssets public state and initialData. Base is derived from wmlDataSource via `augmentPublicDataForSelect` in multipleSSM.
- Added `getBase` (via augmenter) that reads from `wmlDataSource.subscribedStreams[assetId]?.materializedView`; all base consumers (getLocalStandardForm, getStandardForm, updateStandard reducer) use derived base.
- multipleSSM: added optional `augmentPublicDataForSelect` to inject derived fields before selectors run.
- updateStandard reducer: uses `action.payload.base` (supplied by thunk); thunk reads base via getWMLBase and passes to payload.
- fetchAction: no longer returns base; base comes from dataSource after Snapshot.
- Tests: reducer tests updated for payload.base; selector tests for derived base; index.test.ts assertions updated.
...
**Decided: Option A** — WML dataSource owns subscribe/unsubscribe. When personalAssets fetchAction runs, it dispatches `subscribeToStreams([id])` on the WML dataSource slice; the dataSource sends the WebSocket subscribe. When clearAction runs, `unsubscribeFromStreams([id])`. personalAssets does not send subscribe/unsubscribe itself. Initial state comes from Snapshot (sidecar) after subscribe.
...
| charcoal-client | `src/slices/personalAssets/index.api.ts` | **Done.** subscribeAction replaces fetchAction; no WML fetch; wmlDataSource.subscribeToStreams([id]); LifeLine subscription for RequestIds + toast. clearAction: LifeLine unsubscribe + wmlDataSource unsubscribe only (2.5 done). Properties deprecated and stubbed until image uuid-as-filename refactor (getFetchURL removed). |
...
- **Deprecate legacy fetch API**: **Done (client-side).** Removed getFetchURL; subscribeAction stubs properties as `{}`. Character icons and ImageHeader display broken until image uuid-as-filename refactor. Restore: add properties source, populate in subscribeAction; useLibraryImageURL will resume. See personalAssets AGENT.md "Deprecated: Image properties (fetch)".
...
7. **Frontend – personalAssets**: Derive base from dataSource **(2.2, 2.3 Done)**; refactor receiveWMLEvent to only clearPendingEditsByRequestIds (Done); open-asset flow triggers subscribe (no fetch for WML body) **(2.4, 2.6 Done)**; clearAction unsubscribes **(2.5 Done)**. Client Work Item 2 complete.
8. **Tests and cleanup**: WML dataSource tests (implemented); personalAssets reducer tests; getFetchURL deprecated and removed (properties stubbed until image uuid-as-filename refactor).
