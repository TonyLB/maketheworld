...
**Mechanics:** [`lambda/wml/promoteToCanon.ts`](./promoteToCanon.ts) plans a minimal sequence (`planPromoteToCanonSteps`): if the asset is not already in **Library**, enqueue **`Move Asset`** toward **Library**, then **`Canonize Asset`** (Library → Canon inside [`dataSource/mtw-wml.ts`](./dataSource/mtw-wml.ts)). Each step is sent on the messageBus and flushed before the next; the runner **re-reads authoritative zone** between steps so work is skipped if already done (**state-based idempotency**; no `idempotencyKey` on the message). If the asset is **already Canon**, no coordination is sent and no redundant **`Zone Changed`** is published.
