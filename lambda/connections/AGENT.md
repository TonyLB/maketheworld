...
| **`Character Connected`** / **`Character Disconnected`** | `mtw.connections.characters` | **`mtw.ephemera.positions`** ([`dataSource/positions/`](../ephemera/dataSource/positions/)) | Membership apply (`orchestrateCharacterRoomMembership`), `Character Moved` fact, cache/`RoomUpdate` bundle; world copy via fan-in. **Not** session RoomHeader bootstrap. |
